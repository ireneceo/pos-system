...
// ============================================
// Brand General Stock Categories
// ============================================

/**
 * GET /api/brands/:brandId/general-stock-categories
 * 브랜드 일반재고 카테고리 목록 조회
 */
router.get('/brands/:brandId/general-stock-categories', authenticateToken, isBrandManager, async (req, res) => {
  try {
...
/**
 * POST /api/brands/:brandId/general-stock-categories
 * 브랜드 일반재고 카테고리 생성
 */
router.post('/brands/:brandId/general-stock-categories', authenticateToken, isBrandManager, async (req, res) => {
  try {
    const { brandId } = req.params;
    ...
    if (order === undefined || order === null) {
      const maxOrder = await GeneralStockCategory.max('display_order', { where: { brand_id: brandId } });
      order = (maxOrder || 0) + 1;
    }
    ...
    console.error('Create brand general stock category error:', error);
    res.status(500).json({ error: 'Failed to create category' });
  }
});

/**
 * PUT /api/brands/:brandId/general-stock-categories/:categoryId
 * 브랜드 일반재고 카테고리 수정
 */
router.put('/brands/:brandId/general-stock-categories/:categoryId', authenticateToken, isBrandManager, async (req, res) => {
  try {
...
/**
 * DELETE /api/brands/:brandId/general-stock-categories/:categoryId
 * 브랜드 일반재고 카테고리 삭제
 */
router.delete('/brands/:brandId/general-stock-categories/:categoryId', authenticateToken, isBrandManager, async (req, res) => {
  try {
    const { brandId, categoryId } = req.params;
...
/**
 * PUT /api/brands/:brandId/general-stock-categories/reorder
 * 브랜드 일반재고 카테고리 순서 변경
 */
router.put('/brands/:brandId/general-stock-categories/reorder', authenticateToken, isBrandManager, async (req, res) => {
  try {
    const { brandId } = req.params;
    ...
    for (const item of orders) {
      await GeneralStockCategory.update(
        ...
        { where: { id: item.id, brand_id: brandId } }
      );
    }

    res.json({ success: true, message: 'Order updated' });
  } catch (error) {
    console.error('Reorder brand general stock categories error:', error);
