...
### 2026-06-13 (DEV 미배포 — 다음 /배포 대상)
- 본사(브랜드) 계정 구독 — 미래에 시작하는 계정을 항상 "체험(trial)"로 처리하도록 수정. 시작 전인데 정지/즉시청구로 잘못 저장되던 갭(thefire 본사 사례) 방지
- 주방 인쇄 대기열 — 삭제된 주문은 티켓이 다시 찍히지 않도록 제외(유령 티켓 방지). 정상 주문 인쇄는 그대로 (※ 배포 후 매장 실프린터 확인 필요)

### 2026-06-12 오후 (운영 배포 3회 — Backup 082237 / 085130 / 093826, smoke 9/9×3 — 버전 반영 대기)
- 스탭 로그인/PIN 전환 크래시 수정 — 이메일 없는 스탭(PIN 전용)이 로그인·전환하면 "Something went wrong"으로 앱 전체가 죽고 재로그인도 안 되던 문제 (운영 재현→배포 후 같은 계정 정상 실검증)
- 시작 크래시 자동 복구 — 앱이 켜자마자 죽으면 캐시·서비스워커를 1회 자동 삭제 후 재시도 (로그인 상태 유지). "캐시 때문에 로그인 안 됨" 자동 해소
- 업데이트 전달 우회 — Cloudflare에 잘못 캐시된 옛 서비스워커(5/30)를 빌드별 주소로 영구 우회 — 매장 기기가 다시 자동 업데이트 받음
- 통합 오더티켓 스테이션 범위 — 저장해도 "풀오더"로 되돌아가던 문제 근본 수정 (서버가 설정을 읽을 때 새 항목을 깎던 것 + 옛 화면이 덮어써도 선택값을 지키는 보호 + 인쇄용 데이터 누락 보강)
- 브랜드메뉴 카드 — 사진 없을 때 메뉴 이모지 표시 (회색 아이콘 대신)
- 모바일 주문 — 주문 완료 후 홈으로 가도 테이블 번호 유지 (테이블 QR로 들어온 손님이 추가 주문 시 테이블 재입력 불필요. 기존엔 주문 완료가 테이블까지 지움)
- 모바일 테이블 선택 — 테이블 번호 입력 시 화면이 확대되며 레이아웃이 밀리던 것 수정 (iOS 자동 확대 차단)

### 다음 세션 대기 (코드 변경 아님 — 메모)
- ~~구독 시작일/트라이얼 프론트~~ — 이미 완료·배포 확인됨(2026-06-13)
- ~~설정 저장 보호 가드~~ — 이미 완료·동작확인(13/13). 잔여=hydration marker(Irene 결정사항)뿐
- ~~BG dashboard 자동 trial 판정 + user 29~~ — user29 운영데이터 7/1 정렬 + BG 미래시작 강제 trial 코드갭 수정(6/13)
- 기획설계 2건 구현 대기: 삭제/취소 PIN 손실방지 게이트(docs/VOID_PIN_GATE_DESIGN.md) / 브랜드메뉴 레스토랑 적용범위(BRAND_MENU_SYSTEM.md §14)
