...
### 2026-06-02 (주방 인쇄·알림 모델 v2 — DEV, 미배포·실프린터 미확인)
- 주문 취소·테이블 이동 시 주방 자동 통보 — 자동발행 설정과 무관하게 취소표/이동표를 **항상** 주방에 발송(주방이 놓치지 않도록). 발송 후 주문 낸 화면에 "주방에 발송됨" 알림이 뜨고 [재발송] 가능
- 모든 주방 티켓 상단에 **주방(station) 이름 박스** 표기 — 어느 주방으로 가는 표인지 한눈에 (신규·추가·이동·취소 모든 티켓)
- 주방 디스플레이 알림 추가 — 주문 전체취소 알림(빨강) 신설, 테이블 이동이 합치기였을 때 "다른 주문에 합쳐짐" 문구. 알림은 현재 보고 있는 주방 탭 기준으로 표시
- 플로어플랜에서도 주문/품목 취소 시 동일하게 주방 통보 연결
- "취소 티켓 인쇄" 설정 토글 삭제 — 취소는 이제 항상 발송이라 설정 불필요
- (검증) 빌드/타입/렌더 통과. ⚠️ **실제 프린터 종이·주방화면·POS화면 눈 확인은 다음 매장 테스트에서 진행 후 배포** (가이드: `docs/PRINT_RULES_MATRIX.md` § 10)
...
- 주방 디스플레이 단계 되돌림 수정 — 서빙 처리한 주문이 저절로 이전 단계로 돌아가던 문제 차단(오래된 동기화 신호 무시)
- 라이브 오더 누락 방지 — 화면 연결이 잠깐 끊겼다 돌아올 때 그 사이 들어온 주문이 안 보이던 문제 → 재연결 시 자동 새로고침 + 주기적 동기화
- "테이블 비우기" 개선 — 테이블을 비울 때 주문의 테이블번호가 지워지던 문제 → 번호는 보존하고 테이블만 비움
- (사고 복구) 설정 소실 대응 — 프린터·결제 설정이 통째로 비워지던 근본원인 파악, 백엔드 보호 가드는 다음 작업

### 미배포 (다음 세션)
- 설정 저장 보호 가드(빈값 덮어쓰기 차단) / 오더티켓 중복 출력 / POS 화면 정비
