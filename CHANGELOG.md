...
### 2026-06-19 (v3.59 이후 — 밤 배포 예정)
- 직원 PIN 로그인 — 공용 단말에서 직원이 이메일/아이디 없이 PIN만으로 로그인. 온스크린 숫자패드(터치). 단말이 매장을 기억해 PIN만 누르면 됨. (PIN 입력 시도 횟수 제한으로 무차별 추측 차단)
- 현금관리 마감(Cash-up) — 교대 시작 시 개시현금 입력 → 영업 → 마감 시 현금/카드(종류별)/기타를 실제로 세어 입력 → 시스템이 예상과 비교해 "현금 RM5 부족" 식으로 차이 안내. 마감 현금이 다음날 개시현금으로 이월. (글로벌 표준 블라인드 카운트: 예상 금액을 보기 전에 실물을 먼저 세어 입력 → 손실방지) 사이드바 "Cash Up" 메뉴.
- (백스테이지) PayPal 결제 알림 — 사용하지 않는 매장에 봇이 보내던 가짜 요청으로 관리자에게 불필요한 알림 메일이 가던 것 중단(요청은 기존대로 안전하게 차단).

### 2026-06-20
- 현금관리 확장 — 영업 중 현금 입금/출금(거스름 보충·경비 지출) 기록, 마감 예상현금에 자동 반영. 마감 후 Z-Report(마감 요약) 영수증 인쇄 + 캐시 서랍 수동 열기. 결제수단 사전등록(선택).
- 현금관리 보안·정확도 강화 — 취소/삭제된 주문의 결제가 마감 예상현금에 잘못 포함돼 "현금 부족"으로 오인되던 것 수정(직원 누명 방지). 현금관리는 카운터 권한 직원만 사용. 마감 후 현금 이동/재마감 잠금.
- 예약 ↔ 플로어플랜 연결 — 예약에 플로어플랜 테이블을 배정하면, 예약 시간이 임박한 테이블이 플로어플랜에 "예약됨"으로 표시. 손님 도착(체크인) 시 해당 테이블 주문 화면이 자동으로 열리고 인원수가 자동 입력됨. 주문 결제가 끝나면 예약이 자동으로 완료 처리. 같은 테이블·시간 이중예약 차단. 설정에서 "예약됨" 표시 리드타임 조절.
- (백스테이지) UI 통일 — 현금 입출금 입력을 표준 팝업으로 통일.
- (백스테이지) 직원 PIN 로그인 버그 수정 — 익명 상태에서 PIN 로그인이 인증 가드에 막혀 항상 실패하던 문제 수정(미배포 기능, 출시 전 발견). 실라우트 통합검증 33/33 + 회귀 안전망 추가.
