...
### 2026-05-30 (DEV only — 미배포, Irene 테스트 + bless + 배포 대기)
- 모바일 매장식사(dine-in) 주문에 테이블번호 필수 — 매장별 설정 ON 시. 대표/공용 QR로 들어와 테이블 없이 주문되던(Floor Plan에 "픽업 N"으로 뜨던) 문제 해결
- 테이블 없는 손님은 매장에 등록된 Floor Plan 테이블 목록에서 직접 선택 (오타로 엉뚱한 테이블에 꽂히는 것 방지)
- 테이크웨이 전용 QR은 테이블 안 물음(면제), 테이블 QR로 들어온 테이크웨이는 그 테이블에 그대로 표시
- 설정 위치: 설정 → Tables & QR 탭 → "테이블 번호 필수" 토글 (기존 토글 활성화, 기본 꺼짐)

### 2026-05-30 (운영 배포됨 · 버전 결정 보류 — Backup 20260530_042734, smoke 10/10)
- 세트메뉴 옵션 전구간 기록 — 세트에 직접 넣은 옵션 + 각 구성품 개별옵션이 주방티켓·영수증·주문내역·키친디스플레이 모든 곳에 표시
- POS에서도 세트 자체 옵션 입력 가능 (기존엔 구성품 옵션만 받았음)
- 브랜드 메뉴 세트도 "이거 또는 저거"(OR/택1) 슬롯 + 구성품 옵션 빌더로 업그레이드 — 브랜드에서 만든 세트가 매장에 옵션·조건 그대로 전달
- 세트 구성 화면 친절화 — Fixed(항상 포함)/Choice(택1) 설명, 각 구성품이 주문 시 함께 나오는 옵션 표시
- Floor Plan 편집 화면에서 테이블이 안 보이던 문제 수정 + 캔버스 크기 작게(최소 300) 조정 가능
- (다음 세션 검증 대기) 브랜드 세트 산하 매장 실전파 + 세트 옵션 실프린터 출력 + 이머전시 모바일 인쇄
