...
### 2026-06-26 (thefire 잔여 + 합본주문 — 운영 배포 다회, 버전 무변)
- 모바일 크로스셀(상품 추천): RA 상품/카테고리, BG 브랜드메뉴, 모바일 담은 직후 추천 바텀시트
- 부분수량 취소: "3개 중 1개만" 취소(수량 스텝퍼), 주방에 "취소 N / 잔여 M" 표시
- 테이블+테이크웨이 합본 주문: 한 계산서로 합침, 서비스차지는 dine-in 품목만·포장비는 takeaway 품목(매장 설정대로)·세금 전체. 빌/상세/Live Orders에 품목 dine-in/takeaway 표시
- 주방 스테이션 인쇄 매수(1~3장) 설정
- 오프라인 주문 큐: 연결 끊겨도 주문 보관 후 자동 전송(중복 방지)
- 주문 리마크: 입력란에서 이전 메모 검색·선택(자동완성)
- Floor Plan: 테이블번호 크게, 터치 시 버튼 색 굳던 문제 해결, 우측 패널 오더정보 정돈

### 2026-06-25 #2 (하이브리드 로컬 인쇄 — 운영 배포 SW 4.16→4.17, 버전 무변)
- 인쇄 — 인쇄 전담 POS(POS1)가 자기가 넣은 주문·테이블이동·주문취소·아이템취소를 서버 응답을 기다리지 않고 그 자리에서 즉시 인쇄(와이파이 불안정해도 빠름). 다른 기기/모바일 주문은 기존대로 인쇄 전담 POS가 처리. 중복 없이 정확히 1장.
- 인쇄 — 이동/취소/아이템취소 티켓은 기존 표준 디자인(TABLE CHANGED / ORDER CANCELLED / ITEM VOIDED) 그대로 사용.

### 2026-06-25 (백스테이지 운영 배포 — 버전 무변, thefire 무인쇄 근본수리)
- 인쇄 — 다른 설정을 저장할 때 매장 프린터 설정이 실수로 지워지던 문제 해결(주방 자동인쇄가 갑자기 멈추던 사고 근본 차단). 프린터 설정은 레스토랑 관리자만 변경 가능(비상 라우팅 모드는 직원도 켤 수 있음). (운영 배포)
- 인쇄 — 주문이 들어오면 인쇄 전담 POS가 5초 기다리지 않고 즉시 인쇄(다른 기기·모바일 주문 포함). POS·주방·플로어 화면에서도 즉시 나옴. (운영 배포 SW 4.12)
- 인쇄 — 자동인쇄를 껐다 켜도 밀려 있던 예전 주문이 한꺼번에 쏟아지지 않도록 수정.
- 시스템 — 운영 서버 디스크 정리(오래된 배포 백업), 보안 모니터 디스크 용량 경고 해소.

### 2026-06-24
- 직원 이름 표시 — 직원이 로그인/새로고침해도 내부 ID(`r16:server1`)가 아니라 깔끔한 이름(`SERVER1`)으로 표시. 화면·영수증·마감리포트 모두 적용(기존 주문 데이터도 일괄 정리). (운영 배포 SW 4.11, 버전 무변)
- 키친 디스플레이(KDS) — KDS 전용 직원 PIN 게이트를 없애고, POS·플로어플랜과 동일하게 헤더의 'Logged in'(직원 전환) PIN 모달로 통일(동작 일원화).
- 인쇄 — 신규·테이블이동·주문취소·아이템취소 인쇄를 한 방식으로 통일. 누가/어느 기기/어느 계정으로 처리해도 인쇄 전담 POS가 한 장만(중복 없이) 인쇄.
- 인쇄 — 주문/이동/취소를 하면 인쇄 전담 POS가 즉시 인쇄(이전엔 테이블이동·취소가 한참 늦게 나옴).
- 인쇄 — 주문 취소 직후 주문이 삭제돼도 '주문 취소' 안내 티켓이 누락 없이 1번 인쇄.
- 인쇄 — 테이블 이동 티켓 맨 아래에 '이전 테이블(취소선) → 새 테이블' 표기로 어디서 어디로 옮겼는지 표시.
- 인쇄 — 주방 티켓·빌의 시간이 항상 매장 설정 타임존으로 출력(기기 로컬 시간 사용 버그 수정).
- 설정 — POS 권한(직원)에게도 프린터·주방 스테이션 설정을 열어, 관리자 계정 아니어도 프린터 설정/인쇄 가능.
- 권한 — '주문 전용' 서버 역할: 결제와 주문취소만 막고 주문·테이블이동 등은 가능(배포해도 권한 유지).

### 2026-06-23 (백스테이지 운영 배포 — 버전 무변, thefire 실사용 셋업)
- 프린터 설치(Windows 7/8) — QZ Tray 설치 시 자바(Java)까지 자동으로 깔리도록(이전엔 자바 설치 안내창 반복으로 멈춤).
- 인쇄 서명 — 구버전 QZ Tray(Windows 7)에서 매번 뜨던 "허용(Allow)" 팝업 해소(버전에 맞는 서명 방식 자동 적용).
- 자동인쇄 — POS가 2대일 때 같은 주문이 양쪽에서 두 번 인쇄되던 중복 제거(메인 POS가 자동인쇄 전담).
- 모바일 메뉴 — 메뉴 상세를 보고 뒤로 가면 카테고리 탭이 처음으로 돌아가던 문제 수정(보던 탭 그대로 유지).
- 플로어플랜 — 서빙(홀) 전용 직원 화면에 '전체 주문 티켓 인쇄(Full)' 버튼을 크게 노출.

## [v3.62] — 2026-06-23 배포 (Backup 20260623_124849, Smoke 9/9, SW 3.95)

### 2026-06-23 (thefire 실사용 준비)
- 직원 PIN 전환 — POS/플로어플랜에서 직원 이름을 눌러 다른 직원 PIN으로 전환하던 기능이 동작하지 않던 문제 수정(다중 단말 매장에서 매장 정보를 못 보내 실패하던 회귀).
- 시재(현금) 개시 모드 — 교대 시작 시 개시 현금을 **'직전 마감액 자동 이월'** 또는 **'매일 같은 고정 금액'** 중에서 선택 가능(설정 → 운영). 고정 선택 시 매 교대마다 그 금액으로 미리 채워짐(직원이 수정 가능).
- 마감 리포트 글꼴 — 데일리/파이널 마감 인쇄물의 글꼴·스타일을 영수증·주방티켓과 동일하게 통일(가독성).
- 통합 오더티켓 수동 인쇄 — 플로어플랜 테이블 화면에 **전체 주문을 한 장으로** 뽑는 'Full' 인쇄 버튼 추가(누른 그 POS의 프린터로 출력).
- 로그인 직원 PIN 우선 — 매장에 한 번 연결된 단말은 로그인 화면이 **직원 PIN 패드를 먼저** 표시(관리자 로그인을 거치지 않고 PIN만으로 들어감). 관리자/매니저 로그인은 'Use email sign-in' 한 번으로 전환. (단말 연결 기억은 로그아웃해도 유지)
- 설정 QR 인쇄 — 테이블 QR + 빠른입장(픽업/테이크아웃/예약) QR에 **'인쇄' 버튼** 추가(영수증 프린터로, 플로어플랜 테이블 QR 인쇄와 동일).
- 프린터 설치(Windows 7/8) — QZ Tray(인쇄 앱) 설치가 "인터넷 오류"로 실패하던 문제 수정. OS 버전을 감지해 호환 버전을 자동 설치하고, 설치 파일을 안정적인 경로로 받도록 개선.

## [v3.61] — 2026-06-22 배포 (Backup 20260622_204037, Smoke 9/9, 스키마 dev=운영 완전일치)

### 2026-06-22 (플로어플랜 핫픽스 — 추가 배포 Backup 20260622_211326, Smoke 9/9)
- 플로어플랜 — 새 주문 알림음이 매장(dine-in) 테이블 주문에도 울리도록(이전엔 포장/픽업/배달 주문에만 울려 테이블 주문 소리가 안 났음). 라이브오더와 동일.
- 플로어플랜 헤더 반응형 — 화면이 좁으면(≤1440px) Today's Cash Drawer·Final Settlement를 설정(⚙️) 드롭다운으로 수납하고 보기색상 버튼을 약자로 축약해 헤더가 넘치지 않도록(Daily Settlement는 인라인 유지).

### 2026-06-22
- 발주 — 같은 공급업체로 보낸 주문이 따로따로 쌓이지 않고 **한 발주서로 합쳐지도록**(공급업체별 1건). 발주 검토(Pending) 화면 진입 시 자동 통합.
- 발주 — 발주 검토 화면에서 **품목별 삭제(×)** 및 **발주서 폐기(Discard)** 가능. 발주 담는 화면에 검토 화면 바로가기 링크 추가, 우측 패널 이름을 "Planned Order"로 정리.
- 발주 장바구니 — 담은 내역이 페이지를 벗어났다 돌아와도 **유지**되도록(이전엔 사라짐).
- 외부공급업체 발송 — PDF/WhatsApp/Email 발송 내용에 **실제 품목 목록과 총액** 포함(이전엔 품목 수만). WhatsApp 번호 미등록 시 연락처 선택 발송. 품목이 이름으로 표시.
- 외부공급업체 — 다른 매장이 등록한 외부공급업체가 내 검색에 섞여 보이던 노출 차단(내가 등록한 것만). 재료에서 등록 시 등록된 공급업체를 선택하도록 정리.
- 인쇄 — 자동발행 OFF→ON 전환 시 이전 티켓 폭주 / 주문 밀릴 때 신규 자동인쇄 누락 수정(인쇄 방식 자체는 무변경).
- 재고 — 재고 편집 화면 "공급업체 상품 연결" 팝업이 뒤로 가려지던 문제 수정.
- 외부공급업체 상품 등록 — 솔루션 미가입 외부 공급업체 상품을 매장이 직접 등록·관리(프로필에서 추가/수정/삭제, 재고에서 등록). 등록 상품은 발주 가능. (RA 우선)
- 발주 화면 — 브랜드/푸드코트 연결 재고가 발주 목록에 안 뜨던 문제, 입고 발주 '오늘'만 보이던 것 → 전체 기간 기본.
- 인보이스 — '결제대기' 탭 기간 필터 전체(All) 기본(미결제 누락 방지). 모든 역할.
- 공급업체 목록 — 직원(Staff) 계정 수정/삭제 버튼 무반응 수정.
- (내부) 브랜드 재료 목록 타매장 재료 노출 차단 · 미인증/데모 주소 알림메일 반송 차단 · QZ 진단 문의 168건 정리.
