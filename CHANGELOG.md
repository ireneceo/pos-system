...
## v3.103 — 2026-09-25 (SW 5.64-context-cash-delivery-20260925)

### 2026-09-25 19:16 (SW 5.65-reconcile-total-first · 백업 20260925_185824 · 스모크 10/10)
- **인보이스 «총액 우선 대조»** — 대조 화면 맨 위에서 인보이스 총액(+배송비·세금·할인)만 적고 «이 총액으로 확정 (줄은 나중에)». 줄 합과 안 맞아도 됩니다. 결제·청구서는 그 총액, 줄 단가는 원가에 반영하지 않습니다. 줄까지 맞추기는 아래 2단계로
- 이미 적어 둔 줄 단가가 있으면 «N개가 지워지고 원가에 반영되지 않습니다» 확인창
- **갭 표시 통일** — 목록·상세·결제 창이 «청구 총액 − 발주 총액»(배송비·세금 포함) 한 기준. 목록 버튼 «Total only · +RM 2.50» / «Reconciled · −RM 3.20»
- 운영 DB 변경 0 · 서버 변경 0 · 실호출 10/10 · 실브라우저 10/10 · 고장주입 반증 2건


### 2026-09-25 17:06 (SW 5.64-context-cash-delivery · 백업 20260925_165921 · 스모크 10/10)
- **선택 카드 제목 = 내 프로필 이름** — 「Choose where to work」의 첫 카드(이 아이디의 원래 자리)가 브랜드 이름 대신 계정 프로필 이름으로 나옵니다. 부여받은 매장·오너 카드는 그대로 매장 이름
- **데스크탑에서도 다른 곳으로 전환** — 사이드바 아래(언어 위)에 «◐ 이름» 버튼. 접힌 사이드바에선 ◐ 아이콘. 전환할 곳이 2곳 이상인 계정에만 보입니다. 전에는 휴대폰 화면에만 있었습니다
- **현금관리 삭제** — 자동으로 생긴 기록(마감 조정·발주 결제·직원 상환)엔 수정·삭제 버튼 대신 출처 배지가 붙습니다. 서버가 거절하면 이유가 창에 한 줄로 뜹니다(전엔 눌러도 아무 일 없어 보였음)
- **삭제 직후 목록이 바로 갱신** — 저장·삭제 직후 2초 안에 목록을 다시 읽으면 옛 목록이 보이던 앱 전역 결함 수정
- **푸드코트 배송 조건** — 결제 설정에 «이 금액 이상 무료배송 / 미만이면 배송비» 두 칸(브랜드와 같은 카드). 배송 조건 문장에 통화 기호가 붙습니다(«RM 300.00 이상 무료 · 미만 RM 15.00»)
- 🔒 MainLayout 배치 2줄 — Fable 판정 + Irene 승인·사인오프 → print-guard `--bless` 8/8 · verify-all --full 23/23 · 클릭 흐름 3종(현금 6/6 · 전환 7/7 · 푸드코트) · 고장주입 반증 4건 · 운영 DB 변경 0


### 2026-09-25 13:24 (SW 5.63-owner-hat · 백업 20260925_131833 · 스모크 10/10)
- **오너 모자** — 한 아이디에서 오너까지 고를 수 있습니다. 시스템 관리자가 「오너 · 매장」을 부여하면 그 사람의 「Choose where to work」에 오너 카드 1장(소유 매장 전체, 여럿이면 «첫 매장 +N»)이 생기고, 고르면 오너 화면으로 들어갑니다. 지금까지는 역할마다 별도 아이디를 만들어야 했습니다
- 자격 부여 화면에 **역할 선택**이 생겼습니다 — 매장 관리자 / 오너. 전에는 매장 관리자 하나뿐이었습니다
- 🔴 **그 부여 화면을 여는 버튼이 없었습니다** — Managers 목록의 「Manager Details」 창 안에 있는데, 2026-03-03 정리 작업이 창을 여는 코드를 지워 2026-08-20 이후 메뉴로는 열 수 없는 상태였습니다(운영 v3.101 포함). 행 아이콘 `≡` 로 진입점을 복원했습니다. 잘못 부여된 데이터는 없습니다 — 문이 없었을 뿐입니다
- 부여 = 소유행 생성이라 **운영 DB 변경 0건**. 회수하면 다음 요청부터 본래 계정으로 되돌아갑니다(로그아웃 아님). 오너 모자를 쓴 사람에게 오너 청구서가 생기지 않습니다
- Fable 게이트 PASS(마커 `33b588d651c1`) — 보안 경계 2건 diff 대조·권한 판정처 증가 0 · verify-all 23/23 · mount sweep 크래시 0 · 실브라우저 3회 연속 2/2 · 고장주입 반증 · print-guard 8/8
...
- 🔴 브랜드 공급업체 «매장에 공유» — 공유가 실패하면 창에서 이유가 바로 지워지던 버그 수정. 이유는 «매장 이름 — 이유»로 남는다
- 옵션 상품이 있는 업체는 공유 창을 여는 순간 «복사할 수 없음» 안내와 함께 잠긴다
- 브랜드 업체 카드에 «매장 N곳 중 M곳에 사본» / «아직 공유한 매장 없음» · 창에 «사본 없는 매장 모두 선택» · 삭제 확인에 «매장 사본은 남음»

### 2026-09-24 21:38 (SW 5.60-brand-supplier-copy · 버전 미상승 · 백업 20260924_213814)
- 브랜드 공급업체 «매장에 공유» = **매장 사본**. 브랜드 업체 카드의 «Share to stores» 로 고른 매장마다 업체·상품 전체의 사본이 생기고, 이후 브랜드와 매장은 서로 따로 수정한다(동기화 없음)
- 브랜드 → 매장 공급업체 **상속 종료** — 매장 목록·권한·켜기/끄기는 매장 자기 업체만
- 운영 이전: with MIN Cafe 가 실제로 쓰던 브랜드 업체 23곳을 매장 사본으로(껐던 JASMINE·UGS 는 꺼진 채) — 재료 연결 301 · 발주 31 · 청구서 20(RM 4,745.47 발행자) · 원가이력 20 함께 이동, 금액·상태·결제 무변경. 한 번도 안 쓴 6곳은 만들지 않음(브랜드가 다시 공유 가능)
- 매장 연결이 가리키던 **지워진 상품**(New Seoul Mart 멸치액젓 2개)은 사본에도 지워진 채 옮겨 지금 모양 그대로 유지 — 매장에서 새 상품에 다시 연결 권고

### 2026-09-24 20:18 (SW 5.59-owner-po-view · 버전 미상승 · 백업 20260924_201241)
- 오너 — «Order History» 에서 소유 매장을 골라 그 매장 발주를 **보기**(목록·상세·인쇄·인보이스·영수증). 발주를 만들거나 받거나 결제하는 버튼은 오너에게 없다 — 오너의 주문 행위는 «PO Approvals» 승인. 전에는 메뉴에 보이는데 들어가면 거부(403)였다
- 오너 메뉴 «Purchase Order»(작성 화면)는 발주 목록으로 이동

### 핫픽스 — 2026-09-24 17:52 · 18:17 · 19:37 (SW 5.58 유지 · 버전 미상승 · 백업 20260924_174641 / 20260924_181207 / 20260924_193115)
- 🔴 보안·돈 — 옛 결제 기록 API(`/api/invoices/:id/payment`)가 로그인만 하면 아무 청구서나 결제완료로 찍었다(구독 청구서면 돈 안 내고 정지 해제 가능) → 폐기(410). 운영 사용 기록 0 · 화면 호출 0. 자동검사는 실제 결제 경로(결제 제출 → 관리자 확인)로 이전
- 🔴 보안 — 오너 «매장 가져가기» 구멍: 누구나 오너로 가입해 소유자 없는 남의 매장(운영 실매장 8곳, 영업 중 매장 포함)을 승인 없이 자기 매장으로 가져갈 수 있었다 → 자기가 만든(이미 연결된) 매장만 오너로 올리고, 아니면 거부. 소유자 없는 매장 검색도 같은 조건으로
- 🔴 보안 — 옛 가입 주소(`/api/auth/register`)가 로그인 없이 요청에 적힌 역할을 그대로 저장해 누구나 System Admin 계정을 만들 수 있었다 → 폐기(410). 운영 SA 는 1명뿐 — 탈취 흔적 없음
- 비밀번호 찾기 — 이메일 인증 전 계정에는 재설정 메일이 조용히 안 가던 것 수정
- 운영 데이터(Irene 지시): with MIN Cafe 발주 오너 승인 OFF · gitconsulting 에 with MIN Cafe 매장관리자 자격 · 옛 K-DINE Brand 계정(id 11) 이메일 → kdine-brand@gitconsulting.group · 오너 계정 withmin_owner(irene@gitconsulting.group, id 64) 신설·with MIN Cafe 소유 연결

## v3.102 — 2026-09-24 (SW 5.58-seo-canonical-news-20260924)

### 인보이스 대조
- 인보이스 대조 — 한 번 짝지어 준 이름을 다음 인보이스에서 훨씬 잘 알아봅니다(줄번호·잡음이 붙어도, 반대로 사전이 지저분해도). 이미 잘 붙은 이름은 다음 저장에서 망가지지 않습니다
- 인보이스 대조 — 총액이 줄 합계와 안 맞아 저장이 막히면 「총액대로만 저장」으로 적은 총액대로 저장·결제할 수 있습니다. 그때는 줄 단가가 저장되지 않고 원가에도 반영되지 않으며, 화면에 그 사실이 표시됩니다

### 홈페이지 검색 노출 (SEO)
- SEO — 검색엔진이 홈·요금·기능·블로그 글마다 **서로 다른 제목·설명·본문**을 받도록 백엔드가 페이지별 HTML 을 만든다(`/seo-html`, 운영 nginx 교체 뒤부터 적용). 🔴 원인: 크롤러가 받는 첫 HTML 이 모든 주소에서 똑같았다(홈 = /pricing 바이트 동일, canonical 0개)
- SEO — 없는 블로그 글 주소는 404, 뉴스 글의 대표 주소를 /news/ 로(전에는 /blog/ 로 선언해 sitemap 과 어긋남), sitemap 117개
- SEO — `www`·끝 슬래시 주소 301(마케팅 주소만 www→apex — POS 화면을 www 로 쓰는 기기의 저장 설정 보호). nginx 초안 `docs/nginx-purplehere.com.conf`
- 블로그 — 새 글 주소가 겹치면 숫자 대신 `-2`, 중국어 제목은 영어 주소+`-zh`. 기존 글 주소는 그대로, 본문만 고쳐 저장해도 주소가 바뀌지 않는다(전에는 저장마다 주소를 다시 만들었다)
- 운영 nginx 교체 완료(Irene sudo) — 마케팅 주소만 백엔드가 HTML 을 만들고, 실패 시 정적 화면으로 복귀

### 운영
- 운영 디스크 정리 — 배포 백업 보관 기간을 30일에서 7일로(최신 10개는 그대로 보존). 112GB 확보, 사용률 81% → 32%

## v3.101 — 2026-09-24 (SW 5.56-soa-period-20260924)

### 받은 주문 (판매자)
- **「품목 수정」** — 출고 전(주문 접수됨·확인됨) 주문의 품목을 판매자가 직접 고친다(수량·교체·삭제·추가). 그전에는 **판매자에게도 구매자에게도 바꿀 화면이 없었다**
- **주문 금액은 올릴 수 없다** — 발주서는 구매자가 승인한 금액. 더 받아야 하면 종전처럼 거부하고 구매자가 재발주
- 상품은 **이름으로 검색**해 고른다(가격·단위가 항목 아래 표시)
- **「주문 추가」** — 전화·메신저로 받은 주문을 판매자가 구매처 대신 올린다. 오너 승인이 켜진 매장은 **승인 대기로 시작**(승인 우회 없음)
- 수정·추가 모두 구매처에 **무엇이 어떻게 바뀌었는지** 메일과 주문 화면 배너로 안내. 이유 적기는 선택

### 자격 (System Admin)
- 사용자 관리에 **「자격」** — 한 사람에게 다른 매장 자격을 주고 없앤다. 자격이 둘 이상이면 로그인 때 고르고, 로그인 뒤에도 화면에서 바꾼다(**로그아웃 왕복이 사라진다**)
- 🔴 원인: 전환 기능은 이미 있었는데 **부여할 화면이 없어** 아무도 자격을 못 받고 있었다
- 본인 원래 자격은 늘 있고 없앨 수 없다. 지금은 「매장 × 매장 관리자」 조합만

### 월 정산서 (SOA)
- **「January 2000」·미래 발행일·기간 「-」 해결** — 뜻이 다른 값을 한 자리에 겹쳐 쓴 것이 원인이었다(수집 범위 시작이 라벨 자리에, 마감일 기준점이 발행일 자리에)
- 발행일 = **만든 날** 고정 · 기간은 **행에 저장**되어 화면·메일에 그대로
- **기간 고르기**(지난달 / 이번달 / 직접 입력) 추가
- 지난 정산에서 빠졌던 오래된 미납분도 **같이 묶인다**(전에는 영구 누락)
- 메일에 **「N일 안에 결제」** 안내
- 정산서를 취소·삭제하면 안에 묶인 거래 인보이스가 **풀려** 다음 정산서에 다시 잡힌다
- 정산서 번호가 분 단위라 **취소 후 즉시 재발행이 중복으로 막히던 것** 수정

### 내부 (날짜 정확도)
- 🔴 **공용 날짜 경계 함수가 하루씩 밀려 있었다** — 타임존 오프셋 계산에서 밀리초가 빠져 「하루의 끝」이 다음 날 00:00:00.997 이 됐다. 예약·금고·주문 기록 등 13곳이 쓰는 함수로, 날짜 조회 시 다음 날 첫 1초가 섞이고 있었다. 테스트 8건 신설(옛 코드로는 4건 실패)

## v3.100 — 2026-09-24 (SW 5.52-seller-order-amend-20260924)

### 받은 주문 (판매자)
- **「품목 수정」** — 받은 주문의 품목을 판매자가 직접 고친다(수량 변경 · 다른 상품으로 교체 · 줄 삭제 · 줄 추가). 그전에는 **판매자에게도 구매자에게도 품목을 바꿀 화면이 없었다**
- 고칠 수 있는 때는 **출고 전**(주문 접수됨 · 확인됨). 배송을 시작했거나 일부라도 수령된 주문은 수정되지 않는다
- **주문 금액은 올릴 수 없다** — 발주서는 구매자가 승인한 금액이라, 더 받아야 하면 종전처럼 주문을 거부하고 구매자가 다시 발주한다. 금액이 넘으면 저장 버튼이 잠기고 서버도 거부한다
- 고를 수 있는 상품은 **내 카탈로그 중 그 구매자의 재고 자리가 있는 것**만. 단가는 서버가 카탈로그에서 채운다
- **이유 적기는 선택**

### 발주 (구매자)
- **판매처가 주문을 고치면 안내가 온다** — 메일(무엇이 어떻게 바뀌었는지 표 + 전후 금액)과 주문 상세 화면 상단 배너. 이유가 적혀 있으면 함께 보인다
- 이유가 비어 있어도 **변경 내역은 항상** 간다

## v3.99 — 2026-09-20 (SW 5.45-margin-editor-20260920)

### 브랜드 프로덕트
- **편집창 마진도 목록과 같은 계산기** — 사는 값은 「우리 재고 단위 하나」(g당), 파는 값은 「판매 단위 하나」(kg당)라 그냥 빼면 안 된다. 10kg 로 사서 kg 으로 파는 쌀이 **+124537%** 로 보이던 원인. 이제 Sawah Mas = 원가 RM 34.50 → 판매 RM 43.00 / 10 kg/pack (마진 RM 8.50 · 20%)
- 편집창에도 **「단위 환산 불가」** 표시 — 바꿀 수 없는 단위 조합에서 숫자를 지어내지 않는다
- 계산을 테스트로 박제(`productMargin.test.ts` 5건) — 단위 환산을 빼면 4건이 실패한다

## v3.98 — 2026-09-20 (SW 5.44-margin-unit-conversion-20260920)

### 브랜드 프로덕트
- **마진을 판매 단위 기준으로 계산** — `unit_cost` 는 「1단위 값」이 아니라 **기준양(base_quantity) 만큼의 값**이다. 김치 재고아이템 = 10,000g 에 RM 48 → 1kg 판매 원가 **RM 4.80**, 마진 RM 2.70(36%) (전에는 −540%)
- 판매 단위와 원가 단위를 바꿀 수 없으면 「단위 환산 불가」로 표시
- 목록과 편집창이 같은 숫자를 쓰도록 계산기 단일화(`utils/productMargin.ts`)

## v3.97 — 2026-09-20 (SW 5.43-margin-warning-and-reprice-20260920)

### 브랜드 프로덕트
- **마진 경고** — 공급업체 가격(또는 레시피 원가)과 비교한 마진을 금액·비율로 표시하고, 문제일 때 무엇이 비었는지 구분(마진 적음 / 역마진 / 판매가 없음 / 원가 없음)
- 연결 없음은 기존 `Not linked` 배지가 계속 담당

### 발주
- **「현재가로 갱신」** — 판매자가 값을 바꿔도 담아 둔 초안은 담을 때 가격을 들고 있다. 보내기 전에 현재가로 맞추고 총액을 다시 계산(`POST /purchase-orders/:id/refresh-prices`)
- **이미 보낸 발주는 바뀌지 않는다** — 받았거나 결제한 건을 소급하면 재고·청구서·마감이 흔들린다. 보낸 뒤 금액 차이는 인보이스 대조가 담당


## v3.96 — 2026-09-18 (SW 5.42-recipe-card-clarity-20260918)

### 레시피
- **「우리 원가」 배지** — 원가가 두 줄(브랜드 원가 ↔ 우리 원가)로 나오는 레시피에 배지를 붙여 이유를 드러냄. 두 줄은 **그 매장이 재료 원가를 따로 정해 둔** 레시피에만 나온다
- **준비 레시피의 권장 판매가 칸 숨김** — 메뉴에 붙지 않는 레시피라 값이 없는 것이 정상
- **재료 0개 레시피 경고** — 「재료가 없어 원가가 0이고, 팔려도 재고가 줄지 않습니다」

## v3.95 — 2026-09-18 (SW 5.41-personal-money-and-receipt-20260918)

### 발주 결제
- **결제수단 «개인금액» 추가** — 개인 돈으로 산 것. 결제 시점엔 회사 금고가 움직이지 않고, 발주·거래청구서는 결제됨(공급업체는 실제로 받았으므로)
- **개인금액 정산** `POST /purchase-orders/:id/reimburse` — 갚을 때 비로소 회사 돈이 나간다. 현금이면 열린 시프트 금고에서 `source='reimbursement'`, 이체면 금고 무접촉. 이중 정산 409 · 비개인금액 400 · 되돌리기 대칭
- **「개인금액 미정산」 필터**(건수 표시) — 갚을 목록은 새 표가 아니라 발주 행
- **영수증 첨부** `POST /purchase-orders/:id/upload-receipt` — 인보이스(앞으로 낼 청구서)와 분리. 결제된 발주에만 붙는다(미결제 400)
- 결제수단 목록 하드코딩 2곳(발주·청구서) → `PAYMENT_METHODS` 상수 1곳

## v3.94 — 2026-09-18 (SW 5.40-cash-default-direct-purchase-20260918)
- 발주 대기의 「Receive + pay」(직접 구매) 기본 결제수단을 **현금**으로. 다른 모드는 계좌이체 기본 유지
...
### 통화 표기
- 금액이 찍히는 모든 자리를 기호(RM)로 정렬 — 인보이스 발행·연체·결제확인 메일, 발주·반품·거래인보이스 계열 9개 메일(`fmtMoney`), 발주 PDF/인쇄 합계, **왓츠앱 공유 문구**, 신용한도, 발주 화면, 계약 인보이스, 할인 금액, 구독 월요금 라벨, 번역 4개 언어
- 통화를 «안내»하는 자리는 ISO 코드(MYR) — 결제설정 라벨 5역할, 결제수단 미설정 안내 4역할, 브랜드·국가·플랜·지갑 통화 칸
- 주문 활동기록 금액 표기(🔒 인쇄 보호파일 · 표기 2줄 · 인쇄 로직 무접촉)
- `check-currency-display.js` 양방향 차단으로 승격(금액 자리 코드 / 안내 자리 기호 둘 다 실패 처리)

### 준비된 재료
- 레시피 저장·삭제 후 재료 목록 즉시 갱신 — 준비 재료를 만든 직후 새로고침 없이 선택 가능
- 준비 레시피 삭제 규칙: 재고가 남았거나 다른 레시피가 쓰면 거부(사유 표시), 지울 수 있으면 재료를 비활성으로 보존
- ING-UNI-002 확장 — 재료 출처로 «레시피»를 인정(정상 준비 재료 오탐 해소 + 우회 삭제 탐지)
- health-check 「준비 레시피 삭제 게이트」 계약 3건 신설

### 검증
- verify-all --full 21/22 → fable_note 기입 후 21/21 · 실브라우저 mount sweep 675초 크래시 0 · health-check 251/251 · 인쇄 보호파일 8/8 · 고장주입 18/18 · 마이그레이션 없음


## [Unreleased] — 미배포 (개발서버만)

### 2026-09-21
- [Claude Code] 월 청구(SOA) 점검 — 「2000년」 메일·미래 발행일·빈 기간 칸의 원인 확인. 고치는 방향은 판정 대기(코드 변경 없음)

## 2026-09-22 배포 #2 (SW 5.51-bg-supplier-unshared-20260922) [Claude Code]
- 브랜드제너럴도 공급업체를 외부 공급업체로 등록하고 바로 상품을 등록할 수 있습니다
- 브랜드제너럴이 새로 등록한 공급업체는 매장에 자동으로 공유되지 않습니다 (이미 공유 중인 업체는 그대로)
- 오류 창 제목이 «Error» 로 제대로 나옵니다

## 2026-09-22 배포 (SW 5.50-supplier-external-only-20260922) [Claude Code]
- 매장·푸드코트에서 공급업체를 추가하면 외부 공급업체(EXTERNAL)로 등록되고 바로 상품을 등록할 수 있습니다
- 매장·푸드코트의 예전 방식(OWN) 공급업체는 외부 공급업체로 옮겨졌습니다 (운영 3건)

## 2026-09-21 배포 #4 (SW 5.49-sitemap-20260921) [Claude Code]
- 검색엔진용 사이트맵에 블로그·뉴스 글 전부와 빠져 있던 페이지가 들어갑니다 (11개 → 116개 주소)
- 홈 구조화 데이터 중복 정리

## 2026-09-21 배포 #3 (SW 5.48-landing-seo-20260921) [Claude Code]
- 랜딩 페이지 구조화 데이터(회사·FAQ·기사·브레드크럼 등)가 검색엔진에 실제로 출력됩니다
- 랜딩 상단·모바일·하단 메뉴가 실제 링크가 되어 검색엔진이 페이지를 따라갈 수 있고, 새 탭 열기가 됩니다
- 페이지마다 두 개씩 나가던 메타 설명이 하나로 정리됩니다

## 2026-09-21 배포 #2 (SW 5.47-order-date-everywhere-20260921) [Claude Code]
- 브랜드·푸드코트·공급업체의 받은 주문 목록이 구매자가 보낸 순서(발주일)로 정렬되고 기간 필터·시각도 발주일 기준입니다
- 공급업체 대시보드 최근 주문이 발주일 순이고, 구매자가 아직 보내지 않은 발주는 나오지 않습니다
- 발주서 인쇄본과 공급업체 발주 메일의 날짜가 발주일입니다

## 2026-09-21 배포 (SW 5.46-brand-visibility-po-order-20260921) [Claude Code]
- 「All franchises」 브랜드 상품이 배정된 Brand General 계정으로 저장돼도 그 브랜드 매장 발주 화면에 나옵니다
- 가맹점에 보이는 「Buyers outside my franchises」 상품을 매장에서 연결할 수 있습니다
- 브랜드가 받은 주문(Sales Orders)·발주 메일·인쇄본에 브랜드 프로덕트 이름과 SKU 가 나옵니다
- 발주 화면 목록 가격이 실제 주문 단위 가격으로 나옵니다 (예: 43.00/pack)
- 발주 기록이 보낸 순서(발주일)로 정렬되고 날짜 열이 「Order date」가 됩니다
- 영어 화면 발주 상세의 「원가 대조」 버튼이 영어로 나옵니다

## v3.91 (2026-09-17) — 화면·메일의 통화를 전부 RM 으로 · 할인 인보이스 · 발주 카탈로그

### 2026-09-17 #7 — 「MYR」이 보이던 자리를 전부 **RM** 으로 (화면 + 메일 + 문서)
- Irene 「MYR 안쓴다니까. RM이라고」. 1·2차에서 놓친 자리가 남아 있었습니다 — 제 검사가 **화면에 직접 찍히는 글자를 못 보고** 있었기 때문입니다.
- 이번에 **53곳**을 고쳤습니다: 인보이스 대조 화면(「총액 (RM)」·「발주 총액: RM …」), 결제설정 4개 화면(「Bank Transfer (RM)」·「Additional Charges (RM)」), 요금제 3개, 구독 2개(할인 입력 칸), 재료·레시피 4개 탭(「Unit Cost (RM)」), 관리자 대시보드, 브랜드·매장 목록의 통화 표시, 시스템 상품 가격, 플로어플랜 금액.
- **메일과 문서도 같이** — 알림 메일 금액, 인보이스 메일 금액, 거래명세서(브랜드·푸드코트·공급업체) 「Total due」, 발주 인보이스 문서, 지갑 잔액 문구, 추천 커미션·출금 기록.
- 저장하고 비교하는 값은 **코드(MYR) 그대로**입니다 — 결제사가 코드만 받기 때문입니다. 원화·달러 매장은 ₩·$ 로 그대로 나옵니다.
- 같은 실수가 다시 나지 않도록 **자동 검사**를 만들어 배포 게이트에 넣었습니다. 만들자마자 메일·기록 쪽 11곳을 더 찾아냈습니다.

### 2026-09-17 #8 — 할인해 준 것이 인보이스에 보입니다
- 브랜드·푸드코트의 「발행」 버튼이 **매장별 할인을 한 번도 읽지 않고** 정가로 청구하고 있었습니다. 게다가 합계가 0이면 **발행 자체를 건너뛰어**, 100% 할인해 준 매장은 인보이스가 아예 생기지 않았습니다.
- Irene 「인보이스가 0원 무료라도 발행해서 할인해준 건 보여줘야 해」 → 정가·할인액·합계가 줄로 찍히고, 0원이면 **「결제완료」로 발행**됩니다.
- 수익 리포트에 **할인 합계** 칸이 생겼습니다.
- 새로 발행되는 인보이스는 「받는 매장」이 제대로 채워집니다(전에는 비어 있어 「청구 대상 없음」으로 보였습니다).

### 2026-09-17 #9 — 브랜드 상품이 가맹점 발주 카탈로그에 안 뜨던 문제
- 「외부 구매자에게도 공개」를 켜면 **자기 가맹점에서 오히려 사라졌습니다.** 화면 문구는 「밖에서도 주문할 수 있다」인데 실제로는 가맹점 몫에서 빠지고 있었습니다.
- 브랜드가 등록한 상품은 **설정과 무관하게 자기 가맹점 카탈로그에 나옵니다.** 운영 실측으로 with MIN Cafe 에서 18개가 가려져 있었습니다.

### 2026-09-17 #10 — 재고아이템 원가를 공급업체 가격과 맞췄습니다 (운영 28건)
- Irene 「공급업체 가격이 원가인데 기본적으로 같게 넣어줘야 해」 → 연결된 공급업체 가격을 그대로 원가로 넣었습니다.
- 그중 9건은 **1g 값이 1,000g 자리에** 들어가 수백~수천 배 작던 것이었습니다(대파 0.01→10 · 물엿 0.0074→133 · 고추장 0.0099→167.57).
- 레시피 원가가 따라 움직입니다(불고기덮밥 10.85 → 10.94).

### 함께 나간 것
### 2026-09-16 #4 — [Codex] 레스토랑 문의 화면 공통 UI 컴포넌트 통일
- Operation/System Inquiry 화면의 검색·필터·버튼·폼을 기존 공통 컴포넌트로 통일
- 개발 반응형 실측 8/8 통과, 운영 배포 없음


## v3.89 (2026-09-17) — 발주 화면 긴급 수정 · 「1 박스 → 몇 개」 고치기

### 2026-09-17 #5 — 🔥 발주 화면이 품목을 담으면 죽던 문제 (긴급)
- **오늘 오전 배포한 배송비 코드가 발주 화면을 깨뜨렸습니다.** 품목을 담는 순간 화면 전체가 흰 화면(오류 카드)이 됐습니다. 제 잘못입니다.
- 원인은 **계산 순서** — 배송비를 계산하는 줄이 아직 만들어지지 않은 «이 발주의 통화» 를 먼저 쓰고 있었습니다. 장바구니가 비어 있으면 그 줄이 실행되지 않아 **화면을 열어보는 검사로는 잡히지 않았습니다.**
- 순서를 바로잡았고, **같은 종류를 배포 전에 자동으로 잡는 검사**를 새로 만들어 배포 게이트에 넣었습니다. 원래 버그를 다시 넣어 검사가 실제로 막는 것까지 확인했습니다.
- ⚠ 화면에서 **Ctrl+Shift+R** 한 번 눌러 주세요 — 브라우저가 옛 파일을 들고 있으면 같은 오류가 계속 보입니다.

### 2026-09-17 #6 — 무게로 사는데 «개»로 세던 재료 31건을 kg으로 (운영 반영)
- Irene 「발주처가 kg 인데 우리 재고가 piece인게 말이 돼?」 → 말이 안 되는 게 맞았습니다. **저울에 달아서 주문하는** 소고기 양지·연어·닭고기류·토마토·고추·시금치·배추·깐마늘 등 **31건**의 재고 단위를 **kg**으로 바로잡았습니다.
- **숫자는 그대로 두고 라벨만** 바꿨습니다. 현재고·원가·과거 기록·발주 이력은 건드리지 않았습니다.
- **박스·병으로 사는 것은 그대로 뒀습니다** — 진간장은 표기가 「1 L」이어도 실제로 사는 단위는 **병**이라 병으로 세는 게 맞습니다. 그런 것은 「1병 = 1L」처럼 값을 넣어 풀면 됩니다.
- **홍고추 1건은 건너뛰었습니다** — 레시피가 쓰고 있어서 단위를 바꾸면 그 레시피 수량의 뜻이 말없이 바뀝니다. 레시피를 먼저 손보셔야 합니다.
- 결과: 값을 정하셔야 하는 항목이 **273건 → 238건**으로 줄었습니다.

### 2026-09-17 #4 — 「1 박스를 받으면 몇 개 늘어나는지」를 화면에서 고칩니다
- 전에는 거래처를 **연결할 때 한 번만** 물어보고, 그 뒤에는 고칠 칸이 어디에도 없었습니다(연결 끊기뿐). 재료 화면과 브랜드 재고아이템 화면 **두 곳 모두**에 생겼습니다.
- 목록에 **「1 box → 50 piece」** 처럼 보이고, 아직 안 정한 것은 **「확인 필요」** 로 뜹니다. 연필을 누르면 바로 고칩니다.
- 창은 문장으로 물어봅니다 — 「이 업체에서 **1 box** 를 받으면 / 내 재고는 **[ 50 ] piece** 늘어납니다」. 적어 두신 규격으로 계산되는 값이 있으면 **한 번 눌러 넣을** 수 있습니다.
- **값이 1 그대로여도 저장하면 「확인함」으로 기록**되어 목록에서 빠집니다. 「업체의 개 = 우리 팩」처럼 답이 1인 경우(차류 125건)를 위한 것입니다. 나중에 그 업체가 단위를 바꾸면 다시 확인을 요청합니다.
- 저장하면 **아직 받지 않은 발주에도** 같은 값이 적용됩니다. 지금 재고 수량과 이미 받은 발주는 **바뀌지 않습니다**.
- 화면에서 「단위 변환 / Unit conversion / 환산」 같은 말은 **한 군데도 쓰지 않습니다**(4개 언어).

### 2026-09-17 #9 — K-DINE IPC 공급업체 상품 93건 등록 (운영 반영)
- Irene 이 주신 목록 그대로 **공급업체 8곳 · 상품 93건**을 K-DINE IPC 앞으로 넣었습니다(TaiYang Fresh · EH Foodart · New Seoul · LSH · Jasmine · Fandbee · Shopee · Direct). 무게·용량도 표시하신 기준 그대로입니다.
- **기존 것은 하나도 건드리지 않았습니다** — 브랜드가 쓰는 공급업체 상품 313건이 전후로 완전히 같습니다.
- ⚠ **가격은 전부 `RM 0.00`** 입니다(목록에 가격이 없었습니다). 인보이스 대조의 「이 가격 쓰기」나 상품 편집으로 채우시면 됩니다. 0은 「아직 안 정함」이라 원가에 영향이 없습니다.
- 규격 표기 3건은 시스템이 못 읽어 이렇게 넣었습니다 — 랩 `450mm/pkt`→1팩 · 젓가락 `40pair/pkt`→40개/팩 · VIT라면 `840g/6pkt/1CT`→5,040g/박스.

### 2026-09-17 #8 — 원가를 공급업체 가격에 맞췄습니다 (운영 반영)
- **연결된 공급업체 가격이 그대로 원가**여야 하는데 28건이 어긋나 있었습니다. 그중 9건은 **「1g 값」이 「1,000g 값」 자리에** 들어가 수백~수천 배 작았습니다(대파 0.01→10 · 물엿18kg 0.0074→133 · 고추장 0.0099→167.57).
- 고치니 **레시피 재료비가 곧바로 따라 움직였습니다**(불고기덮밥 10.85→10.94). 원가가 레시피까지 제대로 연결돼 있음을 확인했습니다.
- ⚠ 크게 오른 3건은 확인이 필요합니다 — **김치 10kg 48은 맞습니다**(전 값이 절반이었습니다). 소시지·뚜껑은 공급업체 쪽에 「한 묶음에 몇 개」가 비어 있어 생긴 값입니다.

## v3.90 (2026-09-17) — 리포트 매출에서 «취소된 주문» 제거 · 화면마다 다르던 매출 기준 통일

### 2026-09-17 #7 — 매출 숫자 바로잡기
- **리포트가 취소된 주문을 매출로 세고 있었습니다.** 「결제는 됐는데 취소된」 주문이 그대로 매출에 들어가 있었고, 최근 60일 기준 **5,213.30 이 부풀려져** 있었습니다. 그래서 같은 브랜드를 퍼포먼스 화면에서 보면 숫자가 달랐습니다.
- **평균 주문액**이 「매출 ÷ 전체 주문수」였습니다 — 취소·진행중 주문이 분모에 섞여 실제보다 낮게 나왔습니다. 이제 완료된 주문 수로 나눕니다.
- **완료 건수·완료율**이 「completed」만 세어 서빙까지 끝난 주문이 빠져 있었습니다.
- **구독 화면의 매장별 매출**도 같은 이유로 조금 적게 나왔습니다(60일 기준 107.06).
- **푸드코트 리포트·오너 리포트에도 똑같은 문제**가 있어 함께 고쳤습니다.
- 확인 결과 **틀리지 않았던 것**: 브랜드 퍼포먼스 수치, 품목·카테고리 통계, 수익 리포트(인보이스) — 전부 데이터와 정확히 일치했습니다.
- 이제 **모든 화면이 같은 기준**을 씁니다(완료 또는 서빙까지 끝난 주문, 삭제된 것 제외). 기준이 어긋나면 배포가 자동으로 막힙니다.
- ⚠ 리포트의 **「주문 수」는 늘어납니다** — 전에는 취소·미결제 주문이 빠져 있었습니다. 실제로 들어온 주문 수이고, 이행률도 이 기준입니다.

## v3.88 (2026-09-17) — 준비된 재고 · 발주 배송비 · 재료 정리

### 2026-09-17 #3 — 재료 데이터 정리(운영 반영)
> 코드 배포 + 운영 데이터 정리를 함께 했습니다. **매장 메뉴는 전 과정에서 한 글자도 바뀌지 않았습니다.**

- **사라져 있던 재료 4개를 되살렸습니다** — 떡 · 해바라기유 · 쌀 · 식용유. 살아 있는 레시피 21줄이 쓰는 재료인데 목록과 발주 화면에서 빠져 있어 **살 수가 없는 상태**였습니다.
  - ⚠ 정정: 처음엔 「꺼진 재료라 원가도 차감도 0」이라고 적혀 있었지만 **사실이 아니었습니다**. 실제로는 원가도 차감도 정상이었고(떡볶이 원가 4.22 에 떡 1.82 포함), 진짜 피해는 **안 보여서 못 사는 것**이었습니다. 잘못된 설명도 고쳤습니다.
- **레시피가 쓰는 재료는 이제 끌 수 없습니다** — 끄려고 하면 「어느 레시피가 쓰는 중」이라고 알려 드립니다. 먼저 레시피를 바꾸셔야 합니다. 같은 일이 다시 생기지 않게 막은 것입니다.
- **번호가 겹쳐 있던 재료 13건에 새 번호를 줬습니다**(먼저 만든 쪽 번호는 그대로, 나중에 만든 쪽만).
- **주인 없는 공급업체 연결 15건 정리** — 12건은 지우고, 발주 이력이 붙어 있는 3건은 이력을 남기려고 «사용 안 함»으로만 바꿨습니다.
- **입고 수량이 팩 수로만 늘던 10건을 고쳤습니다** — 「소스통 1팩 = 50개」인데 1개로 들어오던 것들입니다. 지금 재고 숫자는 그대로이고 **앞으로 들어오는 것부터** 맞습니다.
- **남은 273건은 Irene 님이 값을 정하셔야 합니다** — 「1캔이 몇 개인지」처럼 사람만 아는 숫자라 기계가 추측하지 않습니다. 화면에서 넣으시면 목록에서 자동으로 빠집니다.

### 2026-09-17 #2 — 주문 수정 기록이 안 남던 문제 · 재료 분류 합치기(운영 반영)
- **주문을 고쳐도 기록이 안 남고 있었습니다.** 「누가 언제 뭘 고쳤나」가 저장 단계에서 조용히 버려지고 있었고(화면엔 아무 표시도 없음) 이제 정상 저장됩니다.
- **재료 분류가 두 벌로 갈라져 있던 것을 한 벌로 합쳤습니다** — 같은 이름 분류가 15쌍 겹쳐 있었습니다. 매장 재료 27건을 매장 분류로 옮기고, 비어 버린 브랜드 분류 14개는 «사용 안 함»으로 뒀습니다. 되돌리기 파일이 있습니다.

### 2026-09-17 #1 — v3.88 배포 (준비된 재고 · 발주 배송비 · 브랜드 메뉴 정리)
- **미리 만들어 두는 재료(준비된 재고)** — 레시피에 「이 결과물을 재료로 씁니다」 스위치가 생겼습니다. 켜면 그 결과물이 재료 목록에 자동으로 생기고, 재고 화면의 **「만들기」** 한 번으로 원재료가 빠지고 준비 재료가 늘고 원가까지 실제 쓴 돈 기준으로 잡힙니다. 판매할 때 빠지는 방식은 **그대로**이고, 준비용 레시피는 메뉴에 붙지 않습니다(두 번 빠지는 것 방지).
- **발주 배송비** — 거래처마다 「얼마 이상 무료 / 미만이면 얼마」 두 칸을 정하면 발주서에 **품목 합계 · 배송비 · 총액** 세 줄로 자동 계산됩니다. **비워 두면 «무료»가 아니라 «규칙 없음»** 입니다(지금 거래처 42곳 전부 비어 있어 아무도 배송비를 물지 않습니다).
- **메뉴의 주인은 매장입니다** — 브랜드 메뉴가 매장과 달라도 됩니다. 브랜드 화면에는 보기 전용 **「매장 자체 메뉴」** 탭과 「어느 매장이 만든 것」 배지만 추가했습니다. **브랜드 레시피는 브랜드제너럴 것** 그대로이고 매장은 고칠 수 없습니다.

## v3.87 (2026-09-16) — 배송 용어 통일 · 반송 메일 주소 교체

### 2026-09-16 #3 — 반송되던 메일 주소 교체(운영 반영) · 배송 용어 통일
> 주소 교체는 **운영 데이터에 바로 반영**했습니다(코드 변경 아님 → 배포 불필요).
> 배송 용어 통일은 **개발서버까지만** — 배포는 Irene 지시 때.

- **help@withmin.info 로 가던 알림을 help@k-dine.com 으로 바꿨습니다.** 그동안 계속 반송되던 원인은 «운영에는 한 번도 적용된 적이 없었기» 때문입니다 — 9/15 의 교체 기록 파일이 개발서버에서 만들어져 배포 때 운영으로 복사되는 바람에 이미 처리된 것처럼 보였습니다.
- 운영 데이터베이스 830개 칸을 전부 훑어 남은 자리를 찾았고, **메일이 실제로 나갈 수 있는 13곳**(매장 관리자 계정 1 · 매장 정보 1 · 고객문의 티켓 10 · 운영요청 티켓 1)을 바꿨습니다. 바꾸기 전 백업과 되돌리기 기록을 남겼습니다.
- ⚠ 그 계정은 **이메일이 곧 로그인 아이디**입니다 — with MIN Cafe 관리자는 이제 `help@k-dine.com` 으로 로그인하십니다(비밀번호 그대로).
- **with MIN Cafe 홈페이지 주소도 `https://k-dine.com` 으로 바꿨습니다**(Irene 지정).
- 손대지 않은 것: **과거 기록 3,400여 건**(활동 로그·원가 이력·시스템 로그) — Irene 지시로 그대로 둡니다. 여기서는 메일이 나가지 않고, 고치면 «그때 누가 했다»가 어긋납니다.
- **«배송» 용어를 하나로 통일했습니다.** 같은 단계를 Ship · Dispatch · Deliver 세 단어로 부르고 있었습니다. 이제 보내는 단계는 **배송 중 / 배송 시작**, 도착은 **배송 완료** 한 가지입니다. Dispatch 는 없앴습니다.
- **화면과 메일 알림 내용까지 4개 언어 전부**(108군데) 맞췄습니다. 한글 오역도 하나 고쳤습니다 — 보내는 단계인데 「업체가 **배송완료**로 표시」라고 떠 있었습니다.
- 발주 상태 값 자체(shipped/delivered)는 그대로입니다 — **보이는 말만** 바꿨습니다.

### 2026-09-16 #2 — 조사·검토만 (코드 변경 없음)
> 배포 대상 없음. 운영 데이터도 바꾸지 않았습니다(조회와 미리보기만).

- **K-DINE 브랜드 레시피가 매장에 제대로 갔는지 확인했습니다.** 레시피 72건은 정상적으로 매장에서 보입니다 — 브랜드 레시피는 매장으로 복사되는 게 아니라 매장이 그대로 읽어가는 구조이고, 분류·재료·연결 어디에도 끊긴 곳이 없었습니다.
- **다만 «보이는 것»과 «재고가 깎이는 것»은 다릅니다.** 9월에 올린 레시피 중 재료가 들어 있는 21건이 어떤 메뉴에도 안 붙어 있어 팔려도 재고가 안 깎입니다. 반대로 메뉴에 붙어 있는데 재료가 하나도 없는 레시피가 8건 있어 화면에는 «연결됨»으로 보이지만 깎이는 양은 0입니다.
- **애드온(치즈·계란 추가 등)은 레시피가 아니라 옵션에 재료를 연결해야 깎입니다.** 현재 매장 옵션 12개 전부 재료 연결이 비어 있습니다.
- **브랜드↔매장 메뉴·옵션 «서로 공유» 요청을 검토했습니다.** 이미 있는 것과 없는 것을 갈라 문서로 정리했습니다 — 잠금 설정은 이미 있고(The Fire 브랜드가 그 방식으로 운영 중), 없는 것은 ①매장이 추가한 메뉴가 브랜드로 올라가는 상시 경로 ②브랜드 화면에서 «이 매장이 추가함»을 보는 자리 ③브랜드 전체에 한 번에 거는 설정입니다.
- **올리기 전 미리보기 표 2개를 만들었습니다** — K-DINE 옵션은 6개가 이미 브랜드에 같은 내용으로 있고 연결만 끊긴 상태이고, with MIN Cafe 227건은 메뉴 227·분류 23·옵션 44가 새로 생깁니다(메뉴를 먼저 올려야 옵션이 붙습니다).
- ⚠ **설계 결정은 Fable 판정 대기** — 한도(429)로 5회 연속 판정을 못 받았습니다. 우회하지 않았습니다.

### 2026-09-16 — 재료 카테고리를 지울 때 «재료를 어디로 옮길지» 선택
> **운영 배포 완료** (SW `5.30-category-reassign-delete-20260916` · 09:31 UTC · 안전 게이트 10/10 · 스모크 10/10 · 백업 `/var/www/backups/20260916_092523`).
> ⚠ Fable 판정 미수령(한도 429, 오늘 3회 시도) — 우회하지 않음. 민감 판정 «일반 변경»(비대상).

- **재료 카테고리를 지울 때 그 안의 재료를 어디로 옮길지 고를 수 있습니다.** 지금까지는 재료가 하나라도 있으면 삭제가 막히고 「먼저 재료들 카테고리를 바꾸세요」라고만 나왔습니다. 옮길 곳을 고르는 칸도 한꺼번에 옮기는 길도 없어서 재료를 하나씩 손으로 옮겨야 했습니다.
- **옮길 곳 목록에 브랜드에서 내려온 카테고리도 함께 나옵니다** — 브랜드 것에는 «브랜드» 표시가 붙습니다.
- **삭제가 막힐 때 이유와 건수가 화면에 뜹니다.** 전에는 서버가 옛 형식으로 사유를 돌려줘 화면에 아무것도 안 보일 수 있었습니다.
- **옮기고 지우는 것이 한 번에 끝나거나 아무것도 안 바뀝니다** — 중간에 끊겨 재료만 옮겨지고 카테고리가 남는 일이 없습니다.
- 비어 있는 카테고리는 종전대로 확인만 받고 지웁니다. 카테고리 만들기·고치기·순서 바꾸기는 한 줄도 안 바뀌었습니다.
- 남은 일: **브랜드 카테고리로 통합하는 것은 이번에 하지 않았습니다** — 브랜드 카테고리에는 매장이 쓰는 Staff Meal·Poultry·Alcohol·Beverage·Kitchen & Supplies 5개가 없고 거기 재료가 43건이라 갈 곳을 먼저 정해야 합니다(Fable 판정 대기).

### 2026-09-15 #3 — 같은 물건이 두 줄로 뜨던 발주 목록 · 거래처 이름 · 매장 어휘
> **운영 배포 완료** (SW `5.29-seller-name-fix-20260915` · 15:21 UTC). 릴리즈 기록 `2026-09-15-link-pairs`.
> ⚠ Fable 판정 미수령(한도 429, 이날 2회 시도). 민감 판정 «일반 변경»(비대상).
> 📌 이 항목은 2026-09-16 에 뒤늦게 옮겨 적었습니다 — 그날 오후 세션이 배포까지 하고 `/개발완료` 없이 끝나 기록이 비어 있었습니다.

- **같은 물건이 발주 목록에 두 줄로 뜨던 것을 한 줄로 합쳤습니다.** 프로덕트가 재고아이템에 연결돼 있으면 재고아이템 줄 하나만 나옵니다 — 재고는 한 곳에 있는데 줄이 둘이라 같은 물건을 두 번 발주할 수 있었습니다. 매장도 같은 규칙입니다. 연결되지 않은 프로덕트는 종전대로 그대로 나옵니다.
- **이미 프로덕트로 팔고 있는 재고아이템에 «프로덕트로 등록»을 다시 누르면 막습니다.** 전에는 그대로 통과해 같은 이름 프로덕트가 두 개 생겼습니다. 재고아이템 화면에는 «판매 중: 상품명» 으로 보입니다.
- **발주 화면에 «/piece · undefined» 로 나오던 자리에 공급업체 이름이 나옵니다.** 그 목록 경로만 이름을 내려주지 않던 것으로, 연결 자체는 정상이었습니다.
- **공급업체 상품을 «고르기»와 «만들어 연결» 두 버튼으로 나눴습니다.** 기존 «External supplier» 는 업체 등록처럼 읽혔지만 실제로는 상품을 만들어 연결하는 것이었습니다. 재고아이템 화면·프로덕트 창 양쪽에 두 버튼이 나란히 있습니다.
- **매장 화면의 «Add as product» 가 «메뉴로 등록», «Also sell as product» 가 «메뉴로도 등록» 으로 바뀝니다**(4언어). 매장에는 프로덕트가 없고 메뉴가 있습니다. 브랜드는 종전대로 «프로덕트» 입니다.
- **발주·재고 목록의 거래처 이름이 회사명으로 나옵니다.** 브랜드명(«with MIN»)이 뜨던 두 자리를 회사명 단일 소스로 돌렸습니다. 회사명이 비어 있는 브랜드만 종전처럼 브랜드명으로 떨어집니다.
- 내부: 반송되는 `help@withmin.info` 주소 교체 스크립트를 만들었습니다(미리보기 기본 · `--apply` 로만 변경 · 되돌리기 스냅샷 · 메일 서버 로그인 계정은 제외). **운영 실행은 아직 하지 않았습니다.**

### 2026-09-15 #2 — 프로덕트 하나에서 재고아이템·공급업체까지 한자리에
> **운영 배포 완료** (SW `5.26-product-stock-link-20260915` → 같은 날 5.29 에 포함). 릴리즈 기록 `2026-09-15-product-stock-link`.
> ⚠ Fable 판정 미수령(한도 429). 민감 판정 «일반 변경»(비대상).
> 📌 이 항목도 2026-09-16 에 뒤늦게 옮겨 적었습니다.

- **프로덕트를 등록할 때 재고아이템도 같이 만들 수 있습니다.** 지금까지는 프로덕트·재고아이템·공급업체 상품을 각각 등록하고 따로 연결해야 했습니다. 이름·단위·기준수량은 프로덕트에 적은 값을 그대로 씁니다.
- **자동으로 만들지는 않습니다** — 체크를 켠 경우에만 만듭니다. 같은 이름의 재고아이템이 이미 있으면 새로 만들지 않고 먼저 물어봅니다(«같은 물건이 두 줄» 사고 재발 방지).
- **프로덕트 창에서 «어디서 사 오나»를 바로 연결합니다.** 재료 화면까지 나갔다 오지 않아도 됩니다.
- **프로덕트 창에 «파는 기준 / 사는 기준 / 마진»이 한자리에 보입니다.** 예: 파는 기준 1 kg/pack RM 15.00 · 사는 기준 14 kg/carton RM 150.00 · 마진 RM 10.71 → RM 15.00 per kg (+40.0%).
- **발주에서 내 재고아이템에 없는 물건을 검색하면, 공급업체 상품에 몇 건 있는지 그 자리에서 알려 줍니다.** 전에는 탭을 직접 바꿔야만 알 수 있어, 살 수 있는 물건을 없는 것으로 알고 나가는 일이 있었습니다.

### 2026-09-15 — 브랜드 여러 개 쓰는 계정의 «다른 브랜드» 누락 · Profile 이메일 인증 안내
> **운영 배포 완료** (개발 반영 SW 5.25 → 같은 날 15:21 UTC 배포분 SW 5.29 에 함께 올라감).
> ⚠ 2026-09-15 작성 당시에는 «미배포» 로 적었으나 사실과 달라 2026-09-16 에 정정했습니다(운영 `sw.js` 실측).
> ⚠ Fable 판정 미수령(한도 429). 민감 판정 «일반 변경»(비대상).

- **주문이 들어오면 Sales Orders 에 빨간 점이 뜹니다.** 브랜드를 두 개 이상 가진 분은 «소속»으로 적힌 한 브랜드의 주문만 세고 있어서, 다른 브랜드로 들어온 주문은 목록엔 보이는데 점이 안 떴습니다. 브랜드 매니저는 종전대로 소속 브랜드 하나만 셉니다.
- **발주 알림 메일 머리글에 그 거래의 브랜드 이름이 나옵니다.** 브랜드를 두 개 이상 가진 분에게 상관없는 쪽 브랜드 이름이 붙던 것을 고쳤습니다. 청구서·예약·승인 등 다른 알림 메일은 한 줄도 바뀌지 않았습니다.
- **Profile 화면에서 이메일 인증 상태를 보고 인증 메일을 다시 받을 수 있습니다.** 지금까지 이 안내는 알림 설정 화면에만 있었고, 정작 이메일을 바꾸는 자리인 Profile 에는 없어서 주소를 바꾸면 알림이 끊긴 걸 본인만 몰랐습니다. 인증된 계정·데모·테스트 계정에는 나오지 않습니다.

### 2026-09-14 #3 — 무료 청구서 확정 · 결제일 · 은행송금 우선
> **운영 배포 완료** (SW 5.23 17:0x · SW 5.24 17:03 · 스모크 각 10/10).
> ⚠ Fable 판정 미수령(한도 429) — Irene 지시로 우회하지 않고 대기.

- **무료(0원) 청구서의 «Confirm» 이 눌립니다.** 브랜드 구독 청구서처럼 매장이 아닌 곳으로 발행된 0원 청구서를 정작 낼 사람이 확정할 수 없었습니다(운영 10건 대기). 금액이 있는 청구서는 종전대로 결제 경로로만 처리됩니다.
- **확정이 실패하면 이유가 화면에 뜹니다.** 지금까지는 눌러도 아무 일이 없는 것처럼 보였습니다.
- **결제를 기록할 때 결제일을 고를 수 있습니다** — 실제로 돈이 나간 날. 기본은 오늘이고 미래는 고를 수 없습니다.
- **결제수단은 은행송금이 먼저 나옵니다**(기본값). 전에는 현금이 기본이라 실수로 시재에서 빠질 여지가 있었습니다.
- **발주 «Mark as Sent» 를 누르면 «받을 발주» 목록에 바로 올라갑니다.** 새로고침해야 보이던 것을 고쳤습니다.
- **영어 화면에 한글이 섞이던 것**을 고쳤습니다(인보이스 대조 결과 안내).
- **브랜드 메뉴의 레시피 이름을 누르면** 레시피 화면이 그 레시피를 검색한 채로 열립니다.

### 2026-09-14 #2 — 레시피 이름을 누르면 그 레시피로
> 개발서버 반영 · **미배포**

- **브랜드 메뉴의 레시피 이름을 누르면 레시피 화면이 그 레시피를 검색한 채로 열립니다.** 지금까지는 이름만 보이고 확인하러 가려면 레시피 화면에서 다시 찾아야 했습니다.
- 연결이 없는 «레시피 없음» 은 눌리지 않습니다.

### 2026-09-14 — 브랜드 메뉴에 «연결된 레시피» 표시
> **운영 배포 완료** (SW 5.22-recipe-tag-20260914 · 04:35 · 스모크 10/10).
> ⚠ Fable 판정 미수령(사용 한도 429) — 릴리즈 기록 `fable_note` 에 명시. 민감 판정 «일반 변경»(비대상).

- **브랜드 메뉴 목록에 연결된 레시피 이름이 보입니다.** 지금까지는 레시피를 연결해도 화면에 표시되지 않아, 메뉴마다 수정 창을 열어 봐야 무엇이 붙었는지 알 수 있었습니다. 운영 K-DINE 브랜드 기준 105개 중 65개가 연결돼 있습니다.
- **레시피가 없는 메뉴는 «레시피 없음»** 으로 표시되고, 마우스를 올리면 «이 메뉴는 재고가 차감되지 않습니다» 라고 알려 줍니다. 한국어·영어·중국어·말레이어 모두 들어갔습니다.
- 표시 전용 변경입니다 — 레시피 연결·푸시·재고차감 동작은 그대로입니다.

### 2026-09-13 #4 — 브랜드 화면 튕김 수정 · 없는 주소 안내 · K-DINE 브랜드 메뉴 연결
> **운영 배포 완료** (SW 5.21-brand-recipe-gate-20260913 · 19:44 · 스모크 10/10 · 백업 `/var/www/backups/20260913_192703`).
> ⚠ Fable 판정 미수령(사용 한도 429 · 이번 세션 5회 시도) — 릴리즈 기록 `fable_note` 에 명시. 민감 판정은 «일반 변경»(비대상).

- **브랜드·푸드코트 총괄이 «브랜드 레시피»·«재고원장» 을 눌러도 대시보드로 튕기지 않습니다.** 메뉴는 보이는데 들어가지지 않던 상태였습니다. 9월 12일 «발주 전용(무료)» 작업에서 매장용 이름(recipe_management)으로 문을 달았는데 역할을 가리지 않아 요금제 어휘가 다른 브랜드까지 막혔던 것입니다. 매장 계정의 무료 등급 차단은 그대로 유지됩니다.
- **없는 주소로 들어가면 흰 화면 대신 안내 화면이 나옵니다** — 돌아가기·시작 화면 버튼이 있습니다. 지금까지는 옛 북마크나 오타 주소가 완전 백지였습니다.
- **공급업체·브랜드·푸드코트 계정에서 화면마다 나가던 권한 없는 주문 조회를 멈췄습니다.** 실패 요청이 계속 쌓여 진짜 오류를 가리고 있었습니다.
- **손가락 조작 기준 정리** — 행 순서 바꾸기 버튼 28→32px, 모바일 표 버튼 최소 크기 30→32px.
- **K-DINE IPC 메뉴 105개가 K-DINE with MIN 브랜드로 연결됐습니다(운영 데이터 작업).** 그동안 매장 독립 메뉴라 브랜드에서 고쳐도 내려가지 않았습니다. 브랜드에 메뉴 105 · 카테고리 14 · 옵션그룹 7(옵션 12)을 만들고 1:1로 이었습니다. 매장 메뉴의 이름·가격·옵션은 그대로이고 잠금도 걸지 않았습니다.
- 참고: 브랜드 메뉴 화면은 **브랜드 소유자 계정**(`help@gitconsulting.group`)으로만 열립니다. 같은 회사의 다른 계정으로는 데이터가 있어도 보이지 않습니다.
- 내부: 버튼 전수검사 도구 신설(화면 168개·클릭요소 8,683개 수집 + 실클릭 검사), 검증 스윕이 백지 화면을 «정상» 으로 넘기던 사각지대 수정.

### 2026-09-13 #3 — 포장단위에 «ea»
> **운영 배포 완료** (SW 5.20-package-unit-ea-20260913 · 10:42 UTC · 스모크 10/10 · 백업 `/var/www/backups/20260913_103714`). Irene 「ea 도 추가해줄래?」

- **기준단위(포장) 제안 목록에 `ea` 가 다시 나옵니다.** 9월 11일에 어휘를 풀네임으로 통일하며 `ea→piece` 로 접었던 말인데, 매장에서 실제로 쓰기 때문에 되살렸습니다. 상품 창 네 곳(브랜드 상품·공급업체 상품·공급업체 프로필·외부 공급업체 등록)이 같은 목록을 씁니다. 이미 저장된 상품의 값은 바뀌지 않습니다.
- 서비스 상품 넣는 법(측정 결과): **주문 방식 «개수로» · 기준단위(포장) `ea` · 포장당 용량 `4` · 취급단위 `hour`** → 규격 «4 hour/ea», 1 ea 주문이 4시간입니다. 포장단위를 비우면 화면이 «4 hour/pack» 으로 떨어지므로 비우지 않습니다.
- 기록만 남긴 것: 브랜드 상품 상세의 «재고 단위» 칸은 **환산에 쓰이지 않는 이름표**이고 실제 차감은 주문 단위(포장단위) 기준입니다(운영 81행 중 78행이 빈 칸). 어느 길로 정리할지는 Fable 판정 대기 — `docs/TRADE_STRUCTURE.md` §5-12.

### 2026-09-13 #2 — 상품 종류: 재고 상품 / 주문제작 / 서비스·기타
> **운영 배포 완료** (SW 5.19-product-kind-20260913 · 10:01 UTC · 스모크 10/10 · 마이그 90/90 · 백업 `/var/www/backups/20260913_095601`). Irene 「주문제작 설정을 해. 재고 있고 없고랑 상관없이 주문할 수 있게.」 · 「주문제작 (재고관리 안함, 배송처리 함), 서비스/기타 (재고관리 안함, 배송처리 안함 … 이렇게 나눠져야 하는 거 아니야?」
> ⚠ Fable 판정 미수령(사용 한도 429 · 3회 시도) — 릴리즈 기록 `fable_note` 에 명시.

- **판매자 상품에 «상품 종류» 가 생겼습니다** — 재고 상품 / 주문제작 / 서비스·기타. «재고를 세는가» 와 «배송이 있는가» 는 서로 다른 축이라 참/거짓 하나로 담지 않고 셋으로 나눴습니다. 기본값은 «재고 상품» 이라 **기존 상품 동작은 그대로**입니다(운영 155행 전부 재고 상품으로 시작).
- **주문제작** — 재고를 두지 않고 주문이 들어오면 만들어 배송합니다. 출고할 때 판매자 자체 재고를 깎지 않습니다. 전에는 재고 0에서 빼려다 «모자람» 기록만 쌓였습니다.
- **서비스·기타** — 재고도 배송도 없습니다. 판매자 발주 화면에 «완료 처리» 버튼 하나만 나오고, 누르면 주문이 바로 완료됩니다(«출고 기록» 안내는 뜨지 않습니다). 물건이 한 줄이라도 섞이면 배송 있는 주문으로 보아 종전대로 출고합니다.
- **서비스 상품은 파는 단위로 «hour» 를 고를 수 있습니다** (컨설팅 시간 등). 재고 상품 목록에는 넣지 않았습니다 — 매장 재고 단위 8개에 없는 값이라 재고로 넘어갈 수 없습니다.
- 레시피·재고아이템이 걸린 상품은 종류와 무관하게 종전대로 구성재료가 빠집니다. 새 주문 상태값은 만들지 않아(완료=received) 목록·통계·배지는 그대로 동작합니다.
- 아직 안 한 것: **운영 상품에 종류를 실제로 지정하는 일**(GIT 소스류 21건·컨설팅 시간 등)은 별도 지시 대기 · 구매자(매장) 화면의 «완료 예정일» 표기는 이번에 판매자 화면만 바꿨습니다.

### 2026-09-13 — B2B 무료 발주 등급
> **운영 배포 완료** (SW 5.15 → 5.18 · 4회 · 스모크 10/10). 우리 POS 를 쓰지 않는 매장도 **무료로 가입해 발주만** 하게 하고, 재고·원가가 자동으로 맞물리는 것을 보여 유료 전환을 유도합니다.

- **판매자가 «누가 주문할 수 있나» 를 고릅니다.** «계약을 맺은 곳에만 판매» 또는 «가입한 곳이면 누구나 주문 가능». 열어 두면 구매자가 가입만 하면 바로 담고 주문할 수 있고, 거래 기록은 첫 주문 때 자동으로 생깁니다.
- **«주문용 상품 링크» 가 생겼습니다.** 공급업체·브랜드의 상품 화면 맨 위 한 줄입니다. 링크 주소·복사·QR·새 창으로 열어보기가 그 줄에 있고, 펼치면 손님에게 보이는 **가게 이름**과 링크 이름을 정할 수 있습니다. 받은 사람은 **로그인 없이** 상품과 가격만 보고 «무료 가입하고 주문» 으로 들어옵니다.
- **브랜드는 상품마다 «내 가맹점 밖 구매자에게도 판매» 를 고를 수 있습니다.** 고른 상품만 링크와 카탈로그에 나오고 가맹점 전용 상품은 그대로 보호됩니다.
- **브랜드 계약에 «공급형» 이 생겼습니다.** 가맹형과 같은 폼·같은 관리 화면을 쓰되 목록을 전체/가맹형/공급형으로 나눠 보고, 새 제안도 종류를 골라 만듭니다.
- **매장의 «온라인 메뉴판(보여주기 전용)» 링크를 설정 → Mobile Order 로 옮겼습니다.** 주문 링크 바로 옆이라 이제 바로 찾을 수 있습니다(전에는 Tables & QR 의 QR 묶음에 묻혀 있었습니다).
- **폼의 «필수» 표시를 한 방식으로 맞추기 시작했습니다.** 별표는 서버가 실제로 막는 칸에만 붙고 저장 버튼을 막지 않습니다(가입·매장 회사정보부터).
- 고친 것: 0원 무료 등급이 요금제 **변경** 목록에 섞여 유료 매장을 무료로 내릴 수 있던 것(가입 화면에는 그대로 보입니다) · 회사정보 저장이 화면이 보내지 않은 칸까지 덮어쓰던 것 · 잠금 표시가 휴대폰에서만 보이던 것.

- **«발주 전용(무료)» 요금제가 생겼습니다.** 0원 · 발주 · 공급업체 · 매입 청구서 · 공지 · 대시보드까지 열립니다. 주문 건수 제한은 두지 않았고 매장은 1곳입니다.
- **무료 요금제는 7일 체험이 붙지 않습니다.** 전에는 가격과 무관하게 체험으로 시작해 8일째 «미납» 으로 떨어졌습니다. 이제 0원이면 바로 이용 상태로 시작합니다.
- **무료 등급에서 재고 메뉴는 사라지지 않고 «잠김» 으로 보입니다.** 누르면 «업그레이드하면 무엇이 자동으로 계산되는지» 안내 화면으로 갑니다 — 발주하면 재고가 쌓이고, 메뉴 원가가 계산되고, 업체별 단가를 비교합니다. 지금 쓰던 발주·공급업체 자료는 그대로 남는다는 것도 함께 알립니다.
- **무료 등급은 발주를 보내기 전에 법인 정보(상호·사업자번호·주소·전화)를 받습니다.** 청구서를 발행해야 하기 때문이며, 유료 매장에는 영향이 없습니다.

### 2026-09-12
> **운영 배포 완료** (SW 5.14 · 07:21~07:26 UTC · 스모크 10/10). 아래 항목은 전부 운영에 나가 있습니다.

- **재고아이템을 지울 수 없을 때 «어디서 쓰는지»를 보여주고 그 자리로 보냅니다.** 그동안은 «Delete Failed» 만 떠서 이유를 알 수 없었습니다. 특히 다른 브랜드에 공유한 사본이 쓰이는 경우에는 내 화면에 연결이 보이지 않아 «연결이 없는데 왜 안 지워지지?» 가 됐습니다. 이제 안내창이 쓰는 곳을 종류별로 보여 주고 «보러가기» 로 그 화면을 엽니다 — 브랜드 레시피, 프로덕트 레시피, 상품 옵션, 매장 재고, 공급처 연결.
  - 레시피 줄을 대신 지워 주지는 않습니다. 다른 브랜드의 레시피를 건드리는 일이라 그 화면에서 직접 빼도록 안내만 합니다.
- **상품 옵션에 쓰이는 재고아이템은 미리 막습니다.** 전에는 검사가 없어 삭제를 누르면 서버 오류로 끝났습니다.
- **재고아이템 카드에 «쓰는 레시피»가 보입니다.** 프로덕트 레시피와 브랜드 레시피를 이름까지 보여 주고, 누르면 그 레시피 화면으로 갑니다. 지우려 할 때가 아니라 평소 목록에서 바로 «이 재료를 어디서 쓰는지» 알 수 있습니다.
- **꺼 둔(비활성) 재고아이템은 레시피 재료 검색에 뜨지 않습니다.** 브랜드 레시피는 이미 그랬고, 매장 레시피 화면도 같게 맞췄습니다. 재고아이템을 끄면 매장으로 공유된 사본도 함께 꺼집니다. 이미 저장된 레시피 줄은 그대로 보입니다.

- **발주 상세의 품목 이름이 공급업체 상품 이름으로 나옵니다** (SW 5.13). 품목 표·입고 창·반품 창에서 공급업체가 부르는 이름이 큰 글씨로 오고, 우리 재고 이름(«영문(한글)»)은 작은 줄·괄호로 남습니다. 발주 대기·인쇄·PDF·공유·인보이스 대조는 이미 그랬고 상세 화면만 반대였습니다. 발주관리 첫 화면의 «발주 제안» 목록은 우리 재고 목록이라 우리 이름 그대로입니다.
  - 판매 상품 연결이 없는 줄(옛 발주·브랜드가 판매자인 발주)은 그대로 우리 이름이 큰 글씨입니다.
  - 발주 공유 문구 테스트 5건이 9월 11일 문구 변경 뒤 실패한 채 남아 있던 것을 함께 고쳤습니다.

- **공급업체 상품 정리 도구**: Irene 님 답으로 고친 행도 **답의 규격으로** 재고 환산값을 다시 계산해 제안합니다. 전에는 검토표를 만든 시점 값만 써서, 답으로 «65 g/piece» 로 고친 호떡의 환산이 1 로 남아 «1개 입고 = 1 g» 이 됐습니다(운영 2차 반영에서 연결 14건을 뒤에 손으로 고쳤습니다). 같은 자리에서 원가가 3배 이상 움직이면 답이라도 자동으로 쓰지 않고 건너뜁니다.
  - 운영 데이터는 2026-09-11 에 이미 정리 완료: 연결 환산 14건(쓰레기봉투 10·8 · 호떡 65 · 갈아만든배 12 · 단팥 3000) · 당면 재료를 1kg 상품으로 재연결(원가 24) · 소금 1 bundle = 5400 g · 멸치액젓 규격 «800 g/bottle»(발주 줄 «800 g × 1 bottle», 원가 19).

### 2026-09-11 #6
> **운영 배포 완료** (서버만 · SW 5.12 그대로 · 21:26 UTC · 스모크 10/10 · 마이그 87/87 · 백업 20260911_212145 · Fable 게이트 통과 f2283598e56c). 배포 뒤 Irene 승인으로 **운영 원가 20행 복원**(21:34 UTC · 20/20 일치 · 백업 `catalog-alignment-20260911/cost-incident-restore-2026-09-11T21-34-38-200Z.json`).

- **재고아이템·재료 원가가 1,000배로 커지던 문제를 고쳤습니다.** 원가가 공급업체 가격을 따라갈 때 «기준양»을 한 번 더 곱하고 있었습니다. 1000 g 들이처럼 기준양이 1 이 아닌 재료에서만 드러났습니다(운영 MSG 18.8 → 18,800 등). 이제 문서 규칙대로 «가격 ÷ 판매자 용량 × 내 기준숫자» 입니다.
- **운영에서 잘못 커진 원가 20개를 원래 값으로 되돌렸습니다.** 고춧가루·사골분말·다시다·흑후추·MSG·무·깐마늘(브랜드 재고아이템·공유 재료), 매장 10 식용유·양파·깐마늘. 레시피 원가·매장 원가표에는 번지지 않았습니다. 재료의 규격 칸은 건드리지 않았습니다.
- 배포 전 검사에 «원가가 공급업체 가격으로 계산한 값과 3배 이상 다른 행» 검사가 생겼습니다.
- 공급업체 상품 정리 도구: 검토표가 연결 재료 원가 변화를 미리 보여 주고 3배 이상이면 확인을 받습니다. 되돌리기는 원가를 다시 계산하지 않고 적용 때 남긴 원가 기록을 거꾸로 씁니다.

### 2026-09-11 #5
> **운영 배포 완료** (SW 5.12-po-line-qty-text · 20:10 UTC · 스모크 10/10 · 백업 20260911_201034 · Fable 게이트 통과).

- **발주 내역 수량이 «10 kg × 2 carton» 한 조각으로 나옵니다** — 발주 대기·발주 상세(발주·입고)·인쇄·PDF·공급업체 메일·WhatsApp/메일 공유·인보이스 대조·공급업체 수신 화면. «1 kg/pack» 규격은 상품·재료 정보 자리에만 남습니다. 공유 문구의 가격 앞 기호는 «@» 입니다.
- **인보이스 목록(매장)**: 브랜드·푸드코트가 보낸 청구서의 발행자가 회사명으로 나오고, 최신 → 오래된 순(구입 청구서는 발주 주문일, 나머지는 발행일)으로 정렬·기간 필터됩니다.
- 공급업체 상품 정리 도구 보정: «같음» 행의 재고 환산 제안도 적용 · 영문명 없는 행도 공급업체를 찾아 둠 · 작업 뒤 스스로 종료.
- ⚠ 이 배포 직전 운영 1차 반영(19:24 UTC)의 원가 전파가 위 #6 의 결함으로 원가 15행(+공유 재료 2)을 부풀림 → #6 에서 복원.

### 2026-09-11 #4
> **운영 배포 완료** (SW 5.11-supplier-catalog-align · 19:03 UTC · 스모크 10/10 · 백업 20260911_185725 · Fable 게이트 통과). 이 배포는 **화면·문서·정리 도구**만 올렸습니다. 공급업체 상품 이름·규격·가격 정리는 운영 검토표를 Irene 님이 확인한 뒤 반영합니다.

- **공급업체에게 가는 발주서(PDF·인쇄·WhatsApp·메일·공급업체 수신 화면)가 공급업체 상품 이름을 그대로 씁니다.** 우리 내부 이름을 덧붙이던 «Buyer ref» 줄이 없어졌고, 공급업체 상품과 연결이 없는 줄은 우리 재고 이름에서 한글 괄호를 떼고 보냅니다.
- **PDF·인쇄 발주서의 수량 칸이 «10 kg» 처럼 단위를 붙여 나옵니다.** 따로 있던 Unit 칸은 없어졌습니다. PDF 에서는 우리가 자동으로 붙인 상품 코드(SP-…)를 숨깁니다.
- **상품·재료 정보의 규격이 «10 kg/BOX» 한 줄로 보입니다.** 공급업체·브랜드·외부 공급업체·푸드코트 상품 카드, 매장 재료·브랜드 재고아이템 카드, 재고 카드의 발주처 칩이 모두 같은 모양입니다. 용량을 모르는 개수 상품은 «1 bottle» 처럼 보입니다.
- **상품 등록 폼의 단위 선택지에는 kg·g·L·ml·piece 만 남았습니다.** pack·bottle 같은 포장 이름은 «Package Unit» 칸에 적습니다(이미 포장 이름으로 저장된 상품은 그대로 선택돼 보입니다). 포장단위 제안에 tub·drum·roll·bundle·sheet 가 추가됐습니다.
- 공급업체 상품 목록 정리 도구(검토표 → 확인 → 적용 → 되돌리기)를 준비했습니다. 가격이 2배 이상/절반 이하로 바뀌거나, 규격 표기를 읽을 수 없거나, 한글과 영문이 다른 물건으로 보이는 줄은 자동으로 고치지 않고 확인을 받습니다. 지난 발주 줄은 받은 발주는 단위·규격 표시만, 아직 받지 않은 발주는 재고 환산값까지 맞춥니다.

### 2026-09-11 #3
> **운영 배포 완료** (SW 5.10-reconcile-honest-read · 16:29 UTC · 스모크 10/10 · 마이그 87/87 · 백업 20260911_162405). 첫 시도는 같은 서버의 다른 프로젝트 빌드와 겹쳐 메모리 게이트가 막았고(운영 무변경), 끝난 뒤 재시도로 나갔습니다.

- **인보이스 대조 화면이 못 읽은 인보이스를 정직하게 알립니다.** 손글씨·흐린 사진이라 한 줄도 못 읽으면 노랑 «자동으로 읽지 못했습니다» 가 뜨고 엉뚱한 줄이 뜨던 고르기 목록을 숨깁니다. 읽었지만 발주와 맞는 줄이 없으면 노랑 «읽었지만 맞는 줄을 찾지 못했습니다» 와 함께 목록은 남겨, 골라 붙이면 다음부터 자동으로 짝지어집니다. 맞는 줄이 있으면 예전처럼 초록입니다.
- **전화번호·주소처럼 숫자가 말이 안 되는 줄은 인보이스 품목으로 받지 않습니다.** 운영 발주 32(손글씨 Guan Kee)에서 전화번호 줄이 품목으로 떠 모든 줄의 고르기 목록에 나오던 원인입니다.
- **외부 공급업체 상품 등록 창에 «재고 환산» 칸이 생겼습니다.** «1 BOX = 10 kg» 처럼 포장당 용량에서 자동으로 채워지고 고칠 수 있습니다. 자동 계산이 안 되는 조합은 적어야 저장됩니다. 전에는 항상 1 로 저장돼 10 kg 박스 3개 입고가 재고 +3 으로 잡힐 수 있었습니다(이미 만든 연결은 그대로).
- 외부 공급업체 상품 등록 API 가 잘못된 업체 번호에 응답 없이 멈추던 문제를 고쳤습니다(이제 «Supplier not found»).

### 2026-09-11 #2
> **운영 배포 완료** (SW 5.09-po-pack-unit · 14:19 UTC · 스모크 10/10 · 백업 20260911_141438). 아래 항목은 전부 운영에 나가 있습니다.

- **발주 줄이 «용량 · 포장단위 × 수량»으로 보입니다.** 예: Kimchi · 10 kg/BOX · × 3 BOX @ 48.00 = 144.00. 담는 화면·발주 대기(Staging)·발주 상세·인쇄·PDF·WhatsApp/메일·공급업체 알림 메일·공급업체 수신함·인보이스 대조에 똑같이 나옵니다.
- **공급업체 상품·브랜드 상품·외부 공급업체 상품에 «기준단위(포장)» 칸이 생겼습니다.** BOX·PKT·Btl·Tin 처럼 인보이스에 적힌 대로 입력할 수 있고, 외부 공급업체 상품 창에서는 «포장당 용량»과 함께 «주문 표시»로 미리 보입니다.
- **발주 줄이 주문한 순간의 용량을 함께 저장합니다.** 공급업체가 나중에 상품 용량을 바꿔도 지난 발주서와 인보이스 대조는 그대로입니다.
- **외부 공급업체 발주 줄 단위가 «piece» 로 잘못 찍히던 문제를 고쳤습니다.** 공급업체 상품 단위를 다른 표에서 찾고 있었습니다. 금액·수량·입고 재고는 영향이 없었습니다.
- 기존 상품 중 단위가 pack/piece/bottle/can 이고 용량이 1 인 상품은 그 값이 포장단위 칸에 채워집니다(빈 칸만). 배포 전에 만든 발주 줄은 용량 문구 없이 예전처럼 보입니다.

### 2026-09-11
> **운영 배포 완료 1회** (SW 5.08 · 08:11 UTC · 스모크 10/10). 아래 항목은 전부 운영에 나가 있습니다.

- **외부 공급업체 인보이스의 «결제함»과 발주의 «Pay»가 한 곳에서 기록됩니다.** 인보이스에서 결제함을 누른 뒤 발주에서 Pay 를 또 누르면 현금 서랍에서 두 번 나가던 문제를 막았습니다. 어느 쪽에서 먼저 냈든 두 번째는 «이미 결제됨»으로 거부됩니다. 결제함을 누르면 결제 수단을 고르는 창이 뜹니다.
- **인보이스 대조에서 적은 총액과 줄 합계가 RM 1 넘게 다르면 저장을 막습니다.** RM 1 이내면 «Rounding adjustment» 한 줄로 맞춰, 인보이스 총액 = 적은 총액 = 낼 금액이 됩니다.
- **구입 인보이스의 «기간»이 발주일 → 수령일로 보입니다.** 발행 시각이 두 번 찍히던 것을 고쳤고, 목록·상세·인쇄에 공급업체 인보이스 번호·일자도 나옵니다.
- **브랜드 총괄·푸드코트 총괄·오너 인보이스 화면도 외부 공급업체 인보이스를 «Mark paid»(결제 창)로 냅니다.** 그동안 가입 공급업체처럼 게이트웨이 «Pay» 가 나왔습니다. 여러 매장 오너와 두 번째 브랜드도 같은 방식으로 결제됩니다.
- **공급업체 켜기/끄기.** 브랜드가 넣어준 공급업체도 쓰지 않으면 우리 매장에서만 끌 수 있고, 끈 공급업체는 발주할 때 나오지 않습니다. 다시 켜면 브랜드 조건 그대로 돌아옵니다.
- **모든 역할의 공지사항 페이지에 «모두 읽음» 버튼**이 생겼습니다.
- **레시피 CSV 다운로드 칸 순서**를 레시피 보기 좋은 순서로 바꾸고 재료를 «0.05 kg» 처럼 표시합니다. 이름을 못 찾은 재료 줄도 빠지지 않습니다.
- **레시피에 쓰이는 재료·상품은 삭제가 막히고** 어떤 레시피가 쓰는지 알려 줍니다. 전에는 삭제하면 레시피의 재료 줄이 조용히 사라졌습니다.
- **매장 원가 계산은 매장 원가를 씁니다.** 수령·대조로 바뀐 원가는 매장 칸에 들어가고, 브랜드가 공유한 재료의 브랜드 원가를 매장이 바꾸지 않습니다.
- **직원(Staff) 계정에는 인보이스 결제 버튼이 보이지 않습니다.** 전에는 보였지만 누르면 권한 오류가 났습니다.

### 2026-09-10
> **운영 배포 완료 7회** (SW 5.01~5.07). 아래 항목은 전부 운영에 나가 있습니다.

- **외부 공급업체 청구서에는 마감일을 붙이지 않습니다.** 결제 조건을 합의한 적이 없는 상대에게 «15일 뒤»를 임의로 붙이고 있었고, 그 때문에 9월 24일에 청구서 15장이 한꺼번에 «연체»로 바뀌고 매장에 근거 없는 연체 메일이 나갈 예정이었습니다. 이미 발행된 것도 함께 비웁니다. 주문일·수령일은 화면에 그대로 나옵니다.
  - 브랜드가 판매자인 청구서 1장은 그대로 둡니다(브랜드 결제조건은 내부 정책입니다).
  - 마감일이 없는 청구서를 «1970년»으로 읽어 연체로 잘못 표시하던 자리 6곳을 함께 막았습니다.
- **모바일 주문에서 예전 테이블 번호가 계속 따라다니던 것을 고쳤습니다.** 지난번에 QR 로 찍은 테이블이 지워지지 않아, 나중에 대표 링크로 들어오면 앉은 자리와 상관없이 옛 테이블로 주문·주방티켓이 나갔습니다. 이제 마지막 조작으로부터 **4시간**이 지나면 테이블·장바구니·주문유형이 함께 비워집니다. QR 을 새로 찍으면 언제나 그 테이블이 이깁니다.
- 키오스크 결제 화면에도 자리비움 정리가 생겼습니다(5분). 앞사람이 결제 직전에 떠나도 다음 손님이 그 장바구니를 물려받지 않습니다. 결제 요청이 나간 뒤에는 화면을 치우지 않습니다.
- 레시피를 **복사**할 때 원가를 다시 계산합니다. 예전에는 저장돼 있던 값을 그대로 옮겨서, 원본에 0 이 박혀 있으면 사본에도 0 이 번졌습니다. 이미 0 으로 저장된 줄도 재료 단가에서 다시 계산합니다(단가를 아직 정하지 않은 재료는 건드리지 않습니다).
- 키오스크 결제 화면의 자리비움 정리가 결제 중에 한 번 미뤄진 뒤 **다시는 동작하지 않던 문제**를 함께 막았습니다. 카드가 거절되고 손님이 떠나도 다음 손님에게 넘어가지 않습니다.

- **화면이 하얗게 죽던 자리 12곳을 고쳤습니다.** 저장이나 삭제가 실패했을 때 «무엇이 잘못됐는지» 안내문이 떠야 할 자리에서 화면이 통째로 멈췄습니다. 브랜드 레시피 저장, 직원 등록·수정, 홈페이지 문의 폼, 인보이스 카테고리 삭제와 메일 발송이 해당됩니다.
- **인보이스 대조 화면이 영어 설정에서도 한국어로 보이던 문장 5개**를 네 언어 모두에 넣었습니다.
- **올린 공급업체 인보이스가 화면에 안 보이거나 깨지던 것**을 고쳤습니다. 서버가 예전에 그 파일들에 «1년간 다시 확인하지 말 것» 딱지를 붙여 보냈고, 서버를 고친 뒤에도 브라우저가 옛 응답을 계속 쓰고 있었습니다.
- **대조를 마친 외부 공급업체 발주는 이제 공급업체가 청구한 금액을 지불합니다.** 그동안은 인보이스를 맞춰 봐서 청구가 RM 480 인 것을 확인해도 현금 서랍에서는 발주 금액 RM 500 이 나갔습니다. 결제창에 두 금액이 나란히 보이고, 인보이스만 올리고 대조를 안 했으면 경고가 뜹니다.
- **발주 목록에서 바로 결제·수령결제·결제 되돌리기**를 할 수 있습니다. 그동안은 발주 상세로 들어갔다 나와야 했습니다.
- **인보이스 화면에서도 공급업체 인보이스를 올릴 수 있습니다.**
- **인보이스 대조 화면에 총액 비교가 생겼습니다.** 발주 총액·입력한 줄들의 합·인보이스에 적힌 총액 셋을 나란히 보여 주고 차이를 금액으로 표시합니다.
- **«10개 시켰는데 8개 왔다» 처럼 수량만 다른 줄도 차이로 잡습니다.** 그동안은 단가만 비교해서 현장에서 가장 흔한 차이를 놓치고 있었습니다.
- **올려 둔 공급업체 인보이스를 자동으로 읽어 대조 화면을 채웁니다.** 사진이나 PDF 를 열면 몇 초 안에 품목·수량·단가와 인보이스 번호·일자·총액이 들어갑니다. 읽기는 브라우저 안에서 처리해 인보이스가 외부로 나가지 않고 비용도 들지 않습니다. 확실한 줄은 초록, 확인이 필요한 줄은 노랑으로 채우고 이름을 못 찾은 줄은 비워 둡니다. 저장은 사람이 눌러야 반영됩니다.
- **한 번 짝지어 저장한 이름을 기억합니다.** 공급업체가 «BAWANG HOLLAND» 라고 부르는 것이 우리 «Yellow Onion» 이라는 것을 저장해 두면 다음 인보이스부터 자동으로 붙습니다.
- 대조 화면에서 저장 후 다시 들어가면 «18.0000» 처럼 소수점이 네 자리까지 나오던 것, 왼쪽은 «7.00» 인데 오른쪽은 «7» 로 보이던 것을 발주 쪽 자릿수에 맞췄습니다.
- 인보이스 줄을 한 번 고르면 다시 바꿀 수 없던 것, 이미 다른 줄이 가져간 줄이 선택 목록에 남아 있던 것을 고쳤습니다.
- 인보이스 일자 칸이 브라우저 언어에 따라 월/일/년으로 보이던 것을 «Sep 08, 2026» 처럼 월을 글자로 보여 주도록 바꿨습니다.
- 청구 수량·단가 칸 옆에 발주한 단위를 표시합니다.
- **온라인 메뉴판을 만들었습니다.** 주문·장바구니·계정이 없고 손님 정보도 받지 않는 보기 전용 화면으로, 홍보용으로 어디에나 공유할 수 있습니다. 옵션은 보이되 선택되지 않습니다. 설정의 QR 카드 목록에서 링크를 받으세요.

### 2026-09-09
- 발주 청구서 마감일이 9월 24일에 한꺼번에 연체 처리되는 건을 다시 확인했습니다. **매장이 정지되거나 독촉 메일이 반복되는 일은 없습니다** — 청구서 16장의 표시가 «연체»로 바뀌고 연체 안내 메일이 한 통씩 나가는 데서 끝납니다. 마감일을 어떻게 할지는 아직 정하지 않았습니다.
- 임대료·거래 청구서에 대해 문서와 코드 주석이 «독촉 메일이 반복해서 나간다»고 잘못 적혀 있던 것을 사실대로 고쳤습니다(동작은 그대로).

### 2026-09-08
- 대조 화면에서 인보이스 내용을 직접 넣는 칸을 맨 위로 올렸습니다. 사진·PDF는 글자를 읽어주지 못하므로 미리보기가 안 열려도 일이 막히지 않아야 합니다.
- 대조 화면 미리보기가 빈칸으로 뜨던 것을 고쳤습니다. 안 열릴 때를 위해 «새 창에서 크게 보기»를 옆에 두었습니다.
- 발주 목록이 세 단계를 구분해 보여줍니다 — «인보이스 확인»(올라왔는데 미대조) / «▲ 차이 N»(값이 다름) / «대조 완료».
- 청구서 목록에 그 발주를 언제 주문했고 언제 받았는지 나옵니다. 아직 안 받았으면 «미수령»으로 표시합니다.

## v3.86 (2026-09-08) — 제목줄 잘림 + 공급업체 가격이 원가로

**태블릿 세로에서 화면 위 제목줄이 잘리던 것을 전 역할·전 페이지에서 고치고, 외부 공급업체 가격을 고치면 재료 원가가 따라가도록 했습니다.**

### 바뀐 것
- 화면 위 제목줄이 태블릿 세로에서 잘리던 것을 고쳤습니다. 제목은 왼쪽, 둘러보기·플랜·남은 기간은 오른쪽에 두 줄로 접힙니다.
- 같은 문제가 전 역할·전 페이지 56곳에 있었습니다. 화면 폭 769~1024 구간 규칙이 아예 빠져 있어서, 그 폭에서 내용이 넘치면 높이가 안 늘어나 잘렸습니다.
- 휴대폰 폭에서도 같은 종류의 잘림을 54곳 고쳤습니다.
- 헤더 오른쪽 묶음을 공용 컴포넌트로 만들었습니다(HeaderRight). 낱개로 붙이면 폭이 좁아질 때 제각각 접힙니다.
- **외부 공급업체 상품 가격을 고치면 그 상품을 사는 재료·재고아이템의 원가가 따라갑니다.** 가입 공급업체 쪽에는 이미 있던 전파가 외부 공급업체 수정 경로에만 빠져 있었습니다. 가격만 저장되고 원가는 옛 값으로 남아, 원가율을 봐도 알 수 없는 종류의 문제였습니다.

### 남은 것
- **소급은 안 됩니다** — 이번 수정 전에 외부 공급업체 가격을 고쳤던 재료는 원가가 옛 값 그대로입니다. 앞으로의 수정만 따라갑니다. 소급 반영은 발주↔인보이스 대조 작업에 포함돼 있습니다.

### 이 라운드에서 틀린 것 (기록)
- 전체 훑기가 파일 이름 패턴으로 돌면서 🔒 인쇄 보호파일 두 개(MainLayout·POSTerminal)에 들어갔습니다. 제외 목록을 안 걸어서 생긴 일이고, 가드가 잡아 즉시 되돌렸습니다(8/8 무변경 확인).
- 처음 훑기의 검사식이 「높이는 풀고 최대높이는 안 푸는」 경우를 통과시켰습니다. 실브라우저로 재다가 오너 매장목록에서 버튼이 11px 삐져나간 것을 보고 찾았습니다.
- 배포 1차는 프론트 빌드에서 멈췄습니다. 같은 서버의 다른 제품 빌드와 겹쳐 메모리 게이트가 막은 것으로, 운영 서버에는 닿기 전이었습니다.

### 2026-09-07
- 브랜드제너럴이 자기 브랜드 앞으로 온 발주를 전부 봅니다. 브랜드를 여럿 운영하면 다른 브랜드로 들어온 주문이 화면에서 통째로 사라지던 문제입니다. 반품 화면도 같았습니다.
- 판매자 화면에서 받은 주문이 「완료」로 표시되고, 결제 상태를 따로 보여줍니다(미결제·결제완료·환불).
- 발주를 받으면 거래 청구서가 반드시 나갑니다. 수령을 끝내는 버튼이 셋인데 청구서를 만드는 건 하나뿐이었습니다. 발주 결제와 청구서 결제도 서로를 따라갑니다.
- **발주에 찍히는 단위가 공급업체 기준으로 바뀌었습니다.** 1kg 들이 소스를 주문했는데 「g」로, 50개들이 소스통을 주문했는데 「piece」로 찍히던 문제입니다. 파는 쪽이 파는 단위(kg·pack)가 그대로 올라갑니다.
- 발주 담는 화면도 같은 단위로 맞췄고, 가격을 주문 단위당 값으로 보여줍니다(그램당 0.0279 → kg당 27.90). 재고 표시는 그대로 취급단위입니다.
- 휴대폰에서 인보이스 올리기 버튼을 누르기 좋게 키웠습니다.
- 배포 백업 정리에서 「최신 10개는 남긴다」가 실제로는 하나도 보호하지 못하던 것을 고쳤습니다.
- 프론트를 고치고 빌드하지 않으면 검증이 통과하지 않도록 막았습니다.
- 주간 서버 리포트가 디스크 원인을 실제 폴더로 알려주고, 자동 보안 업데이트 판정 기준을 실행 기록으로 바꿨습니다.

## v3.85 (2026-09-07) — 브랜드 매출 리포트

**브랜드제너럴의 「리포트」가 이제 브랜드 자신의 매출을 보여줍니다.**

지금까지 리포트 6개 탭은 전부 매장 주문을 집계한 것이라 「성과」 화면과 같은 물건이었습니다.
정작 브랜드가 얼마를 팔았는지 보는 화면은 없었습니다.

### 바뀐 것
- **리포트 = 브랜드 매출** — 브랜드가 청구한 것을 세 갈래로 봅니다: 프로덕트 개별판매(매장 발주 → 거래 청구서) · 구독 판매(브랜드 플랜 월정액) · 로열티·마케팅비 등 수수료. 각각 청구액 / 수금액 / 미수금을 기간별로.
- **매장 판매 분석 6탭은 「성과」 메뉴 밑으로** 이동했습니다. 화면 내용은 그대로이고 위치와 제목만 바뀝니다. 매장 목록에서 매장을 누르면 그쪽으로 갑니다.
- **매출 누락 신호** — 수령은 끝났는데 청구서가 안 나간 발주 건수를 화면에 한 줄로 띄웁니다.
- **브랜드 성과 화면에 오늘·어제** 버튼이 생겼습니다. 라이브 오더와 같은 방식이고, 날짜는 브라우저 시간이 아니라 매장 설정 시간대 기준입니다.
- 재료·상품 데이터의 중복과 잘못된 숫자를 정리하는 도구를 만들었습니다(아직 실행 전). 같은 중복이 다시 생기면 배포가 막히도록 검사 3건을 넣었습니다.

### 운영에서 찾은 것 (수정은 별건)
수령이 끝난 발주 **14건(합계 RM 4,020.57) 전부에 거래 청구서가 없습니다.**
수령 처리 길이 두 개인데 매장이 실제로 쓰는 쪽이 청구서를 만들지 않습니다.
월간 정산서도 청구서를 묶는 구조라 연쇄로 0장이고, **브랜드 매출 기록이 비어 있던 진짜 이유**가 이것입니다.

### 이 라운드에서 틀린 것 (기록)
- 재료를 합칠 때 따라갈 표 목록을 **컬럼 이름만 보고** 만들었는데 둘은 재고아이템을 가리키는 것이었습니다. 그대로 돌렸으면 브랜드 레시피 4줄이 망가졌고 안전장치는 못 잡았을 것입니다. 상위 검증에서 적발됐습니다. 목록의 근거는 이름이 아니라 DB 제약 실측입니다.
- 돈이 새는지 보는 검사를 만들었는데 **기준을 그 API 자신의 응답으로 잡아** 방어를 없애도 통과했습니다. DB에서 독립적으로 기준을 만들어 고쳤습니다.
- 상품 중복을 매장 **이름**으로 묶어 136건이라고 봤는데, 같은 이름 지점이 3개라 생긴 착시였습니다. 실제는 10건입니다.

---

## v3.84 (2026-09-04) — 화면 정돈 + 직접 배송

**재고아이템·프로덕트 화면의 버튼과 배지가 카드 밖으로 나가던 것을 잡고, 배송사에 직접 배송을 넣었습니다.**

### 바뀐 것
- **재고아이템 카드 버튼** — 활성/비활성 버튼이 늘면서 한 줄에 5개가 되어 라벨이 잘리고 카드 밖으로 나갔습니다. 이제 화면이 좁으면 버튼이 아래로 접힙니다(1366px 노트북에서 3줄, 넓으면 2줄).
- **프로덕트 목록의 연결 배지** — 포장재 이름이 길어(최대 89자) 배지가 카드 레이아웃을 밀어냈습니다. 이제 남는 폭 안에서만 자라고, 전체 이름은 마우스를 올리면 보입니다. 재고아이템 다이렉트는 이름을 생략합니다(프로덕트와 같은 물건이라 중복).
- **프로덕트 목록 활성/비활성 필터** — 기본이 **활성만**입니다. `비활성만` / `전체 상태` 를 따로 고를 수 있습니다.
- **배송사에 `Self Delivery`** — 출고를 찍을 때 자기 차로 직접 갖다 주는 경우 고를 것이 없었습니다. 목록 맨 앞에 들어가고, 추적 번호 없이도 출고가 끝까지 통과합니다.

### 이 라운드에서 세 번 틀렸습니다 (기록)
버튼 정돈 하나를 세 번 고쳤습니다 — ①버튼 등분(라벨 잘림) ②`min-width:0` 을 쓴 수정(줄바꿈 대신 축소가 일어나 여전히 잘림) ③grid 로 바꾼 대안(잘림은 없어졌으나 Delete 가 고아로 남고 카드가 550px). 정답은 자식에 min-width 를 주지 않는 flex 줄바꿈이었습니다.
더 중요한 것은 **제 측정이 잘못된 대상을 재고 있었다**는 점입니다 — 측정 도구가 버튼을 카드로 잡아 "이상 없음"이 공짜로 나왔고, 카드 폭이 이상했던 신호를 넘겼습니다. 상위 검증에서 적발됐습니다.

---

## v3.83 (2026-09-04) — 재고아이템 다이렉트

**프로덕트·메뉴가 재고아이템을 직접 가리키게 했습니다. 1개 팔면 그 재고아이템 1개가 빠집니다.**

「재료 다이렉트」로 이으면 뒤에서 `"<이름> (auto)"` 레시피가 몰래 만들어져 물려 있었습니다.
화면에는 레시피가 안 보이지만 데이터에는 있었고, 규칙("재고아이템 또는 레시피, 둘 중 하나")에 없는 세 번째 길이었습니다.

### 바뀐 것
- 프로덕트·메뉴 폼의 재료 입력이 **선택 칸 하나**로 바뀌었습니다. 수량 칸과 줄 추가가 없습니다 — 그대로 1:1 이라서입니다.
- 레시피와 재고아이템 중 하나를 고르면 다른 쪽이 자동으로 비워집니다. 둘 다 보내면 저장이 거부됩니다.
- 목록에 연결 상태가 배지로 보입니다: `재고아이템 다이렉트` / `Recipe: <이름>` / `연결 없음`.
- 매장 메뉴 폼에서 **브랜드가 공유한 재고아이템을 아예 못 고르던 것**을 고쳤습니다.
- 재고아이템에 다이렉트로 붙은 프로덕트는 발주에서 그 프로덕트로 살 수 없습니다 — 재고아이템을 사야 합니다.
- 기존 `(auto)` 레시피 61건(전부 컵·소스통·천끈 같은 포장재)이 다이렉트 연결로 정리됐습니다.

### 잡은 결함
- 저장은 거부되면서 **남의 매장 재료가 이미 붙어 버리던 것** — 검증이 쓰기보다 뒤에 있었습니다.
- 새 마이그레이션이 운영에서 아무 일도 안 하고 끝나도록 되어 있었습니다. 그대로 나갔으면 POS·모바일 메뉴가 전부 멈출 뻔했습니다.
- 테스트 잔재를 치우다 검사 두 개가 조용히 건너뛰며 "전부 통과"로 집계되던 것.

---

## v3.82 (2026-09-04) — 재료 목록 통합

**레시피에 연결되는 재료와 재고아이템이 서로 다른 목록이던 것을 하나로 합쳤습니다.**

원인은 프로덕트를 등록하면 시스템이 그 프로덕트를 재료로 한 줄 더 자동 복제하던 기능이었습니다.
2026-06-08·07-05 에 그렇게 생긴 두 번째 줄 때문에 레시피는 옛 줄을, 발주·재고는 새 줄을 쓰게 갈라져
사서 넣어도 레시피 쪽 숫자가 움직이지 않았습니다. (레시피 연결이 끊긴 적은 없습니다.)

### 바뀐 것
- 재료는 **재고아이템(Stock Items) 한 곳에서만** 만듭니다. 다른 입구 4곳을 막았습니다.
- 브랜드 레시피에서 **재고아이템과 GIT 프로덕트를 한 목록**에서 고릅니다. K-소스도 여기서 고릅니다.
- 재고아이템의 이름·단위·활성 상태를 고치면 브랜드 쪽에 자동으로 따라갑니다(한 방향).
- 재고아이템에 **매장 공유** 설정이 생겼습니다. 쓰는 중인 것을 끄려 하면 어디에 쓰이는지 알려주고 거부합니다.
- 재고아이템 화면의 "매장에 실제로 얼마나 있나" 표시가 처음으로 값을 갖습니다.
- `Shared Ingredients` 메뉴가 없어지고 재고아이템으로 이동합니다. 같은 화면을 두 이름으로 부르던 것도 정리했습니다.
- 데모 계정 앞으로 알림 메일이 나가지 않습니다(반송 메일 원인).

### 안전장치
- 인스펙션 불변식 4건 + 계약 검사 4건 신설 — 같은 분열이 다시 생기면 **배포가 막힙니다.**
- 고장주입 8건 전부 반증 성립(방어를 되돌리면 실제로 실패하는 것을 확인).
- CLAUDE.md 에 규칙 추가: 재료·상품·가격에 **새 목록·자동 복제 경로를 만들지 않는다.**

### 함께 반영된 운영 데이터 정정
- K-DINE IPC GIT 연결 7건 가격 정정(9/9 일치)
- with MIN Cafe 연결 50건을 GIT 프로덕트로 이관(50/50), 중복행 50건 비활성
- 재고아이템 카테고리 중복 정리(18건, 중복 0)

### 검증
`verify-all --full` 17/17 (실브라우저 8역할 크래시 0) · health-check 212/212 · 인쇄 보호파일 8/8 무변경 · 인쇄 라우트 34/34.

### 새 문서
`docs/TRADE_STRUCTURE.md`(구입·판매·브랜드 구조 단일 기준) · `docs/INGREDIENT_UNIFICATION_DESIGN.md`(통합 설계)
...
### 2026-09-07
- 발주를 수령하면 거래 청구서가 반드시 나가도록 고쳤습니다 — 수령을 끝내는 버튼이 셋인데 청구서를 만드는 건 하나뿐이었고, 매장이 실제로 쓰는 버튼이 그 하나가 아니었습니다
- 발주 결제와 청구서 결제가 서로를 따라갑니다 — 한쪽에서 결제하면 다른 쪽도 결제로 표시되고, 되돌리면 미수로 복귀합니다
- 받으면서 그 자리에서 낸 건은 청구서가 처음부터 '결제됨'으로 나갑니다 (미수로 내면 다음 달 정산서에 또 실립니다)
- 월간 정산서가 이미 결제된 청구서를 묶지 않습니다
- 청구서 비고의 수령일이 매장 시간대 기준이 됩니다 (아침 8시 전 수령이 전날로 찍히던 것)
- 배포 백업 30일 보관 정책 — 배포마다 1GB 쌓기만 하고 지우는 코드가 없어 운영에 210개·134G가 쌓였습니다. 다음 배포에서 61GB가 회수됩니다

### 2026-09-03
- 알림함의 상대시간(방금·5분 전·어제)이 아주 오래된 브라우저에서 알림함 전체를 오류 화면으로 만들던 문제 수정 — 그런 기기에서는 예전 영어 표기로 자동 대체되고 화면은 정상 동작합니다

## [SW 4.85] — 2026-09-06 배포 (제품 버전 미상승 — 매장 체감 화면 변화 없음)

### 2026-09-06 — 메일 반송 근절
- 데모 계정으로 알림 메일이 계속 나가 "주소를 찾을 수 없다" 반송이 관리자에게 쌓이던 문제를 해결했습니다. 원인은 발송 규칙이 아니라, 실제로 인증한 적 없는 계정에 "인증 완료" 표시가 잘못 남아 있던 것이었습니다
- 관리자가 계정을 만들 때와 공급업체가 직원을 만들 때 인증을 건너뛰던 경로를 닫았습니다. 이제 항상 인증 메일이 나가고, 링크를 누르기 전까지는 알림 메일을 받지 않습니다(로그인·사용은 그대로)

### 2026-09-06 — 배포 안정화
- 배포가 중간에 아무 말 없이 멈추던 문제를 해결했습니다. 이제 실패하면 이유가 배포 기록에 남습니다
- 단위 정리 작업이 잘못된 값 하나 때문에 전체 배포를 멈추던 것을 고쳤습니다. 처리할 수 없는 값은 "사람이 정할 것" 목록으로 넘어가고 배포는 계속됩니다
- 상품·재고 번호가 행을 지웠다 다시 만들면 이미 쓴 번호를 다시 주던 것을 고쳤습니다(인보이스 번호 포함, 형식은 그대로)

### 2026-09-06 — 브랜드 매니저·정리
- 브랜드 매니저가 자기 브랜드와 매장 목록을 볼 수 있게 됐습니다. 결제 정보(카드사 설정·은행 계좌·구독 금액)는 매니저에게 보이지 않으며, 결제 설정 메뉴도 매니저 화면에서 숨겨집니다
- 매장 재료 등록 화면에 기준단위·기준양 칸이 생겼습니다. 서버가 그 값을 받지 않던 것도 함께 고쳤습니다
- 재료의 기준 숫자에 0 을 넣으면 저장이 거부됩니다(원가 계산이 무너지는 값)
- 사용되지 않던 옛 처리 경로 5개와 열 수 없던 화면 2개를 정리했습니다
- 레시피·재료가 연결되지 않은 상품 수가 배포할 때마다 매장별로 기록에 남습니다(배포를 막지는 않습니다)

## [v3.81] — 2026-09-03 배포


### 2026-09-03 #5 — 언어 전환·문의 읽기 (SW 4.77)
- **화면이 언어를 따라가지 않던 문제** — 원인은 소스에 직접 쓴 한글을 잡아내는 검사가 없던 것이었습니다.
  검사기를 새로 만들어 앞으로는 배포 전에 걸립니다(번역 대상이 아닌 데이터는 이유를 적어 예외 표시)
- **솔루션 개발이슈 화면이 4개 언어로 나옵니다** — 버튼·제목·칸 이름 24건. 배포 내용 본문은 우리 내부
  개발 노트라 한글 그대로 둡니다
- **알림함의 '방금·5분 전·어제' 가 사용자 언어로 나옵니다** — 지금까지 중국어·말레이어 사용자에게는
  영어로 나왔습니다. 문구를 우리가 들고 있지 않고 브라우저 표준을 씁니다
- **문의 목록의 긴 글이 2줄로 잘리고 '더 보기' 로 펼쳐집니다** — 7개 문의 화면이 모두 같습니다
  (운영 문의 SUPP-2026-9449-255 요청)
- **배포기록 화면 아래에 접수된 문의가 함께 보입니다** — 문의 목록에서 실시간으로 읽습니다.
  이 화면은 문의 상태를 바꾸지 않습니다. 이번 배포가 닫은 문의 번호도 '반영: <태그>' 로 표시됩니다
- 좌측 메뉴에서 솔루션 개발이슈가 커뮤니케이션 아래 별도 자리로 옮겨졌습니다
- 플로어플랜의 오프라인 저장 안내 2건·티켓 머지 안내 2건도 4개 언어로 나옵니다
- (내부) 실브라우저 확인 중 결함 2건을 배포 전에 자체 적발·수정했습니다 — 문의 본문이 이중으로 잘려
  펼쳐도 안 보이던 것 / 기록 항목이 빈 줄로 뜨던 것(게이트에 형식 검증 추가)

### 2026-09-03 #4 — 문의 입구 + 오류 화면에서 바로 접수 (SW 4.76)
- **어느 화면에서든 문의로 바로 갈 수 있습니다** — 좌측 메뉴 맨 아래에 **문의하기**가 생겼습니다.
  역할에 맞는 문의 화면으로 바로 갑니다. **요금제와 무관하게 모든 매장에 열립니다** —
  지금까지 문의 메뉴는 Professional 이상에만 있어서 Basic 매장에는 입구가 없었습니다
- **화면이 깨졌을 때 그 자리에서 오류를 보낼 수 있습니다** — "Something went wrong" 화면에
  **이 오류 보내기** 버튼과 "무엇을 하던 중이었나요?" 한 줄이 생겼습니다. 누르면 문의가 바로 접수됩니다.
  오류 내용·화면 주소·계정·앱 버전이 자동으로 붙어서, 따로 설명하지 않아도 재현할 수 있습니다
- 같은 오류를 여러 번 눌러도 접수는 1건입니다. 로그아웃 상태면 로그인 후 문의하시라고 안내합니다
- (내부) 구현 중 전 역할이 진입 즉시 크래시할 결함을 자체 적발해 수정했고,
  그 결함을 일부러 되돌린 빌드로 검사기가 실제로 잡는지까지 확인했습니다

### 2026-09-03 #3 — 솔루션 개발이슈 화면 (SW 4.75)
- **배포할 때마다 개발 현황이 남습니다** — 시스템 관리자에 **솔루션 개발이슈** 화면이 생겼습니다.
  회차별로 무엇이 달라지는지 · 무엇을 확인해야 하는지 · 완료 · 이슈 · 진행중 · 앞으로 할 것 ·
  어떻게 검증했는지가 남습니다
- 기록이 없거나 칸이 비면 **배포가 막힙니다** — 사람 기억이 아니라 기계가 강제합니다

### 2026-09-03 #2 — 앱 설치 (SW 4.74)
- **아이패드·맥에서 설치 버튼이 반응하지 않던 문제 수정** — 아이패드가 데스크탑으로 보고하는 탓에
  기종을 잘못 읽고 있었습니다
- 윈도우는 윈도우용 앱, 안드로이드는 안드로이드 앱, 아이패드·맥은 웹앱(PWA) 설치로 안내합니다
- 기종별 설치 방법을 그림 대신 단계로 안내하는 창이 생겼습니다

### 2026-09-03 #1 — 발주 수령 + 브랜드 레시피
- **발주를 나눠 받아도 받은 양이 제대로 쌓입니다** — 수령 처리가 두 곳에 따로 적혀 있어
  받은 수량이 어긋나던 것을 한 곳으로 합쳤습니다
- **브랜드 레시피 수정이 7주 동안 저장되지 않던 것 수정** — 저장을 누르면 오류가 났습니다
- **레시피 등록·수정 창의 버튼이 아래로 밀려 내려가던 것 수정** — 이제 창 하단에 고정됩니다

### 2026-09-02 #4 — 판매 재고 차감 + 레시피
- **팔린 만큼 재고가 빠지기 시작합니다** — 지금까지 POS 로 받은 주문은 재고를 깎지 못하고
  있었습니다. 주문에 기록된 메뉴를 재고 쪽에서 알아보지 못했기 때문입니다(세트 메뉴는 정상이었습니다).
  이제 레시피가 걸려 있고 재고 수량이 입력된 품목부터 판매할 때마다 재고가 줄어듭니다
- **재고가 0 이라 못 깎은 경우와 메뉴를 못 알아본 경우를 구분해 기록합니다** — 예전에는 둘 다
  "레시피 없음"으로 뭉뚱그려져 문제가 보이지 않았습니다
- **레시피 다운로드에 조리법이 들어갑니다** — 예전 파일은 재료 한 줄이 한 행이라 조리법이 들어갈
  자리가 없었습니다. 이제 **레시피 한 건이 한 줄**이고, 재료요약·조리법 칸이 있으며 재료는 한 칸에
  모여 나옵니다. 브랜드·레스토랑 관리자 모두 같은 버튼으로 받습니다
- **K-DINE with MIN 브랜드 레시피 반영** — 기존 45건 조리법 보완, 신규 7건 추가
  (해물비빔밥·육개장·사전준비 5종). 기존 레시피와 재료 연결은 건드리지 않았습니다

### 2026-09-02 #2 — 발주 결제·현금관리 (화면까지)
- **발주를 받으면서 그 자리에서 결제할 수 있다** — 발주 대기 화면에 **"받을 발주"** 목록이 생겼고
  [수령 + 결제] / [수령만] 을 고를 수 있다. 발주 상세에서도 같은 동작과 **[결제 취소]** 가 가능하다
- **누르기 전에 무엇이 일어나는지 먼저 보여준다** — 재고가 늘고 · 결제가 기록되고 ·
  (현금이면) **열린 시프트의 현금서랍에서 그 금액이 나간다**. 되돌리기가 쉽지 않은 동작이라 그렇게 했다
- **현금서랍에 반영되지 않은 경우를 숨기지 않는다** — 시프트를 안 열었으면 결제는 기록하되
  "현금서랍엔 반영되지 않았습니다"라고 알린다. 열린 시프트가 둘 이상이면 **아예 거절한다**
  (아무 시프트나 고르면 그 시프트의 마감 금액이 틀어진다)
- **결제 취소는 지우는 게 아니라 되돌린다** — 같은 금액의 입금이 기록으로 남고, 마감 기대금액이
  저절로 원래대로 돌아온다. 재고와 수령 기록은 건드리지 않는다
- 결제 수단 3가지(현금·계좌이체·카드). 현금이 아니면 발주에만 기록되고 현금서랍은 그대로다.
  "외상"은 별도 수단이 아니라 **아직 결제하지 않은 상태**다
- 4개 언어 지원
- (내부) 수령 처리가 두 곳에 따로 적혀 있던 것을 한 곳으로 합쳤다 — 응답이 한 글자도 달라지지
  않는 것을 실호출로 증명. 돈 계약 8건은 상시 검사에 들어가 배포 때마다 자동으로 확인된다

### 2026-09-02
- **메뉴에 "저재고 알림 기준" 칸 추가** — 지금까지 메뉴 화면으로 만든 상품은 이 값을 넣을 방법이
  없어서 **재고가 떨어져도 알림을 받을 수 없었다**
- **발주 화면에서 "그대로 파는 상품"을 재고 항목으로 고를 수 있게** —
  내 재고 목록에 그런 상품이 *상품* 표시와 함께 같이 나오고, 공급업체를 붙여 그대로 주문할 수 있다.
  공급업체 카탈로그에서는 기존 상품에 연결하거나 **[상품으로 등록]** 으로 새로 만들 수 있다.
  **판매가는 반드시 사람이 넣어야 하고**, 공급가가 판매가로 자동 복사되지 않는다
- **재고 항목을 "그대로 팔기"로 등록하는 버튼 추가**(매장 재료 화면 · 브랜드 재고 화면) —
  판매가만 넣으면 상품이 만들어진다. **재고는 그 재고 항목 한 곳에만 남는다**(팔리면 거기서 빠진다) —
  같은 물건이 두 곳으로 갈라지지 않게 하기 위해서다. 이미 그렇게 팔고 있으면 알려주고 다시 만들지 않는다
- (내부) "그대로 팔기"로 등록한 상품을 **실제로 출고해 보니** 재고가 재고 항목에서만 빠지고
  (20 → 17) 상품 수량은 그대로였다 — 같은 물건이 두 곳에서 빠지지 않는다는 것을 실호출로 확인하고
  이 검사를 상시 안전망에 넣었다
- **브랜드 재고 화면에도 "외부 공급업체로 등록"이 생겼다** — 지금까지 이 입구는 매장 화면에만 있어서,
  브랜드는 솔루션에 가입하지 않은 업체에서 사는 물건을 화면에서 등록할 방법이 없었다
  (두 화면이 같은 부품을 쓰도록 정리했다)
- (내부) 검증 게이트 자체의 구멍 2건 수리 — 커밋이 바뀌어도 이전 검증 도장이 유효하던 것,
  게이트 파일이 바이너리로 취급돼 변경을 검토할 수 없던 것

## [v3.80-6] — 2026-09-01 배포 (버전 미상승, Irene 지시)

- **표가 잘리던 것이 남아 있던 화면 8곳 마저 수정** — 매출·활동내역·시스템로그·구독관리(관리자)·
  관리자 관리·구독(매니저)·매출(매니저), 그리고 이 화면들이 공유하는 **공용 표 부품 하나**.
  앞선 배포에서 고친 것과 **같은 방식**(세로는 그대로, 가로만 밀어 볼 수 있게)이라 모양 변화는 없다
- (내부) **표를 못 찾던 사각지대를 없앴다** — 이 화면들의 표는 실제로는 `<table>` 이 아니라
  격자(grid)로 그려져 있어서, 실측 도구가 **표로 인식조차 못 하고** 지나가고 있었다.
  격자형 표도 재도록 도구를 고쳤다
- (내부) **4개 역할이 같이 쓰는 화면 3개(`/pos/manager/*`)가 어떤 반응형 측정에도 없었다** — 목록에 추가
- (내부) 직원 목록(브랜드·푸드코트) 두 화면도 측정 목록에서 빠져 있던 것을 추가.
  **고친 화면이 측정 목록에 없으면 "0건"은 통과가 아니라 검사 누락**이다
- 전 역할 재측정 **112건 → 결함 0건** · 인쇄 생명선 8/8 무변경 · 디자인 가드 신규 위반 0

## [v3.80-5] — 2026-09-01 배포 (버전 미상승, Irene 지시)

- **직원 목록 표가 노트북 화면에서 잘리던 것 — 브랜드·푸드코트 화면에도 있었다** — 같은 화면을
  역할별로 복사해 쓰고 있어서, 매장 화면만 고쳤을 때 나머지 둘은 그대로였다. 셋 다 공용 부품으로 통일
- **공급업체 대시보드 표가 잘리던 것 수정** — 1025px 화면에서 표 오른쪽이 잘려 안 보였다.
  카드 모양은 그대로 두고 표만 가로로 밀어 볼 수 있게 했다
- **대시보드 "최근 주문" 표 3곳(브랜드·푸드코트·매장) 같은 방식으로 정리** — 지금은 안 잘리지만
  열이 늘거나 글자가 길어지면 같은 문제가 나던 구조
- (내부) 반응형 실측 도구를 8개 역할로 확대. 더불어 **도구 자체의 거짓 통과를 막았다** —
  토큰이 만료·무효면 로그인 화면을 재고도 "결함 0건"으로 보고하던 것을 `AUTH_FAIL` 로 차단(exit 1).
  고장주입으로 재현→수정→재주입 실패까지 증명했다

## [v3.80-4] — 2026-09-01 배포 (버전 미상승, Irene 지시)

- **목록 표가 노트북 화면에서 오른쪽이 잘리던 것 수정** — 열이 많은 목록(발주 받은 내역 등)을
  1025~1400px 화면(13~14인치 노트북·태블릿 가로)에서 보면 오른쪽 끝 열(작업 버튼)이 잘려서
  아예 손이 닿지 않았다. 이제 표 안에서 가로로 밀어 볼 수 있다. 공용 표 컴포넌트를 고쳐
  이 표를 쓰는 **27개 화면이 한 번에** 정상화됐다. 모바일(카드 모양으로 바뀌는 것)은 그대로
- **재고 대시보드에 "입고 예정" 추가** — 이미 발주해서 들어오는 중인 품목·수량·도착예정일을
  재주문 제안 바로 위에 보여준다. 같은 재료를 또 주문하는 것을 막는 목적. 수량은 재고 목록이
  쓰는 값을 그대로 써서 목록의 "↧ N incoming" 과 숫자가 항상 같다
- (내부) 반응형 실측 도구 추가 — 표가 화면에서 잘리는지 실브라우저로 여러 폭에서 자동 측정
- (내부) 배포 전 자동 점검이 "받은 발주" 화면(브랜드)을 한 번도 안 열어보고 있었다 — 점검 목록에 추가
- **직원 목록 표를 공용 부품으로 통일** — 같은 모양을 따로 만들어 쓰고 있어서 위 문제가 이 화면에만
  남아 있었다. 화면이 좁아질 때 카드로 바뀌는 기준도 다른 화면과 어긋나 있던 것이 함께 맞춰졌다

## [v3.80-3] — 2026-09-01 배포 (버전 미상승, Irene 지시)

- **레시피 목록 CSV 다운로드** — 레스토랑 관리자·브랜드 제너럴 양쪽 레시피 화면에서 목록을 엑셀(CSV)로 내려받기. 화면에 보이는 그대로(검색·분류 필터 적용) 받아지고, 레시피별 재료·수량·단위·원가가 한 줄씩 정리돼 엑셀에서 바로 계산·필터 가능

### 2026-08-31 — **운영 배포 완료** (버전 미상승, Irene 지시)
- **공급업체에 보내는 서류에서 우리 내부 품목명 제거** — 왓츠앱·메일·발주서·인보이스가 공급업체가 쓰는 상품명만 사용 (받는 쪽 창고에 없는 이름이라 대조에 방해)
- **왓츠앱 발주 문구 정리** — 품목명 굵게, 품목 수 표시, 줄마다 수량·단가·소계. 품목 코드는 공급업체가 실제로 쓰는 코드일 때만 표시
- **발주서 PDF 페이지 여백** — 2장부터 내용이 종이 맨 위에 붙던 것 수정. 표 머리글이 매 장 반복되고 품목 줄이 페이지 경계에서 잘리지 않음
- **발주 하나를 보냈는데 목록이 통째로 사라지던 것 수정** — 보낸 건만 목록에서 빠지고 나머지는 그대로 유지
- **전체 발주 보내기에 확인창 추가** — 몇 건·얼마인지 확인 후 진행
- **입고 버튼 분리** — "전량 입고"(전부 도착) / "부분 입고"(일부만 도착 시 품목별 수량 입력)
- **인보이스 발행 알림이 관리자에게 안 가던 것 수정** — 매장당 1명만 기록하는 옛 항목을 읽고 있어서, 관리자가 2명이면 1명만 받고 기록이 빈 매장에서는 아무에게도 안 갔다. 한 곳은 다른 매장 관리자에게 발송되고 있었다
- **직원이 알림 설정 화면을 찾을 수 있게** — 개인 설정인데 매장 설정 권한에 묶여 있어 직원 대부분이 메뉴를 볼 수 없었다. 메뉴 이름도 "시스템 설정" → "알림"으로 정정
- **새 주문 알림 추가** — 새 주문이 들어오면 매장 관리자·직원 기기로 알림. 각자 끄거나 방해금지 시간 설정 가능 (기기에서 알림 허용 1회 필요)
- **테스트 표시가 배포마다 되살아나던 문제 해결** — 브랜드 운영자 계정이 배포할 때마다 테스트로 표시돼 알림을 못 받던 것. 이제 배포가 알아서 바로잡는다
- **발주·인보이스에 단위 표시** — 수량 옆에 kg·g·pack 등 단위가 함께 나온다
- **공급업체에 보내는 서류는 공급업체 상품명으로** — 인보이스·발주 메일이 공급업체가 쓰는 상품명을 쓴다 (우리 내부 재고명은 빠짐)
- **발주 목록 표시방법 선택** — "공급업체 상품명" / "우리 재고명" 전환 버튼. 선택은 기억된다
- **발주 목록 합계바 하단 고정** — 스크롤해도 건수·금액·전체제출이 화면 아래에 붙어 있다
- 인보이스 수량이 소수를 지원하도록 확장 (kg·g 발주가 정수로 깎이던 문제 예방)

### 2026-08-30
- **수동으로 인쇄하는 매장에 뜨던 "밀린 티켓" 경고 없앰** — 자동인쇄를 꺼두고 직접 인쇄하는 매장인데,
  진단 화면이 그걸 모르고 계속 경고를 띄웠다. 자동인쇄가 켜진 매장의 진짜 밀림은 그대로 잡는다.
- (내부) 자동 점검이 시험 계정을 고를 때 계정 표시가 아니라 소속 브랜드가 데모인지로 판정 — 실제 브랜드에
  시험 데이터가 만들어질 수 있던 경로 차단

## [v3.80-2] — 2026-08-30 배포 (버전 미상승, Irene 지시)

- **브랜드 상품도 kg·g 단위로 발주 가능** — 브랜드가 "1kg에 얼마"로 파는 상품을 매장이 2.5kg처럼
  소수로 담을 수 있다. 그전에는 브랜드 상품이 무조건 낱개 주문으로만 보였다.
- **브랜드 상품 등록에 주문 방식 선택 추가** — 개수로 주문(팩·박스) / 무게·부피로 주문(kg, g, L, ml).
  기본은 개수라, 손대지 않으면 지금까지와 똑같다.
- **메뉴·상품에 재료를 넣다 실패해도 기존 재료가 사라지지 않음** — 재료를 고쳐 저장할 때 시스템이
  기존 재료를 먼저 지우고 새로 넣는데, 중간에 문제가 생기면 지운 것만 남아 쌓아둔 재료가 통째로
  없어졌다. 게다가 화면에는 "저장됨"으로 보여서 없어진 줄도 몰랐다. 이제 실패하면 전부 원래대로
  돌아가고, 무엇 때문에 실패했는지 화면에 뜬다.
- **같은 재료를 두 번 넣을 수 없게 됨** — 브랜드 상품 레시피에 같은 재료가 두 줄 들어가면
  입고할 때 재고와 원가가 두 배로 계산됐다.
- (내부) 재료 저장 실패 상황을 배포 전 자동 점검에 추가 — 같은 문제가 다시 생기면 배포가 막힌다.
- **신규 발주 화면이 휴대폰에서 제대로 보임** — 장바구니가 화면 절반을 무조건 차지해서 상품 목록이
  한 줄로 눌려 있었다(작은 폰에서는 상품 카드가 한 장도 안 들어갔다). 이제 장바구니는 화면 아래 한 줄로 접혀 있고,
  담은 개수와 합계가 거기 바로 뜬다. 탭하면 펼쳐지고, 펼쳐도 상품 목록이 보인다. 넓은 화면은 그대로
- **브랜드 재고도 발주와 연결** — 재고 화면에서 입고하고 발주에서 또 받으면 재고가 두 번 더해지던 것을
  매장에 이어 브랜드에도 적용. 입고 창에서 진행 중 발주를 보여주고 "이 발주 건으로 처리"를 고를 수 있다
- 화면 이동 중 취소된 요청이 오류로 기록되던 것 일부 정리

---

## [v3.80] — 2026-08-28 배포

### 2026-08-30 배포 (버전 미상승 — Irene 지시)

### 발주
- **발주 상세에서도 "받았다"를 누를 수 있음** — 목록에는 있는데 상세 화면에는 입고 버튼이 없어서,
  발주해 놓고 물건을 받아도 상세에서는 아무것도 못 하던 문제. 운영 발주가 전부 "제출됨" 상태에 머물러 있어
  사실상 모든 발주가 여기 걸려 있었다. 이제 목록과 상세가 같은 기준으로 동작한다.
  작성 중(draft)·오너 승인대기 발주는 그대로 막힌다(승인을 건너뛰지 못하게)
- **재고에서 입고할 때 진행 중인 발주를 알려줌** — 같은 물건을 재고 화면에서 입고하고 발주에서 또 받으면
  재고가 두 번 더해지던 문제. 이제 입고 창에서 그 품목이 담긴 발주가 있으면 목록으로 보여주고,
  "이 발주 건으로 처리" 를 고르면 발주 수량·상태까지 함께 갱신된다. 발주와 무관한 입고는 지금처럼 그대로 가능

### 표시
- **발주 상세 모바일 화면 정리** — 번역이 빠져 화면에 코드가 그대로 보이던 두 단계(운송 중·도착)를 4개 언어로 채움.
  같은 내용을 두 번 그리던 배송 추적을 접이식으로 바꿔 첫 화면이 짧아짐.
  버튼 줄이 모바일에서 어긋나던 것과 아이콘 버튼 높이가 옆 버튼과 다르던 것을 맞춤.
  품목 카드가 항목마다 다섯 줄씩 차지하던 것을 세 줄로 줄임

### 내부
- 발주 승인 안전장치가 소스에서 변수 **이름을 찾는 방식**이라 이름만 바꿔도 실패하던 것을,
  실제 규칙(승인대기·작성중은 입고 불가 + 두 입고 경로가 같은 기준 사용)을 검사하도록 교체
- **오너 승인대기 발주가 3번의 배포에서 연속으로 저장 안 되던 문제 수정** —
  승인대기 상태값을 넣는 작업이 매번 정상 실행됐는데, 같은 배포의 다른 작업이 30초 뒤
  상태 목록을 통째로 덮어쓰면서 그 값을 지우고 있었다. 지우는 쪽 2곳을 "값을 더하기만 하는" 방식으로 바꿈.
  오너 연결 매장(Seoul BBQ House · Downtown Pizza · Sunset Cafe)이 발주를 제출하면 터졌을 잠복 결함
- 배포 안전장치 추가 — 개발서버에 있는 상태값이 운영에 없으면 **배포를 멈춘다**.
  전에는 차이를 감지하고도 "대체로 무해"로 넘겨서 위 문제가 3번 통과했다
### 발주
- **같은 품목을 다시 담으면 수량이 합쳐짐** — 전에는 어제 담은 장바구니에 오늘 같은 품목을 담으면 줄이 하나 더 생겨,
  같은 물건이 두 줄로 발주되곤 했다. 이제 장바구니(작성 중)에서는 줄이 늘지 않고 수량만 늘어난다.
  이미 제출한 발주서는 건드리지 않고 새 장바구니가 따로 생긴다
- **발주 이메일에 품목 내역이 다 나옴** — 그전에는 발주번호·구매자·총액·상태만 있어서 뭘 주문했는지 보려면
  화면에 들어가야 했다. 이제 품목명·수량·단가·금액 표와 총액이 메일 본문에 바로 나온다(20줄 초과 시 나머지는 건수로 요약).
  판매자 접수 메일과 오너 승인대기 메일 둘 다

### 표시
- **수량에서 불필요한 소수점 제거** — `1.00 pack` → `1 pack`. 실제로 소수인 값(레시피의 0.25 kg 등)은 그대로 유지.
  판매자 주문 접수, 발주 목록 추천 수량, 공급업체 재고 이동, 레시피 재료 수량에 적용
- **메뉴명과 페이지 제목 통일** — 사이드바는 "Sales Orders"인데 페이지 제목은 "Live Orders"로 달랐던 것을 4개 언어 모두 맞춤
  (브랜드·푸드코트·공급업체 공용 화면)

### 내부
- 배포 스크립트: 새 마이그레이션의 **첫 실행 실패가 조용히 넘어가던 구멍**을 막음.
  파일이 이미 운영에 있는 경우는 종료코드로 배포를 세웠지만, 처음 복사해 실행하는 경로만 실패를 삼키고 있었다
...
### 2026-08-28
- with MIN 재고 장부 정렬 — 회사(브랜드 제너럴) 화면에만 있던 재고아이템 288개를 브랜드 재고 장부로 옮김.
  이름이 겹치는 62개는 기존 재고에 합치고(새로 만들지 않음), 나머지 226개는 새로 등록.
  이미 걸려 있던 공급업체 연결 285개를 그대로 가져와 **옮긴 재고를 바로 발주할 수 있음**.
  원본 목록과 과거 발주 이력은 손대지 않고 그대로 보존
- 깃컨설팅 판매상품 81개를 매장 발주 목록에 등록 — 재고 수량을 세지 않는 **발주 전용** 항목으로 담김
- 결과: with MIN Cafe 화면에서 보이는 재고 157 → 460개, 바로 발주 가능한 것 66 → 432개
- **신규 화면 "일괄 연결"**(재고/구매 메뉴) — 재고·공급업체 상품·연결을 한 화면에서 한 번에 처리.
  이름이 정확히 같은 것만 자동 제안하고 **확정은 사람이** 한다(잘못 합쳐지는 사고 방지).
  담을 수 없는 상품은 사유와 함께 그대로 보여주고 숨기지 않음. 이미 연결된 것은 다시 눌러도 중복 안 생김
- **커버리지 탭** — 공급처가 없어 발주 못 하는 재료, 재료가 연결 안 된 레시피를 한눈에
- 가져올 때 공급가를 재고 원가에 반영할지 행마다 선택(기본 꺼짐 — 손으로 정한 원가를 덮어쓰지 않음)
- 잘못 만들어져 있던 빈 공급업체 등록 1건 정리(나머지 1건은 기존 재고 12개가 참조 중이라 보존)
- 공급업체 화면과 오너 화면이 열리지 않던 문제 수정 (배포 전 검증에서 발견·수정)

### 2026-08-27 (운영 배포 완료 — Irene 지시로 버전 미변경, v3.78 유지)
- 발주서에 공급업체 상품명·SKU 표시 — 공급업체에게 실제로 나가는 문서(인쇄본·WhatsApp·이메일)에
  공급업체의 판매품목명과 SKU 가 주 표기로 나가고, 우리 내부 이름은 이름이 다를 때만 "Buyer ref:" 로 병기.
  받는 쪽이 자기 창고에서 상품을 대조할 수 있게 됨. 인쇄본에 SKU 열 신설.
- 위 표시는 **기존에 발주한 건에도 그대로 적용** — 데이터 작업 없이 옛 발주서도 바뀜
- 공급업체 수신함의 발주서 Print 버튼이 눌러도 인쇄본이 열리지 않던 문제 수정
  (인쇄 화면이 공급업체 계정에 허용된 경로 밖에 있었음). 공급업체 전용 인쇄 경로를 추가해 해결하고,
  공급업체 화면 권한 폭은 넓히지 않음 — 남의 발주서는 여전히 볼 수 없음을 실측 확인

### 2026-08-25 (운영 배포 완료 — 버전 미변경 여부 확인 대기)
- (미배포) 발주 장바구니·품목 목록에 포장 표기 표시 — 공급업체 상품명에 적힌 `50SETS X 5PKTS/CTN` 같은 그룹 단위를 그대로 노출
- (운영 데이터) GIT Consulting 재고 실사 반영 · 옛 표기 중복 상품 21건 정리(한글 병기로 통일, 상품 109→90건)
- (운영 데이터) 신규 품목 2건 등록(플라스틱 투명컵·뚜껑, 판매가 원가+40%) + 재고 18팩씩 + 공급업체 연결
- (운영 데이터) TK 1000 매입 단가 입력 (1 CTN RM80 = 팩당 RM16)
- (미배포) 재고관리에서 공급처가 연결된 품목까지 "연결 안 됨"으로 표시되고 주문 기능이 사라지던 문제 수정
  — 연결 여부를 재고부족 발주제안이 아니라 실제 공급처 연결로 판정 (브랜드 285건 · 매장 77건이 발주 불가로 보이던 상태)
- (미배포) 재고관리 목록: 사진 없을 때 뜨던 빈 회색 네모 제거, 품목명 칸 확대 및 이름 줄바꿈(잘리지 않음)
- (미배포) 발주 장바구니: 품목명 전체 표시, 현재 재고 함께 표시, 우측 패널 드래그로 폭 조절(기억됨)
- 발주 수량 화살표가 0.01 씩 오르던 문제 수정 — 팩·개·박스처럼 세는 단위는 1씩, 무게·부피만 소수.
  발주 카트 · 수령 수량 · 반품 수량 세 곳 모두 적용 (단가·단위환산 비율은 그대로 소수)

### 2026-08-24 (운영 배포 완료 — 버전 미변경, v3.78 유지)
- 발주 화면 Stock Items 가 통째로 안 보이던 문제 수정 (재고 추적 상태를 잘못 읽어 전 품목이 숨겨짐)
- 발주 화면 카테고리 필터가 비어 있던 문제 수정
- 발주 목록 보기: 긴 상품명 2줄 표시, 카테고리·현재 재고 함께 표시 (리스트·카드 공통)
- 재고관리에 카테고리 필터 · 재고 0 필터 추가
- 재고관리에서 공급처가 연결되지 않은 품목을 구분 표시 (발주 가능 여부를 목록에서 바로 확인)
- 발주 승인 대기 상태가 저장되지 않던 문제 수정 (오너 승인이 켜진 매장의 발주 제출 실패)
- 앱 캐시 버전 갱신 — 배포 후 화면이 옛 상태로 남던 문제 완화
- (미배포) 브랜드 재고 수량에 매장 보유분을 더하던 계산 제거 — 다음 배포 포함

### 2026-08-22
- 브랜드 재고 화면에 산하 매장 보유분 참고 표시 (D2) — 적용 화면 확정 후 배포 예정

## [v3.78] — 2026-08-22 배포

**레시피 없는 상품도 재고가 관리됩니다**

- **그대로 파는 상품(캔음료·병맥주·완제품 등)의 재고가 이제 판매하면 줄어듭니다.** 지금까지는 레시피가 있는 상품만 재고가 빠졌고, 레시피가 없는 상품은 팔려도 재고가 그대로였습니다. 레스토랑 관리자(메뉴)와 브랜드 제너럴(프로덕트) 양쪽 모두 적용됩니다.
- 메뉴/상품 편집 화면에서 **"이 상품 재고 관리"를 켜고 수량과 단위를 입력**하면 됩니다. 레시피를 연결한 상품에는 이 항목이 나타나지 않습니다(둘 중 하나만 씁니다).
- 재고보다 많이 팔려도 재고가 마이너스로 내려가지 않고, 채우지 못한 수량은 기록에 남습니다.
- **판매가나 원가가 비어 있는 상품을 목록에서 바로 알 수 있습니다.** 목록 위 요약에서 건수를 누르면 해당 상품만 골라 볼 수 있습니다.
- **변형 상품을 다시 등록할 수 있습니다** — 이름이 비슷한 상품(예: 종이볼 L/M/S)을 등록할 때 확인창이 뜨고, "그래도 등록"을 누르면 진행됩니다. 완전히 같은 이름·SKU는 계속 차단됩니다.
- 브랜드 재고 내역에 매장에서 발생한 사용분이 어느 매장 것인지 표시됩니다.


## [v3.76] — 2026-08-20 배포

**공개 로그인 카드가 실제 매장을 열던 문제 차단**

- 로그인 페이지의 테스트 계정 카드로 실제 영업 매장에 들어갈 수 있던 문제를 막았습니다. 이제 데모 매장이 아닌 곳에 연결된 계정은 빠른 로그인이 거부됩니다.
- 테스트(QA) 계정 카드는 운영 로그인 페이지에서 더 이상 표시되지 않습니다. 데모 체험 카드 5종은 그대로 사용할 수 있습니다.
- 로그인 실패 시 실제 사유가 표시됩니다(기존에는 관계없는 안내 문구가 표시됐습니다).
- 데모 브랜드/푸드코트 산하 체험용 지점 5곳의 분류를 데모로 정정했습니다.

## [v3.75] — 2026-08-20 배포

**멀티 역할/매장 선택 로그인 (1계정 여러 자격)**

- **계정 하나로 여러 매장을 오갈 수 있습니다** — 지금까지는 다른 매장을 보려면 그 매장 계정으로 다시 로그인해야 했습니다. 이제 시스템 관리자가 권한을 주면, 로그인한 뒤 **"어디서 일할지" 고르는 화면**에서 매장을 골라 들어갈 수 있습니다
- **고를 것이 하나뿐이면 화면이 안 뜹니다** — 지금 쓰시는 모든 매장이 여기 해당합니다. 권한을 실제로 부여하기 전까지는 **화면도 동작도 전과 똑같습니다**
- **본래 자기 자리로는 언제든 돌아옵니다** — 선택 화면에 "기본" 표시가 붙은 카드가 항상 있습니다
- **권한은 더해지는 게 아니라 바꿔 쓰는 것입니다** — 다른 매장 자격으로 들어가 있는 동안에는 원래 보던 매장이 닫힙니다. 한 번에 한 곳만 봅니다
- **한 창에서 바꾸면 같은 브라우저의 다른 탭도 함께 따라옵니다** — 안 그러면 POS나 주방 화면이 열려 있는 탭에서 **결제와 자동 인쇄가 아무 안내 없이 멈춥니다**. 계산·인쇄 중인 화면에서는 바로 바꾸지 않고 안내를 먼저 띄웁니다
- **매장 POS로 지정된 기기에서는 전환 버튼이 없습니다** — 현장에서 사람을 바꾸는 건 기존 PIN 전환이 그대로입니다. 다른 매장 자격으로 그 기기의 카운터 화면에 들어가면 안내와 함께 막습니다
- **권한을 회수하면 로그아웃이 아니라 본래 계정으로 돌아옵니다** — 안내가 뜨고 선택 화면으로 갈 수 있습니다. 회수된 매장은 즉시 닫힙니다
- **부여·회수는 시스템 관리자만 할 수 있습니다** — 매니저 상세 화면의 "매장 접근 권한"에서. 삭제된 매장에 남은 권한도 표시돼 정리할 수 있습니다

**개발 안전장치**

- **검증 없이 "완료"라고 끝낼 수 없게 하는 정지 장치를 넣었습니다** — 결제·권한·DB 같은 민감한 변경이 검사 합격 기록 없이 마무리되려 하면 시스템이 막습니다. 검사받은 뒤 코드를 한 줄이라도 고치면 합격 기록이 자동으로 무효가 됩니다
- 화면 검사 도구가 **아무것도 검사하지 않고 "통과"로 끝나던 문제**를 고쳤습니다

## [v3.74] — 2026-08-20 배포

- **은행송금 확인 버튼(Confirm Payment)이 눌리지 않던 문제를 고쳤습니다** — 라이브 오더 화면에서 손님이 올린 송금 증빙을 확인하려고 눌러도 **아무 반응이 없었습니다**. 화면 내부 오류로 요청 자체가 서버에 전송되지 않던 것이고, **6월 5일부터 계속 그 상태였습니다**(테이블 화면·POS 결제창에서는 정상 동작했기 때문에 드러나지 않았습니다)
- **이제 실패하면 이유가 화면에 보입니다** — 지금까지는 서버가 거부하거나 인터넷이 끊겨도 아무 표시 없이 창이 닫혀서, 매장에서는 "됐는지 안 됐는지" 알 수 없었습니다. 이제 창이 그대로 열린 채 사유가 뜨고 다시 시도할 수 있습니다
- **응답이 없으면 15초 후 자동으로 알려줍니다** — 이전에는 무한정 기다리며 버튼이 잠긴 채로 남았습니다
- **결제는 됐는데 주문이 주방으로 안 넘어간 경우를 따로 알려줍니다** — 이전에는 조용히 넘어가 티켓이 안 나갔습니다
- 🔧 같은 종류의 오류가 다른 화면에도 있는지 전체 점검했고(562개 파일), **다른 곳에는 없었습니다**. 앞으로 같은 실수가 배포되지 않도록 자동 검사를 추가했습니다
- 🔧 상위 모델(Fable) 교차검증에서 위 자동 검사의 **빈틈 하나를 찾아 메웠습니다** — 버튼에 바로 붙은 형태의 코드는 검사를 그냥 통과하고 있었습니다(같은 사고가 또 나도 못 잡는 상태). 지금은 잡습니다
- 테이블 화면에서 결제 확인이 실패했을 때 **실패 사유가 뭉개져 나오던 것**을 라이브 오더 화면과 똑같이 맞췄습니다

## [v3.73] — 2026-07-31 배포

### 2026-07-31 (결제 시각 기록 + 배포 스모크가 실매장을 건드리던 것 수정)
- **결제 시각이 이제 기록됩니다** — 지금까지 매장 POS 에서 결제를 완료하면 "결제했다"는 사실만 주문에 남고, **언제 받았는지는 어디에도 저장되지 않았습니다**. 이제 결제 건별로 시각·금액·수납한 직원이 남습니다
- **마감(시재)이 정확해집니다** — 결제 시각이 없어서 마감이 "주문을 받은 시각"으로 대신 계산하고 있었습니다. 그래서 **교대가 바뀐 뒤에 받은 돈이 앞 교대로 잡혔습니다**. 이제 실제로 받은 시각 기준입니다
- **부분 결제 잔액을 서버가 정확히 압니다** — 지금까지 결제 완료 주문의 "받은 금액"이 0으로 남아 있었습니다
- **온라인 결제(카드·PayPal)가 동시에 두 번 요청돼도 한 번만 기록됩니다** — 두 요청이 거의 동시에 들어오면 같은 결제가 원장에 두 번 남아 매출·마감이 2배로 잡힐 수 있었습니다 (실제 발생 이력은 없음 — 켜지기 전에 막았습니다)
- 매장에서 보이는 화면·동작은 **변화 없습니다**. 이미 지난 주문의 기록은 손대지 않습니다(없는 결제 시각을 지어내지 않기 위해)
- 🔧 **(중요) 배포 점검이 실제 영업 중인 매장에 테스트 주문을 넣고 있던 것을 고쳤습니다** — 배포할 때마다 실매장에 RM1 테스트 주문이 만들어지고 취소됐고, 그 취소 안내표가 그 매장 **주방 인쇄 대기열에 쌓이고 있었습니다**(자동인쇄를 켰다면 유령 취소표가 주방에서 나올 수 있었습니다). 이제 데모 매장에서만 돌고, 데모가 아니면 아예 중단합니다
- 🔧 실매장에 쌓여 있던 테스트 주문 **165건을 이력에서 정리**했습니다 — 그 매장의 6~7월 **취소 통계가 165건에서 실제값 1건으로 정상화**됐습니다 (데모 매장 236건도 함께 정리)

## [v3.72] — 2026-07-27 배포

### 2026-07-27 #2 (AI 카메라 서빙 + 윈도우앱 인쇄)
- **AI 카메라 서빙이 이제 실제로 동작합니다** — 기능과 화면은 이미 있었는데 Enterprise 요금제에 이 기능이 등록돼 있지 않아 매장이 쓰면 "플랜에 포함되지 않음" 오류가 났습니다. 등록했습니다 (Enterprise 전용, 하위 요금제는 그대로 차단)
- **카메라를 서빙 화면 안으로 옮겼습니다** — Floor Plan > Items 안에서 "사진으로 찾기" 버튼을 누르면 촬영 → 해당 주문 항목을 찾아 바로 서빙합니다. 못 알아보면 목록에서 직접 고를 수 있습니다
- **처음 쓰는 사람도 바로 인식됩니다** — 이전에는 첫 촬영이 실패한 뒤에야 메뉴 사진 학습이 시작돼, 처음 써 본 사람은 반드시 "못 찾음"을 봤습니다. 이제 카메라를 켜는 순간 준비됩니다
- 사진 인식 요청 한도가 IPv6 환경에서 우회될 수 있던 것을 막았습니다
- **윈도우 앱 0.1.10** — 인쇄 관련 8건 수정: 같은 티켓이 중복으로 나오던 경우, 정상 인쇄인데 실패로 보고돼 다시 찍히던 경우, 용지 없음·커버 열림을 성공으로 처리하던 것, 프린터 이름이 바뀌면 조용히 전부 멈추던 것, 1m 넘는 긴 영수증이 소리 없이 잘리던 것 등
- ⚠️ 윈도우 앱은 **실제 프린터 종이 확인이 아직 남아 있습니다** — 매장 배포 전 1회 확인 필요

### 2026-07-27 (소켓 인증 강제 전환 — 운영 적용됨, 코드 배포 없음)
- **실시간 주문 연결에 로그인 확인을 강제했습니다** — 이제 로그인하지 않았거나 위조된 접속은 실시간 주문·고객화면 연결이 아예 거부됩니다. 이전에는 접속만 하면 다른 매장 화면에 가짜 장바구니를 띄우거나 진행 중인 결제에 회원을 붙여 포인트를 적립시키는 것이 가능했습니다
- 정상적으로 로그인한 매장 기기는 **아무 변화가 없습니다** (전환 전 20시간 동안 운영 접속 291건이 전부 정상 토큰이었음을 확인한 뒤 전환)
- 이 변경은 **코드 배포가 아니라 서버 설정 전환**이라 앱·화면 업데이트가 필요 없습니다
- ⚠️ 기록 정정: 아래 "2026-07-26 #2 마감 기대금액" 항목은 **이미 운영에 반영되어 있습니다**(v3.71 배포에 함께 실려 나감 — 2026-07-27 체크섬 실측으로 확인). 미배포 목록에 남아 있던 것은 기록 착오입니다

### 2026-07-26 #2 (마감 기대금액 근본 수정)
- **마감(시재) 화면의 예상 금액이 0으로 나오던 문제를 고쳤습니다** — 교대를 마감하면 예상 현금·카드·이월렛이 모두 0으로 계산돼, 세어 넣은 현금 전액이 "초과"로 표시될 상황이었습니다. 매장이 아직 마감을 닫은 적이 없어 실제 피해는 없었습니다
- 원인은 마감이 **분할결제 기록만** 보고 있었고, POS 의 일반 결제는 주문에 기록되어 마감이 그것을 못 보던 것입니다. 이제 분할결제는 결제 건별로, 일반 결제는 주문 기준으로 함께 집계합니다
- **직원식(staffMeal)은 예상 금액에서 빠집니다** — 실제로 받은 돈이 아니라서, 포함하면 마감 때 없는 돈이 "부족"으로 잡혔습니다
- **Z-Report 의 결제 건수**도 이제 실제 결제 건수를 표시합니다 (매출은 있는데 건수가 0으로 찍히던 문제)
- 참고: 마감 금액과 대시보드 매출은 기준이 달라(마감=결제 완료 기준 / 대시보드=주문 완료 기준) 항상 똑같지 않을 수 있습니다. 정상입니다
- 취소·삭제·미결제 주문은 종전대로 예상 금액에서 제외됩니다 (직원이 부당하게 "부족" 판정을 받지 않도록)
- 인쇄·주문·결제 동작은 변화 없습니다. 데이터베이스 변경도 없습니다

## [v3.71] — 2026-07-26 배포

### 2026-07-26 (손님 정보·화면 보호 강화 + 안전망 확충)
- **다른 매장 손님의 개인정보를 볼 수 없도록 막았습니다** — 로그인한 계정이면 누구나 다른 매장 손님의 이름·전화번호·이메일·포인트 적립 내역을 조회할 수 있던 경로를 차단했습니다. 이제 자기 매장(및 관할 매장) 손님만 조회됩니다
- **다른 매장 손님의 포인트를 건드릴 수 있던 경로를 차단했습니다** — 적립·사용·조정·환원·가입포인트 5개 경로 전부에 매장 확인을 넣었습니다
- **고객 표시화면(Customer Display) 보호** — 다른 매장의 고객 화면에 가짜 장바구니를 띄우거나, 화면을 강제로 초기화하거나, 진행 중인 계산에 엉뚱한 회원을 붙여 포인트를 적립시킬 수 있던 실시간 경로를 막았습니다. **매장에서 보이는 동작은 그대로입니다**(현재 설정에서는 기록만 남기고 차단은 다음 단계에서 켜집니다)
- **모바일 주문에서 엉뚱한 매장에 주문이 붙던 경우를 없앴습니다** — 매장 정보를 불러오지 못한 상태에서 주문을 시작하면 손님 세션이 다른 매장(1번)에 묶일 수 있었습니다. 이제 매장이 확정되지 않으면 진행하지 않고 다시 시도하도록 안내합니다
- (내부) 매장 경계 자동 점검 범위를 넓히고, 드러난 50개 지점을 실제 호출로 전수 확인했습니다 — **무관한 계정에는 전부 차단(실제 유출 0)**
- (내부) 안전망 추가 — 소켓 경계 테스트 15건 신설, 배포 게이트에 계약 테스트(금액공식·설정보존·소켓경계) 편입, 상시 회귀 점검 42건 → 49건
- 주문·결제·인쇄 동작에는 변화가 없습니다. 데이터베이스 변경도 없습니다

### 2026-07-25 (매장 정보 보호 강화 — 다른 매장 자료 열람·변경 차단)
- **다른 매장의 정보를 볼 수 없도록 막았습니다** — 그동안 우리 시스템에 로그인한 계정이면 누구나(직원·공급업체 계정 포함) 다른 매장의 결제 설정·프린터 설정·사업자등록번호·세금번호·계좌정보까지 조회할 수 있었습니다. 이제 자기 매장과 관할 매장만 열립니다
- **손님용 매장 페이지(QR 링크)에서 내부 정보가 빠졌습니다** — 로그인 없이도 매장의 내부 설정과 사업자 정보가 노출되던 것을 손님에게 필요한 항목만 남기도록 정리했습니다
- **다른 매장을 임의로 '정지' 시킬 수 있던 경로를 차단했습니다** — 정지되면 그 매장이 로그인을 못 하게 되는 심각한 문제였습니다
- **다른 매장의 설정을 덮어쓸 수 있던 경로를 차단했습니다** — 읽기는 막혀 있었는데 저장만 열려 있었습니다
- **다른 매장의 실시간 영업 정보 노출을 차단했습니다** — 손님 이름·전화번호·주문 내역·매출·결제 증빙까지 조회되던 경로입니다
- **매장 목록이 역할에 맞게 좁혀졌습니다** — 매장 관리자·직원·공급업체 계정에 전체 매장 목록(당일 매출 포함)이 내려가던 것을 자기 매장만 보이도록 수정했습니다
- 매장 운영에는 변화가 없습니다. 인쇄·주문·결제 동작은 그대로이며, 데이터베이스 변경도 없습니다

## [v3.70] — 2026-07-24 배포

### 2026-07-24 #3 (모바일 주문 이월렛 종류 기록 — 운영 배포)
- **모바일(QR) 주문에서도 이월렛 종류가 기록**됩니다 — 그동안 POS에서 받은 결제만 종류가 남고, 손님이 폰으로 QR 결제한 주문은 종류가 비어 있었습니다. 입점몰 매출보고에서 Touch 'n Go를 따로 집계하려면 이 부분이 반드시 필요합니다
- 매장이 취급 이월렛을 **1개만** 지정했으면 손님에게 묻지 않고 자동으로 기록되고, **2개 이상**이면 결제 화면에서 손님이 고릅니다. 지정하지 않은 매장은 종전과 동일하게 묻지 않습니다
- 오프라인 상태에서 **나눠 결제(분할결제)** 한 건도 카드/이월렛 종류가 보존됩니다 (기존에는 이 경우만 유실)

### 2026-07-24 #2 (카드·이월렛 결제 종류 설정 통일)
- **카드도 매장이 취급하는 종류만 고를 수 있습니다** — 그동안 결제화면에 Visa/Master/Amex/Debit/Other 5개가 항상 떴는데, 이제 설정 > 결제수단에서 **우리 매장이 받는 카드만** 지정하면 그것만 표시됩니다. Amex를 안 받는 매장에 Amex 버튼이 뜨지 않습니다
- **카드도 1종만 지정하면 자동 기록** — 캐셔가 매번 누를 필요가 없습니다 (이월렛과 동일)
- **이월렛도 "필수 여부"를 매장이 켜고 끌 수 있습니다** — 그동안은 취급 이월렛을 2개 이상 고르면 무조건 필수였는데, 이제 끄면 바쁠 때 건너뛸 수 있습니다. 결제화면에도 필수(*)인지 선택(Optional)인지 표시됩니다
- 카드와 이월렛이 **같은 규칙**으로 동작합니다 — 취급 종류 지정 + 필수 여부, 1개면 자동 기록, 2개 이상이면 결제 시 선택
- (내부) 기존 매장 설정은 그대로 유지됩니다 — 카드 종류를 지정하지 않은 매장은 기존 5종이 그대로 보이고, 이월렛 필수 여부는 기존 동작(2개 이상이면 필수)이 기본값. 운영 전 매장 판정 대조 결과 **배포 즉시 동작 변화 0** 확인

### 2026-07-24 (배포 전 전수 점검 + 주방티켓 유실 방지)
- **오래 열려 있던 테이블의 추가주문 티켓이 안 나오던 문제를 근본 수정** — 하루(24시간)가 넘도록 닫지 않은 테이블에 메뉴를 추가하면, 그 추가분 주방티켓이 조용히 인쇄되지 않을 수 있었습니다. 이제 **"주문을 언제 받았는지"가 아니라 "인쇄가 언제 필요해졌는지"** 를 기준으로 판단해, 오래 열린 테이블이라도 새로 추가한 라운드는 정상 인쇄됩니다
- 취소·이동·품목취소 안내표는 종전대로 **주문이 오래됐어도 반드시 인쇄**됩니다 (직원이 지금 한 동작이므로)
- 인쇄가 켜지지 않은 매장에서 미인쇄 표시가 무한 누적돼 **신규 주문이 인쇄 대기열 밖으로 밀리던 문제**의 방어는 그대로 유지됩니다
- (내부) 배포 전 기존 운영 기능 전수 회귀 점검 — 주문관리·결제·인쇄·데이터베이스·전 역할 화면을 실제 호출로 검증, 기존 기능 이상 없음 확인
- (내부) 과거 이월렛 결제를 GrabPay로 일괄 기록하려던 작업은 **하지 않기로 결정** — 어떤 이월렛이었는지 알 수 있는 근거가 없어 사실과 다르게 기록될 수 있고, 몰 보고 결과도 동일하기 때문입니다. 이월렛 종류 구분은 매장이 설정에서 취급 이월렛을 지정한 **이후 주문부터** 기록됩니다

### 2026-07-23 (입점몰 매출보고 + 이월렛 종류 구분)
- **입점 쇼핑몰 매출 자동보고 연동** — IOI Mall(The Fire 매장)의 시간별 매출을 몰 시스템(Tangent)에 자동 보고하는 연동을 완성하고, 몰 인증·전송이 정상 동작함을 확인했습니다. 운영 가동은 몰이 정식(production) 접속 정보를 발급하면 켜집니다
- **이월렛(전자지갑) 종류 구분** — POS 결제 시 이월렛을 Touch 'n Go / GrabPay / Boost / ShopeePay / DuitNow 등으로 구분해 기록할 수 있습니다. 설정 > 결제수단에서 **매장이 취급하는 이월렛을 지정**하면, **한 가지만 쓰는 매장은 결제 시 자동으로 기록**(캐셔가 따로 고를 필요 없음), 여러 개면 결제할 때 선택합니다. 몰 매출보고에서 Touch 'n Go를 별도로 집계하기 위한 기능입니다
- (내부) 결제 종류 기록이 POS·플로어플랜·라이브오더 등 **모든 주문 경로에서 일관**되게 저장되도록 정비했습니다

### 2026-07-22 (윈도우 데스크탑앱 테스트 수정 3건, 운영 배포)
- **예약 테이블 표시 깨짐 수정** — Floor Plan에서 예약된 테이블의 글자(테이블명·인원·예약시간)가 원 밖으로 넘쳐 겹치던 문제를 고쳤습니다. 이제 원 안에 깔끔하게 표시되고, 공간이 부족하면 말줄임(…)으로 정리됩니다 (Windows·macOS 동일)
- **프린터 오류 알림 개선** — 자동인쇄 실패 알림 띠에서 "Dismiss(닫기)"를 누르면 **같은 오류로 다시 뜨지 않습니다**(잠시 후 재등장하던 문제 해결). 알림 위치도 **화면 하단**으로 옮겨 상단 메뉴·버튼을 가리지 않습니다
- **예약 체크인 시 상태 자동 반영** — Floor Plan에서 예약 테이블에 "Check in (New Order)"로 주문을 시작하면 예약 상태가 **자동으로 '착석(Seated)'** 으로 바뀌고 주문과 연결됩니다. Reservations 화면으로 돌아오면 최신 상태가 반영됩니다
- (참고) 윈도우 앱 다운로드 시 나오는 보안 경고는 앱 오류가 아니라 미서명 설치파일 특성입니다 — "추가 정보 → 실행"으로 정상 설치되며, 경고 제거(코드서명 인증서)는 매장 확대 시 검토 예정

## [v3.69] — 2026-07-16 배포

### 2026-07-16 (인쇄 자가진단 & 원격 지원, 운영 배포)
- **인쇄 문제를 매장이 스스로 진단**할 수 있습니다 — 설정 > 프린터의 "전체 점검" 버튼 하나로 기기·프린터·설정을 한 번에 확인하고, 무엇이 문제인지(원인)·어떻게 고치는지(가이드)·지금 눌러 고칠 수 있는 버튼까지 한 화면에 보여줍니다 (막힌 티켓 비우기, 주방 프린터 설정 등)
- 실제로 종이가 나오는지 확인하는 **테스트 인쇄(텍스트·이미지 2종)** 를 제공하고, "나왔어요/안 나왔어요"를 눌러 기록합니다
- **자동인쇄가 실패하면 화면 맨 위에 알림 띠**가 떠서 매장이 즉시 알 수 있습니다 (자동으로 다시 시도 중임을 안내하고, 계속 실패하면 무엇을 확인할지 알려줍니다)
- 관리자는 **전 매장의 인쇄 상태를 원격으로 한눈에** 볼 수 있습니다 (Admin > Print Health) — 미인쇄 건수, 인쇄 담당 기기의 마지막 응답 시각·앱 버전 등. 매장에 전화하기 전에 상황을 파악할 수 있습니다
- 자동으로 확인할 수 없는 항목은 "확인 필요"로 정직하게 표시합니다 (되는 것처럼 위장하지 않음)
- (내부) 인쇄 동작 자체는 전혀 바뀌지 않는 읽기 전용 진단 — 기존 인쇄 코드 무접촉, 기기별 상태를 서버에 올리는 채널만 추가

### 2026-07-15 #3 (안드로이드 앱 다운로드, 운영 배포)
- 안드로이드 태블릿에서 POS에 로그인하면 **"Download for Android" 버튼**이 떠서 **네이티브 앱을 바로 받을 수 있습니다** (윈도우 앱과 동일한 방식). 설치하면 WiFi·블루투스로 주방 프린터에 인쇄할 수 있습니다
- 안드로이드에서는 인쇄가 안 되는 웹앱(PWA) 대신 **네이티브 앱으로 안내**합니다 (손님 주문 화면에는 안 뜸)
- (참고) 안드로이드 앱은 주문 자동인쇄를 실제 태블릿에서 최종 확인 후 정식 사용 권장

### 2026-07-15 #2 (브랜드 메뉴 레시피 연결 수정 + 공지 댓글 삭제 안정화, 운영 배포)
- 브랜드 메뉴를 만들 때 **"Linked Recipe(연결 레시피)" 목록에 '레시피 관리'에서 등록한 레시피가 뜨지 않던 문제**를 수정했습니다 — 이제 등록한 브랜드 레시피가 정상적으로 목록에 나타나 연결할 수 있습니다 (매장 메뉴와 동일한 방식으로 통일, 재고 차감도 이 레시피를 사용)
- 공지 댓글을 삭제할 때 실패해도 아무 반응이 없어 "삭제가 안 된다"고 보이던 것을 수정 — 이제 삭제가 안 되면 이유를 화면에 표시합니다
- (내부) 브랜드 메뉴 레시피 연결에 소유권 검증 추가 — 다른 브랜드의 레시피는 연결 불가

### 2026-07-15 (윈도우앱 빌 백지 정석 수리 + Floor Plan 헤더/캐시버튼, 운영 배포)
- 윈도우 데스크탑앱에서 **빌(영수증)이 백지로 나오던 문제를 근본 수리**했습니다 — 이제 로고·한글 디자인 그대로 인쇄됩니다. 프린터 드라이버를 거치지 않고 앱이 직접 그려 보내는 방식(오더티켓이 나오던 검증된 경로)이라, 값싼 영수증 프린터에서도 안정적입니다 (앱 0.1.9)
- 실제 주문 없이 인쇄를 점검할 수 있는 **자가진단(빌·오더티켓 한 번에 인쇄)** 을 앱 진단화면에 추가했습니다
- Floor Plan 상단 헤더가 일부 화면 크기(1600급 모니터·최대화)에서 **두 줄로 접히던 것**을 수정 + 현금 관련 버튼(Today's Cash Drawer·Open Drawer)을 **나란히** 정리했습니다
- 데스크탑앱 다운로드가 항상 최신 버전을 받도록 정리 (설치 피드 최신화)
- (내부) PayPal 웹훅 알림 오탐 제거 — 결제 동작 변화 없음
- ⚠ 인쇄 디자인은 매장 실제 프린터 종이 확인 후 최종 확정 예정

### 2026-07-14 #2 (윈도우 데스크탑앱 인쇄 — 원격 진단 + 티켓 유실 수정 + 앱 0.1.8, 운영 배포)
- 인쇄가 실패해도 "인쇄됨"으로 처리돼 **종이가 안 나온 티켓이 사라지던 문제**를 수정했습니다 — 이제 실패하면 다시 인쇄를 시도합니다
- 인쇄 문제가 생기면 **매장에 가지 않아도 원인을 알 수 있습니다** — 인쇄할 때마다 앱 버전·프린터·성공 여부·실제로 출력된 내용이 서버에 기록됩니다
- 데스크탑앱 창 제목에 **버전이 표시**됩니다 (예: Purple POS 0.1.8)
- 데스크탑앱 **자동 업데이트가 실제로 적용**됩니다 — 하루 종일 앱을 켜두는 매장에서 업데이트가 영영 설치되지 않던 문제 수정. 영업 중에는 절대 재시작하지 않고, 새벽 시간에 아무도 사용하지 않을 때만 조용히 설치합니다

### 2026-07-14 (개발서버 안정화 — 서비스 무영향)
- 개발서버가 며칠간 반복해서 멈추던 문제를 근본 수리했습니다 (메모리 고갈로 서버 전체가 얼어붙던 것 — 서버 사양 문제가 아니라 안전장치 부재였습니다)
- 무거운 작업이 서버를 마비시키기 전에 자동으로 차단·중단시키는 4중 보호막을 넣었습니다
- SSH 무차별 대입 공격 차단(fail2ban) + 비밀번호 로그인 차단

### 2026-07-13 #5 (안드로이드 앱 검증 도구 — 운영/앱 동작 무변경)
- 안드로이드 인쇄 검증이 "앱이 티켓을 못 찍는다"고 3번 실패했는데, 원인은 앱이 아니라 검증 도구 자체였습니다 — 검증이 매장의 주방 스테이션 프린터 설정을 무시하고 엉뚱한 프린터로 인쇄를 시도하고 있었습니다 (앱 코드는 변경 없음)
- 검증이 실매장과 같은 방식(주방 스테이션별 프린터)으로도 티켓이 **옳은 프린터로** 나오는지 확인합니다
- 검증 중 서버가 멈추던 문제 방지 — 안드로이드 에뮬레이터가 서버 메모리를 다 쓰면 아예 시작하지 않습니다
- 검증이 중단돼도 테스트 매장의 프린터 설정이 원래대로 복구됩니다

### 2026-07-13 #4 (배포 도구 — 운영 무영향)
- 배포 스크립트가 스키마 확인에 실패했을 때 이유 없이 멈추던 문제 수정 (원인을 표시하고, 일시적 오류는 재시도합니다)
- 배포 전 운영 백업이 실패해도 그냥 진행되던 문제 수정 — 백업이 실제로 만들어졌는지 확인한 뒤에만 배포합니다 (백업 없이 배포되면 되돌릴 수 없습니다)

---

## 2026-07-13 #3 — 운영 배포 (반품 재고 환원 수정)
- **브랜드에서 받은 물건을 반품하면 재고가 제대로 줄어듭니다** — 반품을 승인해도 매장 재고가 그대로였고(깎았다가 도로 올림), 브랜드 본사 재고도 복구되지 않던 문제를 수정했습니다
- 반품 시 단위 환산이 빠져 있던 문제 수정 (입고는 환산해서 더하는데 반품은 환산 없이 뺐습니다)
- 반품 처리 기록이 남습니다 (입고는 기록이 남는데 반품은 아무것도 안 남았습니다)
- 같은 반품이 두 번 승인되어 재고가 두 번 환원되거나 크레딧노트가 두 장 발행되던 문제 차단
- 입고한 수량보다 많이 반품 신청할 수 없습니다 (여러 번 나눠 신청하면 통과하던 문제)

---

## 2026-07-13 — 운영 배포 (브랜드 재료 실사·발주제안 + 발주 오너 승인)

### 발주 오너 승인
- **발주가 나가는 모든 경로에 오너 승인이 적용됩니다.** 재고관리의 일괄 발주와 외부업체 수동 전송이 승인 없이 바로 나가던 문제를 막았습니다 (설정 → 운영 탭에서 켜고 끕니다. 오너가 연결돼 있으면 기본 켜짐)
- **승인 전 발주는 공급업체에게 보이지 않습니다** — 임시저장(장바구니) 발주와 승인 대기 발주가 공급업체 화면에 그대로 노출되던 문제를 막았습니다
- 승인 대기 중인 발주는 입고 처리로 끝낼 수 없고, 대신 **작성자가 철회**할 수 있습니다
- 승인 대기 수량도 "입고 예정"에 잡혀 같은 재료를 중복 발주하지 않습니다
- 외부업체 발주는 오너 승인 후 **발주 상세 화면에서 WhatsApp·이메일로 발송**합니다 (승인 전에는 발송 버튼이 잠깁니다)

### 브랜드 재료 실사 · 발주 제안 · 매장별 재고 설정
- **재고 실사에 브랜드 재료가 포함됩니다.** 세어 넣은 수량은 그 매장에만 반영되고, 같은 브랜드의 다른 지점 재고는 그대로입니다
- **발주 제안에 브랜드 재료가 뜹니다.** 대시보드 제안과 재고 목록의 일괄 발주 체크박스 양쪽 모두에서 담을 수 있습니다
- **최소재고·리드타임·사용량 설정을 매장마다 따로 정합니다.** 브랜드가 정한 값이 기본이고, 매장이 바꾸면 그 매장에만 적용됩니다. 재료 이름·단위·공급처는 여전히 브랜드가 관리합니다
- 재고 부족 알림과 품절 판정도 매장이 정한 최소재고 기준으로 뜹니다
- 사용량 자동계산이 다른 지점의 입고까지 합산하던 문제 수정 — 이제 매장별로 계산됩니다
- 보안: 다른 매장의 재고 실사를 열람·수정·완료할 수 있던 문제 차단

### 2026-07-13 (운영 배포됨 — 브랜드 재료 실사 · 발주 제안 · 매장별 재고 설정)
- **재고 실사에 브랜드 재료가 포함됩니다.** 세어 넣은 수량은 그 매장에만 반영되고, 같은 브랜드의 다른 지점 재고는 그대로입니다
- **발주 제안에 브랜드 재료가 뜹니다.** 대시보드 제안과 재고 목록의 일괄 발주 체크박스 양쪽 모두에서 담을 수 있습니다
- **최소재고·리드타임·사용량 설정을 매장마다 따로 정합니다.** 브랜드가 정한 값이 기본이고, 매장이 바꾸면 그 매장에만 적용됩니다 (지점마다 회전율이 다릅니다). 재료 이름·단위·공급처는 여전히 브랜드가 관리합니다
- 재고 부족 알림과 품절 판정도 매장이 정한 최소재고 기준으로 뜹니다
- 사용량 자동계산이 다른 지점의 입고까지 합산하던 문제 수정 — 이제 매장별로 계산됩니다
- 보안: 다른 매장의 재고 실사를 열람·수정·완료할 수 있던 문제 차단
...
## 2026-07-12 #4 — 운영 배포 (브랜드 재고 공유)

- **브랜드가 정한 재료를 매장이 그대로 씁니다.** 브랜드에 등록한 재료가 소속 매장의 재고·발주·레시피 화면에 자동으로 나타납니다(매장에서는 수정 불가, 브랜드가 관리). 매장별 재고 수량과 단가는 매장마다 따로 유지됩니다
- **매장이 브랜드 재료를 바로 발주할 수 있습니다.** 브랜드가 공급처를 연결해두면 소속 매장이 그 공급처로 주문합니다(계약도 브랜드 것을 물려받습니다)
- **브랜드를 여러 개 운영하는 경우, 두 번째 브랜드에도 공급업체·상품을 연결할 수 있습니다** — 이전에는 대표 브랜드에만 가능했습니다
- 재고 입고·차감·폐기가 매장별로 정확히 기록됩니다 — 같은 브랜드의 다른 지점 재고가 섞이지 않습니다
- **보안 강화**: 재고 입고·차감·설정·수정·삭제에 소유권 확인을 추가했습니다(다른 매장의 재료를 건드릴 수 없습니다). 레시피에 다른 브랜드 재료를 넣는 것도 차단됩니다
- 공급업체 화면 정리: **Direct → 외부업체(External)** 로 이름을 바꿨습니다. 외부업체는 "Find Suppliers"(가입 공급업체 검색)에 더 이상 섞이지 않고, 외부업체 상세에 뜨던 계약 안내도 없앴습니다
- 발주 화면 리스트 보기를 다시 만들었습니다 — 열이 어긋나고 넓히면 가운데가 비던 문제 해결, 창 크기에 따라 항목이 자연스럽게 접히고 줄이 정렬됩니다. 카드/리스트 전환 버튼은 다른 화면과 같은 디자인으로 통일하고 오른쪽에 배치했습니다

### 2026-07-12 #3 (운영 배포됨 — 발주 보기 전환 · 재료 연결 해제)
- 발주 화면에서 상품을 카드 또는 리스트로 볼 수 있습니다. 내 재고와 공급업체 카탈로그 각각 따로 선택할 수 있고, 선택한 보기는 다음에 들어와도 그대로 유지됩니다 (레스토랑 관리자·브랜드 총괄 공통)
- 재료 수정 화면에서 연결된 공급업체 상품을 해제할 수 있습니다 — 연결은 되는데 끊을 방법이 없었습니다. 해제해도 공급업체 상품 자체는 지워지지 않습니다

### 2026-07-12 #2 (운영 배포됨 — 프랜차이즈 맵)
- 프랜차이즈 맵에 매장이 지도로 표시됩니다 — 기존 매장들은 좌표가 없어 지도가 비어 있었습니다. 주소로 좌표를 채우고, 이후 매장은 저장 시 자동으로 잡힙니다
- 주소가 두 번 반복되어 표시되던 문제 수정 (예: "… Petaling Jaya, Selangor, Petaling Jaya, Selangor")

### 2026-07-12 (운영 배포됨 — 발주)
- 브랜드 발주에 공급업체 이름이 표시됩니다 — 브랜드에서 받는 발주는 이름이 비어 있었습니다
- 발주 수신처를 회사명과 브랜드명으로 함께 표시합니다 (예: "GIT Consulting (with MIN)")
- 발주서 PDF에 주문한 매장 이름·주소가 표시됩니다 — 공급업체가 받는 문서에 주문자가 비어 있었습니다
- 발주서 PDF 버튼을 누르면 인쇄창이 바로 뜨지 않고 문서를 먼저 보여줍니다. 다운로드·인쇄 버튼을 위아래에 배치했습니다
- 공급업체별로 각각 발주를 제출할 수 있습니다 — 전체 제출 버튼 하나뿐이었습니다
- 발주 수량 표시를 정리했습니다 (1.00 → 1)

### 2026-07-11 #3 (미배포 — 설정 정리)
- 매니저(브랜드·푸드코트 총괄) 설정 화면에서 실제로 동작하지 않던 항목 제거 — 회사정보·브랜드·구독 탭이 표시만 되고 저장되지 않던 가짜 화면이었습니다. 회사정보는 회사 정보 페이지, 브랜드는 브랜드 관리, 구독은 구독 화면에서 그대로 사용합니다
- 매니저 구독 화면의 '일괄 청구' 버튼 제거 — 구독 청구서는 자동으로 발행되므로 불필요합니다

### 2026-07-11 #2 (임차인 임대료 청구 — 신규 기능)
- 임대료 청구가 시스템 안에서 됩니다 — 계약에 월 임대료·관리비·청구일을 넣으면 매달 임대료 청구서가 자동 발행되고, 입점 매장은 기존 인보이스 화면에서 그대로 받아 결제합니다
- 임대 관리 화면이 실제로 동작합니다 — 입점사별 월 임대료·납기일·납부/연체 현황이 실데이터로 표시됩니다(이전에는 항상 빈 화면이었습니다)
- 임대 관리 화면에 푸드코트 총괄이 들어갈 수 없던 문제 수정 — 정작 임대료를 청구하는 당사자가 막혀 있었습니다
- 계약이 종료되면 임대료 청구가 자동으로 멈춥니다. 같은 달에 청구서가 두 번 발행되지 않습니다

## [v3.68] — 2026-07-11 배포 (Backup 20260711_170702, Smoke 9/9, 안전게이트 9/9)

- 관리자 대시보드·리포트의 매출/주문/직원 수가 실제 데이터로 표시됩니다 — 일부 화면(매니저 대시보드, 오너·푸드코트 리포트의 운영 지표)이 임의의 숫자를 보여주고 있던 문제를 바로잡았습니다. 새로고침할 때마다 값이 달라지지 않습니다
- 삭제한 주문이 매출에 계속 잡히던 문제 수정 — 매니저 매출·리포트에서 삭제 주문이 정확히 제외됩니다
- 푸드코트 총괄 리포트가 비어 보이던 문제 수정 — 소속 매장의 주문이 정상적으로 집계됩니다(다른 푸드코트 주문은 여전히 보이지 않습니다)
- 구독(플랜) 변경이 실제로 저장됩니다 — 매니저 구독 화면에서 플랜을 바꿔도 새로고침하면 원래대로 돌아가던 문제 해결. 예약된 다운그레이드 표시 및 취소 가능
- 요금을 대신 결제하는 브랜드·푸드코트 관리자가 해당 매장의 플랜을 변경할 수 있습니다 — 본인이 결제 주체인데도 "결제자에게 문의하세요"로 막히던 문제 해결
- 플랜 화면의 가격이 실제 청구 금액과 일치합니다 — 화면에는 옛 가격이, 청구는 실제 가격이 적용되던 불일치 해결. 플랜 비교표의 주문/직원/메뉴 한도도 실제 플랜 기준
- 업그레이드 시 발행되는 청구서의 납기일이 과거 날짜로 잡혀 즉시 연체가 되던 문제 수정
- 시스템 관리자 구독 화면에서 매장 목록·플랜 변경·정지·해지가 실제로 동작합니다(이전에는 화면에서만 바뀌고 저장되지 않았습니다)
- 네트워크가 잠깐 끊겼을 때 로그아웃되던 문제 수정 — 통신이 한 번 실패해도 로그인 상태가 유지됩니다
- 열어도 항상 비어 있던 관리자 화면 정리(시스템 설정·보안·백업·푸드코트 관리·입점사 지원) — 해당 기능은 기존 화면(사이트 설정, 결제 설정, 로그, 계약 관리, 문의)에서 그대로 사용합니다. 빈 화면으로 안내하던 링크도 실제 기능으로 연결

**신규 기능 (이번 배포로 처음 운영 반영)**
- AI 음식인식 서빙(Track A): 서빙 화면에 메뉴 사진 썸네일 + 탭하면 옵션·테이블 상세 + "메뉴 사진 갤러리". 직원이 접시를 사진과 대조해 서빙 (Enterprise 전용)
- AI 음식인식 서빙(Track B, 1단계): 카메라로 접시 촬영 → 지금 조리완료된 주문 중 매칭 → 확인 후 서빙. 현재는 색 기반 로컬 인식(무비용)이며, 정확도 높은 진짜 이미지 AI는 다음 단계에서 연결 예정. 원본 사진 무보존
- 내부 품질: 주문·금액·결제 무결성 + 매장 간 데이터 접근(IDOR) 배포 전 자동검사 확장

## 이전 배포 (버전 미상승 — 인프라/인쇄 핫픽스, 2026-07-08~10)

### 2026-07-10 (개발 인프라 — 내부용, 공지 대상 아님 · 운영 배포됨, 버전 미상승)
- 개발 안전망 대폭 강화(사용자 체감 기능 없음): 모든 검증을 명령 1개로 돌리는 러너, 민감 변경(결제·인쇄·DB·보안) 자동 감지, 위험한 파일 편집 사전 차단, 배포 검증 게이트 확대(실브라우저 크래시 검사 포함)
- DB 마이그레이션 등록 누락 자동 차단 — 새 마이그레이션을 배포 목록에 빠뜨려 운영 스키마가 어긋나던 위험을 구조적으로 방지
- 새 개발자/AI 세션 온보딩 문서 + 세션이 끊겨도 안전 규칙이 이어지도록 정리
- E2E 자동 테스트 확장(b~f): 모바일 주문·POS 결제·플로어플랜·설정·주방화면(KDS)을 데모 매장에서 실제로 태워 주문 생성→결제→단계이동→인쇄 파이프라인까지 자동 검증(매번 자동 정리, 운영 무영향). 배포 검증의 실브라우저 크래시 검사에 발견된 간헐 오탐(flake) 근본 수리(실패 화면 1회 재검 — 진짜 크래시는 여전히 차단)
### 2026-07-09 (with MIN 인쇄 데스크탑앱 0.1.7 — 운영 배포됨, 버전 미상승)
- 데스크탑 앱 0.1.7: 일부 USB 감열 프린터에서 영수증이 하얗게(백지) 나오던 문제 수정 — 앱이 영수증을 실제로 그린 뒤 인쇄하도록 개선. 첫 인쇄 후 작업표시줄에 남던 유령 창 제거
- 자동 업데이트 개선: 새 버전이 준비되면 "지금 재시작" 알림이 한 번 뜨고, 종일 켜둔 POS도 업데이트를 놓치지 않도록 재확인. (이전에 앱이 조용히 업데이트 안 되던 근본 원인 해결)
- 진단 화면: 종이 없이 인쇄 내용을 PDF로 미리 확인하는 "Render check" 도구 추가 — 프린터 문제인지 앱 문제인지 매장에서 바로 판별
- 주방 스테이션(BAR 등) 프린터 미지정 경고: Settings에서 프린터를 안 고른 스테이션은 인쇄가 안 되므로, 노란 경고로 표시해 지정하도록 안내
- 오더티켓 디자인 인쇄: Settings→Printer→인쇄형식을 'graphic'으로 두면 오더티켓도 영수증처럼 디자인으로 인쇄 (기존 설정 그대로 활용)
### 2026-07-08 (with MIN 인쇄 백지 수정 + 데스크탑앱 0.1.2 — 운영 배포됨, 버전 미상승)
- 인쇄 백지 수정: 일부 USB 감열 프린터에서 영수증·주방티켓이 하얗게 나오던 문제 해결 — 영문 메뉴 매장은 프린터가 확실히 알아듣는 텍스트 방식으로 자동 인쇄(한글 메뉴는 기존 이미지 방식 유지). 앱 새로고침만으로 반영
- 데스크탑 앱 0.1.2: 앱을 닫으면 다시 안 열리던(재부팅해야 열리던) 문제 해결, 상단 File/Window 메뉴 제거·앱 아이콘 정상화·첫 실행 시 최대화, 이후 업데이트는 자동 설치
- 데스크탑 앱 안에서 뜨던 "Install App"(PWA 설치) 버튼 숨김 — 이미 앱이므로 불필요
- 윈도우 브라우저의 "Install App" 버튼은 이제 윈도우 전용 데스크탑 앱을 받도록 연결(인쇄 되는 정식 앱)

## [v3.67] — 2026-07-06 배포 (Backup 20260706_134639, Smoke 9/9, 안전게이트 6/6, SW 4.58)

### 2026-07-06 (모바일 중복 주문 방지 + 내부 안전망 확장 — 운영 배포)
- 모바일 주문 중복(같은 주문이 2번 들어가던 문제) 방지: 결제 시 카트 기준 안정 키로 중복 요청을 하나로 합침. 네트워크가 느리거나 두 번 눌러도 주문은 한 번만 생성됨
- 모바일 결제 요청에 20초 타임아웃 적용 — 응답이 늦어도 멈추지 않고 안전하게 처리
- 모바일 장바구니 가독성: 담긴 총수량 배지, 수량 굵게 표시
- 내부 품질: 주문·금액·결제 무결성 자동검사(인스펙션 하니스) + 매장 간 데이터 접근(IDOR) 정적 검사기 신설 — 배포 전 자동으로 잡음(사용자 화면 무관)

### 2026-07-05 (with MIN 공급망 정리 + 공급업체 페이지 — 운영 배포됨)
- 공급 리스트 대량 등록: 355개 품목을 재고·공급업체·판매품목으로 일괄 등록하고, 우리 재고 이름(영문+한글)과 공급업체 판매품목·SKU를 함께 정리
- 브랜드가 사고(매입) 파는(판매) 품목 구조 완성: 브랜드가 매입하는 공급업체를 지정하고, 같은 품목을 판매품목으로도 재고에 연결 — 레스토랑 메뉴↔재고 연결과 동일한 방식
- 공급업체 화면 정리: 우리 솔루션 밖 업체는 모두 "Direct"로 통일(중복 "OWN" 제거), 브랜드가 등록한 공급업체·재고는 산하 매장에 "Brand"로 표시, 카테고리 연결
- 공급업체 편집/삭제: 외부(Direct) 공급업체를 화면에서 바로 편집·삭제(발주 이력은 보존)
- 브랜드 공급업체 목록·발주 화면 접근 복구(플랜 모듈 누락 수정)
- 내부 품질: 공급망 구조 자동검사 도구(인스펙션 하니스) 신설 — 잘못 연결된 데이터를 배포 전 자동으로 잡음

### 2026-07-04 #2 (재고 공급처 표기 정리 — 미배포)
- 재고관리: 재고에 딸린 옛 "기본 공급업체" 칸을 읽기 전용으로 정리. 이제 공급처는 항목별 "공급처 연결(판매품목·SKU·단가)"로만 관리 — 화면상 "내 재고 이름"과 "공급업체 판매품목 이름"이 섞이던 혼동 제거
- 기존에 입력돼 있던 공급업체 이름은 그대로(읽기 전용) 보이며, 다음 발주 때 공급처를 연결하면 올바른 SKU·단가로 정식 이관됨(기존 데이터 손실 없음)

### 2026-07-04 (공급업체 판매품목 이름/SKU 표시 + 브랜드 재고관리 수정 — 운영 배포됨)
- 재고 발주: 공급처 목록·발주서·공급업체 수신함에서 "우리 재고 이름/코드"와 "공급업체가 파는 판매품목 이름·SKU(품번)"를 함께 표시. 공급업체 발주서(PDF)에 공급업체 SKU가 찍혀 창고에서 바로 찾을 수 있음
- 외부 공급업체 상품 등록: 판매품목 이름(기본값=우리 재고명, 수정 가능)과 SKU를 직접 입력하도록 개선
- 브랜드 재고관리(브랜드 모드): 재고 설정 저장, 발주(공급처) 연결, 알림 닫기, 입출고 이력 기록이 정상 동작하도록 수정

### 2026-07-03 (Brand General / Restaurant Owner 전수 점검 — 결함 32건 수정)
- 보안: 활동로그·인보이스·알림설정·구독 목록의 타 매장 접근 차단 강화
- 브랜드 관리: 브랜드 코드 중복/삭제 시 페이지 멈춤 수정, 주소 전체 항목 저장, 목록 내보내기(Export) 동작
- 브랜드 상품/메뉴: 상품·옵션그룹 삭제 오류 수정, 저장 실패 시 안내 메시지 정상 표시
- 리포트·성과: 매출 집계 정확도 개선(누락 주문 포함), 성장률·최다판매 표시 수정, 검색 필터 입력 정상화
- 매니저 매출: 실제 주문 기반 매출 집계로 전환(기존 임시 0 표시 제거)
- 전화번호 입력: 국가번호가 다른 저장 번호도 정확히 표시·수정되도록 수정
- 오너: 공지 댓글·업무 매뉴얼 작성 오류 수정, 알림 설정 토글 정상화
- (신규 설계) AI 음식 인식 서빙 기능 기술 설계 문서 작성
...
## [v3.66] — 2026-07-01 배포 (Backup 20260701_201256, Smoke 9/9, SW 4.54, 안전게이트 통과)

### 2026-07-01 (KDS 세트 단계 동기화 + 마감/실시간/속도 보완)
- 주방화면(KDS)과 플로어플랜의 **세트 메뉴 구성품 단계가 이제 일치**합니다 — KDS에서 세트 구성품을 시작하면 플로어플랜 우측 상세에도 같은 단계로 즉시 보입니다.
- **최종 마감(Final Settlement)** 에 오늘 매출 요약(순매출·주문유형·소스·취소·미결)과 **취소(void) 요약**을 추가했습니다.
- **고객 디스플레이**가 우측 주문 패널이 열려 있는 동안 안정적으로 계속 표시됩니다(끊겨도 자동 복구).
- **같은 테이블에 별도 주문**을 만들 수 있는 "New Order" 버튼을 추가했습니다(스탭밀 등 각각 따로 결제·주문).
- **주방 티켓에 메모**(품목별 특별요청 + 주문 메모)가 모바일·POS 모두 정확히 찍힙니다.
- **화면 전환/뒤로가기가 빨라졌습니다**(정적 자산 즉시 로드).
- **KDS**: 취소 주문 확인 버튼 상시 표시 + 오늘 취소 목록, 주방 스테이션별 소리 켜기/끄기(탭마다), 되돌리기는 아이템 단위로 정리.
- 단계·상태 변경이 모든 화면에 **더 빠르게 반영**되도록 실시간 동기화를 보강했습니다.

## [v3.65] — 2026-06-30 배포 (Backup 20260630_010745, Smoke 9/9, SW 4.46, 안전게이트 4/4)

### 2026-06-30 (오프라인 모드 + IOI Mall + POS UX 일괄 배포)
- 입점몰 매출 보고(IOI Mall) — 시간대별 매출을 몰 POS 시스템에 자동 전송하는 연동을 추가했습니다(설정 ▸ Mall Sales Reporting). 운영 자격증명을 받은 매장만 켜집니다.
- POS 상단 헤더 — 작은 화면(1024~1536)에서도 버튼이 한 줄로 정돈되고, 'Dashboard/Customer Display/Open Drawer/POS Terminal' 표기가 한국어로 나옵니다.
- 팝업·모달 테마 — 고대비·다크 모드에서 옵션·세트·확인 팝업이 화면 테마를 따라 또렷하게 보입니다(이전엔 흰 박스로 따로 놀던 문제).
- POS 카테고리 — ▾ 버튼으로 카테고리 전체를 그 자리에서 여러 줄로 펼쳐 한눈에 보고, 고르면 자동으로 접힙니다.
- 모바일 QR — 테이블 QR을 다시 스캔하면 항상 그 테이블로 새로 시작되어, 이전 테이블 번호가 잘못 남던 문제를 막습니다.

### 2026-06-29
- 오프라인 모드(POS1 허브) — 인터넷이 끊겨도 카운터에서 ①주문 접수 ②주방 티켓 로컬 인쇄 ③복구 시 자동으로 전부 서버에 전송(중복 없이·누락 없이)이 됩니다. 끊긴 동안 보관된 주문은 라이브 주문 화면 상단에 "보관 N건"으로 표시되고 연결되면 사라집니다. (※ 매장 프린터로 실제 출력 확인 후 배포)
- 주방 디스플레이(KDS) — 주문 보기에서도 품목마다 **되돌리기(↺)** 버튼이 생겨, 한 품목만 이전 단계로 내릴 수 있고 주문은 그에 맞춰 이동합니다.
- 주방 디스플레이(KDS) — 주방 티켓이 인쇄되지 않은 주문을 화면 배지·상단 "미인쇄 N"·팝업으로 알려주고, 거기서 바로 **다시 인쇄**할 수 있습니다.
- 오프라인 메인 POS 지정 — 인터넷이 끊긴 동안에는 매장이 정한 **메인 POS 1대에서만** 주문을 받고, 나머지 기기는 "주문은 메인 POS에서" 안내 화면으로 잠겨 주문 누락·중복을 막습니다. 메인 POS에는 "이 기기가 모든 주문·인쇄를 처리한다"는 안내가 뜨고, 주방 프린터가 안 되면 비상모드로 모든 티켓을 이 프린터로 받을 수 있습니다. 설정 ▸ 프린터에서 "오프라인 메인 POS" 토글로 지정합니다. (※ 매장 프린터로 실제 출력 확인 후 배포)

## [v3.64] — 2026-06-28 배포 (Backup 20260628_202137, Smoke 9/9, 안전게이트 4/4)

### 2026-06-28 (스탭밀 · 마감 통합 · POS UX)
- 스탭밀(직원식) — 결제 시 'Staff Meal' 선택하면 품목마다 **직원 이름**을 넣을 수 있고(이미 쓴 이름 자동완성), 하루치 직원식을 모은 **Staff Meal Settlement 정산서**를 인쇄(품목별 직원명 + 합계).
- 마감(Settlement) 통합 — Floor Plan·Live Orders 상단의 Daily/Final 버튼을 **'Settlement ▾' 하나로 통합** + Staff Meal Settlement 추가.
- Final Settlement — 시재(현금 드로어)를 안 쓰는 매장도 **매출 기준 최종 마감**이 가능.
- POS 다크 모드 — 어두운 화면에서 선택 항목·강조 글씨가 잘 안 보이던 가독성 문제 개선.
- POS 품목 메모 — 작은 화면에서 누르면 **팝업 + 화면 키보드**(터치 단말도 입력 가능) + 자주 쓰는 메모 빠른 선택. 메모는 주방티켓/빌에 출력.
- 옵션 품절 — 상품처럼 **옵션도 품절 표시·선택 차단**(롱프레스 토글, 모바일/POS 반영).
- 인보이스 — 미결제 인보이스가 기간 필터에 가려지지 않고 **항상 표시**, 같은 이름 매장은 **지점명으로 구분**, 시스템 인보이스 **결제전/결제완료 보기** 필터.
- (백스테이지) thefire 공지 미수신 근본수리(매 배포 때 테스트로 재분류되던 원인 제거 + 놓친 공지 백필), 개발서버의 실 Stripe 키 격리.

### 2026-06-27 (브랜드메뉴 — 가맹점 동기화 수정)
- 브랜드메뉴 — 브랜드(본사)에서 세트 메뉴 구성품을 추가·변경하면 가맹점 매장에도 그대로 반영되도록 수정(이전엔 일부 세트 구성이 가맹점에 안 내려가던 문제). /배포 시 적용.
- 주방 인쇄 안정화 — 한산한 시간 뒤 첫 주문이 늦게 인쇄되던 문제 해결(QZ 연결 유지), 주방 스테이션 티켓을 통합 오더티켓보다 먼저 발행해 BAR 등이 뒤늦게 나오던 문제 개선, 아이템 취소표를 해당 주문 회차 기준으로 표시. (thefire 라이브 대응, 운영 반영 완료)
...
## [v3.63] — 2026-06-27 배포 (Backup 20260627_061214, Smoke 9/9, SW 4.23)
- 모바일 주문 — 메뉴 상세에서 "담기"를 누르면 짧은 "✓ 담음" 확인 후 자동으로 메뉴 리스트로 돌아가고, 화면 하단에 항상 보이는 장바구니 바(수량·금액·주문하기)가 떠서 같은 메뉴를 두 번 주문하는 실수를 막아줌.
- 모바일 추천(애드온) — 추천 카드에 사진/이모지가 안 나오던 문제 해결(사진 없으면 메뉴 이모지, 그것도 없으면 기본 식기 아이콘으로 항상 표시).
- 모바일 추천 — 추천을 담은 뒤 같은 상품에 다시 들어가도 추천이 사라지지 않고 담은 항목은 ✓로 표시. "담는 도중 화면이 갑자기 사라지던" 문제 해결.
- 모바일 추천 — 추천 기능을 켜면 추천 상품을 일일이 연결하지 않아도 다른 카테고리 상품으로 자동 추천(원하면 카테고리/상품 지정으로 더 정교하게).
