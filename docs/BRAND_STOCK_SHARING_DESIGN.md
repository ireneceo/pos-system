...
## 구현 결과 (2026-08-20 dev 완료 · Fable 게이트 대기)

설계 순서대로 구현했고, 도중에 설계에 없던 결함 2건을 실측으로 잡아 함께 고쳤다.

### 설계대로 구현한 것
| 설계 항목 | 구현 |
|---|---|
| 1. 알림 소음 제거 | `useInventoryData.ts` — `min_stock > 0` 일 때만 부족/품절 판정 |
| 3. 소프트 링크 | `product_ingredients.linked_ingredient_id` + 멱등 마이그 + 사람 확인 연결 도구(`link-product-ingredients.js`, dry-run 기본·모호는 미연결) |
| 링크된 실재고 노출 | `GET /product-ingredients` 가 `linked_stock`/`linked_store_total`/`linked_stores`/`linked_unit` 첨부. **`current_stock` 무접촉**, 미연결은 `null`(0 아님 — "연결 없음"과 "재고 0"은 다른 사실) |
| 4. 알림→발주 담기 | 링크 있으면 `Add to Order`(장바구니 담기 — 즉시 발주 아님, 판매자별로 묶어 보내기 위함), 없으면 `Link required` 표시 |

카트 저장은 발주 화면(`NewPurchaseOrderPage`)의 `po-cart:brands:{id}` 키·`CartRow` 모양을 **그대로** 쓴다. 다른 형식으로 쓰면 담긴 게 화면에 안 나타나는 조용한 실패가 된다.

### 설계에 없던 것 — 실측으로 드러난 결함 2건

**① 배치 없는 매장은 팔아도 재고가 안 줄었다 (P5, 가장 큰 것)**
FIFO 차감은 **배치(입고 로트)** 에서만 뺀다. 배치가 없거나 부족하면 `deducted_quantity=0` 이 되고, 예전 코드는 그 값으로 재고를 줄였다 — 음식은 나갔는데 장부만 그대로. 실제 소비량(`actualDeductQty`)으로 줄이고, 배치로 못 덮은 몫은 `[batch_shortfall N]` 로 거래 메모에 남긴다. 레시피 재료·옵션 재료·수동 차감 3경로 동일 규칙.
*증명(트랜잭션 롤백):* 배치 0인 재료에 2 요청 → FIFO 가 덮은 양 0 → 옛 계산 15(안 줄어듦) vs 현재 계산 13.

**② FIFO 함수가 두 벌이었다**
`routes/inventory-extra.js` 에 `services/inventoryDeductionService.js` 와 같은 함수가 복사돼 있었다 — 한쪽만 고치면 "주문 차감은 맞는데 수동 차감은 틀린" 식으로 갈라진다. 사본을 지우고 서비스에서 가져다 쓴다.

또한 "레시피가 없어서 차감을 건너뛴" 횟수를 `results.skipped_no_recipe` 로 센다. 상품 754개 중 레시피 연결이 1개뿐이라 차감이 전 기간 0건이었는데도 경고 더미에 묻혀 아무도 몰랐다.

### P1 중복 등록 — 규칙을 셋으로 나눈 이유
운영 데이터로 검출력을 재보니 **이름 완전일치는 실제 중복 20쌍을 하나도 못 잡았고**(0쌍), 괄호를 뺀 키는 정확히 20쌍을 잡았다. 그런데 같은 규칙이 정당한 변형(`갈색 종이 밥그릇(L)` vs `(M)`)도 잡는다. 그래서:
- **하드 차단** = SKU 또는 이름 완전일치
- **소프트 확인** = 괄호 변형 → 409 이되 `force: true` 로 통과
- **제안 전용** = 숫자+단위 토큰까지 지운 느슨한 키 → **차단하지 않는다**(규격이 괄호 밖에 있는 중복을 사람 눈에 올려주는 용도. `GET /brand-products/similar`)

유사도 점수·퍼지 매칭은 쓰지 않는다 — 비결정적이라 "왜 걸렸는지" 설명할 수 없다.

### P4 브랜드 거래내역
`entity_type='brand'` 행만 보던 것을 **브랜드 + 산하 매장**으로 넓혔다. 브랜드 재료의 실제 소비는 매장에서 일어나고 그 거래는 restaurant 스코프로 쌓이기 때문에, 예전 화면은 "입고만 있고 나간 적 없는" 그림이었다. 행마다 `source_scope`(brand/restaurant)·`source_name` 을 붙여 부호를 오해하지 않게 한다.

### 남은 사전조건 (Fable 지시)
- 백엔드 고장주입을 **pm2 재시작 후 실서버 HTTP 경유**로 재실행
- health-check 영구 계약 4건 추가: 완료→차감 트랜잭션 / 배치0 차감 / P1 하드·소프트 409 / min0 알림 제외
- 그 후 별도 Fable 게이트 → 배포

### 사고 기록 (재발 방지)
배포 격리를 위해 `git checkout HEAD -- <재고 9파일>` 을 실행해 **미커밋 수정을 전량 소실**시켰다(전 파일 재구현). 격리 전 반드시 `git stash` 또는 사본 보관. 미추적 신규 파일은 이동만 했기에 무사했다.
