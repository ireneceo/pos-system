...
**🔴 코드가 스스로 이 설계를 레거시로 표시해 두었다** — `inventoryDeductionService.js:194`:
> `⛔ **확장 금지(레거시).** products.current_stock 는 재고아이템과 별도 저장소라 이원화된다.`

즉 "레시피 없는 상품의 자체 재고칸"은 **재고 아이템과 따로 노는 임시 저장소**임이 이미 명시돼 있다.
GIT 의 문제가 정확히 이것이고(발주 입고는 재고 아이템에, 판매 차감은 자체 칸에서),
매장 쪽에도 **같은 이원화가 잠재**한다.

**⇒ 고칠 방향 (두 역할 공통)**
- 레시피 **있음** → 레시피에 연결된 **재고 아이템**에서 차감 *(현행 유지)*
- 레시피 **없음** → **그 상품에 연결된 재고 아이템**에서 차감 *(연결 신설 — 지금 없다)*
- `products.current_stock` / `brand_products.current_stock` 은 **차감 경로에서 은퇴**시킨다
  (레거시 데이터 이관 경로 필요)
...
### A-00. Irene 모델 vs 실제 개발 — **절반만 돼 있다** (2026-09-01 실측)

> Irene: *"프로덕트나 메뉴 = 재고아이템, 프로덕트 레시피 재료나 메뉴 레시피 재료 = 재고아이템들(여러 개)
> 이렇게 루트 있는 거잖아. 이미 개발 그렇게 한 거 아니였어?"*

| 루트 | Irene 모델 | 실제 개발 |
|---|---|---|
| 레시피 **있음** | 레시피 재료 = **재고아이템 여러 개**에서 차감 | ✅ 그렇게 돼 있다 |
| 레시피 **없음** | 프로덕트/메뉴 = **재고아이템 1개** | ❌ **안 돼 있다** |

**근거**
1. **프로덕트·메뉴를 저장해도 재고아이템이 생성되지 않는다.**
   `ProductIngredient.create` 는 재고 라우트(`routes/product-ingredients.js`)에만 있고,
   프로덕트 저장 경로에 없다. 메뉴 저장(`routes/menu.js`)에는 `Ingredient.create` 자체가 없다.
   ⇒ **프로덕트 ↔ 재고아이템 1:1 이 성립한 적이 없다.**
   (`ingredients.brand_product_id` 는 매장 공유 미러용이고 BG 재고가 아니다 — Irene 확정)
2. 대신 상품 행의 **별도 재고칸**(`current_stock`)을 쓴다. 코드가 스스로 레거시로 표시 —
   `inventoryDeductionService.js:194`: *"⛔ 확장 금지(레거시). `products.current_stock` 는
   재고아이템과 별도 저장소라 이원화된다."*

⇒ **개발 당시에도 임시임을 알고 적어뒀고, 정식 모델로 넘어가지 않은 상태.**
   GIT 포장재(레시피 없는 프로덕트)가 정확히 이 구멍에 걸렸다.
