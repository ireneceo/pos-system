...
#### 무엇이 브랜드 제너럴의 재고인가 — Irene 확정 (2026-09-01)

> Irene 원문 ①: *"브랜드 제너럴 <<< 자기 재고 관리."*
> Irene 원문 ②: *"브랜드 재료는 브랜드제너럴의 재고와 상관없어. 브랜드제너럴의 재고와 상관있는 건
> 프로덕트와 프로덕트 레시피의 재료야."*

| 대상 | BG 재고와의 관계 | 테이블 | 실측 |
|---|---|---|---|
| **프로덕트** (BG 판매상품) | **파는 것** | `brand_products` | — |
| **프로덕트 레시피의 재료** | **= BG 의 재고** ✓ 발주 입고가 여기 쌓인다 | `product_ingredients` (`owner_user_id=23`) | **288건**, 수량>0 **23건** (포장재 등) |
| **브랜드 메뉴 · 브랜드 레시피 · 브랜드 재료** | 🔴 **어떤 상관도 없음.** 매장에 내려보내는 것 | `brand_menus` · `recipes`(brand_id) · `ingredients`(brand_id) | 브랜드 재료 146건 수량 0 — **정상** |
| 매장 재고 | 각 매장 자기 것 | `ingredients` (`restaurant_id`) | 매장별 |

- **BG 체인**: 프로덕트 → 프로덕트 레시피 → **재료(= BG 재고)** — `product_recipe_ingredients` → `product_ingredients` (실측 확인)
- 🔴 **브랜드 관련 것은 전부 BG 재고와 무관하다** (Irene: *"브랜드관련 레시피 메뉴 등등은 브랜드제너럴
  재고와 어떤 상관도 없다고"*). 브랜드 메뉴 → 브랜드 레시피 → 브랜드 재료 체인은 **매장에 내려보내는 것**이고,
  BG 가 사고 파는 재고와 **한 줄도 겹치지 않는다.** BG 재고 계산에 절대 끌어들이지 말 것.
- ⛔ "브랜드 재고"라는 말은 쓰지 않는다. **재고는 각자 관리**(Irene).
- ⛔ 브랜드 재료 146건의 수량이 0 인 것은 **결함이 아니다** — BG 재고가 아니기 때문. (내 1차 보고 오류)

#### 재고관리 2루트 — **RA·BG 완전 대칭** (Irene, 2026-09-01)

> Irene: *"레시피가 있는 경우와 없는 경우 2가지 재고관리 루트가 있잖아. 이건 레스토랑 관리자랑 브랜드제너럴이랑 같은데?"*
> → **맞다.** 실측 결과 두 역할의 코드 구조가 동일하고, **결함도 동일**하다.

| 루트 | 레스토랑 관리자 | 브랜드 제너럴 |
|---|---|---|
| **레시피 있음** | 메뉴 → 레시피 → **재료**에서 차감 ✓ | 프로덕트 → 레시피 → **재료**에서 차감 ✓ |
| **레시피 없음**(그대로 파는 물건) | `products.current_stock` 차감<br>(`inventoryDeductionService.js:191~205`) | `brand_products.current_stock` 차감<br>(`seller-orders.js` ship 브랜드 분기) |

`inventoryDeductionService.js:194` 의 `⛔ 확장 금지(레거시)` 주석은 **같은 물건을 별도 재고아이템으로도
만들면 이원화된다**는 경고다. 저장 위치 자체는 Irene 모델대로 맞다 → 자세한 것은 위 A-00 참조.

**⇒ 고칠 방향 (두 역할 공통)**
- 레시피 **있음** → 레시피에 연결된 **재고아이템들**에서 차감 *(현행 유지)*
- 레시피 **없음** → **그 프로덕트/메뉴 자체 수량**에서 차감 *(현행 유지)* +
  **입고가 그 수량으로 들어가게** 고친다 *(현재 별도 재고아이템으로 들어간다 — 이것이 결함)*

#### 결함: 판매상품 → **GIT 자기 재고** 연결이 없다

- **재고 아이템**(`product_ingredients`)에 **판매상품을 가리키는 컬럼이 아예 없다** (실측: 참조 컬럼 0개).
...
- ⇒ **GIT 이 팔아도 GIT 재고(투명컵/뚜껑 18 pack)는 영원히 안 줄어든다.**
...
### A-00. 레시피 없는 프로덕트 = 수량이 프로덕트에 그대로 있어야 한다 (Irene 확정)

> Irene: *"프로덕트나 메뉴 = 재고아이템, 프로덕트 레시피 재료나 메뉴 레시피 재료 = 재고아이템들(여러 개)
> 이렇게 루트 있는 거잖아."*
> Irene: *"프로덕트나 메뉴 = 재고아이템 이런 프로덕트나 메뉴의 경우에는 **재고수량이 프로덕트나 메뉴에
> 수량이 그대로 있어야지**."*

| 루트 | 수량이 사는 곳 |
|---|---|
| 레시피 **있음** | 레시피 재료 = **재고아이템 여러 개** |
| 레시피 **없음** | **그 프로덕트/메뉴 자체** (`brand_products.current_stock` / `products.current_stock`) |

**⇒ 지금 차감 코드가 보는 자리는 맞다.** 출고(ship)가 `brand_products.current_stock` 을,
매장 판매가 `products.current_stock` 을 보는 것은 **Irene 모델대로다.**
⚠ 내가 이 자리를 "레거시라 틀렸다"고 보고한 것은 **오판**이다.
(`inventoryDeductionService.js:194` 의 "이원화" 경고는, 같은 물건을 **별도 재고아이템으로도 만들면**
이원화된다는 뜻이지 이 저장 위치가 틀렸다는 뜻이 아니다.)

#### 🔴 진짜 결함 — 입고가 프로덕트로 안 들어간다

GIT 이 UGS 에서 산 포장재는 **그 프로덕트가 아니라 따로 만든 재고아이템**으로 입고됐다.

| | 어디 | 수량 |
|---|---|---|
| GIT 이 실제로 받은 것 | `product_ingredients` (재고아이템, 별도로 만든 것) | 투명컵 **18 pack** · 뚜껑 **18 pack** |
| GIT 이 파는 것 | `brand_products` (프로덕트) 의 자체 수량 | **0** · `track_stock=0` |

같은 포장재인데 **물건이 두 개**가 됐다. 발주 화면이 **재고아이템만 주문 대상으로 삼기 때문**이다
(PO 라인이 `product_ingredient_id` 를 참조). 레시피 없는 프로덕트를 **직접 보충할 경로가 없다.**

**⇒ 고칠 것 2가지**
1. **레시피 없는 프로덕트는 발주·입고가 그 프로덕트 수량으로 들어가게 한다.**
   (발주 대상에 프로덕트를 포함 + `receive` 가 프로덕트 수량을 올리는 분기)
2. **이미 재고아이템에 들어가 있는 수량을 해당 프로덕트로 이관**한다 (투명컵 18 · 뚜껑 18 등).
   이관 후 중복 재고아이템은 정리한다 — 안 그러면 계속 이원화된다.
3. `track_stock` 게이트 재검토 — Irene: *"재고추적은 항상되는 거 아니야?"*
   현재 이 프로덕트들은 `track_stock=0` 이라 수량이 있어도 차감이 안 걸린다.

### A-000. 재고아이템 등록 루트 — **어느 입구로 들어와도 하나로** (Irene 지시 2026-09-01)

> Irene 원문: *"재고 아이템은 이런 루트에서 등록되어야 해. 프로덕트나 메뉴를 등록할 때 이대로
> 재고아이템으로 함께 등록, 공급업체 상품등록할 때 재고아이템으로도 등록하기, 재고아이템 등록 시
> 공급업체 상품으로도 등록하기와 프로덕트로도 등록하기, 공급업체 선택하고 없으면 신규등록,
> 프로덕트나 메뉴 등록으로 연결하면서 필요한 정보 추가로 받기, 판매가 등"*

**입구 3개, 결과는 하나** — 물건이 둘로 갈라지는 것(지금 GIT 포장재가 걸린 문제)을 **등록 단계에서 원천 차단**한다.

| 시작점 | 함께 등록되는 것 | 그 자리에서 추가로 받는 정보 |
|---|---|---|
| **① 프로덕트 / 메뉴 등록** | → **재고아이템** 을 그대로 같이 등록 | 재고 단위·기준수량 |
| **② 공급업체 상품 등록** | → **재고아이템** 으로도 등록 | 우리 쪽 재고명·단위(공급업체 표기와 다를 수 있음) |
| **③ 재고아이템 등록** | → **공급업체 상품** 으로도 등록<br>→ **프로덕트 / 메뉴** 로도 등록 | **공급업체 선택 — 없으면 그 자리에서 신규 등록**<br>프로덕트/메뉴로 연결 시 **판매가 등** 필요한 값 |

**역할별 대상 테이블**

| | 재고아이템 | 판매물 | 공급업체 상품 | 연결 |
|---|---|---|---|---|
| 브랜드 제너럴 | `product_ingredients` | `brand_products`(프로덕트) | `supplier_products` | `ingredient_seller_products` |
| 레스토랑 관리자 | `ingredients`(restaurant) | `products`(메뉴) | `supplier_products` | `ingredient_seller_products` |

**원칙**
- **한 물건 = 한 재고아이템.** 입구가 달라도 새로 만들지 않고 **기존 것에 연결**한다(중복 생성 방지 검사 필수).
- **레시피 없는 프로덕트/메뉴**는 그 자체가 재고아이템이므로 **수량은 프로덕트/메뉴에 그대로** 둔다(A-00).
- 공급업체가 목록에 없을 때 **화면을 떠나지 않고 신규 등록**할 수 있어야 한다.
- 프로덕트/메뉴로 연결할 때 **판매가**를 그 자리에서 받는다.
  ⛔ 공급업체 원가를 판매가 자리에 복사하지 않는다 — 2026-08-28 마진 0 사고
  ([[reference_supplier_cost_copied_as_price]]).
- **채번 주의**: 상품 SKU·재고 코드가 건수 기반이라 삭제 후 재등록 시 번호가 재발급된다
  ([[reference_count_based_code_numbering]]). 이 흐름을 만들 때 같이 손본다.

### A-0. 이번 건의 범위 — **레시피 없는 프로덕트(포장재)** 하나

> Irene: *"내가 물어본 재고 빠지는 건 패키지니까 레시피 없는 프로덕트야."*

PO-R10-20260827-001 의 6품목은 전부 **포장재 = 재판매 = 레시피 없는 프로덕트**다.
따라서 이 건은 **"레시피 없음" 루트 하나**의 문제이고, "레시피 있음" 루트는 이미 정상 동작한다.

```
GIT 이 UGS 에서 포장재 입고  →  BG 재고(product_ingredients)  투명컵 18 pack · 뚜껑 18 pack
GIT 이 with MIN 에 ship      →  brand_products.current_stock (0, track_stock 꺼짐)   ← 여기서 끊긴다
```

**⚠ 연결은 자동 매칭으로 때울 수 없다 — 사람이 한 번 지정해야 한다.**
6품목 중 프로덕트명과 BG 재고명이 일치하는 건 **2개뿐**(MTP 투명컵·플랫뚜껑).
나머지 4개(TK 1000 사각통 · 3.25OZ 소스통 · PE 비닐봉투 · SC-800)는 프로덕트가 **공급업체식 이름**이라
이름으로는 못 찾는다. 이름매칭으로 때우면 **엉뚱한 재고를 깎을 위험**이 있다.
→ **프로덕트 등록/수정 화면에 "이 프로덕트는 어느 재고에서 나가는가" 선택**을 둔다.
...
2. **연결 신설** — `product_ingredients`(= BG 재고)에 프로덕트 참조가 없다. 프로덕트와 자기 재고를 잇는
