...
## 5. Fable 판정 (2026-09-01 · Irene 승인 대기)

> 판정자 Fable. 아래는 결정이고, Irene 이 승인하면 §6 순서로 착수한다. Irene 이 뒤집으면 그 항목만 다시 판정한다.
> Opus 가 §4-B 에 쓴 *"레시피 없음 → 프로덕트 자체 수량에서 차감(현행 유지)"* 는 **이 판정으로 바뀐다** (Q1 참조).

### 5-1. 결정 요약

| # | 결정 | 한 줄 근거 |
|---|---|---|
| **Q1** | **프로덕트 ↔ 재고아이템 1:1 연결.** 레시피 없는 프로덕트/메뉴는 **자기 재고아이템 하나를 반드시 가진다**(등록 시 자동 생성 또는 기존 것 선택). **수량은 그 재고아이템에 산다.** 프로덕트 화면은 그 수량을 보여주고 거기서 고친다. 발주 라인은 지금처럼 재고아이템만 가리킨다(프로덕트를 발주 대상에 넣지 않는다). | M7-① 이 이미 "프로덕트 등록 → 재고아이템 함께 등록"이라 답이다. 수량이 재고아이템에 있으면 입고·차감·최소치·실사·배치·발주 제안이 **전부 지금 코드로 그대로 돈다**. 프로덕트에 수량을 따로 두면 그 전부를 프로덕트용으로 다시 만들어야 하고, 지금 GIT 처럼 둘로 갈라진다(`inventoryDeductionService.js:194` 경고가 바로 그 뜻). |
| **Q2** | **사람 확인 먼저, 자동은 뒤.** ①운영 읽기로 "프로덕트 ↔ 같은 이름·단위 재고아이템" 후보표를 뽑는다 ②이름이 다른데 수량이 들어 있는 것(GIT 4건 등)은 Irene 이 표에서 짝을 확정한다 ③배포 마이그가 확정 짝을 먼저 잇고, 나머지는 자동 규칙(후보 1개면 연결 / 0개면 프로덕트 값으로 재고아이템 신설 / 2개 이상이거나 양쪽 다 수량>0 이면 잇지 않고 목록에 남김). **연결 시 재고아이템 수량이 진실**, 프로덕트 쪽 수량은 로그만 남기고 더하지 않는다. 재고아이템 행은 지우지 않는다. | 자동 이름매칭 2/6 뿐이라 사람이 지정해야 한다. 순서를 거꾸로(자동 신설 먼저) 하면 GIT 4건이 **새 중복**을 만든다. 더하면 이중 계상. |
| **Q3** | **다중 공급처 유지.** 등록 입구(②③)에서는 공급처 **하나만** 고르게 하고, 추가 공급처는 지금처럼 재고아이템의 공급처 목록에서 붙인다. **선호 공급처는 항상 정확히 1개**(하나를 선호로 찍으면 나머지는 자동 해제, 서비스 함수 한 곳). | 이미 매장 재고 14%(55건)가 쓰고 있고 새우 13.90~33.00 처럼 **가격 비교가 그 기능의 존재 이유**다. Irene 조건은 "복잡하면 제외"인데 복잡함은 데이터가 아니라 **등록 입구**에 있고, 입구는 하나만 고르게 하면 단순해진다. 빼면 with MIN 55건을 지워야 한다. |
| **Q4** | **원가 = 선호 공급처 링크의 가격.** 평균가·최근가 없음. **가격 0 인 링크는 "원가 미입력"으로 표시**(0 원가로 조용히 계산하지 않는다) — 재고 목록·레시피 원가 합계에 "미입력 재료 N개" 표시. 링크를 지우지는 않는다. | [[reference_cost_two_paths]] 그대로("공급업체 가격이 그대로 원가"). `is_preferred` 611건이 이미 그 구조. 0 은 가격이 아니라 빈칸이다. |
| **Q5** | **`track_stock` 게이트 제거 — 재고추적은 항상.** 차감·입고·출고 4곳의 `if (!track_stock) skip` 을 없앤다. 컬럼은 남기되(드롭 금지) 마이그로 전부 1, UI 토글 숨김, 발주 화면의 "숨김" 필터 제거. 품목을 안 쓰려면 `is_active` 로 끈다(그게 원래 그 용도). | M6. 8/22 에 기본값을 껐던 이유는 "수량 0 품목이 알림을 채운다"였는데, 저재고 알림은 이미 `min_stock > 0` 일 때만 뜬다(`inventory-core.js:134`) — 스위치 없이도 조용하다. GIT 포장재가 안 빠진 직접 원인이 이 스위치다. |
| **Q6** | **구매자 선수령 허용 유지 + 판매자 쪽 구멍 봉합.** 구매자가 배송 전에 수령하면 판매자에게 알림 + 화면에 *"구매자가 수령 확인했습니다 — 출고(배송완료) 처리할까요?"* 가 뜨고, **판매자의 출고 처리는 `shipped_at` 이 비어 있는 동안 상태와 무관하게 1회 허용**(재고 차감은 그때 1번). 상태는 뒤로 안 돌린다. | 현실은 판매자가 버튼을 안 누르고 물건이 먼저 온다(외부 공급업체는 아예 안 누른다). 그런데 **판매자 재고 차감은 ship 에만 붙어 있어서** 선수령이면 판매자 재고가 영영 안 빠진다 — 이 구멍이 Q6 의 진짜 문제다. M8 원문("받았다고 누르면 배송완료로 할거냐고 안내")이 정확히 이 처리다. |
| **Q7** | **이 개편에서 제외 — 별건(K-DINE 메뉴)으로 기록.** 정식 해법은 **"주문 조건 가격 규칙"**(주문에 식사가 있으면 음료 RM3 로 자동 적용, POS 가 판단) — 메뉴 2줄도 옵션도 아니다. 그때까지 임시는 **메뉴 2줄(같은 레시피 연결)**. | 2줄은 직원이 골라야 해 실수하고, 옵션은 품목 안에서 주문 전체를 못 본다. 자동 규칙만 둘 다 해결한다. 임시로 2줄을 쓰는 건 재고가 같은 레시피로 빠지기 때문(수동 할인은 PIN 게이트를 탄다). 이 개편에 섞으면 범위가 는다. |

### 5-2. 설계 절단면 (Q1·Q5 의 구체)

- **링크 컬럼** — RA `products.stock_ingredient_id` → `ingredients.id` / BG `brand_products.stock_product_ingredient_id` → `product_ingredients.id`. 둘 다 nullable + **UNIQUE**(1:1 강제).
- **불변식** — 레시피(`recipe_id`/`product_recipe_id`) 없음 ∧ 세트 아님 ⇒ 링크 **필수**. 레시피 있음 ⇒ 링크 NULL(레시피 재료가 재고). 세트 ⇒ 둘 다 NULL(구성품이 빠진다). 인스펙션 하니스에 불변식으로 등록해 위반 0 을 배포 게이트에서 본다.
- **수량 읽기·쓰기** — `products.current_stock` / `brand_products.current_stock` 는 **더 이상 쓰지 않는다.** 프로덕트 API 는 링크된 재고아이템 수량을 `current_stock` 자리에 **읽어서 돌려준다**(기존 화면 무수정). 프로덕트 폼에서 수량을 고치면 재고아이템에 쓴다(실사 기록 남김).
- **차감 경로 교체(3곳)** — 판매 차감 `inventoryDeductionService.deductInventoryForOrder`(RA): 레시피 없음 → 링크된 ingredient 를 `stockFor/applyStock` 로 차감(브랜드 공유 재료 오버레이 그대로 탄다) / 판매자 출고 `seller-orders.js` brand 분기: 레시피 없음 → 링크된 product_ingredient 차감 / 입고 `purchase-orders-workflow.js` receive·mark-received: `track_stock` 분기 제거. 링크가 없으면 **건너뛰되 센다**(`skipped_no_stock_link`) — 마이그 후 0 이어야 한다.
- **등록 입구 ①의 서버 기본값** — 레시피 없는 프로덕트 생성 시 링크가 안 오면 서버가 같은 이름·단위로 재고아이템을 **자동 신설**해 잇는다. (UI 선택기는 P2. P1 부터 새 프로덕트가 링크 없이 생기지 않게 하는 장치.)
- **원가 복사 금지** — 입구 ③에서 프로덕트를 같이 만들 때 **판매가는 필수 입력·원가로 미리 채우지 않는다**([[reference_supplier_cost_copied_as_price]]).

### 5-3. 결제·현금관리 절단면 (M9·M10)

- `purchase_orders` +`payment_status ENUM('unpaid','paid','refunded')` 기본 unpaid · `payment_method ENUM('cash','bank_transfer','card')` · `paid_at` · `paid_by_user_id` · `cash_movement_id`(되돌리기용). **"외상"은 별도 방법이 아니라 unpaid 상태**다(나중에 `/pay`).
- `cash_movements` — `source` 에 `purchase_order` **expandEnum 으로 추가** · `purchase_order_id` 컬럼 추가(범용 reference_id 대신 명시 FK).
- **현금 규칙** — 구매자가 **매장(restaurant)** 이고 **열린 시프트가 있을 때만** 드로어 출금(`type=out`) 을 그 시프트에 만든다. 시프트가 없으면 결제는 기록하되 드로어 출금은 안 만들고 화면에 *"시프트가 열려 있지 않아 드로어 출금으로 기록되지 않았습니다"* 를 띄운다(막지 않는다 — 시프트 밖 지출은 드로어 돈이 아니다). BG 구매자는 드로어가 없으니 결제 기록만.
...
- **한 번에 처리(D)** — `POST /purchase-orders/:id/receive-and-pay` **단일 트랜잭션** 엔드포인트. 프론트가 receive → pay 두 번 부르는 방식 금지(중간 실패 = 받았는데 미결제). 이를 위해 receive 본체를 `services/purchaseOrderReceive.js` 로 빼서 `/receive` 와 공유(로직 복제 금지). 결제 본체도 `services/purchaseOrderPayment.js`(`recordPayment`/`reversePayment`) 한 곳.
- **화면** — staging 에 "받을 발주(수령 가능 상태)" 블록 추가 → [수령+결제 완료] / [수령만]. 발주 상세에도 같은 버튼. 확인창은 재고 증가·결제 기록·드로어 출금(해당 시) 세 가지 효과를 명시.

### 5-4. 양방향 확인 절단면 (M8 · Q6)

- 판매자 → 구매자: 알림은 있으니 **화면 단서만** 추가 — 구매자 발주 목록 행에 *"업체가 배송완료로 표시"* 배지 + [수령 확인] 버튼.
- 구매자 → 판매자: 알림 카테고리 `buyer_received` 신설(NOTIFICATION_CATEGORIES) + 판매자 주문 목록에 *"구매자 수령 확인 — 출고 처리할까요?"* 행 강조 + [출고 처리] 버튼(`shipped_at IS NULL` 동안 허용, 1회, 재고 차감 그때).

## 6. 규모 · 착수 순서 · 게이트 (Fable 확정)

**대.** 링크 컬럼 2 + 결제 컬럼 5 + cash_movements 컬럼 1 + ENUM 확장 1 + 등록 흐름 3화면 + 알림 1종 + 차감 경로 3곳 + 데이터 이관. 운영 마이그레이션 포함, 돈·재고 무결성 접촉.

### 착수 순서 — 단계마다 따로 배포, 따로 게이트

| 단계 | 내용 | 왜 이 순서 | 게이트 |
|---|---|---|---|
| **P0 실측** | §6-A 의 운영 읽기 7건 → 후보표를 Irene 에게 | 마이그 규칙과 Q2 짝 확정에 숫자가 필요하다. 코드 0줄 | 없음(읽기만) |
| **P1 정합** | 링크 컬럼·불변식·차감 경로 3곳 교체·`track_stock` 게이트 제거·읽기전달·서버 자동신설·마이그(확정 짝 + 자동 규칙)·인스펙션 불변식 | **GIT 이 팔아도 안 빠지는 결함 자체**를 고치는 단계. 나머지는 이 링크 위에 선다 | Fable 전체 게이트(재고 무결성·운영 마이그). 고장주입: 게이트 제거 코드를 되돌리면 데모 매장 판매 차감 테스트가 실패해야 한다 |
| **P2 입구 3개** | 프로덕트 폼 재고 블록(기존 선택/신설) · 공급업체 상품 → 재고아이템 · 재고아이템 → 공급업체(신규 포함)+프로덕트(판매가 필수) | P1 뒤에 새로 생기는 프로덕트가 링크 없이 생기지 않게 UI 로 닫는다 | 경량(UI) + 원가→판매가 복사 금지 반증 1회 |
| **P3 양방향** | `buyer_received` 알림 · 판매자 [출고 처리](`shipped_at` 1회) · 구매자 배지+[수령 확인] | 재고 흐름이 맞아진 뒤에야 "언제 빠지나"를 사람에게 알리는 의미가 있다 | 경량 + 출고 2회 눌러도 1회만 차감 반증 |
...
### 6-A. P0 운영 실측 (읽기 전용 · Opus 실행)

1. 레시피 없는 비세트 프로덕트 수: RA `products`(recipe_id·product_recipe_id NULL, is_set_menu=0, deleted_at NULL) 매장별 / BG `brand_products`(product_recipe_id NULL) 오너별. 그중 `track_stock=1`·`current_stock>0` 각각.
2. 1 의 각 프로덕트에 대해 같은 소유 범위의 재고아이템 중 **정규화 이름+단위 일치** 후보 수(0/1/2+) 집계 + **양쪽 다 수량>0 인 충돌** 목록.
3. GIT 6품목 짝 후보표: 프로덕트(id·이름·단위) ↔ product_ingredients(id·이름·단위·수량) — Irene 확정용.
4. `ingredients`·`product_ingredients` 에서 `track_stock=0` 이면서 `current_stock>0` 인 행 목록(게이트 제거 시 **차감이 시작되는** 것들).
5. `ingredient_seller_products`: 링크 2개 이상 품목 중 선호 개수 ≠ 1 인 것 / 선호 링크 중 `unit_price=0` 인 것(개수·소유자).
6. `cashier_shifts`: 매장당 열린 시프트 1개 불변식이 코드에 있는지 + 현재 열린 시프트 있는 매장 수.
7. `purchase_orders` 에서 `status='received' AND shipped_at IS NULL`(선수령으로 판매자 차감이 빠진 건) 개수·판매자별.

### 게이트 공통
- 매 단계 `verify-all` + `check-sensitive-diff` + 인쇄 보호파일 무접촉(이 개편은 인쇄 파일을 열 이유가 없다).
- 마이그는 전부 멱등 + `migrations.registry.json` 등록(P1 짝 적용 = `manual`, 컬럼·ENUM = `deploy`). ENUM 은 `expandEnum()` 만.
