...
| # | 결정할 것 | Opus 가 모은 근거 |
|---|---|---|
| **Q1** | 레시피 없는 프로덕트/메뉴의 **입고 경로** — 프로덕트를 발주 대상에 넣을지, 재고아이템과 1:1 로 묶을지 | 현재 PO 라인은 `product_ingredient_id`/`ingredient_id` 만 참조. 프로덕트를 가리키는 자리가 없다 |
| **Q2** | 기존에 갈라진 수량 **이관 범위와 방법** (투명컵 18·뚜껑 18 등) + 중복 재고아이템 처리 | 프로덕트명↔재고아이템명 일치 2/6 → **자동 매칭 불가, 사람이 지정해야 함** |
| **Q3** | **다중 공급처를 유지할지 뺄지** — Irene: *"복잡하면 멀티 공급업체 등록 기능 제외하는게 맞을 것 같고"* | 실사용 매장 14.3%(55건), 가격 다름 78%, **`is_preferred` 가 전 건 지정돼 있어 원가 기준은 이미 하나**. 빼면 with MIN 55건 정리 필요 |
| **Q4** | 다중 공급처 유지 시 **원가 기준 명문화** (선호 공급처 가격? 최근가?) 와 **가격 0 링크 처리** | `is_preferred` 611건 지정. 0.00 링크 다수 존재 |
| **Q5** | **`track_stock` 을 없앨지** — M6 대로 항상 추적으로 갈지, 유지하되 기본 켜기로 갈지 | 현재 GIT 포장재는 전부 `0` 이라 수량이 있어도 차감이 안 걸린다 |
| **Q6** | 구매자가 **배송 전에 먼저 수령**할 수 있는 현행(`submitted` 부터 수령 가능)을 유지할지 | 유지 시 C 의 "판매자에게 안내"가 반드시 필요 |
| **Q7** | 음료 **조건부 가격**("음식 주문 시 RM3") 처리 — 별도 메뉴 2줄 vs 옵션 | 시스템 미지원 (K-DINE 메뉴 건, 별건) |

## 6. 규모 · 게이트

- **대.** DB 컬럼 4개 + ENUM 확장 + 등록 흐름 3개 화면 + 알림 1종 + 재고 차감 경로 + 데이터 이관
- **운영 마이그레이션 포함** → 배포 안전 게이트(ENUM parity · migration registry) 대상
- **돈(현금관리)·재고 무결성 접촉** → Fable 검증 게이트 대상, 검증 규율 4조항 전부(고장주입 반증 포함)
- 착수 순서는 Fable 이 정한다. Opus 는 판정 없이 실행·실측만 한다.
