...
> **📌 2026-09-11 재검토 조건 충족 → 스냅샷 추가 (Fable 판정 · Irene 「권고대로 해」)**
> Irene: 「포장양은 표시 안되고 포장 단위만 나오면 어떻게 해? … 1kg 단위면 1 kg X 1pack 이렇게 되는 거 아니야?」
> 확정 발주 줄 8개 표면(Staging·상세·인쇄·PDF·WhatsApp/메일·공급업체 알림메일·공급업체 수신함·인보이스 대조)이
> 규격을 그려야 하게 됐다 → `purchase_order_items.base_quantity`·`base_unit` 스냅샷 신설, 판매 상품 3표에 `package_unit`.
> `base_quantity`(용량)는 스냅샷하고 `order_mode` 는 여전히 스냅샷하지 않는다(줄 단위에 이미 반영됨).
> 단일 기준 = `docs/TRADE_STRUCTURE.md` §2-2 «판매 상품·발주 줄도 다섯 칸을 따른다».
...
- **2026-09-11 외부 공급업체 등록 창(`RegisterExternalSupplierModal`, RA 재고·BG 공용)** — 이 창이 연결을 `unit_conversion: 1` **고정**으로 보내던 마지막 자리였다. 같은 날 배포로 창에 «포장당 용량 10 kg · BOX» 를 적을 수 있게 되자 10 kg 박스 3개 입고 → 재고 +3(맞는 값 +30) · 원가 kg당 48(맞는 값 4.8) 위험이 커졌다(Fable 판정).
  - 조치: 창에 **«재고 환산» 칸** — «1 {포장단위} = [값] {재고 단위}». 기본값 `utils/unitConversion.ts` `defaultLinkConversion(용량, 내용물 단위, 재고 단위)`: 같은 단위 → 용량 · 변환 가능(kg→g) → 환산 · 불가(bottle→ml, tray→kg) → **빈 칸(필수 입력)**. 사람이 고치면 그 값 우선. 0 이하·빈 값은 저장 차단.
  - 기존 연결 소급 **안 함**(위 ⛔ 그대로).
...
> **⚠ 한계 — 손글씨 인보이스는 못 읽는다 (2026-09-11 · Fable 판정 · Irene 「fable 판단 오면 바로 고쳐」)**
> Irene: 「고르라는 항목이 전혀 맞지도 않아 … 리스트는 2-3개 나와 … 이미지 제대로 스캔을 못하는 거야? 수준이 낮아?」 (운영 발주 32)
> - 실측: 발주 32 Guan Kee 인보이스 = **인쇄 양식지 + 숫자·이름 수정 전부 손글씨**. tesseract(인쇄체 전용)로 1800 축소 입력 → 손글씨 0줄, 파서 통과 1줄 = 전화번호 줄(수량 19 · 단가 −3,221,326); 원본 3278×5370 입력 → 평균 신뢰도 50, 파서 통과 6줄 전부 쓰레기. **해상도·설정 문제가 아니다.**
> - 화면이 짝 0줄인데도 초록 «읽어 오른쪽을 채웠습니다» 를 띄우고, 6줄 모두의 «이 줄 고르기» 목록에 전화번호 줄만 떴다.
> - **수정 A (품질 게이트)** `InvoiceReconcilePage.tsx` `readResult` 세 상태 — 발주 기본값은 어느 경우에도 유지(§1 «다른 줄만 사람이 고친다»):
>   - `no_lines` 파서가 품목 줄로 인정한 줄 0(손글씨·흐린 사진 — 발주 32) → 노랑 «이 인보이스는 자동으로 읽지 못했습니다(손글씨·흐린 사진). 왼쪽 원본을 보면서 발주 값과 다른 줄만 고쳐 주세요.» + 고르기 목록 없음
>   - `no_match` 읽힌 줄은 있는데 발주와 짝 0 → 노랑 «읽었지만 발주와 맞는 줄을 찾지 못했습니다. 아래 목록에서 골라 붙이거나 직접 입력하세요.» + **목록 보임**
>   - `ok` 짝 1개 이상 → 초록 그대로
>   - ⛔ `no_match` 에서 목록을 숨기지 않는다(Fable 게이트 보정): 그 목록이 이름 사전(`invoice_name`)을 처음 가르치는 유일한 자리라, 숨기면 인쇄명이 안 겹치는 공급업체는 영원히 자동 읽기를 못 쓴다. 첫 구현(짝 0 이면 목록 숨김)이 이 거짓말을 새로 만들어 게이트에서 막혔다.
> - **수정 B (파서 상식 검사)** `invoiceMatcher.ts` `parseInvoiceLine`: 수량·단가 >0 · 금액 < 1,000,000 · 숫자 3개 줄은 수량×단가 ÷ 금액이 **10배 이내**. ±10% 로 조이지 않은 이유 = 한 자리 오독 줄(28.00→8.00, 3.5배)은 버리지 않고 amount_mismatch «확인 필요» 로 보여 줘야 한다(계약 (b)).
> - 계약 테스트 `invoiceMatcher.guard.test.ts` (g) 발주 32 OCR 전문 → 0줄 · (h) 서울마트 인쇄 13줄 → 12줄 · 전화번호 줄 null · 오독 줄 유지. 고장주입(상식 검사 블록 전체 제거 → (g)·전화번호 FAIL) 성립.
> - **보류(Irene 결정)**: 손글씨를 읽으려면 클라우드 비전 OCR 뿐 — 위 D1(무료·브라우저·외부 전송 0)을 뒤집는 결정. 운영 첨부 중 손글씨 비율을 잰 뒤 판단(Fable 권고: 절반 넘으면 검토).
...
**🔴 2026-09-24 — 저장과 매칭의 전제가 어긋나 학습이 안 되고 있었다** (Irene 「학습해서 다음에 알 수 없어?」).
매칭 1단계는 OCR 잡음을 예상해 느슨하게 만들었는데(「사전 낱말이 전부 들어 있으면」), 화면이
**읽힌 줄 이름을 통째로** 사전에 넣어(`r.parsed.name`) 그 느슨함을 무력화했다 — 잡음 낱말이 다음
인보이스에 똑같이 재현돼야만 걸렸다. 고친 것 셋:
- **저장값을 매칭과 같은 규칙으로 정규화** — `aliasFor()`(= `tokens()` 결과를 join). 백엔드는 정규화를
  따로 구현하지 않는다(저장만). `tokens()` 에 **숫자만인 토큰 제거** 추가(두 자리 줄번호 「14 …」·「17 …」).
- **1단계를 양방향 부분집합으로** — 어느 한쪽 낱말이 다른 쪽에 전부 들어 있으면 짝. 옛 사전에 잡음이
  통째로 저장돼 있어 「사전이 지저분하고 줄이 깨끗한」 경우가 실제로 많다. 2단계처럼 **전역 greedy**.
- **학습 퇴화 방지** — `aliasToRemember()`: 사전으로 붙은 줄(`viaAlias`)은 사전을 갱신하지 않는다.
  안 그러면 잘 붙은 깨끗한 사전을 이번에 읽힌 잡음으로 덮어쓴다. 사람이 직접 고른 줄은 항상 갱신.

**「총액대로만 저장」** (2026-09-24 · Fable D6) — 사진 판독이 엉망일 때 적은 총액으로 저장·결제하는 길.
`total_only: true` 면 ⓐ줄 값을 **전부 비우고**(이전 대조 값이 남아 「줄까지 맞춘 것」으로 보이지 않게)
ⓑ매장 원가·전파·소급·이름사전 **전부 건너뛰고** ⓒ헤더(총액 **필수**)와 `invoice_reconciled_at` 만 저장
ⓓ총액 불일치 검사 생략. 청구서에는 차액 크기와 무관하게 **`Supplier invoice difference` 한 줄**이 붙어
청구서 총액 = 적은 총액 = 낼 금액이 한 숫자가 된다.
판정 `isTotalOnlyReconcile(po, items)` 는 **새 칸 없이 도출**(대조 시각 있음 AND 모든 줄 단가 null).
화면은 저장이 막힌 그 자리에 버튼 하나(별도 토글 없음) + 「총액만 맞춤」 배너.
⚠ 표시 조건은 **`totals.blocked || totalMismatch`** 여야 한다 — `totalMismatch`(서버 400) 만 보면
화면이 먼저 저장을 잠가 **버튼이 영영 안 뜬다**(Fable 게이트 적발).

**계약**: `dev-frontend/src/utils/invoiceMatcher.guard.test.ts` **19건** — 픽스처가 **실제 발주 33 의 19줄 +
실제 OCR 텍스트**다. 고장주입: 1:1 제거 / 사전 제거 / 문 제거 / 양방향→한방향 / viaAlias 분기 제거 /
숫자토큰 제거 뺌 / 총액만 모드 원가 무접촉 제거 — 전부 반증됨.
⚠ **화면 jest 는 배포 게이트 밖**이다: `CI=true npx react-scripts test --testPathPattern invoiceMatcher.guard`

---

### 8-6. 총액 우선 대조 + 헤더 갭 표시 (2026-09-25 · Fable 판정 · **A 구현·운영 배포 SW 5.65** · C(AI) 는 Irene 결정 대기)

> **구현 결과(2026-09-25 [Claude Code])** — A-1~A-4 그대로. 갭 계산 한 곳 `dev-frontend/src/utils/reconcileGap.ts`(`reconcileGap`·`formatGap(gap, 통화코드)`·`isTotalOnly`). 서버 변경 0 — `total_only` 경로와 `TOTAL_MISMATCH` 안전망 그대로. 반증: 서버 줄 null 화 제거 → 실호출 2건 실패 · 갭 부호 반전 → 단위 테스트 실패.

> Fable 이 지정한 자리는 「8-2 바로 아래 8-5」였으나 §8-5(브랜드·푸드코트 청구서 결제)가 이미 있어 **8-6** 으로 둔다. 내용은 판정 원문 그대로.

> Irene 원문: 「그리고 인보이스를 넣고 실제 발주한 거랑 금액이 다르거나 배송비를 추가해야 할 때 그걸 못하고 있어. 최종 아이템 매칭을 일일이 안해도 그냥 최종 금액 픽스하게 할 수 없을까? 그냥 갭차이 - + 표시해서 저장하는 거 되면 좋겠어. 일일이 틀린 거 다 잡는게 어려운게 텍스트를 제대로 불러오지를 않아. ai 추가하면 해결돼?」

**한 줄 결론** — Irene 이 원하는 것(줄 매칭 없이 최종 금액 고정 + 갭 ± 저장)은 **서버에 이미 있고(`total_only`) 운영에도 배포돼 있다.** 문제는 그 길이 **「실패한 뒤에만 나타나는 예비 버튼」**이라 사람 눈에 없는 기능이고, 저장 뒤 **갭이 목록·상세에 안 보인다**는 것이다. 답은 AI 가 아니라 **그 길을 1차 경로로 앞에 세우고 갭을 헤더 기준으로 보여주는 화면 재배치**다. 서버 변경은 거의 0. AI(비전 판독)는 별개 사안으로 조건부 시범만 권고.

**0. 실측 사실**
- 운영 청크 `/static/js/548.65db1aa0.chunk.js` 에 `saveTotalOnly`·`blockedShort` 가 있다 → 09-24 게이트가 잡은 「버튼이 영영 안 보이던」 결함의 수정판이 **운영에 있다**(Irene 브라우저가 옛 SW 캐시를 물었을 가능성은 남음 — 확인 불가).
- 화면은 「총액 칸에 값을 넣고 → 줄 합과 RM 1 넘게 다를 때만」 노란 상자+버튼이 **맨 아래 Actions 밑**에 뜬다(`InvoiceReconcilePage.tsx:1035`). 총액을 안 적으면 저장이 그냥 통과하고 계산값이 총액이 된다 — 「줄을 다 못 맞추겠으면 총액만 적어라」는 안내가 **어디에도 없다**. 「못 하고 있어」의 가장 유력한 원인(순위 ①안내 부재 ②옛 SW 캐시 ③총액은 적고 배송비 칸을 안 채워 계산값과 어긋나 빨간 문구만 봄).
- 목록 배지 `▲ 차이 N · ±금액` 은 `purchase-orders-crud.js:335-362` 의 **줄 기준 SQL**. 총액만 모드는 줄 값이 전부 null 이라 배지가 「대조 완료」로만 보이고 **갭이 사라진다**. 줄 모드여도 배송비·세금 차이는 배지에 안 잡힌다. 헤더 갭은 결제 모달(`ReceivePayModal.tsx:139`)에서만 취소선으로 보인다.
- `total_amount = subtotal + tax_amount + delivery_fee`(`purchase-orders-crud.js:216`) → 갭 = `invoice_total − total_amount` 는 같은 기준끼리의 비교. **새 칸 불필요.**
- 총액만 모드에서도 `invoice_delivery` 는 저장되고(`cost-reconciliation.js:330`), 청구서에 「Supplier invoice difference」 한 줄이 붙어 청구서 총액 = 적은 총액 = 낼 금액(D6, 09-24 실호출 11/11).

**A. 최종 형태 — 「총액 우선 대조」(기존 `total_only` 를 1차 경로로 승격, 새 모드 아님)**
- **A-1** 헤더 블록을 첫 화면 최상단의 1차 경로로: [인보이스 총액][배송비][세금][할인] 4칸(이미 있음) + 아래 **갭 한 줄** 「발주 RM X → 청구 RM Y = **+RM Z**」(+ 주황 / − 초록 / 0 회색). 총액 칸에 값이 있으면 항상 표시.
- **A-2** 1차 버튼 **「이 총액으로 확정 (줄은 나중에)」** = 기존 `save(true)`. **총액이 적혀 있으면 항상 활성** — 줄 합 일치를 기다리지 않는다. 줄 매칭·OCR 은 아래 「줄까지 맞추기(원가 반영)」 2차 영역으로. 맨 아래 노란 예비 상자는 **제거**(버튼 2개 금지). 서버 400 `TOTAL_MISMATCH` 경로는 안전망으로 유지.
- **A-3** 이미 줄 값이 저장된 발주에서 총액 확정을 누르면 확인창 「줄 단가 N개가 지워지고 원가에 반영되지 않습니다」 — D6 「반쪽 진실 금지」 유지. 서버는 그대로(줄 전부 null 화).
- **A-4** **갭 표시 = 헤더 기준으로 통일.** 목록·상세·결제 모달 셋 다 `invoice_total − total_amount`. 총액만 모드 판정은 기존 칸으로 도출(`invoice_reconciled_at` 있음 AND `reconcile_invoiced_lines === 0`) → 「총액만 · +RM Z」 / 「대조 완료 · +RM Z」, 줄 차이 N 은 보조 텍스트. **DB 칸 추가 0 · 마이그 0.**
- **A-5** 원가 정책 무변경 — 총액 확정은 매장 원가·전파·소급·이름사전에 **무접촉**. 「갭」은 관측값이지 원가가 아니다.

**B. 배송비를 더해야 할 때의 경로**
- 지금도 길은 하나: 대조 화면 배송비 칸(발주 예상값 프리필) → 실제 청구 배송비로 고침 → 저장. 총액을 함께 적었으면 총액 = 줄합+세금+배송−할인 이어야 통과, 아니면 총액 확정 버튼.
- A 적용 후: 총액 + 배송비 두 칸만 적고 「이 총액으로 확정」 → `invoice_total`(낼 금액)·`invoice_delivery` 저장, 청구서 헤더 charges 에 배송비, 차액 줄로 총액 일치. 배송비 칸 밑 「발주 때 예상 X · 차이 +Y」 유지.
- ⛔ 발주 자체(`delivery_fee` 스냅샷·`total_amount`)에 배송비를 덧붙이는 「발주 수정」 경로는 **만들지 않는다.** 예상(발주) vs 실제(청구)를 나란히 남기는 것이 이 기능의 존재 이유(§2).

**C. 「AI 추가하면 해결돼?」**
- **총액 픽스는 AI 와 무관하다.** AI 가 있어도 사람이 숫자를 확정해야 하고(파싱 실수가 원가로 굳는 걸 막는 게 이 화면의 존재 이유), 손글씨·흐린 사진은 어떤 판독기든 틀린다. **A 가 먼저다.**
- 「텍스트를 제대로 못 불러온다」의 실체(§8-2 실측): ①인쇄 인보이스는 tesseract 가 19/19 줄을 읽고 약한 고리는 **이름 매칭**(사전으로 한 번 가르치면 다음부터 자동) ②**손글씨 인보이스(발주 32 Guan Kee)는 0줄** — 종류가 다른 입력.
- 비전 AI 는 ②손글씨와 ①첫 만남 이름 매칭을 **실제로 개선한다.** 그러나 09-10 D1(무료·브라우저·외부 전송 0·비용 0)을 뒤집는 결정이고, 인보이스 이미지가 외부로 나가며 건당 비용이 생긴다 — **Irene 결정 사안.**
- **Fable 권고**: 지금은 A 만. AI 는 「AI 로 읽기」 버튼 1개 · 매장 설정 opt-in · 조건 5개 전제로 시범 — ⓐ서버가 이미지 중계(키는 서버·인쇄 폴러 자원 무접촉) ⓑ결과는 tesseract 와 같은 자리(사람 확인 전 저장 0) ⓒtesseract 는 기본·무료 경로 유지, AI 는 실패 시에만 ⓓ건당 비용·호출 수를 매장별 기록 ⓔ실패율 실측 후 확대.
- **결정 기준**: 공급업체 중 손글씨·불량 사진 비율이 **절반을 넘으면** 붙일 가치가 있고, 아니면 사전 학습으로 충분. 운영 DB 로 못 재니(읽기 차단·OCR 결과 미저장) **Irene 이 공급업체 수로 답하는 것이 가장 빠르다.** 비용 수치는 착수 세션이 산정해 붙인다.

**D. 착수 전 실측 (코드 변경 0)**
1. Irene 이 「못 하고 있다」던 발주 번호 1개 — 그 모양(외부 공급업체·인보이스 첨부·총액 적었는지)을 개발 38 매장에 재현.
2. 개발 재현: 총액을 줄 합과 RM 5 다르게 → 노란 상자 버튼 보임 → 총액 확정 → 목록 배지가 「대조 완료」로만 나오고 갭 0 인지(예측: 그렇다 = A-4 근거).
3. 총액+배송비만 적고 총액 확정 → `invoice_delivery`·`invoice_total` 저장, 청구서 charges 에 배송비, 차액 줄, 결제 모달 낼 금액 = 적은 총액 (실호출).
4. 운영 09-24 권고 쿼리(「대조 시각 있음 AND 전 줄 단가 null」 발주 0건) — 운영 읽기 열리면 1회.
5. Irene 이 새로고침 후에도 버튼이 안 보이면 그때 ②옛 SW 캐시 원인 조사.

**E. 범위·게이트** — 규모 중(프론트 `InvoiceReconcilePage`·`PurchaseOrdersPage`·`PurchaseOrderDetailPage`·`ReceivePayModal` 라벨, i18n 4언어). 백엔드 0~1줄. 🔒 보호파일 무접촉. `check-sensitive-diff` 가 찍으면 **게이트 판정 1회(이 사안 2회차)**. 구현 중 되묻기 없이 팀원 판단.
**반증 의무**: 총액 확정에서 줄 값이 지워지는지(고장주입: null 화 제거 → 원가 오염 재현) · 헤더 갭 부호 · 총액만 배지 도출.
...
# 발주 ↔ 인보이스 원가 대조 (설계 확정 2026-09-08 · 미구현)

> Fable 설계 판정 2회 + Irene 컨펌 완료. 구현 착수 전 이 절이 단일 기준이다.
> 발단: Irene "레스토랑관리자나 스탭이 발주한 외부공급업체의 인보이스를 올리면 그 인보이스랑
> 실제 주문내용 매칭할 수 있어? … 이거 원가계산에 너무 중요해서. ai 안쓰고 지금 개발된 구조에서 가능해?"

## 0. 결론 — AI 없이 된다. 단 "사진 자동 판독"만 빼고

사진·스캔은 글자 인식 없이 못 읽는다. 그런데 그게 없어도 전부 성립한다 —
**사람이 숫자만 확정**하면 대조·강조·전파·이력은 결정적으로 계산된다.

**실측(2026-09-08 운영)**: 직원이 올린 인보이스 2장 모두 **이미지 전용 PDF**였다
(`이미지 1 · 글꼴 0 · 텍스트 블록 0 · JPEG`). 즉 현장의 기본형은 "종이를 찍은 사진"이다.
자동 추출 경로는 있어도 되지만 **주 경로가 아니다.**

## 1. 입력 3층

| 공급업체가 주는 형태 | 처리 | 타이핑 |
|---|---|---|
| 종이 → 사진 (**현장 기본형**) | 화면 좌: 사진 / 우: 발주 라인 기본값. **다른 줄만** 고침 | 다른 줄 수만큼 |
| 왓츠앱·메일 **글자** | 붙여넣기 칸 → 매칭기 | 없음 |
| 프로그램으로 뽑은 **글자 PDF** | 브라우저에서 텍스트 자동 추출(pdf.js, 결정적) | 없음 |

**매칭기 = 순수 함수 하나**(프론트, 서버 저장 없음). 줄 단위 분해 → 숫자 3개(수량·단가·금액)
정규식 추출, 나머지가 이름 → 발주 라인 `seller_product_name` 과 정규화 토큰 겹침 점수
→ 금액 = 수량×단가 정합이면 가점. 결과는 라인마다 **`매칭됨` / `확인 필요` / `미매칭`** 3상태.
⛔ **자동 매칭 결과는 저장되지 않는다.** 사람이 확인을 눌러야 `invoiced_*` 에 들어간다 —
파싱 실수가 원가로 굳는 것을 막는 유일한 자물쇠.

## 2. 저장 — 새 표는 `cost_change_logs` 하나뿐

기존 행에 칸 추가:
- `purchase_order_items`: `invoiced_unit_price`, `invoiced_quantity` (nullable, null = 미대조)
- `purchase_orders`: `external_invoice_uploaded_by_user_id`, `invoice_number`, `invoice_date`,
  `invoice_total`, `invoice_tax`, `invoice_delivery`, `invoice_discount`,
  `invoice_reconciled_at`, `invoice_reconciled_by_user_id`

⛔ 발주 스냅샷 `unit_price` 는 **절대 덮어쓰지 않는다.** "예상(발주) vs 실제(청구)" 두 값이
나란히 남는 것이 이 기능의 존재 이유다. 세금·배송을 라인 단가에 섞지 않는다(헤더로 분리).

**신규 모델 `cost_change_logs`**: `subject_type`(seller_product|ingredient|product_ingredient|po_item)
· `subject_id` · 구매자 `entity_type/entity_id` · 판매자 `seller_type/seller_entity_id`
· `old_value` · `new_value` · `unit` · `source`(invoice_reconcile|seller_edit|manual|backfill|retro_apply)
· `purchase_order_id`(근거 인보이스) · `batch_id`(소급 되돌리기) · `changed_by_user_id`
· `changed_by_name`(스냅샷) · `note` · `changed_at`.

쓰는 지점은 **`services/costSync.js` 안 한 곳** — `recomputeUnitCost` 가 UPDATE 하는 순간에
old/new 를 안다. 호출부마다 로그를 쓰지 않는다.

**`activity_logs` 를 쓰지 않는 이유 3가지**: ①`restaurant_id` FK 라 BG·푸드코트 구매자가 못 쓴다
②`changes` 가 JSON 이라 그래프·평균의 소스가 못 된다 ③"이 대상의 마지막 변경"을 1쿼리로 못 뽑는다.

**두 질문을 구분한다**: *시장 가격 변동*(얼마에 사 왔나)은 발주/청구 라인에서,
*우리 원가 기준 변경*(누가 언제 바꿨나)은 `cost_change_logs` 에서 읽는다.

## 3. 원가 전파 — 새 전파 코드를 쓰지 않는다

진입점은 **공급업체 판매상품가 하나**. 대조 화면에서 라인별로 "이 청구가로 판매상품가를 바꿀까?"
를 묻고, 바꾸면 `recomputeForSellerProduct('supplier', id)` 가 이미 있는 길로
매핑 → 재료/재고아이템 `unit_cost` → 거울 → 레시피까지 흐른다.

범위 규칙 하나: **누가 샀느냐 = 누구 원가가 바뀌느냐.**
매장 발주 → 그 매장 `ingredients` / BG 발주 → `product_ingredients` → 거울 → 브랜드 레시피.
매장 대조가 브랜드 재료를 바꾸는 경로는 **없다**(브랜드 재료는 매장 읽기전용).

**⚠ 매장 원가 오버레이(`restaurant_ingredient_costs`)는 범위 안이다** — 1차 설계에서 "범위 밖"
으로 뒀다가 실측에서 뒤집혔다. dev 12행이지만 **운영 73행 · 3개 매장**이고 거의 전부가
외부 공급업체 매핑을 갖는다. 전파가 오버레이를 덮을지 말지 구현 시 반드시 결정한다.

## 4. 과거 발주 소급 — 기본 "안 함"

과거 라인 `unit_price` 는 그 시점 합의가격이고 결제·마감 기대금액·SOA 에 이미 쓰였다.
소급하면 결제된 발주 금액이 바뀌어 원장이 깨진다.
→ 묻되 **기본 off**, 대상은 같은 판매자 상품의 **미결제·미수령 라인만**, 결제분 불변.
...
**외부 공급업체는 로그인도 결제 확인도 못 한다. 업로드된 인보이스가 원본이고 우리 청구서는 추정치다.**

자동 발행을 끄는 안은 **기각**(인보이스 화면에서 결제관리가 불가능해진다).
채택: **자동 발행 유지 + 외부 건은 "매입 미지급 기록"으로 취급.**

필요한 변경 4개:
1. 청구서 목록·상세에 **"외부 공급업체 — 업로드 인보이스가 원본"** 배지 + 파일 링크 + 인보이스 번호/일자
2. ⛔ **게이트웨이 결제 차단** — `Restaurant/InvoicesPage.tsx:1128` 의 `Pay Now` 가 발행자 구분 없이
   뜬다. 외부 발행자면 **"결제함 체크"** 만. 서버도 `create-payment-intent`/`create-paypal-order`
   에서 400(프론트 폭 ≤ 서버 폭).
   *실측: `getStripeForIssuer` 에 **플랫폼 폴백이 없다** — 키 없으면 throw. 운영 공급업체 10·15 는
   결제 설정이 없어 지금 눌러도 에러로 끝나고 우리 계정에서 돈이 나가지 않는다. 핫픽스 불요, P1 범위.*
3. 대조 확정 시 청구서 라인·총액·세금·배송·할인을 **업로드 값으로 덮어쓴다**(외부 건만).
   대조 전엔 "미대조 · 추정" 표시.
4. 미대조 상태에서도 체크는 허용하되 "미대조 금액으로 결제 처리됨" 경고 1줄(막지 않는다 —
   매장이 이미 현금으로 냈을 수 있다).

**운영 #127·#128(TRD-SUP15·TRD-SUP10, 2026-09-08 자동 발행)은 지우지 않는다.**
이 구조에 그대로 맞는 행이고, P1 이 나가면 직원이 이미 올린 PDF 2장으로 대조해 정정된다.

## 6. 가격 변화 표시 (발주 화면)

> Irene: "다음 발주할 때도 해당 아이템이 어떤 가격변화가 있었는데 보이게 … 올랐다 안올랐다 아이콘"

소스는 **발주 라인**(새 표 없음): 같은 구매자·같은 매핑의 과거 `received` 라인,
**실효가 = `COALESCE(invoiced_unit_price, unit_price)`**.
서버가 seller-sources/catalog 응답에 상품당 1건씩 붙인다(그룹 쿼리 1개, 라인마다 호출 금지):
`price_history { last_price, last_at, prev_price, trend, n, avg, min, max }`.
표시는 **기하 글리프** `▲ +8%` / `▼ -3%` / `—` (이모지·lucide 아님 — RA 디자인 표준).
기준은 *현재 판매상품가 vs 마지막 실효가* = "지금 발주하려는 가격이 지난번 실제로 낸 값보다 높은가".

## 7. 표시 동기화 ("여기저기 담당자·날짜")

단일 API `GET /api/cost-changes?subject_type=&subject_id=` (최신 1건 + 이력) 를 세 곳이 같이 읽는다:
재고/재료 상세 원가 칸 · 공급업체 상품 목록 가격 칸 · 레시피 원가 칸.
표시 문구: "마지막 변경 · 담당자 · 날짜 · 출처(인보이스 PO#)".

## 8. 절단 순서

| 단계 | 내용 | 상태 |
|---|---|---|
| **P0** | 외부 공급업체 가격 PUT → costSync 전파 결함 수정 + `uploaded_by` 기록 | ✅ **운영 배포 완료 (2026-09-08 · v3.86)** |
| **P1** | 위 1~7 전부 | ✅ **운영 배포 완료 (2026-09-08 · SW 4.91·4.95)** |
| **P2** | 구매·원가 리포트(평균·±폭·그래프). RA Reports "구매·원가" 탭 신설 | ✅ **운영 배포 완료 (2026-09-08 · SW 4.97)** |

### 구현 결과 (2026-09-08)

전부 하루에 나갔다. 구현하면서 **설계에 없던 것 세 가지를 정해야 했다** — 아래는 그 결정과 근거다.
⚠ 이날 배포는 **Fable 판정 없이** 나갔다(한도 초과 429, 2회 시도). 사후 검토 문서 `docs/FABLE_REVIEW_2026-09-08.md`.

1. **판매가를 고칠 수 있는 범위** — 설계 §3 은 "대조 화면에서 판매상품가를 바꾼다"까지만 정했다.
   실제로는 **내가 등록한 외부 공급업체 상품만** 허용해야 했다. 가입 공급업체 상품가는 그쪽 소유이고,
   dev 실측에서 한 상품에 매핑이 4개 걸려 있어 **한 매장의 대조가 다른 세 곳 원가를 움직였다**(발주 18건·라인 56건 도달).
2. **이미 결제된 청구서** — §5-3 은 "대조 확정 시 청구서를 업로드 값으로 덮어쓴다"만 말한다.
   결제된 청구서까지 덮으면 낸 금액과 총액이 갈리고 SOA·정산이 그 위에 있다. → **미결제 건만** 고치고 사유를 돌려준다.
...
③`issued_at` 은 소급하지 않는다(지난달 SOA 가 흔들린다).

**P0 결함 상세**: 같은 `SupplierProduct.update` 두 경로 중
`routes/supplier-directory.js:1204`(매장이 외부 공급업체 상품 수정)에만 `costSync` 호출이 없다.
`routes/supplier-products.js:815~823`(가입 공급업체 자기 상품 수정)에는 있다.
→ 외부 공급업체 가격을 고쳐도 재료 원가가 안 따라간다. 이 설계 전체가 이 한 줄 위에 선다.
수정은 815 라인 패턴 그대로(커밋 뒤 호출 · try/catch · 실패는 로그) + dev 실호출 증명.

**원가 정책은 안 바뀐다** — `reference_cost_two_paths` 유지. 원가는 여전히 **공급업체 현재가 1개**.
"평균 ±폭·그래프"는 **리포트(관측)** 이지 원가 결정 정책이 아니다. 평균을 원가로 쓰는 분기는 만들지 않는다.
