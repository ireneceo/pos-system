...
#### 재료 vs 재고 아이템 — Irene 구분 (2026-09-01)

> Irene 원문: *"브랜드 제너럴 <<< 자기 재고 관리. 그리고 브랜드 레시피에 연결되는 재료가 해당 브랜드의
> 재고가 되는 것은 별개의 문제야. 그렇게 브랜드 재료를 가져가서 레시피 연결해서 재고가 되는 건
> 재고 아이템인거잖아."*

| 부르는 이름 | 무엇 | 테이블 | 실측 건수 | 수량>0 |
|---|---|---|---|---|
| **브랜드 재료** | 레시피에 연결되는 것 (K-Gochujang·Korean Leek·Chicken Breast …) | `ingredients` (`brand_id=2`, 매장 없음) | **146** | **0건** |
| **재고 아이템** | 실제 재고로 관리되는 것. **발주 입고가 여기 쌓인다** (포장재 등) | `product_ingredients` (`owner_user_id=23`) | **288** | **23건** |
| 매장 재고 | 각 매장 자기 것 | `ingredients` (`restaurant_id`) | 매장별 | — |

- **브랜드 레시피(Recipe)** → `ingredients`(브랜드 재료) 를 쓴다 *(실측: K-Bulgogi·Korean Leek 등)*
- **프로덕트 레시피(ProductRecipe)** → `product_ingredients`(재고 아이템) 를 쓴다 *(실측: AUS BRISKET 등)*
- **같은 물건이 양쪽에 63건 겹쳐 있다** (이름 일치 기준)

> ⛔ "브랜드 재고"라는 말은 쓰지 않는다. **재고는 각자 관리**(Irene). 위 두 목록은 GIT 이 가진 것이고,
> 매장 재고는 매장 것이다.

**⇒ 별개의 문제 두 개**
1. **(지금 할 일)** 판매상품 → **재고 아이템**(`product_ingredients`) 연결. 팔면 여기서 빠져야 한다.
2. **(별개)** 브랜드 재료를 가져다 레시피에 연결해 **재고 아이템이 되는 경로**.
   지금은 목록이 갈라져 브랜드 재료 146건이 전부 수량 0 이다 → [[project_brand_stock_two_lists_split]]
...
- ⇒ **GIT 이 팔아도 재고 아이템(투명컵/뚜껑 18 pack)은 영원히 안 줄어든다.**
