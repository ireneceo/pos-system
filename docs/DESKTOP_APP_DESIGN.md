...
## 🔴 운영 실태 (2026-07-27 Fable 검증 — 문서 주장보다 이게 우선한다)

**현재 이 앱을 실제로 쓰는 운영 매장은 0개다.**

- 운영 `print_device_status` 전수 17행 = `web-rawbt 8 / web 5 / web-qz 4`, **`windows-app` 0행**
  (v3.69 자가진단이 로그인한 모든 기기를 주기 리포트하므로 표본 누락이 아니다).
- `detectPlatform()`(`utils/printDiagnostics.ts:64`)은 앱 안이면 `window.__NATIVE_PRINT` 브릿지로
  **반드시** `windows-app` 을 찍는다 → 0행은 측정 착오가 아니라 **아무도 안 켰다**는 뜻.
- 파일럿이던 with MIN(rid=10)조차 현재 `platform=web, printer_mode=browser` 로 리포트(최근 2026-07-27).
- 네이티브 인쇄 텔레메트리 `[print-trace] CLIENT` 는 **2026-07-01 이후 0건**.

**따라서 아래 문서의 "운영 배포 완료 / 검증 완료" 는 "빌드·호스팅·피드·CTA 까지 완료" 로 읽어야 한다.**
호스팅 정합(exe·`latest.yml` sha512·크기·별칭 바이트)은 실측 확인됨. 반면 **0.1.9 graphic 빌 종이 확인,
야간 자동업데이트 실작동(0.1.7→0.1.9), Floor Plan 헤더 눈 확인은 전부 미실증**(2026-07-15 "내일 확인" 이후 기록 없음).

**다음 할 일 1순위 = with MIN 이 왜 앱을 껐는지 확인.** (불편해서 브라우저로 회귀? 그냥 안 켬?)
그 답이 이 앱의 진짜 완료 여부와 나머지 항목의 우선순위를 결정한다.
**Irene 결정 대기**: exe 코드서명 인증서(미서명 → SmartScreen 경고, 신규 매장 설치 최대 마찰).
