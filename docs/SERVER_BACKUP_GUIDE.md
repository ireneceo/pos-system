...
| 무엇 | 언제 | 저장 위치 | 보관 |
|---|---|---|---|
| 운영 백엔드·프론트 빌드 | **매 배포** | `/var/www/backups/<YYYYmmdd_HHMMSS>/` | **7일 + 최신 10개** |
...
- **7일 초과분 삭제**, 단 **최신 10개는 나이와 무관하게 보존**(배포가 뜸해도 롤백 대상이 남게).
- 환경변수로 조절: `BACKUP_KEEP_DAYS`(기본 **7**) · `BACKUP_KEEP_MIN`(기본 10).

**🔴 2026-09-24 — 30일 → 7일로 줄였다.** 운영이 디스크 81% 경고를 보냈고 `/var/www/backups` 가
**148G(배포 백업 147개)** 였다. **규칙은 정상 작동했고 «지울 게 없었을» 뿐이다** — 9월 한 달에
**121번 배포**해 그 전부가 30일 이내였다. 하루 3~6배포인 곳에서 30일치를 통째로 들고 있을 이유가 없고,
롤백은 사실상 직전 것을 쓴다. 최신 10개 보존은 그대로라 배포가 뜸해도 롤백 대상은 남는다.
1회 정리 결과: **113개 삭제 · 112GB 확보 · 디스크 81% → 32%**(사람이 만든 13개 폴더는 무접촉).
⚠ 2026년 1월 백업 1개(137MB)는 파일 소유자가 달라 권한 거부로 남았다 — `sudo rm -rf` 필요.
되돌리려면 `deploy-to-production.sh` 의 `BACKUP_KEEP_DAYS` 값만 30 으로.
- ⚠ **2026-09-07 수정** — 이 "최신 10개 보존"이 **실제로는 0개를 보호하고 있었다.** 보호 목록을 `head -N` 결과(줄바꿈 구분) 그대로 `case " $KEEP " in *" $d "*` 로 비교해서 어느 항목도 일치하지 않았다. 시뮬레이션으로 재현(옛 방식 보호 0건 / 새 방식 10건)하고 `| tr '\\n' ' '` 를 붙여 고쳤다. 30일 넘은 백업이 마침 적어서 사고로 드러나지 않았을 뿐이다.
