...
## 8.8 🔒 통합티켓(Consolidated Order Ticket) — 단일 진실 (2026-06-29 Irene 확정, 매장 무관 통합수정)

> 통합티켓 = **카운터 사본**. 주방 스테이션 티켓과 별개. 하루 종일 디버깅 끝에 확정 — 재논의/되돌리기 금지.

### 핵심 4규칙
1. **발행 횟수 = 1 동작당 정확히 1장.** 회차(order_group)가 여러 개여도 통합은 **합쳐서 1장**(스테이션만 회차별로 쪼개 발행). 구현 = 하이브리드 배치 루프에서 **통합은 `bi===0` 에서만 발행, 나머지 회차는 `__consolidatedClaim:{skip:true}` 로 억제**(`hybridKitchenPrint.ts`).
2. **내용(무엇이 찍히나):**
   - **신규주문** = 주문 전체 품목.
   - **추가주문(+Round)** = **추가한 주문내역 전체**(이번에 추가한 품목 전부, **주문 전체 아님**). 기존(이미 찍힌) 품목 제외.
   - **테이블이동·주문취소·아이템취소(재발행)** = **주문 전체**(서브 포함). 취소는 취소줄(voided), 아이템취소는 해당 품목만 줄긋기.
   - 구현 = `_fullOrderItems` 가 통합 내용을 고정: 재발행이면 전체주문, 신규/추가면 `kitchenItemsRaw`(이번에 안 찍힌 전체). 회차 쪼개짐과 무관하게 전체가 통합 1장에 나오게.
3. **중복 0 = ATOMIC claim을 "발송 직전"에.** `PATCH /api/consolidated-print/:id/claim` 가 `consolidated_printed_at` NULL→now 를 **1명만 win**(`consolidated-print.js`). win한 패스만 발송, lose는 skip. **claim과 발송이 한 몸**이라 claimed=true 인데 발송 안 되는 **orphan 0**.
   - ⛔ **"미리 선점(early-stamp/preStamped)" 절대 금지** — 2026-06-29 사고: 하이브리드가 통합을 미리 claim 해두고 발송이 끊기면 통합이 영영 안 나옴(claimed=true·CLIENT 발송 없음). 폴러는 이미 선점됐다고 막혀 못 찍음. **claim은 반드시 sendUnifiedTickets 발송 시점에.**
   - 새 회차(+Round/이동/취소/아이템취소)는 백엔드가 `consolidated_printed_at=null` 리셋(merge 466 / move 1289 / cancel 1625 / void 2581) → 다음 회차 정상 재발행.
4. **하이브리드(POS1 다이렉트) vs 폴러(클라우드) = 같은 claim 공유.** POS1은 자기 주문을 스테이션+통합 다이렉트로, 폴러는 다른 기기(모바일/노트북) 주문을. 둘 다 위 ATOMIC claim 으로 한쪽만 발송 → 경로 무관 중복 0. **모든 매장 동일**(특정 매장 분기 없음 — 주문 데이터/설정 기준).

### 통합 vs 스테이션 (혼동 금지)
- **스테이션(주방)** = 회차별 + **서브된 것 제외**(`status!=='served' && !=='completed'`, move 1236·1265 / cancel 1617 / void 2587). 이미 나간 건 다시 안 만들게.
- **통합(카운터)** = 위 2번 내용 규칙. 전체주문 재발행(이동/취소)엔 서브 포함.

### 절대 하지 말 것 (2026-06-29 사고 교훈)
- 통합 early-stamp/preStamped (orphan).
- stale 재무장 창(10초)을 source/매장별로 늘리기 — 이동·취소·머지가 재무장 경유 인쇄라 **다 막혀 안 나옴**(되돌림). 느린 BAR 재무장 폭주는 **하트비트**로만 막는다(창 유지).
- 통합을 회차(배치)마다 발행 — N장 나옴. `bi===0` 만.

상세 디버깅 히스토리·진단법 = `docs/PRINT_DEBUG_HISTORY_2026-06-29.md`, 메모리 [[reference_consolidated_ticket_unified_2026_06_29]].
...
## 🔒 pending-print 창 신선도 규칙 (2026-07-23 도입 → 2026-07-24 기준 교정, Irene 확정)

> `GET /orders/restaurant/:rid/pending-print` = 자동인쇄 폴러가 읽는 **서버 큐 창**. 이 창의 **모집단 규칙**만 다룬다 — 인쇄 방식·라우팅·타이밍·주체는 이 규칙과 무관(무변경).

### 왜 신선도 경계가 필요했나 (2026-07-23)
서버는 주문 생성 시 프린터 설정과 무관하게 `needs_print=true` 를 찍는데, 이 플래그를 **지우는 주체는 자동인쇄 폴러가 도는 기기뿐**(`/printed`·`print-claim`·`print-dismiss`). 그래서 **자동인쇄를 안 켠 매장은 플래그가 영영 안 지워지고 무한 누적**된다(운영 실측 2026-07-23: K-DINE IPC 1,616건, The Fire 86, with MIN 65). 창은 **oldest-20** 이므로 옛 행이 20칸을 영구 점유하면 **정말 새 주문이 창 밖으로 밀려 자동인쇄가 통째로 멈춘다**(= 2026-06-22 `print-dismiss` 를 만든 그 사고의 재발). `dismiss` 는 폴러가 돌 때만 작동해 **폴러가 아예 없는 매장에서는 방어가 안 된다** → 서버가 "창은 항상 신선한 것만 담는다"를 보장한다.

### 판정 기준 = "인쇄 필요가 발생한 시각" (2026-07-24 교정)
최초 구현은 주문의 `createdAt` 을 기준으로 삼았는데, 이는 **틀린 기준**이었다. 판정해야 할 대상은 주문이 태어난 시각이 아니라 **인쇄 필요가 발생한 시각**이다.
- **결함(실증)**: 24h 넘게 열려 있던 테이블에 +Round 를 넣으면 `needs_print` 가 "지금" 켜지는데도 창에서 탈락 → **그 라운드 주방티켓이 무음 유실**.
- **교정**: 컬럼 `orders.print_needed_at` 신설, 창 판정 = **`COALESCE(print_needed_at, createdAt) >= now - 24h`**. 기존 행(NULL)은 `createdAt` 폴백이라 2026-07-23 동작과 동일 → **배포 즉시 동작 변화 0**. 백필하지 않는 것이 설계(마이그↔코드 배포 갭에 생긴 NULL 행까지 폴백이 봉인).

### 스탬프 규칙 (⛔ 재시도에는 절대 찍지 않는다)
| 구분 | 대상 | `print_needed_at` |
|---|---|---|
| **새 인쇄 필요** | 주문 생성 · 라운드/머지(`mergeItemsIntoOrder` = add-items·merge-items·생성머지·자동머지·테이블머지 전부) · 테이블이동 · 주문취소 · 아이템 void · 모바일 생성/자동머지 | **NOW 스탬프** |
| **재시도·정리** | 죽은-claim 자동복구 · `/print-rearm` · `/print-claim` · `/print-dismiss` · `/printed` | **⛔ 스탬프 금지** |
| **오프라인 재생** | `printedOffline` (이미 로컬 인쇄됨) | 기존 값 보존 (생성 시 null) |

**왜 재시도에 찍으면 안 되나**: 죽은-claim 복구 UPDATE 는 **나이 상한 없이** 재무장한다. 여기에 스탬프하면 인쇄가 고장난 매장에서 `claim ↔ re-arm` 핑퐁이 옛 행을 **영원히 신선**하게 만들어 위 누적 방어가 통째로 붕괴한다. (구조적 안전판: claim 은 폴러가 **창에서 본** 주문에만 발생하므로, 창 밖 옛 행은 핑퐁 자체가 성립하지 않는다 — Fable 3사이클 시뮬 실증.)

### 경계 면제 — 재발행(취소/이동/void 안내)
`pending_reprint IS NOT NULL` 분기에는 **신선도 경계를 절대 걸지 않는다.** 어제 들어온 주문을 "지금" 취소/이동/void 한 것은 직원의 **현재 동작**이라 반드시 인쇄돼야 한다(주문 생성일이 오래됐다는 이유로 취소표를 버리면 **2026-06-24 취소표 분실 사고가 그대로 재발**). 재발행은 1회 인쇄 후 `pending_reprint=null` 로 정리되므로 창을 잠식하지 않는다.

### 영향 없는 것
**수동 인쇄(Kitchen Ticket 버튼)는 `order_items` 로 찍으므로 `needs_print` 와 무관** = 오래된 주문도 언제든 그대로 인쇄 가능(영향 0).

### 회귀 안전망 (health-check `print` 카테고리에 박제)
1. 25시간 지난 미인쇄 주문 → 창 **제외** (누적 방어)
2. 25시간 열린 주문의 **+Round → 창 포함** (라운드 티켓 유실 방지, 실 API `add-items` 경유)
3. 오래된 **재발행(취소표) → 창 포함** (2026-06-24 계약)
4. 재시도 경로(죽은-claim 복구) → **스탬프 0 + 창 밖 유지** (부활 없음)

되돌리면 정확히 위 테스트가 실패한다 = 회귀 자동 감지.
