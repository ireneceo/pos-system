...
## 준비된 재고 (1차 가공 재고) — 🟡 Fable 설계 판정 대기 (2026-09-12 접수)

> **상태: 안건만 접수. 설계·결정 없음. 구현 착수 금지.**
> 이 절에는 **Irene 원문**과 **팀원이 실측한 사실**만 적는다. 어느 길로 갈지는 Fable 판정 뒤에 이 절을 고쳐 쓴다.
...
### 5. 아직 안 본 것 (Fable 판정 전 팀원이 모을 수 있는 사실)

- `services/costSync.js` 가 중간 단계를 만났을 때의 전파 범위
- 운영 DB 에 「불고기 양념육」처럼 **이미 재료로 등록된 1차 가공품**이 몇 건인지 (있으면 그게 오늘의 우회 방식이고, 이행 대상이 된다)
