...
## 준비 레시피 삭제 게이트 + 카드 표시 (2026-09-18, v3.92·v3.96 배포)

### 삭제는 스위치 OFF 와 같은 규칙을 따른다
- 예전엔 **삭제가 OFF 보다 약한 방어**였다 — OFF 는 「재고가 남았거나 쓰는 레시피가 있으면」 409 로 막는데, 삭제는 그냥 지워 **준비 재료 행이 남았다**(개발서버 재현 확인).
- 이제 `routes/recipes.js deleteRecipeWithPrepGate` 가 한 트랜잭션에서 **disable → 메뉴 연결 해제 → 삭제** 순서로 돈다. 거부 시 409 `PREP_INGREDIENT_IN_USE` 와 사유(재고 몇 남음 / 쓰는 레시피 목록)를 화면 알림으로 보여준다.
- 사실 관계: `recipes` 는 **soft delete 가 아니다**(paranoid 아님·`deleted_at` 없음). FK `fk_ingredients_source_recipe`(ON DELETE SET NULL)가 실제로 발동해, 삭제 뒤 준비 재료는 **비활성 + 출처 없음**이 된다 — 그것이 설계된 최종 상태다(재고 0·배치·장부 보존).
- SQL 로 레시피를 직접 지우는 우회는 브랜드 행이면 **ING-UNI-002** 가 잡는다(아래). 코드 경로 회귀는 health-check 「준비 레시피 삭제 게이트」 계약 3건이 받친다.

### ING-UNI-002 확장 — 출처는 **셋**이다
「브랜드 재료는 출처를 가짐」 검사가 출처를 둘(Stock Item·프로덕트)만 인정해 **정상 준비 재료를 「출처 없음」으로 세고 있었다.** 운영 재료 #1132(brand 2, 09-18 생성)가 그 경우여서 **배포가 이 검사에서 막힐 상태**였다. `source_recipe_id` 를 셋째 출처로 인정해 바로잡았고, 덤으로 우회 삭제(출처 셋 다 NULL·활성)를 이 검사가 잡는다. 한계: 매장 소유 행은 수기 재료가 정상적으로 출처가 없어 같은 방식으로 구분할 수 없다.

### 레시피 카드 표시 규칙 (Irene 신고에서 나옴)
- Irene 「이거 다 브랜드레시피인데 왜 어떤 건 표시되고 어떤 건 표시 안되고 있어?」
- **원가가 두 줄(Brand Cost ↔ My Cost)로 나오는 조건은 하나다** — `isRestaurantAdmin && owner_type==='brand' && restaurant_ingredient_cost !== null`, 즉 **그 매장이 재료 원가를 따로 정해 둔 레시피**. 운영 실측: 매장 8 준비 레시피 7개 중 `Prepared Beef Brisket` 1건만 오버라이드가 있었다.
- 고장이 아니라 **근거가 안 보였던 것**이라 같은 조건으로 **「우리 원가」 배지**를 붙였다(판정을 새로 만들지 않고 원가 블록과 같은 조건 재사용).
- **준비 레시피는 Suggested(권장 판매가) 칸을 숨긴다** — 메뉴에 붙지 않는 «파는 물건이 아닌» 레시피인데 전부 `RM 0.00` 으로 떠 값을 안 넣은 것처럼 읽혔다.
- **재료 0줄 레시피에는 경고**를 띄운다 — 「재료가 없어 원가가 0이고, 팔려도 재고가 줄지 않습니다」(운영 3건: 비빔밥 재료 · 반찬미역국 · S-Seaweed Soup).
