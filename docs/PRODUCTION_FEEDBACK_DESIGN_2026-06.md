...
# Round 2 — IOI Mall Food Court 신규 피드백 (2026-06-21)

> 출처: 운영 support_tickets (IOI Mall Food Court, 6/21). Round 1(6/18~19) 6건은 이번 사이클에 구현·검증 완료(통화/QZ/취소사유/PIN/시재/예약). 아래는 6/21 신규. ⑤ 오프라인은 별도 자문으로 분리.

| # | 항목 | 유형 | 규모 | 처리 |
|---|------|------|------|------|
| ① | 발주관리 추가요청(BG=레스토랑 동일구조 + 오너 승인) | 기능 | 중 | PURCHASE_ORDER_SYSTEM §G |
| ② | base64 sweep 메일 매장 인박스 노이즈 | 위생 | 소 | 본 문서 R2-② |
| ③ | 메일 발신전용 표시 | 기능 | 중 | 본 문서 R2-③ |
| ④ | 문의 리스트 텍스트 truncate | UI | 소 | 본 문서 R2-④ |
| ⑤ | 오프라인 복원력 sync 아키텍처 | 자문 | 대 | **별도 분리(미착수)** |

## R2-② base64 sweep 메일 매장 인박스 차단
**문제:** 개발용 주간 "Base64 inline images detected" sweep 알림이 매장 support 인박스로 노출("이게 뭐냐").
**설계:** QZ 진단 노이즈(§2) 동일 패턴. sweep 알림은 System Admin 채널 전용 — 매장 support_tickets 인박스 조회에서 제외(category 또는 source 격리). 매장에 메일/티켓 미생성. 기존 노이즈는 운영 정리(Irene 승인+백업).

## R2-③ 메일 발신전용 표시
**요청:** 시스템 발송 모든 이메일에 "발신전용·수신불가" 안내. 유저 설정 메일서버 발송분 정책 문의 포함.
**설계:** 공통 `emailLayout(bodyContent)` 푸터에 발신전용 안내 1곳 추가(4언어 locales). 모든 플랫폼 메일 자동 적용. 유저가 자기 메일서버로 보내는 고객대상 메일도 동일 푸터 옵션(향후) — 1차는 플랫폼 발신 메일 전체.

## R2-④ 문의 리스트 truncate
**요청:** System Inquiry 리스트에 본문 전체 말고 적정 길이로 잘라 표시(엔터 가독성 저하). 상세는 클릭.
**설계:** 인박스/문의 리스트 본문 미리보기 `truncate`(단일 라인, 말줄임) + 개행 제거. 상세 모달/페이지에서 전체. 통일 스타일.
