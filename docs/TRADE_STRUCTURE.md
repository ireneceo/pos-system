...
②판매자 «배송비 기준»(기준선 금액 + 고정 배송비 두 값)을 어디에 둘지 — `SupplierCompany` 확장 / 계약(Contract·SupplierContract) / **브랜드·푸드코트까지 포함하는 판매자 공통 자리**.
  ⚠ 브랜드가 판매자인 실거래가 8건 있는데 브랜드에는 칸이 없다. 새 자리를 만드는 것은 「기존 개념에 새 목록·새 표를 추가 금지」 규칙에 걸리므로 **구조 문서를 먼저 고치고 판정**해야 한다.
