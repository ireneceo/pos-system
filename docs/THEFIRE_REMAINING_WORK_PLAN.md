...
## ★ 현재 최우선 (2026-06-27, Irene "너무 중요") — 네트워크 회복력
> thefire 고질문제의 뿌리 = 인터넷 불안정(§0). 매장이 **동글/핫스팟**으로 인터넷 쓰는 상황까지 포함해 "끊겨도 영업이 안 멈추는" 회복력을 완성한다. 아래 A·B 는 §0·§9 의 발전형(같은 네트워크 테마). 인쇄 보호규칙 유지(둘 다 인쇄 무관 = 표시/안내/주문생성 안전망, billPrint 무접촉).

### A. 핫스팟/동글 사용 안내 + 연결상태 가시화 (신규)
- **배경:** 매장이 유선 대신(또는 백업으로) USB 4G/5G 동글·휴대폰 핫스팟으로 인터넷 사용. 직원이 "지금 끊겼는지 / 동글로 돌고 있는지" 모름 → 주문·인쇄 문제를 솔루션 탓으로 오해.
- **A-1 안내(문서/화면):** 동글 적용 시 셋업·운영 가이드 — POS를 핫스팟에 연결하는 법, 끊겼을 때 대처, **데이터 소모 주의(메뉴 이미지 등 계량요금)**, 권장 동글/요금제. 매장 배포용 1장 가이드 + 앱 내 도움말.
- **A-2 연결상태 표시(기능):** POS/모바일 상단에 온라인/오프라인 배지 — 끊기면 "오프라인 — 주문은 저장 후 자동전송" 안내. `navigator.onLine` + 서버 헬스 핑. 배너 패턴 재사용([[reference_session_state_stale_banner]] 식). 인쇄가시성 #7(미인쇄 표시)과 시각 일관.

### B. 오프라인 주문 무손실 — 보완·완성 (§9 발전)
- **현황:** `utils/offlineOrderQueue.ts` 로 **주문 생성(POST /orders)** 만 큐잉+멱등(idempotency_key)+재연결 flush 구현·배포됨(#9, OrderContext/PaymentPage/MobileApp 연결).
- **보완 목표("끊겨도 문제 없는" 수준):**
  - [ ] **명확한 피드백:** 오프라인 큐잉 시 "주문 저장됨 — 연결되면 자동 전송" 토스트/배지(현재 조용히 큐잉되면 직원이 확신 못 함). A-2 와 묶음.
  - [ ] **생성 외 동작:** 추가주문(add-items)·취소·결제가 오프라인 중일 때 처리(현재 생성만 보호). 최소 "오프라인이라 지금 불가, 연결 후" 명확 차단 또는 큐.
  - [ ] **메뉴 캐시:** 동글/와이파이 끊겨도 메뉴 로드되게(SW 캐시 실측 필요) — 안 그러면 끊긴 순간 주문 입력 자체 불가.
  - [ ] **인쇄 연동:** 오프라인 큐 주문은 재전송·서버도달 후에야 needs_print→인쇄. 이 지연을 직원이 알게(A-2 배지 + #7 인쇄가시성 연결).
  - [ ] **flush 신뢰성:** online 리스너 외 주기 재시도 + 실패 backoff + 큐 가시화("전송 대기 N건").
  - [ ] **검증:** 오프라인→주문→온라인 복귀 전 사이클 실측(데모, 안전계약). 중복0·분실0.

---

## 완료 현황 (2026-06-27 갱신)
- **§1 하이브리드 인쇄** 1-A/1-B ✅ 출시(v4.16/4.17). 1-C BAR·1-D +Round = 매장 물리확인 대기(아래 원문).
- **§2 부분수량취소 ✅ / §3 합본빌 ✅ / §4 머지 served ✅ / §5 stale-write OCC ✅ / §6 스테이션매수 ✅ / §7 설정 로드-resilience ✅ / §10 FloorPlan UI ✅** — 전부 운영배포(2026-06-26). 실프린터 종이확인만 Irene 잔여.
- **§8 결제후 빌 즉시화 ⬜** — thefire 는 빌POS 에서만 결제 → 현재 불필요(다른기기 결제 시).
- **§9 오프라인 큐 = 주문생성만 ✅ → 위 §B 로 보완 진행.**
- **§11** 리마크 ✅ / 크로스셀 ✅(v3.63) / takeout뷰 = 측정→수정불필요.
- (별건) **브랜드메뉴 가맹점 동기화** ✅ (2026-06-27 thefire 3매장 옵션·세트 동일화, 세트전파 영구수정 /배포 대기).

---

## 0. 매장 측 (코드 아님 — 간헐오류의 뿌리) — ★ 위 네트워크 회복력 A 와 한 묶음
- **POS1·POS2 인터넷을 유선 이더넷으로**(프린터는 이미 유선 LAN ✓). POS↔서버가 와이파이라 끊김 → 자동인쇄 지연·Serve 되돌림·설정로드 실패·429 다 여기서 옴.
- **동글/핫스팟을 쓸 경우** = 위 §A 안내·연결상태 가시화로 보완(유선이 불가한 매장 현실 대응).
