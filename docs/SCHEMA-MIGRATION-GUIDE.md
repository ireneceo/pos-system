...
### 🔴 2026-08-30 — 위 "하드코딩 금지"를 안 지켜 3배포 연속 소거가 났다

위 규칙은 2026-08-24에 이미 적혀 있었는데 **기존 마이그 2개가 지키지 않고 있었다.** 결과:

`migrate-po-status-pending-approval.js` 가 매 배포 정상 실행돼 값을 넣고 자기검증까지 통과했는데,
**29초 뒤 같은 배포의 `sprint6-migration.js` 가 9개짜리 하드코딩 목록으로 통째 교체하며 그 값을 지웠다.**
레지스트리 실행 순서가 **파일명 정렬**이라 `sprint6` 이 항상 뒤에 온다. `sprint7-migration.js` 도 같은
하드코딩이었고 가드 덕에 발현만 안 했을 뿐 같은 지뢰였다.
→ 배포 로그는 `[OK] completed` 였고, 마이그·레지스트리·배포 루프는 **전부 정상**이었다. 결함은 목록 하드코딩 하나였다.

**그래서 규칙을 코드와 게이트로 옮겼다 (문서만으로는 안 지켜졌다):**
1. **공용 유틸 `scripts/lib/enumExpand.js`** — 이걸 쓰면 하드코딩이 불가능하다.
   ```js
...
2. **배포 게이트 `scripts/check-enum-parity.js`** — dev ENUM 값이 운영에 없으면 **배포를 차단**한다
   (다른 타입 차이는 WARN 유지). 예전엔 이 자리가 갭을 정확히 감지하고도 `usually harmless` 로 3회 넘겼다.
   검사기가 ENUM 컬럼을 0개 검사하면 통과가 아니라 `exit 2`(검사기 고장)로 떨어진다.
3. `CLAUDE.md` "DB 스키마 변경" 체크리스트에도 같은 원칙을 박았다.

**기존 마이그를 수정할 때 `MODIFY COLUMN ... ENUM(` 하드코딩이 보이면 그 자체가 결함 후보다.**
가드(`if (!includes(...))`)가 있어도 안전하지 않다 — 가드가 걸리는 순간 남의 값을 지운다.
예외는 그 컬럼의 **전체 값이 하드코딩 목록의 부분집합임이 실측된 경우뿐**.
