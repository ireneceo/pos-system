...
## Claude Code·Codex 공동 작업 규칙

사용자는 목표만 말하고 담당자를 지정하지 않는다. 각 도구가 요청을 자동 분류한다. Codex는 단순 작업이면 수행·자체 검증하고, 복잡한 작업이면 담당자 지정을 요구하지 않고 Claude Code의 Fable·Opus 흐름이 필요하다고 안내한다.

- Fable은 기존 코드 조사·개발기획·설계·복잡한 검증을 담당하고, Opus는 승인된 설계를 구현한다.
- Codex는 보조 개발자로서 명확한 단순·반복 작업, 장시간 명령 실행, 자기 변경의 검증만 수행한다.
- Codex는 요구사항을 기획하거나 구조를 결정하지 않는다. 모호하거나 선택지가 있으면 작업을 멈추고 사용자에게 구체 지시를 요청한다.
- Claude Code와 Codex는 서로 자동 위임하지 않는다. 사용자가 현재 선택한 도구의 작업 흐름만 따른다.
- 작업 기록은 `[Claude Code]`, `[Codex]`, `[공동 검토]`로 실제 주체를 구분한다.
...
- **스크립트마다 실행 위치(cwd)가 다르다** — 게이트·시딩·health-check = `dev-backend`, playwright·i18n·build = `dev-frontend`.
  cwd 오류는 "실패"가 아니라 **"실행 무효"**로 나타난다(명령이 아예 안 돌고 출력만 빈다) → **출력이 비었으면
  결과를 읽지 말고 위치부터 의심**할 것. 실측(2026-08-20): 한 세션에서 Opus 2회·Fable 3회 밟았고,
  그중 한 번은 부여가 실행되지 않은 상태로 "생존 확인"을 돌릴 뻔했다.
- **`verify-all` 을 빌드·e2e 와 동시에 돌리지 말 것** — 실측(2026-08-20): e2e 브라우저와 겹쳐
  `print-route-guard` 가 300초를 넘겨 **거짓 실패**했고(단독 실행은 34/34 통과), 그 사이 빌드는
  메모리 게이트에 막혀 **옛 번들로 e2e 를 돌리는** 2차 사고까지 났다. 순차로 돌리고,
  **빌드 종료코드를 확인하지 않은 e2e 결과는 무효**로 취급한다.
