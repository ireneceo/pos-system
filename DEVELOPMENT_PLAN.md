...
> **최종 업데이트:** 2026-09-02 (**v3.80 유지 · 운영 배포 6회**). **발주·재고 정합 P1·P2 운영 반영 + GIT 포장재 합치기**. Irene 질문 *"위드민이 주문했는데 GIT은 언제 재고가 빠져?"* 의 답을 양쪽에서 닫았다 — **사는 쪽**(P1: 추적 스위치 폐기 + 입고가 프로덕트로 들어오는 경로 신설, 갈라져 있던 GIT 포장재 4쌍을 프로덕트로 합침)과 **파는 쪽**(P2: 출고를 `shipped_at` 1회 규칙으로 바꾸고, 구매자가 먼저 받으면 판매자에게 알림 — 운영 수령 8건이 전부 출고 없이 수령돼 판매자 재고가 한 번도 안 빠지고 있었다). P3(등록 입구) 절반은 개발서버. 게이트 훅 자체 결함 2건도 수리(클린 트리 지문이 커밋을 구분 못 해 P1 마커가 P2 에 유효로 뜨던 것 / 게이트 스크립트가 바이너리라 diff 리뷰 불가). ⚠ 검증 과정에서 **내 테스트가 두 번 거짓말했다**(기본값에 기댄 가짜 통과 / 로그를 바이트-문자 혼동으로 잘라 읽은 가짜 실패) — 둘 다 고장주입·로그 실물 대조로 잡았다. 아래는 같은 날 #3 내역.
>
