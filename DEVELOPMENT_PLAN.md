...
> **최종 업데이트:** 2026-09-01 #3 (**v3.80 유지 — Irene "올리지마" · 운영 배포 4회** — CHANGELOG `[v3.80-3]`~`[v3.80-6]`). ①**레시피 목록 CSV 다운로드**(`RecipesTab.tsx` 1파일 · 백업 `20260901_143912`). ②**목록 표 가로 잘림 근본 수정** — 공용 `DataTableContainer` 의 `overflow:hidden` 탓에 열 많은 표가 1025~1400px(13~14인치 노트북·태블릿 가로)에서 **스크롤이 아니라 잘려** 오른쪽 Actions 열에 손이 닿지 않았다. 세로 hidden 유지 + 가로만 auto → 이 부품을 쓰는 **27개 화면 일괄 정상화**. + **재고 대시보드 "입고 예정"**(재주문 제안 위 · 목록의 `on_order_quantity` 재사용, 백엔드 변경 0 · 백업 `20260901_164201`). ③**전 역할 마무리** — StaffPage 가 **역할별 3벌 복사**돼 있어 공용 부품 수정만으론 브랜드·푸드코트가 안 고쳐졌다(셋 다 공용 컨테이너로 통일). 8역할×47라우트×2폭=**94 측정 → `SupplierDashboard` 1건 검출·수정 → 재측정 0건**. 대시보드 "최근 주문" 3곳 동일 계약 적용(백업 `20260901_183315`). ④**남은 화면 8곳 + 공용 격자 표 부품** — 공용 `TableComponents.Table` 은 `<table>` 이 아니라 **styled.div + display:grid** 라 실측 도구가 **표로 인식조차 못 하고** 지나가고 있었다(탐지 사각지대). 격자형 표 탐지를 넣자 매출·활동내역·시스템로그·구독관리·관리자 관리·매니저 구독/매출 8곳이 드러나 같은 계약으로 수정. `/pos/manager/*` 3라우트 + 브랜드·푸드코트 직원목록 2라우트가 **어떤 측정에도 없던 것**도 상시 목록에 등록 — **고친 화면이 측정 목록에 없으면 "0건"은 통과가 아니라 검사 누락**이다. 재측정 112건 → 0건, verify-all 16/16(백업 `20260901_202730`). 넷 다 안전게이트 9/9 · mount sweep 크래시 0 · 스모크 10/10 · 번들해시 일치. 실측 도구 `dev-frontend/scripts/responsive-audit.js` 신설(8역할) — **도구 자체의 거짓 통과(무효 토큰이면 로그인 화면을 재고 "0건")를 `AUTH_FAIL` 로 차단**하고 고장주입으로 증명. 아래는 같은 날 #2 내역.
>
> **이전:** 2026-09-01 #2 (**v3.80 유지 · 운영 코드 배포 없음**). ①**레시피 목록 CSV 다운로드**(RA·BG 공용 `RecipesTab.tsx` 1파일, 백엔드 변경 0, 실브라우저 양쪽 PASS — **개발서버만, 미배포**). ②**SC-450/SC-800 용기 가격 정정**(운영 16행) — 1팩=50개 기준 원가 30.00/43.00 · 판매가 36.00/51.60(+20%). ⚠ 1차에 낱개가를 pack 칸에 넣어 **50배 싸게** 만들었다가 Irene 지적으로 같은 세션 재정정. ③**K-DINE 브랜드 레시피 정리**(42→68건) — 빠진 17건 + 옵션 9건을 **제목·Suggested Price 만**(내용 비움·재료 안 지어냄), 기존 26건 Suggested Price 를 확정 가격표로 교정. ④**K-DINE 신메뉴는 문서화만** — `docs/KDINE_MENU_AND_RECIPE_PLAN.md`. 매장 기준 진짜 신규는 **7건**(66은 브랜드 계층 총계였고 내 오보). ⑤**발주·재고 정합 개편 총정리** — Irene 질문 *"위드민이 주문했는데 GIT은 언제 재고가 빠져?"* 에서 출발해 **레시피 없는 프로덕트는 팔려도 재고가 안 빠진다**는 구조 결함까지 규명. `docs/PURCHASE_ORDER_SYSTEM.md` 끝에 Irene 확정 모델 M1~M10 + 실측 + 요구사항 A~E + **Fable 결정대기 Q1~Q7** 로 정리. **설계·판단은 Fable**. 상세=`.claude/session-state.md`.
>
> **이전:** 2026-08-31 #3 (**v3.80 유지 · 운영 배포 8회**). 운영에서 with MIN 실발주를 넣으며 나온 피드백을 그 자리에서 반영. ⑨**공급업체에 나가는 문서는 공급업체 상품명만**(우리 내부명 제거) + 단위 표시 — 왓츠앱·메일·인쇄본·거래 인보이스 4경로 통일. ⑩**왓츠앱 문구 가독성** — 품목명 굵게·품목 수·줄별 소계, 한 줄 유지(번호·줄바꿈은 넣었다가 Irene "이럴거면 그냥 한줄이 낫겠어"로 철회). **SKU 는 실제 공급업체 코드만** — `SP-*` 는 우리 자동채번이라 받는 쪽이 모른다(LSH 61개 중 57개만 실제코드, 나머지 4개 공급처는 0개). ⑪**PDF 2장부터 종이 끝에 붙던 것** — 원인은 CSS 가 아니라 `invoicePdf.ts` 가 잘라낸 이미지를 (0,0)에 얹어 모든 장 여백이 0이었던 것. 좌우12·상하14mm + 표 머리글 매 장 반복. ⑫**하나 제출했는데 목록이 통째로 사라지던 것** — 서버는 정상이었고(한 건만 submitted) 목록 새로고침이 실패 시 화면을 비우던 것이 범인. 실패해도 목록 유지 + 보낸 카드만 제거(실브라우저 3/3). + Submit All 확인창, **전량/부분 입고 버튼 분리**(부분 입고는 원래 되는데 목록에 길이 없었다). 상세=`.claude/session-state.md`.
>
> **이전:** 2026-08-31 #2 (**v3.80 유지 — Irene "버전 올릴 필요없어" · 운영 배포 4회 완료**). ⑥**발주·인보이스에 단위가 숫자를 따라다니게** + **공급업체에 나가는 문서는 공급업체 상품명**(우리 내부명 제거) + staging **표시방법 토글**(공급업체 상품명/우리 재고명) + **합계바 하단 고정**. 덤으로 `invoice_items.quantity` 가 정수라 kg·g 발주(2.5kg)가 인보이스에서 **2로 깎일** 잠재결함을 소수로 확장(금액 무영향 확인). ⑦**실브랜드 운영자를 배포가 자동 복구**하도록 매장 축과 대칭화 — Irene "배포할 때 운영서버가 알아서 해?" 질문으로 발견. 그전엔 브랜드 축에 "이미 찍힌 것을 푸는 경로"가 없어 사람이 손으로 고쳐야 했다. 운영 결과 `is_test` 32→30(대상 2계정), 수동 조치 대기 0. ⑧SW 4.64→4.66.
>
...
## ✅ 완료: 발주서 공급업체 상품명·SKU 표시 + 공급업체 인쇄 버튼 결함 (2026-08-27, 운영 배포)

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| 공급업체 문서에 판매품목명·SKU | 인쇄본·WhatsApp·이메일에 공급업체 판매품목명+SKU 가 주(主), 내부명은 이름이 다를 때만 `Buyer ref:` 로 병기. 인쇄본에 SKU 열 신설 | ✅ 완료 |
| 기존 발주에도 적용 | `ingredient_seller_product_id` FK read-time 조인 → **데이터 마이그 불필요**. 6월 생성 PO#170 으로 실브라우저 확인 | ✅ 완료 |
| 판매품목명·SKU 조인 단일 소스화 | 같은 조인이 `purchase-orders-crud`(sellerSource 연관)와 `seller-orders`(FK) **두 곳에 서로 다른 방식**으로 복사돼 있던 것을 `utils/sellerProductIdentity.js` 로 통일 | ✅ 완료 |
| 발주 목록(staging)에도 부착 | staging 에서 보내는 WhatsApp/이메일도 판매품목명·SKU 포함 | ✅ 완료 |
| **기존 결함**: 공급업체 Print 버튼 미동작 | 수신함 버튼이 구매자 경로를 열었으나 공급업체 허용 경로(`/pos/supplier/*`) 밖이라 튕김. **가드가 두 겹**(라우트 requiredRole + ROLE_ROUTES)이라 한 줄로는 안 고쳐짐 | ✅ 완료 |
| 공급업체 전용 인쇄 경로 신설 | `/pos/supplier/orders/:id/print` (`Supplier Admin`+`System Admin`). **ROLE_ROUTES 무접촉 = 권한 폭 안 넓힘**. 판매자 모드는 쿼리가 아니라 **경로로 강제** | ✅ 완료 |

### 설계 근거
`docs/STOCK_ITEM_VS_SUPPLIER_PRODUCT_DESIGN.md` ③-4 / ⑥ P0-3 (2026-07-04 Fable 판정)의 **미착수 잔여분**.
공급업체 인쇄 버튼은 **갈래 B**(네임스페이스 편입) 채택 — 갈래 A(구매자 경로 개방)는 프론트 폭 > 서버 폭이 되어 기각.

### 검증
`verify-all --full` **16/16**(mount sweep 8역할 659초, 🔒 인쇄 8/8 무접촉) · 실 API 11/11 · 공급업체 수신함 4/4 ·
타사 발주 차단 3/3(404·누출 0) · e2e **6케이스 3회 연속** · 단위테스트 5/5 ·
**고장주입 반증 4건 전부 성립**(백엔드 조인 / 인쇄본 렌더 / 공유 문자열 / 버튼 분기) · 민감도 판정 비대상.
**DB 스키마 변경 0** → 롤백 = 코드 되돌리기만.

### 운영 배포
2026-08-27 05:48 UTC · 스모크 **10/10** · 백업 `20260827_053408` · 버전 미상승(Irene 지시, v3.78 유지)

### 수정된 파일
- `dev-backend/utils/sellerProductIdentity.js` (신규)
- `dev-backend/routes/purchase-orders-crud.js`
- `dev-backend/routes/seller-orders.js`
- `dev-frontend/src/App.tsx`
- `dev-frontend/src/pages/IncomingOrders/IncomingOrdersView.tsx`
- `dev-frontend/src/pages/PurchaseOrders/PurchaseOrderPrintPage.tsx`
...
## ✅ 완료: 발주 운영 실사용 피드백 반영 (2026-08-31 #3)

> 운영에서 `with MIN` 실발주 33품목을 넣으며 나온 요청을 그 자리에서 반영. 배포 8회.

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| 공급업체 문서 = 공급업체 상품명만 | 왓츠앱·메일·인쇄본·거래 인보이스에서 우리 내부 재고명 제거(받는 쪽 창고엔 없는 이름). 매핑 없는 라인만 폴백 | ✅ 완료 |
| 왓츠앱 가독성 | 품목명 굵게 + 품목 수 + 줄별 소계 + 단위. 한 줄 유지 | ✅ 완료 |
| SKU 는 실제 코드만 | `SP-<n>-<n>` 자동채번 숨김. 단일 소스 `isRealSupplierSku()` 5곳 적용 | ✅ 완료 |
| PDF 페이지 여백 | 잘라낸 이미지를 (0,0)에 얹어 모든 장 여백 0이던 것 → 좌우12·상하14mm. 인보이스 PDF 도 함께 개선 | ✅ 완료 |
| 브라우저 인쇄 페이지 나눔 | `@page` 여백 + 표 머리글 매 장 반복 + 행·합계 블록 안 잘림 | ✅ 완료 |
| 목록이 통째로 사라지던 것 | 조회 실패 시 `setPos([])` 로 화면을 비우던 것 → 실패해도 유지, 보낸 카드만 제거 | ✅ 완료 |
| Submit All 확인창 | 전량 발송은 되돌릴 수 없는데 합계바 하단 고정으로 오발 위험이 커졌다 → 건수·금액 확인 | ✅ 완료 |
| 전량/부분 입고 분리 | 부분 입고는 원래 지원(상세 품목별)인데 목록에 길이 없었다 → 2버튼 | ✅ 완료 |

### 운영 데이터 정리 (with MIN, rid 10)
- 실발주 33품목 생성(API 경로) · Cherry Tomato 중복 재고 정리 · Guan Kee 치킨 단위 통일 + 없던 품목 2개 신설
- 커피 1kg 단위 전환 · Hokkaido 식빵 이름·단가 정정(0.80 → 6.30)

### 수정된 파일
- `dev-frontend/src/utils/poShare.ts` · `dev-frontend/src/utils/invoicePdf.ts`
- `dev-frontend/src/pages/PurchaseOrders/{PurchaseOrderStagingPage,PurchaseOrdersPage,PurchaseOrderPrintPage,PurchaseOrderDetailPage}.tsx`
- 4언어 `purchaseOrders.json` · `dev-frontend/public/sw.js`
...
## 📋 설계 총정리: 발주·재고 정합 개편 (2026-09-01, 미착수 — 설계·판단은 Fable)

Irene 질문 *"위드민이 이거 주문했는데 GIT consulting 에서는 언제 재고가 빠져?"* 에서 출발.
실측 답은 **안 빠진다** 였고, 파고들어 구조 결함까지 규명했다.

### 규명한 것

| | 내용 |
|---|---|
| **결함 1** | **프로덕트/메뉴를 저장해도 재고아이템이 생성되는 경로가 없다** → 프로덕트 ↔ 재고아이템 1:1 이 성립한 적이 없다. GIT 포장재는 UGS 발주를 **별도 재고아이템**으로 받아(투명컵/뚜껑 각 18 pack) 파는 프로덕트 수량은 0 · `track_stock=0` → **배송을 눌러도 차감 0건** |
| **결함 2** | 구매자→판매자 **수령 알림 0곳** (`purchase-orders-workflow.js` 에 알림 코드 없음). 판매자→구매자는 알림만 있고 **화면 안내 없음** |
| **결함 3** | `purchase_orders` 에 **결제 컬럼 0개**, `cash_movements` 에 발주 참조 자리 없음(`source` ENUM = manual/settlement) |
| **관찰** | `RECEIVABLE_STATUSES` 에 `submitted` 포함 → **판매자 배송 전에 구매자가 먼저 수령 가능** |
| **관찰** | 같은 재고아이템에 공급처 여러 개 — 매장 재고 **14.3%(55건)**, 그중 **78%가 가격 상이**, `is_preferred` 는 611건 지정돼 이미 쓰이고 있음 |

### Irene 확정 모델 (원문 인용으로 문서에 보존)
- **"브랜드 재고"라는 개념은 없다. 재고는 각자 관리.**
- **BG 재고 = 프로덕트 + 프로덕트 레시피의 재료.** 브랜드 메뉴·레시피·재료는 **어떤 상관도 없다**.
- 재고관리 **2루트, RA·BG 동일** — 레시피 있음 → 재고아이템 여러 개 / 레시피 없음 → **프로덕트·메뉴 자체에 수량**.
- **재고추적은 항상 되는 것.** · **재고아이템 등록 입구 3개** — 어디로 들어와도 결과는 하나.

### 산출물
- **`docs/PURCHASE_ORDER_SYSTEM.md`** 맨 끝 — 발단 / 모델 M1~M10 / 운영 실측 / 요구사항 A~E /
  **Fable 결정대기 Q1~Q7** / 규모·게이트. **단일 소스.**
- 메모리 `project_po_stock_integrity_redesign`

### 자진 보고
이 구조를 파악하는 동안 **진단을 네 번 뒤집었다** (연결 없음 → 있음 → 브랜드 재료 무관 → 저장 위치는 맞고 입고가 문제).
Irene 이 매번 잡아줬다. 원인은 **내가 "브랜드 재고" 같은 없는 용어를 만들어 쓴 것**.
→ 다음부터 **Irene 이 쓰는 말(프로덕트 / 메뉴 / 재고아이템 / 재료)만 쓴다.**

---

## ✅ 완료: 레시피 CSV 다운로드 + K-DINE 레시피/가격 정리 (2026-09-01)

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| 레시피 목록 CSV 다운로드 | RA(`/restaurant/:id/recipe-management`)·BG(`/pos/recipes`) 공용 `RecipesTab` 1파일. 화면 필터/정렬 그대로, 재료 단위 19열 펼침, 재료 없는 레시피도 1행 보존. 백엔드 변경 0 · 신규 유틸 0 · 신규 로컬버튼 0 | ✅ 개발서버 (미배포) |
| SC-450/SC-800 용기 가격 정정 | **1팩 = 50개.** 원가 22.50/38.25 → **30.00/43.00**, 판매가 27.90/45.90 → **36.00/51.60**(+20%). UGS→GIT→with MIN 6개 체인 + 중복 판매링크 2개 = 운영 16행 | ✅ 운영 반영 |
| K-DINE 브랜드 레시피 정리 | brand 2 레시피 **42 → 68건**. 빠진 17건 + 옵션 9건을 **제목·카테고리·Suggested Price 만** 생성(내용 비움). 기존 26건 Suggested Price 를 확정 가격표로 갱신(19→15.9 등 정수 반올림 오류 교정) | ✅ 운영 반영 |
| K-DINE 신메뉴 문서화 | 확정 가격표 + 옵션그룹 11개 + 기존 레시피 35건 대조. **매장 기준 진짜 신규는 7건**(66건은 브랜드 계층 총계였고 내가 잘못 말한 숫자) | ✅ 문서만 |

### 수정된 파일
- `dev-frontend/src/pages/RecipeManagement/RecipesTab.tsx`
- `docs/KDINE_MENU_AND_RECIPE_PLAN.md` (신규)

### 자진 보고 — 이번 세션 실수
1. **낱개 가격을 pack 칸에 넣어 운영을 50배 싸게 만들었다.** Irene *"1팩이 30링43링깃인데"* 로 잡혔다. 넣은 뒤 **낱개로 되나눠 대조**했으면 막았다 → 규칙화 [[reference_packaging_unit_is_pack]]
2. **"신메뉴 66건"이라고 잘못 보고.** 브랜드 계층 총계였고 매장 기준 신규는 7건. Irene 이 되물어 정정.
3. **레시피 내용을 지어냈다** — 소시지 1줄=40g, 밥 200g, 재료 원가 0. Irene *"마음대로 레시피 만들지 말고 내용은 비워둬"* 로 전부 제거.
4. **이미 지시받은 것을 두 번 되물었다** (중복 링크 가격). Irene *"왜 자꾸 같은 소리를 해"*.
