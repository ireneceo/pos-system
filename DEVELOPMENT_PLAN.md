...
> **최종 업데이트:** 2026-09-01 (**v3.80 유지 · 운영 배포 없음 · 운영 데이터 작업만**). ①**레시피 목록 CSV 다운로드**(RA·BG 공용 `RecipesTab.tsx` 1파일 — 목록 API가 이미 재료를 내려줘 백엔드 변경 0, 공용 `csvDownload.ts` 재사용). 재료 단위 19열로 펼치고 재료 없는 레시피도 1행 보존. 실브라우저 RA/BG 둘 다 PASS. **개발서버만, 미배포.** ②**SC-450/SC-800 용기 가격 정정**(운영 16행) — 1팩=50개 기준 원가 30.00/43.00 · 판매가 36.00/51.60(+20%). ⚠ 1차에 낱개가를 pack 칸에 넣어 **50배 싸게** 만들었다가 Irene 지적으로 같은 세션에 재정정 → [[reference_packaging_unit_is_pack]]. 중복 판매링크 2개도 같은 값으로 정리. ③**K-DINE 브랜드 레시피 정리**(brand 2, 42→68건) — 빠진 레시피 17건 + 옵션 레시피 9건을 **제목·카테고리·Suggested Price 만** 생성(내용 비움, 재료 지어내지 않음) + 기존 26건 Suggested Price 를 확정 가격표로 갱신. ④**K-DINE 신메뉴는 문서화만** — `docs/KDINE_MENU_AND_RECIPE_PLAN.md`. 적용은 Irene 지시 대기. 상세=`.claude/session-state.md`.
>
