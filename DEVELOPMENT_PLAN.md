...
> **최종 업데이트:** 2026-08-27 (**발주서에 공급업체 상품명·SKU 표시 — 2026-08-27 운영 배포 완료** (스모크 10/10 · 백업 20260827_053408 · DB 스키마 변경 0 · 버전 미상승은 Irene 지시). Irene 지시: "발주할 때 발주관리에서는 한글이름 들어간 우리 재고표시 말고 공급업체 상품 이름으로 표시해야 하는 거 아니야?" + "기존에 발주한 발주리스트에서도 바꿔줘". ①공급업체에게 **실제로 나가는 문서**(인쇄본·WhatsApp·이메일)에 판매품목명+SKU 가 주(主), 내부명은 `Buyer ref` 로 병기 — 그동안 두 이름이 같이 보인 건 내부 화면뿐이고 받는 쪽은 자기 상품을 대조할 수 없었다 ②**기존 발주에도 데이터 마이그 없이 적용**(FK read-time 조인, 6월 생성 PO 로 실브라우저 확인) ③판매품목명·SKU 조인을 **단일 소스**(`utils/sellerProductIdentity.js`)로 통일 — 같은 조인이 두 곳에 서로 다른 방식으로 복사돼 있었다 ④**함께 고친 기존 결함**: 공급업체 수신함의 발주서 Print 버튼이 눌러도 안 열리던 문제(공급업체 허용 경로 밖이라 튕김) → 공급업체 전용 인쇄 경로 신설, **권한 폭은 안 넓힘**(ROLE_ROUTES 무접촉, 구매자 경로는 여전히 차단됨을 실증). 검증: `verify-all --full` 16/16 · e2e 6케이스 3회 연속 · 단위테스트 5/5 · **고장주입 반증 4건 전부 성립** · 🔒 인쇄 8/8 무접촉 · 스키마 변경 0. Fable 게이트 **PASS**. 남은 것 = A4 인쇄 종이 확인(Irene 눈, 운영에서). 상세=session-state.)
>
