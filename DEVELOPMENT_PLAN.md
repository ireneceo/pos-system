...
> ③ 🔴 **Fable 이 잡은 결함** — 그 버튼이 **영영 안 보이는** 상태였다(화면이 먼저 저장을 잠가 서버 400 이 도달하지 않음). 표시 조건 1줄로 수정하고 **Fable 이 직접 브라우저에서** 잠김→버튼 보임→저장 200→배너까지 확인.
> ④ 🔴 **디스크 81% 경고 처리** — 운영 백업 폴더가 148G(147개). 보관 규칙은 정상이었고 **9월에만 121번 배포해 전부 30일 이내**라 지울 게 없었다. `BACKUP_KEEP_DAYS` **30→7**, 운영 1회 정리로 **113개·112GB 확보(81%→32%)**.
> ⑤ 검증: 계약 테스트 19/19 · 서버 실호출 16/16 · **고장주입 4종 반증** · verify-all 22/22 · **Fable 게이트 통과**(마커 `f8cac9eecc63`). **배포는 하지 않았다.**

> **최종 업데이트:** 2026-09-24 #2 — [Claude Code] **v3.101 운영 배포 (SW `5.56-soa-period-20260924`, 12:21 UTC · 안전게이트 10/10 · 스모크 10/10).**
> ① **판매자 주문 품목 수정 + 주문 추가** — 출고 전만 · 총액 못 올림 · 오너 승인 존중 · 구매처에 변경내역 자동 안내.
> ② **자격 부여 화면** — 멀티 컨텍스트 전환은 이미 있었는데 **부여할 화면이 없어 아무도 자격을 못 받고 있었다**. Irene 의 GIT↔with MIN 로그아웃 왕복이 여기서 끝난다.
> ③ **월 정산서 정합** — 「January 2000」·미래 발행일·기간「-」의 원인은 **뜻이 다른 값을 한 자리에 겹쳐 쓴 것**. 기간 고르기·잔여분 포함·「N일 안에」·취소 시 자식 풀기 추가.
> ④ 🔴 **공용 날짜 경계 함수가 하루씩 밀려 있었다** — 타임존 오프셋에서 밀리초가 빠져 「하루의 끝」이 다음 날 00:00:00.997. 예약·금고·주문기록 등 13곳 공통 결함. 테스트 8건 신설(옛 코드 4건 실패).
> ⑤ 검증: 실호출 **128건** · 고장주입 5종 반증 · verify-all 22/22 · 🔒 인쇄 보호파일 8/8 무변경 · **Fable 게이트 통과**(마커 `d3750ed4760b`).
> ⑥ 다음: ④인보이스 대조 → ⑤SEO → ⑥공급업체 공유. **운영 DB 읽기 허가·발주 41 실물이 병목.**

> **최종 업데이트:** 2026-09-24 — [Claude Code] **v3.100 운영 배포 완료 — 판매자 주문 수정(amend) (SW `5.52-seller-order-amend-20260924`, 05:21:39 UTC · 스모크 10/10 · 백업 `20260924_051557`).**
> Irene 「GIT consulting에 주문들어온거 … 아래 아이템이 잘못 들어왔어. 더 싼거. 주문내역 좀 바꿔줘」 → 「내가 수정 가능하게 해주면 돼」.
> ① **그때까지 품목을 바꿀 화면 경로가 아무에게도 없었다** — 구매자용 `PUT /purchase-orders/:id` 는 서버에만 있고 부르는 화면이 0건, 판매자 라우트엔 수정 자체가 없었다.
> ② `POST /api/seller-orders/:id/amend` — 수량·교체·삭제·추가. 출고 전(`submitted`·`confirmed`)만. 단가는 서버가 판매자 카탈로그에서 채운다(요청 본문 불신).
> ③ 🔒 **총액 불증가 게이트** — 원칙(Fable): 발주서는 구매자의 돈 약속이라 판매자가 동의 없이 늘릴 수 없다. 이 한 줄로 오너 승인 우회도, 「제안→승인」 왕복 상태도 필요 없어졌다. 더 받아야 하면 종전대로 Reject+재발주.
> ④ **이유는 선택, 변경 내역은 항상** — `tracking_info` 이벤트에 담아 새 테이블 0·상태 ENUM 무변경. 구매자에게 소켓+메일(변경 표)+상세 상단 배너.
> ⑤ 검증: **verify-all --full 22/22** · 실호출 **27/27** · **고장주입 반증**(총액 게이트 빼니 600→700 통과, 원복 후 지문 동일) · mount sweep 681.9초 크래시 0 · 🔒 인쇄 보호파일 8/8 무변경.
> ⑥ **Fable 1회만**(설계 판정). Irene 「fable 꼭 필요할 때만 써」 → 게이트 판정은 부르지 않고 기계 게이트로 증명, 근거는 릴리즈 기록 `fable_note` 에.
> ⑦ **배포 후 운영 실측**: sw.js 5.52 · 판매자 chunk `8790.a01aae95`·구매자 chunk `9088.c848a695` 에 새 코드 실림 · 4언어 반영 · 주요 경로 200 · pm2 online(재시작 1회) · **배포 기인 오류 0건**. DB 스키마 변경 0건이라 롤백은 백업 복원만으로 끝난다.
> ⑧ 🔴 **PO-R8-20260924-001 짜장소스 교체는 Irene 이 화면에서** — 운영 DB 읽기가 분류기에 막혀(2회 거부) 내가 그 주문의 상태·품목을 확인하지 못했다. 이 기능의 첫 실사용이 된다.
...
> **이전:** 2026-09-18 #5 — **v3.96 운영 배포 (SW `5.42-recipe-card-clarity-20260918`, 13:34 UTC · 스모크 10/10 · 오늘 5번째).**
...
> ④ 증명: 계약 4건 신설 **inventory 47/47** · 고장주입 2/2(금고 무접촉·이중정산 각각 해당 계약만 깨짐) · 실호출 8가지 · `verify-all --full` 21/22(fable_note 공란만) · 마이그 멱등+레지스트리.
> ⑤ **Fable 최소화 적용** — 이 사안은 설계 판정 1회만 받고 게이트 판정은 생략, 근거를 배포기록 `fable_note` 에 남겼다.
> ⑥ **v3.94**(같은 날): 「Receive + pay」 기본 결제수단 현금.

> **최종 업데이트:** 2026-09-18 #2 — **v3.93 운영 배포 (SW `5.39-direct-purchase-and-recipe-rows-20260918`, 10:53 UTC · 스모크 10/10 · 오늘 2번째).**
> ① **발주 대기 화면이 정리됐다** — Irene 「이게 왜 여기 남아? … 그냥 오더히스토리에 들어가면 되는 건데」. 2026-09-02 에 넣은 「To receive」 블록이 09-10 에 이력 행으로 같은 버튼이 들어가며 **완전한 중복**이 됐다. 제거하고 대체물도 두지 않았다.
> ② **직접 구매** — Irene 「직접 사왔을 때 사용할 거」. 초안 행 「Receive + pay」 한 번에 보냄·받음·결제. ⛔ 초안을 `RECEIVABLE_STATUSES` 에 넣는 손쉬운 길은 기각(오너 승인 우회 재개·판매자 포털에 없는 주문이 received 로 노출). 승인 매장은 **400 + 통째 롤백**을 실호출로 확인.
> ③ 🔴 **내 검사 도구가 개발 데모 매장에 시험 데이터를 흘리고 있었다** — Fable 게이트가 배포를 막고 잡았다. 계약 정리가 발주·품목만 지워 **고아 거래청구서 11장 · 재고 배치 16개 · 드로어 출금 7건**이 남았고(FIFO 가 유령 배치를 먹고 마감 기대금액이 어긋남), 이를 감시할 `hcLeakFingerprint()` 는 **정의만 있고 호출이 0곳**이었다. 공용 정리로 교체 + `inventory_batches` 추가 + 감시기 배선(전후 비교, 늘면 실패). 고장주입은 처음에 엉뚱한 곳(sweep)에 걸려 다시 걸었다.
> ④ **내 실수 3건**: 레시피 수정을 **빌드 전에 「고쳤다」고 보고**(styled 주석 백틱으로 빌드가 죽는 상태였다) · 그 수정 주석의 기호가 디자인 게이트에 걸림 · SW 버전을 빌드 시작 뒤에 올려 빌드가 3회.
> ⑤ **Irene 지시**: 「이제 fable 사용 최소화 해」 — 한 사안 1회 기본, 조사·구현·검증은 팀원, 기계 게이트로 증명.

> **최종 업데이트:** 2026-09-18 — **v3.92 운영 배포 완료 (SW `5.38-currency-rm-everywhere-20260918`, 07:25 UTC · 스모크 10/10).**
> ① **메일 금액이 「MYR 220.00」으로 나갔다** — Irene 「일단 이메일료. myr이 표시되어서 나와. 링깃은 모두 RM표시가 되어야 해」. 인보이스 발행 메일의 금액 한 줄과, `fmtMoney` 한 곳(발주·반품·거래인보이스 계열 **9개 메일**)이 코드를 찍고 있었다. **어제 신설한 게이트가 이걸 놓친 채 「✅ 0건」이라 답했다** — 제외 규칙의 `^\s*(const|let)` 가 바로 그 줄을 건너뛰었고, 「어느 통화인지 가리키는 문구는 코드가 정확하다」는 문구 기반 예외가 「Monthly Fee (MYR)」까지 통과시켰다. 둘 다 폐기.
> ② **규칙이 하나가 아니라 둘로 확정** — Irene 「통화를 안내할 때는 MYR, 실제 금액 표시는 RM」. 금액 자리는 RM(41+4곳), 통화 안내 자리는 코드 MYR(28곳 되돌림 — 결제설정 라벨 5역할·결제수단 안내 4역할·브랜드·국가·플랜·지갑 통화 칸). 판정은 문구가 아니라 **금액 인접**으로만 가른다.
> ③ **1차에서 빠졌던 왓츠앱 공유 문구 4곳** — 통화를 변수에 담아 **다음 줄에서** 금액을 조립하는 형태라 한 줄 검사로는 원리상 안 잡힌다. 게이트에 축약변수 검사를 신설해 잡았다.
> ④ **게이트를 양방향 차단으로 승격**하는 과정에서 그 게이트가 두 군데 고장나 있었다 — 제외 규칙이 역방향 검사를 시작조차 못 하게 했고, 종료코드가 `reverse` 를 보지 않아 **출력만 하고 통과**했다. 「출력과 종료코드는 같은 목록을 본다」를 주석으로 박았다. 반증 18/18.
> ⑤ **준비된 재료 2건** — (a) 만든 재료가 레시피 선택기에 안 나온 것은 화면이 목록을 마운트 시 한 번만 읽어서였다(저장·삭제 후 전체 재조회로 수정) (b) **준비 레시피 삭제가 스위치 OFF 보다 약한 방어**였다 → 재고·사용처가 있으면 409 거부(사유 표시), 아니면 재료 비활성 후 삭제. health-check 계약 3건 신설.
> ⑥ 🔴 **배포를 막고 있던 인스펙션 결함** — ING-UNI-002 가 레시피 출처(준비 재료)를 출처로 안 쳐서, 운영 #1132(09-18 04:54 생성)를 「출처 없음」으로 세고 있었다. 셋째 출처를 인정하도록 고쳤고 배포 후 운영 실측 0건.
> ⑦ **Fable 1차 판정의 전제가 틀렸다** — 지시받은 고아 인스펙션은 `recipes` 가 soft delete 라는 전제였으나 실측은 완전 삭제·FK SET NULL. 그 상태는 원리상 없어 **항상 통과하는 검사**가 될 것이라 심지 않고 사실을 다시 올려 002 확장+계약으로 대체.
> ⑧ **준비된 재료 전 구간 실호출**(데모 매장): 저장→선택기 즉시→만들기(실제 수율 4.8kg · 원재료 −6·−0.4 · 원가 6.24→6.50)→메뉴 직결 400 거부→**판매 완료 시** 준비재료 −0.6, 원재료 소급 차감 0.
> ⑨ **Irene 대기**: 준비 재료를 실제로 돌리려면 ①메뉴 레시피에 재료로 넣기 ②재고 「만들기」 · 실프린터 확인 1회 · 결제설정 「Bank Transfer (MYR)」 눈 확인(어제 RM 으로 보신 자리, 규칙대로 되돌림 — 원하면 즉시 복구).
...
> ③ **배포 대기**: 통화 RM 표시(화면 13곳·인보이스 설명문) · 할인 인보이스(0원도 발행·예상청구액·할인 합계) · 발주 카탈로그 external_buyers 노출 · parser 어휘 2줄. 전부 게이트 통과.
> ④ **Irene 확인 대기**: 원가 3건(소시지·뚜껑 묶음 여부) · 공급업체 93건 가격 · 환산값 238건 · 배포 여부.
> ⑤ **남은 부채**: 브랜드 재료 28건 누락(09-04 F5 OFF 이후 · 별건 설계 필요) · 날짜 경계 타임존 · 인보이스 2,000건 상한 · 저장값 RM→MYR 마이그.

> **최종 업데이트:** 2026-09-17 #3 — **v3.89 운영 배포(SW 5.35).** 발주 화면 긴급 수정 + 「1 판매단위 = 몇 재고단위」 화면 수정·확인 기능.
> 🚀 **2026-09-17 12:03 UTC 배포 완료** — 스모크 10/10 · 백업 `/var/www/backups/20260917_115831` · 마이그 `migrate-add-conversion-confirmed` 운영 반영(칸 3개 추가, 데이터 변경 0).
> ① **🔥 내가 오늘 오전 배포한 코드가 운영 발주 화면을 죽였다** — `NewPurchaseOrderPage.tsx` 의 `groups` useMemo 가 **아래에 선언된** `cartCurrency` 를 참조(TDZ, `Cannot access 'mn' before initialization`). 자동 제보 2건(11:16 Electron · 11:21 Chrome) + Irene 콘솔. 장바구니가 비면 `.map` 콜백이 실행되지 않아 **mount sweep 이 원리상 못 잡는 종류**다. 선언을 위로 올리고 의존성 추가.
> ② **재발 방지 게이트 신설** `scripts/check-hook-tdz.js` — useMemo 본문이 뒤에 선언된 훅 const 를 참조하면 fail-closed(useMemo 만·주석/그림자 이름 제외 → 602파일 0건). verify-all 등록. **고장주입으로 반증**(원래 버그 재주입 → 그 줄 지목 → 원복). Fable 도 직접 같은 반증 수행.
> ③ **「1 □ 받으면 재고 □ 늘어남」 화면 수정 기능** — 매장 재료(114건)·BG 재고아이템(159건) 두 화면. 문장 빈칸 UI(「단위 변환/환산」 명사 전면 제거, 4언어+용어집), 제안값 버튼, 「확인 필요/확인됨」 표시.
> ④ **「1 이 맞다」를 담는 칸 3개** `conversion_confirmed_at/_by/_pair` — 저장 = 확인. 값이 1 이어도 R-SC-007 에서 빠지고, **확인은 단위쌍에 묶여** 판매자 단위가 바뀌면 자동 무효.
> ⑤ **BG 쪽 수정 경로 신설**(PUT 자체가 없었다) · 매장 쪽 `parseFloat(x)||1` 제거(0·빈값·글자 400) · 저장과 미입고 발주줄 전파를 **한 트랜잭션**으로.
> ⑥ **배포가 2회 막혔던 것도 원인 해결** — 코드 회귀가 아니라 개발서버 MySQL 연결 천장(50)이었다. 배포가 마이그 95개 직후 health-check 를 돌리는데 **스크립트가 서버용 큰 풀(20)** 을 열어 넘쳤다(Max_used 51 · Aborted 195). `config/database.js` 를 서버 20 / 스크립트 5 로 분리.
> ⑦ 검증: verify-all --full **20/20**(새 게이트 포함) · mount sweep 681초 크래시 0 · jest 241/241 · health-check **248/248** · 🔒 인쇄 보호파일 8/8 무변경 · 고장주입 6종 반증 · IDOR 404 · Fable 게이트 2회 통과(설계·게이트).
> ⑧ **남은 것**: kg 으로 사는데 개수로 세는 **31건 단위 정정**(그중 5건은 Irene 눈 확인) · 환산값 **273건 입력** · 연결 화면 기본값 1 제거 · 운영 baseline 분리 · ING-UNI-022/024/025.

> **최종 업데이트:** 2026-09-17 #2 — **운영 데이터 부채 4종 정리 + 배포 2회 추가(SW 5.33).** 오늘 운영 배포 3회(v3.88 · 주문기록 ENUM+분류 합치기 · 데이터 부채 백엔드).
> 🚀 **2026-09-17 09:32 UTC 배포 완료** — 스모크 10/10 · 백업 `/var/www/backups/20260917_092712` · DB 덤프 `db_predeploy_20260917_092712.sql.gz` · mount sweep 재사용(번들 동일).
> ① **주문 수정 기록이 통째로 버려지고 있었다** — `order_actions.action_type` ENUM 에 `'updated'` 가 없어 저장이 `Data truncated` 로 실패하고 `[orderAuditLog] silent fail` 로만 삼켜졌다(화면 무표시). ENUM expand-only 로 추가해 운영 반영 확인. 마지막 실패 로그 **07:25:20**(적용 전) 이후 재발 0.
> ② **재료 분류 합치기(운영 적용)** — 매장 재료 27건 이동 · 비어버린 브랜드 분류 14개 비활성 · 이름 중복 **15쌍 → 0쌍**. ⚠ 내가 먼저 「23건」이라고 보고했다가 스크립트가 27건을 옮겼다 — **활성만 세었던 내 집계가 틀렸고 스크립트가 맞았다**(꺼진 매장 소유 행도 매장 것).
> ③ **데이터 부채 4종(운영 적용)** — 되살림 재료 4·재고아이템 4 · 재채번 13 · 환산비 10 · 고아 매핑 삭제 12·비활성 3. 각각 되돌리기 스냅샷 있음. 대상 검사 **ING-UNI-020 · 023 · R-SC-003 · R-SC-008 전부 0**. R-SC-007 **273행은 «사람이 정할 값»** 으로 운영 baseline 등재(화면에서 넣으면 자동으로 빠짐).
> ④ **끄기 가드 신설** — 레시피가 쓰는 재료·재고아이템은 409 `IN_USE_BY_RECIPES`. 문자열 `"false"`·`"0"` 우회 구멍도 정규화로 막고 4종 입력 전부 고장주입 확인.
> ⑤ **매장 메뉴(products) 무접촉 — 증거를 다시 세웠다.** 배포 직전 DB 덤프 3개(07:32·08:31·09:27)와 현재 라이브의 `products` 가 **바이트 단위 동일**(md5 `9c00aed9dfa545a0211662d22d5fe48a` · 376,198 바이트). 이 구간이 분류 합치기·부채 적용·배포 3회를 전부 감싼다. 마지막 메뉴 수정 07:30:19 = 내 첫 적용(08:01) 이전의 매장 직원 편집.
> ⑥ ⚠ **내 검사기가 고장나 있었다** — 오전 내내 무접촉 증거로 쓴 `MD5(GROUP_CONCAT(...))` 지문이 `group_concat_max_len` 기본값 **1024 에 잘려 전체 31,555 바이트 중 앞 3%(약 25행)만** 해시하고 있었다. 「지문 동일」은 그만큼 약한 증거였다(결론은 같았다). 검증 규율 3조 「판정 기계부터 의심하라」 사례.
> ⑦ ⚠ **1차 적용이 덜 고쳤다** — 수정 쿼리에만 `is_active = 1` 을 붙여 검사 정의(활성 무관)와 어긋나 코드 중복 2쌍·고아 4건이 남아 재적용. 교훈 = 검사와 수정은 같은 SQL(메모리 등재).
> ⑧ **남은 것**: 환산값 273건(Irene 입력) · 기존 부채 3종(ING-UNI-022/024/025) · 운영용 인스펙션 baseline 분리 · 운영 읽기 계정 `claude_ro`.

> **최종 업데이트:** 2026-09-17 — **v3.88 운영 배포(SW 5.32).** 준비된 재고(1차 가공) · 발주 배송비 · 재료 분류 정본 · 브랜드↔매장 메뉴 «레스토랑 기준» 정리.
> 🚀 **2026-09-17 07:38 UTC 배포 완료** — 스모크 10/10 · 백업 `/var/www/backups/20260917_073246` · 마이그 93/93 · **배포 후 운영 에러 0건**. 운영 실측: SW `5.32-prep-stock-delivery-fee-20260917` · 새 칸 4종 생성 확인 · **운영 발주 42건 전부 배송비 0 · 총액식 불일치 0건**(돈 무변동).
> ① **준비된 재고** — 재료의 세 번째 출처(`ingredients.source_recipe_id`). 「이 레시피의 결과물을 재료로 씁니다」 스위치 → 재료 자동 생성 → 재고 화면 「만들기」 한 번에 원재료 차감·준비 재료 증가·배치·장부(`production`)·매장 원가(실제 쓴 돈 ÷ 실제로 나온 양). **판매 차감 코드 0줄 변경**, 준비 레시피는 메뉴에 못 붙음(이중 차감 방지).
> ② **발주 배송비** — 판매자별 두 칸(무료배송 기준 · 기준 미만 고정액)으로 자동 계산. 총액식 `subtotal + tax + delivery_fee`, 두 벌로 복제돼 있던 총액 함수를 한 곳으로. **미설정은 «무료»가 아니라 «규칙 미적용»**(운영 공급업체 42곳 전부 비어 있어 부과로 두면 전원이 물게 됨).
> ③ **재료 분류** — 정본 = 매장 소유 한 벌, 브랜드 소유 벌 쓰기 중단(403). 합치기 스크립트(드라이런 기본·`--undo`) 신설. **운영에 중복 15쌍·브랜드 분류를 쓰는 재료 158건이 남아 있어 적용은 Irene 지시 대기.**
> ④ **브랜드↔메뉴** — Irene 「지금은 레스토랑 기준이 맞아」 로 공유 모드·매장 추가 차단을 만들었다가 전부 제거. **메뉴의 주인은 매장**, 브랜드 메뉴는 달라도 됨. 남긴 것은 표시뿐(「○○ 가 만든 것」 배지 · 「매장 자체 메뉴 (보기 전용)」 탭). **브랜드 레시피는 브랜드제너럴 것 — 매장 수정·삭제 403(실호출·고장주입으로 재확인).**
> ⑤ 검증: verify-all 18/18 · mount sweep 682초 크래시 0 · health-check 247/247 · 서버 jest 241/241 · 화면 jest 69/69 · 인쇄 보호파일 8/8 무변경 · **고장주입 8종 전부 반증**. Fable 게이트 2회차 보류→조건 이행→통과.
...
> ④ **Fable 판정 미수령 — 한도(429) 5회 연속**(어제 4 + 오늘 1). 우회 안 함. Irene 이 «판정 없이 할 수 있는 준비» 를 선택해 같은 문서 **F 절에 검토표 2개** 작성: K-DINE 옵션은 **매장 7개 중 6개가 브랜드에 같은 이름·같은 옵션 수로 이미 있고 연결만 끊긴 상태**(새로 생길 것은 쓰는 메뉴 0개인 「111111」 1개) · with MIN Cafe 227건은 **메뉴 227 + 카테고리 23(이름으로 합쳐짐) + 옵션그룹 44** 가 새로 생기고 **옵션을 먼저 돌리면 연결이 0건**이 된다(메뉴→옵션 순서).
> ⑤ **이 세션 코드 변경 0건 · 빌드 없음 · 운영 쓰기 0건**(운영은 SELECT 와 `--apply` 없는 미리보기만). 기계 판정 `check-sensitive-diff` **비대상**(문서 2건). **확인 불가**: verify-all·build 는 대상이 없어 돌리지 않았고, 운영 화면 육안 확인도 하지 않았다.
>
> **이전 업데이트:** 2026-09-16 #2 — **운영 배포(SW 5.30). 재료 카테고리를 지울 때 «재료를 어디로 옮길지» 선택.**
> ① Irene 「카테고리 삭제할 때 다른 카테고리에 넣을지 선택하게 해줘.」 → 전에는 재료가 **1건이라도 있으면 400 거부**하고 「먼저 바꾸라」만 돌려줬다(`routes/ingredient-categories.js` 매장·브랜드 DELETE 둘 다). 옮길 곳 입력도 일괄 이동 경로도 없어 사람이 하나씩 옮겨야 했고, 그 400 이 레거시 `{ error }` 형식이라 화면에 사유가 안 뜰 수 있었다.
> ② 공용 헬퍼 `resolveReassign` 하나로 두 DELETE 를 묶고 `reassign_to_category_id` 를 받아 **한 트랜잭션에서 재료 이동 + 카테고리 삭제**. 허용 범위 밖·자기 자신은 400 `INVALID_TARGET`. 옮길 수 있는 범위 = 매장이면 자기 것 + 자기 브랜드 활성 카테고리, 브랜드면 같은 브랜드 것. **재료 수는 category_id 로만 센다** — 브랜드 카테고리는 여러 매장이 함께 쓰므로 매장으로 좁히면 남은 재료를 놓친다.
> ③ 화면은 재료가 남아 있을 때만 «어디로 옮길까요» 선택 모달로 바뀌고, 목록에 **브랜드 카테고리도 «브랜드» 표시와 함께** 나온다(4언어). 빈 카테고리·생성·수정·순서변경은 무변경.
> ④ 검증: verify-all --full **19/19** · 배포 안전 게이트 **10/10** · 🔒 인쇄 보호파일 8/8 무변경 · mount sweep 8역할 크래시 0 · 실호출 **11/11** · **고장주입 반증 1건**(허용범위 검사를 빼니 남의 매장 카테고리로 **200 통과** → 원복 후 sha256 동일 `a34f2abe…`, 흔적 0, 실데이터 무변경) · 스모크 10/10. 운영 실측: 새 코드 3곳 · 새 문구가 운영 번들에 실림 · 익명 401. **확인 불가**: 이번 변경분 단독 타입 검사는 안 돌렸다(빌드는 경고 0 통과).
> ⑤ ⚠ **Fable 판정 미수령**(한도 429 · 오늘 3회). 우회 안 함. 민감 판정 «일반 변경»(비대상).
> ⑥ **손대지 않은 것**: 「브랜드 카테고리로 통합」은 길이 갈려 Fable 판정 대기 — 브랜드 14개에 매장이 쓰는 **Staff Meal·Poultry·Alcohol·Beverage·Kitchen & Supplies 5개가 없고 재료 43건**이 거기 붙어 있다. 갈리는 6지점 = `docs/TRADE_STRUCTURE.md` ⑧.
> ⑦ **배송비 실측 답**(⑦ 절): 발주 총액식이 `total_amount = subtotal, tax_amount = 0` 이라 **배송비는 무료로 정한 게 아니라 계산식에 자리가 없다**. 대조에서 적은 값은 `invoice_delivery` 로 저장만 되고 발주 총액엔 안 들어간다. Irene 확답 — 「외부업체 등록 = 레스토랑 공급업체 등록」(Carrier 화면 아님).
>
> **이전 업데이트:** 2026-09-16 — **기록 정리. 2026-09-15 오후 작업 2건(프로덕트↔재고아이템 연결 · 목록 «두 줄» 합치기)이 기록되지 않은 채 운영에 올라가 있었다.**
...
> ① 브랜드 메뉴 카드에 연결된 레시피 이름이 보입니다. 서버는 이미 `linkedRecipe` 를 내려주고 있었고 **화면만 그리지 않던 것**이라 표시 전용 변경(4언어 포함). 실브라우저 렌더 2/2(양성·음성 대조) · 게이트 18/19 · 스모크 10/10.
> ② **팀원의 구조 이해 오류 2건을 정정**했습니다 — (가) 「푸시 갱신 경로에 레시피 연결이 없다」 → 실제로는 `brandMenuSyncService.js:231` 에 «매장이 자기 레시피를 지정했으면 덮어쓰지 않는다» 규칙이 이미 있었음 (나) 「재료는 브랜드·가격은 매장은 담을 칸이 없다」 → 실제로는 **매장 원가층 `restaurant_ingredient_costs`(운영 77행)** · 재료 거울 158 · 공급업체 연결 831 · 재고 오버레이 159 로 **이미 구현된 구조**. 둘 다 좁게 보고 단정한 것이 원인.
> ③ 규칙 변경(②의 덮어쓰기)은 **하지 않았습니다** — 운영 492건 전수 실측 결과 어긋난 것 1건(다음 푸시 때 자동 정합), 덮어쓰기로 바꿔도 바뀔 데이터 0건, 매장 고유 레시피를 지울 위험만 있었습니다.
> ④ Fable 구조정리 5건을 `docs/TRADE_STRUCTURE.md` 에 **사실·위치·측정일만** 적어 접수(판단 없음).
>
> **이전 업데이트:** 2026-09-13 #4 — **브랜드 화면 튕김 긴급수정 운영 배포(SW 5.21) + K-DINE 브랜드 메뉴 105건 연결(운영 데이터).**
...
> 부수 확인: 이 저장소의 **타입 검사가 두 겹으로 막혀 있었음**(힙 OOM + 라이브러리 d.ts 문법). 되살리는 법·기준 439건을 `docs/BUYER_FREE_TIER_DESIGN.md §7-1-1` 에 남김. 이번 변경 신규 타입 오류 0.
>
...
> 다음: 되돌리기 매핑 사본 가격 복원(Fable 잔여) → 게이트 → 배포 → 공급업체 상품 정리 2차 반영.
>
> **이전 업데이트:** 2026-09-11 #5 — **운영 배포(SW 5.12). 발주 내역 «10 kg × 2 carton» · 인보이스 목록 발행자 회사명·최신순.**
>
> **이전 업데이트:** 2026-09-11 #4 — **운영 배포(SW 5.11). 공급업체 상품 이름·규격 정리 — 화면·문서·정리 도구(데이터 반영은 Irene 확인 뒤).**
> Irene 「이 내용이 우리 공급업체들 이름이야. 영문(한글) 이렇게 이름 좀 다 맞춰줘 … 1kg/pack 이런식으로, 그리고 수량을 1pack, 2pack」 · 「레시피나 재고관리에 사용하는 단위는 포장 기준수량에 사용하는 단위와 같아야 해」 · 「아이템명은 공급업체쪽에는 영어만 … New Seoul Mart 만 영어(한글)」 · 「기존 발주한 내용들이나 POs에 있는 것도 정보 맞춰서 나오게 해줘」
> ①공급업체 문서 5곳: 판매 상품 이름 그대로 · Buyer ref 제거 · 수량+단위 한 칸 ②규격 한 줄 «10 kg/BOX» — 판매 상품·재료·재고아이템 카드·발주처 칩(`sellerSpecLabel`·`stockSpecLabel`) ③폼 취급단위 kg·g·L·ml·piece ④정리 도구 `scripts/catalog-alignment.js`(검토표·적용·되돌리기 · 가격 배수/해석 불가/짝 2개/한글 딴 물건 → 확인 · 지난 발주 줄 received 라벨·규격만/미입고 환산까지).
> 검증: jest 서버 34 · 화면 19 · 고장주입 6/6 · dev API 11/11 · dev 적용→되돌리기 스냅샷 일치 · verify-all --full 19/19 · Fable 게이트 통과(마커 8ca23508b568) · 배포 스모크 10/10 · 백업 20260911_185725.
> 운영 검토표(오프라인 예상): 수정 284 · 추가 26 · 확인필요 39 · 연결 환산 146 · 지난 발주 줄 49. 기존 재료 pack→g/kg 수렴·발주 33 소규모 정리는 별도 사안.
>
> **이전 업데이트:** 2026-09-11 #3 — **운영 배포(SW 5.10). 인보이스 대조 정직 읽기 + 외부 공급업체 등록 창 재고 환산.**
> Irene 「고르라는 항목이 전혀 맞지도 않아 … 이미지 제대로 스캔을 못하는 거야? 수준이 낮아?」(운영 발주 32)
> ①원인: Guan Kee 인보이스가 **손글씨 양식지** — 브라우저 무료 OCR(인쇄체 전용)은 원리적으로 못 읽는데, 파서가 전화번호 줄을 품목으로 받고 화면은 초록 «읽어서 채웠습니다». → 대조 화면 세 상태(no_lines 노랑·목록 없음 / no_match 노랑·목록 유지 / ok 초록) + 파서 상식 검사(수량·단가>0 · 금액<100만 · 3숫자 줄 10배 이내).
> ②외부 공급업체 등록 창 `unit_conversion: 1` 고정 제거 → «재고 환산» 칸(`defaultLinkConversion`, 변환 불가면 필수 입력). ③`loadOwnedExternalSupplier` NaN 무응답 → 404.
> 운영 조회(ssh·운영 백엔드 계정 SELECT): 포장단위 이행 supplier 184/352 · brand 121/155 · piece draft 10줄 2건(제출 0) · 인보이스 첨부 5장 중 손글씨 2(같은 종이 No 257506 이 발주 22·32 두 곳) → 손글씨 유료 OCR 안 붙임.
> 검증: jest 28/28 · 고장주입 E2·F 성립 · 실브라우저 대조 3경우 · 재고 환산 창→입고 재고 36→66 · 원가 18→12 (10/10) · verify-all --full 18/19(deploy-ready 만) · Fable 게이트 PASS · 배포 스모크 10/10.
>
> **이전 업데이트:** 2026-09-11 #2 — **운영 배포(SW 5.09). 발주 줄 «용량 · 포장단위 × 수량».**
> Irene 「발주할 때 기본용량 포장단위가 있고 거기에 수량이 올라가는 거잖아. 수량에 포장단위가 붙는거고.」
> ①결함: 발주 줄 단위 함수가 공급업체 상품을 매장 메뉴 표에서 찾아 번호가 겹치면 «piece» 가 찍혔다(운영 Kimchi × 2 piece) → 판매자 종류별 표 매핑.
> ②판매 상품 3표 `package_unit` · 발주 줄 `base_quantity`/`base_unit` 스냅샷(`migrate-seller-package-unit.js`, 멱등) · 규칙 단일소스 서버 `utils/poLineSpec.js` / 화면 `unitConversion.ts` · 확정 발주 줄 8표면 표시 · 입력 3곳.
> 검증: API 12/12 · jest 10/10 · 고장주입 A~D · verify-all --full 19/19(mount 690.7s) · Fable 게이트 PASS(`f463eda63a99`) · 배포 스모크 10/10 · 백업 20260911_141438.
> 후속(Fable): 외부공급업체 등록 창의 연결 환산비 1 고정(기존 지뢰 §2-⑤) 설계 판정 1회 · 인보이스 대조 «줄 고르기» 실측(운영 발주 32 원본 필요).
>
> **이전 업데이트:** 2026-09-11 — **운영 배포 1회(SW 5.08). 결제 한 손 · 대조 총액 · 거래청구서 날짜 · 매장 원가 칸 · 공급업체 켜기/끄기 · 브랜드/푸드코트/오너 외부 인보이스 결제 · 공지 모두읽음 · Staff 결제버튼 가림.**
>
...
> **페이지 가로넘침은 전 화면 0건**이었고 진범은 내부 탭 스트립이었다.
> ②키오스크 = 새 경로가 아니라 기존 모바일오더의 **표시 모드**(주문·결제 로직 무접촉). 폭 1120px·유휴 90초 리셋·
> 완료화면 45초 복귀·계정 nav 숨김·설정에 주소/QR 카드(4개 언어).
> ③장바구니를 `CartContents.tsx` 로 단일화 — `/cart` 와 키오스크 옆 패널이 **같은 금액식**을 쓴다(실측 일치 확인).
> ④기존 결함 발견: 하단 고정 바 5곳이 `right:0` 미해제로 **`max-width` 가 한 번도 안 먹고 있었다**(손님 폰 포함).
> 검증: **verify-all --full 18/19** · mount sweep 693초 크래시 0 · 인쇄 보호파일 8/8 무변경 · 고장주입 1건 반증.
> 실패 1건은 `deploy-ready`(릴리즈 기록·SW 버전) — **배포를 안 해서 정상적으로 빨강**.
> ⚠ **Fable 판정 다섯 번 연속 429.** 사실은 `docs/FABLE_REVIEW_2026-09-08.md` 【E】 절에 정리. **미배포·검토 대기.**
> ⛔ POS Terminal(1651px)·KDS(99px) 는 🔒 인쇄 보호파일이라 **손대지 않음 — 판정 필요.**
>
...
> `utils/recipeCost.js` 로 재료 단가에서 직접 계산한다(환산 불가 줄만 화면 값 폴백, 있던 값을 0 으로 덮지 않는다).
> 검증: verify-all --full 19/19 · health 227/227 · 인쇄 보호파일 8/8 · e2e 3건×3회 9/9 · 고장주입 2방향 반증 ·
> 실브라우저에서 손상 레시피를 열어 저장 → DB 0.00→5.00 복구 확인. 배포 백업 `20260909_080518` · 스모크 10/10 · 마이그 81/81.
> ⚠ Fable 게이트 판정은 **이번에도 429(사용 한도)** 로 못 받았다. 기계 판정이 돈 영역으로 찍는 `invoices-list.js` 는
> 운영본과 바이트 단위로 동일하게 두어 배포에서 실질 제외했다.
> 남은 것: RA 자기 레시피·프로덕트 레시피 저장 경로(매장은 자기 재고아이템 원가 + 오버라이드를 써야 한다),
> 운영에 이미 0 으로 박힌 값의 일괄 복구(운영 쓰기 결정 필요).
>


> **최종 업데이트:** 2026-09-03 #5 (**v3.81 · 운영 배포 5회**). **언어 전환이 실제로 되게 + 문의 읽기.** Irene 신고 *"언어별로 화면이 안 바뀐다"* 의 원인은 **소스에 박힌 한글을 잡는 게이트가 없던 것** — `i18n:verify` 는 번역 파일만 봐서 `t()` 0개인 전면 한글 페이지가 그대로 통과했다. **신규 게이트 `check-i18n-hardcoded.js`**(jsx-text / jsx-attr / ui-message 3종, `t('키','한글')` 폴백은 위반 아님, 예외는 이유 강제 + 사용 건수 항상 출력)를 verify-all 과 배포 스크립트(3b/10) 양쪽에 **fail-closed** 로 연결. 함께: 개발이슈 화면 문구 24건 4개 언어 / 알림함 상대시간을 `Intl.RelativeTimeFormat` 로(중국어·말레이어에 영어가 나오던 것) / 플로어플랜 안내 4건 / **문의 본문 2줄+더보기 공용 `ClampText` 7화면**(운영 문의 SUPP-2026-9449-255) / 배포기록 화면의 '접수된 문의' 섹션 + `resolves` 칸(상태 자동변경 0) / 좌측 메뉴 개발이슈 별도 섹션. **자체 적발 3건**: 내가 보고한 '한글 82파일' 이 오측정이었고(줄 끝 개발자 메모를 화면 문구로 셈 — 실제 8파일 54건) 그 숫자가 Irene 우선순위 승인의 전제였다 → 정정 후 재승인 / 문의 본문 자르기는 2026-06-21 에 이미 있었는데 그 위에 또 얹어 **이중으로 잘려 펼쳐도 안 보였다**(착수 전 기존 구현 미확인, 6화면에서 기존 clamp 제거) / 기록 항목을 문자열로 쓰면 화면에 빈 줄로 뜨던 것(게이트에 형식 검증 추가, 고장주입 반증). **고장주입 5건 전부 성립**. verify-all 16/16 · 배포 안전게이트 10/10 · mount sweep 674.5초 크래시 0 · 스모크 10/10. 운영 실측 `sw.js 4.77` · 번들 해시 일치 · 4개 언어 키 존재. **남은 육안 확인**: '더 보기' 펼침 · 접수 문의 섹션 실데이터 · 언어 전환 시 개발이슈 화면. **배포 1회 실패 후 재시도** — 첫 시도의 프론트 빌드가 메모리 게이트에 막힘(같은 서버 `/opt/planq` tsc 3.8GB 동시 실행), 그 빌드가 끝난 뒤 성공(우리 코드 무관, 남의 빌드 죽이지 않음). 아래는 같은 날 #4 내역.

> **이전:** 2026-09-03 #4 (**v3.81 · 운영 배포 4회**). **문의 입구 + 오류 화면에서 바로 접수.** Irene 요구 *"오류 있는 거 바로 바로 넣기 편하면 좋겠는데"*. 좌측 메뉴 하단 **문의하기**(접힘/펼침 2변형, 역할별 기존 문의 화면 재사용, 신규 화면 0) + ErrorBoundary fallback 의 **이 오류 보내기**(누르면 티켓 즉시 생성, 오류·스택 20줄·componentStack·경로·역할·계정·빌드해시·UA·시각 자동 첨부, sessionStorage 로 세션 내 1회). **Fable 적발 1건**: RA/Staff 분기만 `isRouteAllowed` 를 건너뛰어 역할별 기준이 달랐고, `system_inquiry` 모듈이 Professional 이상에만 있어 **Basic 매장(운영 대다수)엔 입구가 없었다** → Irene 컨펌으로 **플랜 무관 전면 개방**(문의·오류 신고는 유료 기능이 아니라 서비스 채널). **구현 중 자체 적발**: supportPath useMemo 를 의존 선언보다 위에 둬 전 역할 진입 즉시 TDZ 크래시할 코드였다(v3.37 계열) — 선언 뒤로 이동. **고장주입 4건 전부 반증 성립**(중복 억제·로그아웃 가드·실패응답 + **TDZ 를 되돌린 빌드로 mount sweep 실행 → 전 역할 ERROR_BOUNDARY**, 게이트가 이 결함을 실제로 잡는다는 증명). verify-all **한 회차 16/16**(mount sweep 676초 크래시 0), 실호출 201/200/401. **함께**: 운영 문의 **open 13건 → 1건**(답변 12건 작성·저장 확인, resolved 8·closed 4 — 이미 고쳐진 것과 구식·데모·자동진단 구분), **K-소스 거래처 표시 수정**(brand 2 회사명 비어 있어 브랜드명으로 뜨던 것 → `GIT Consulting`, 무접촉 지문 증명). **확인 불가 2건**: 실기기에서 크래시 화면 육안 확인 / 같은 오류 루프 실환경 재현 — 첫 자동 티켓 때 확인. **적발된 구조 문제**: 소스 하드코딩을 잡는 i18n 게이트가 없어 t() 0개 페이지가 통과했다(전수 82파일·t 미사용 26). 게이트 신설은 다음 회차. 아래는 같은 날 #3 내역.

> **이전:** 2026-09-02 #4 (**v3.80 유지 · 운영 배포 9회**). **판매 차감이 한 번도 작동하지 않고 있었다.** K-DINE IPC 에 브랜드 레시피를 연결하기 전 Fable 이 요구한 사전 실증("이미 연결된 1건이 정말 깎는가")에서 드러났다 — 육개장이 60일 43회 팔렸는데 차감 원장 0건. 원인은 재고 부족이 아니라 **읽는 쪽과 쓰는 쪽의 계약 불일치**: POS 는 라인에 `product_id` 를 싣지 않고 `menuItem.id` 에 상품을 담는데 차감 서비스만 `product_id || id` 를 읽어 **카트 임시 id(`order-<ts>`)를 상품 id 로 오인**했다. 운영 완료 라인 **1,444건 중 product_id 를 가진 라인 0건** = POS 주문 전체·전 매장. 세트는 `set_components` 가 숫자 id 를 103/103 갖고 있어 정상이었다(그래서 세트만 깎여 왔다). 수정은 새 규칙을 만들지 않고 **인쇄가 이미 쓰던 `resolveProductId` 재사용**(보호파일은 export 1줄만, 인쇄 계약 10/10 로 무변화 증명 후 bless). 기존 계약 3건이 못 잡은 이유는 **`product_id` 를 손으로 넣어 서비스를 직접 호출**했기 때문 — POS 모양 라우트 실호출 계약을 신설하고 고장주입으로 반증했다. 소급 없음. 함께: **K-DINE with MIN 레시피 45건 보완·7건 신규 운영 반영**(재료 링크 267→267 무변화·스펙 밖 23건 무접촉), **레시피 다운로드가 "재료 1건=1줄"이라 조리법 열이 아예 없던 것**을 레시피 1건=1줄로 재구성. 아래는 같은 날 #3 내역.
>
> **이전:** 2026-09-02 #3 (**v3.80 유지 · 운영 배포 6회**). **발주·재고 정합 P1·P2 운영 반영 + GIT 포장재 합치기**. Irene 질문 *"위드민이 주문했는데 GIT은 언제 재고가 빠져?"* 의 답을 양쪽에서 닫았다 — **사는 쪽**(P1: 추적 스위치 폐기 + 입고가 프로덕트로 들어오는 경로 신설, 갈라져 있던 GIT 포장재 4쌍을 프로덕트로 합침)과 **파는 쪽**(P2: 출고를 `shipped_at` 1회 규칙으로 바꾸고, 구매자가 먼저 받으면 판매자에게 알림 — 운영 수령 8건이 전부 출고 없이 수령돼 판매자 재고가 한 번도 안 빠지고 있었다). P3(등록 입구) 절반은 개발서버. 게이트 훅 자체 결함 2건도 수리(클린 트리 지문이 커밋을 구분 못 해 P1 마커가 P2 에 유효로 뜨던 것 / 게이트 스크립트가 바이너리라 diff 리뷰 불가). ⚠ 검증 과정에서 **내 테스트가 두 번 거짓말했다**(기본값에 기댄 가짜 통과 / 로그를 바이트-문자 혼동으로 잘라 읽은 가짜 실패) — 둘 다 고장주입·로그 실물 대조로 잡았다. 아래는 같은 날 #3 내역.
>
> **이전:** 2026-09-01 #4 (**v3.80 유지 · 운영 배포 4회 + P1 개발서버 완료(미배포)**). **발주·재고 정합 개편 P1** — Irene 질문 *"위드민이 주문했는데 GIT은 언제 재고가 빠져?"* 에서 출발. **설계·판정은 Fable.** 첫 판정(수량을 재고아이템으로 옮겨 링크로 잇기)은 Irene 반박 *"제대로 구조자체는 되어 있던 거 아니야?"* 로 **과설계 철회** → 구조는 그대로 두고 **결함 2개만** 수리: ①**재고추적 스위치가 꺼져 있으면 차감을 건너뛰던 것**(포장재 6개 전부 꺼짐) — 게이트 전부 제거·스위치 폐기, 특히 `applyStock` 안쪽에 계산을 다 하고 마지막에 버리던 지점이 숨어 있었다 ②**입고가 프로덕트로 들어올 길이 없던 것** — 발주 라인·공급처 연결에 프로덕트 자리 신설(+`stockTarget` 이 넷 중 하나 강제). 부수로 **재고아이템 삭제가 실은 숨기기여서 새로고침하면 되살아나던 회귀**를 잡아 진짜 삭제로 교체. 검증: 고장주입 2회 성립(첫 회는 픽스처가 기본값에 기대 **가짜 통과** — 지적받아 반증 가능하게 수정) · 실호출 7/7(발주→수령 0→10→판매 7) · 삭제 6/6 · verify-all 16/16 · 인쇄 8/8 무접촉. **확인 불가 1건**: BG 판매자 ship 실차감(데모에 계약 없음) — 배포 후 운영 실발주로 검증. 커밋 `5e04aa50`. 아래는 같은 날 #3 내역.
>
> **이전:** 2026-09-01 #3 (**v3.80 유지 — Irene "올리지마" · 운영 배포 4회** — CHANGELOG `[v3.80-3]`~`[v3.80-6]`). ①**레시피 목록 CSV 다운로드**(`RecipesTab.tsx` 1파일 · 백업 `20260901_143912`). ②**목록 표 가로 잘림 근본 수정** — 공용 `DataTableContainer` 의 `overflow:hidden` 탓에 열 많은 표가 1025~1400px(13~14인치 노트북·태블릿 가로)에서 **스크롤이 아니라 잘려** 오른쪽 Actions 열에 손이 닿지 않았다. 세로 hidden 유지 + 가로만 auto → 이 부품을 쓰는 **27개 화면 일괄 정상화**. + **재고 대시보드 "입고 예정"**(재주문 제안 위 · 목록의 `on_order_quantity` 재사용, 백엔드 변경 0 · 백업 `20260901_164201`). ③**전 역할 마무리** — StaffPage 가 **역할별 3벌 복사**돼 있어 공용 부품 수정만으론 브랜드·푸드코트가 안 고쳐졌다(셋 다 공용 컨테이너로 통일). 8역할×47라우트×2폭=**94 측정 → `SupplierDashboard` 1건 검출·수정 → 재측정 0건**. 대시보드 "최근 주문" 3곳 동일 계약 적용(백업 `20260901_183315`). ④**남은 화면 8곳 + 공용 격자 표 부품** — 공용 `TableComponents.Table` 은 `<table>` 이 아니라 **styled.div + display:grid** 라 실측 도구가 **표로 인식조차 못 하고** 지나가고 있었다(탐지 사각지대). 격자형 표 탐지를 넣자 매출·활동내역·시스템로그·구독관리·관리자 관리·매니저 구독/매출 8곳이 드러나 같은 계약으로 수정. `/pos/manager/*` 3라우트 + 브랜드·푸드코트 직원목록 2라우트가 **어떤 측정에도 없던 것**도 상시 목록에 등록 — **고친 화면이 측정 목록에 없으면 "0건"은 통과가 아니라 검사 누락**이다. 재측정 112건 → 0건, verify-all 16/16(백업 `20260901_202730`). 넷 다 안전게이트 9/9 · mount sweep 크래시 0 · 스모크 10/10 · 번들해시 일치. 실측 도구 `dev-frontend/scripts/responsive-audit.js` 신설(8역할) — **도구 자체의 거짓 통과(무효 토큰이면 로그인 화면을 재고 "0건")를 `AUTH_FAIL` 로 차단**하고 고장주입으로 증명. 아래는 같은 날 #2 내역.
>
...
## ✅ 완료: 발주·재고 정합 개편 P1·P2 + GIT 포장재 합치기 (2026-09-01~02)

**설계·판정은 Fable, 구현·실측은 Opus.** 첫 판정(수량을 재고아이템으로 옮겨 링크로 잇기)은
Irene 반박 *"제대로 구조자체는 되어 있던 거 아니야?"* 로 **과설계 철회** → 구조는 그대로, 결함만 수리.

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| P1 결함① 재고추적 스위치 폐기 | 꺼져 있으면 판매 차감·입고·반품을 건너뛰던 게이트를 20곳 넘게 제거. `applyStock` 안쪽에 **계산을 다 하고 마지막에 버리던** 지점이 숨어 있었다 | ✅ 완료 |
| P1 결함② 입고 경로 신설 | 발주 라인·공급처 연결에 `product_id`/`brand_product_id` 추가 + `stockTarget` 이 넷 중 하나 강제 + `receiveIntoProduct` | ✅ 완료 |
| P1 부수 수리 | 재고아이템 "삭제"가 실은 숨기기여서 **새로고침하면 되살아나던 회귀** → 진짜 삭제로 | ✅ 완료 |
| GIT 포장재 합치기 | 컵 18·뚜껑 18·SC-450 5·SC-800 7 을 프로덕트로 이동, 원본 비활성, 원장 8줄 합 0. 단위 정합 6건 | ✅ 완료 |
| P2 출고 1회 규칙 | 상태가 아니라 `shipped_at` 기준. 두 번째 출고는 409. 이미 받은 발주는 상태를 뒤로 안 돌림 | ✅ 완료 |
| P2 수령 → 판매자 알림 | `buyer_received` 카테고리·템플릿 4언어. 판매자 화면 "출고 처리 필요" 배지 + 액션 | ✅ 완료 |
| 게이트 훅 결함 2건 | 지문이 커밋을 구분 못 하던 뒷문 / 스크립트가 바이너리라 리뷰 불가 | ✅ 완료 |
| P3-① 메뉴 폼 최소재고 | 이 칸이 없어 **메뉴로 만든 상품은 저재고 알림을 받을 방법이 아예 없었다** (**미배포**) | ✅ 완료 |
| P3-② 카탈로그 → 프로덕트 | 서버 분기·목록 API + **화면**: 내 재고 목록에 프로덕트가 재료와 함께(+*Product* 배지), 카탈로그 [Add as product] (판매가 필수) (**미배포**) | ✅ 완료 |
| P3-③ 재고 → 프로덕트/외부공급업체 | `register-as-product` RA·BG(재료 ×1 레시피·중복 409·한 트랜잭션) + 공유 부품 2개. **BG 재고 화면에 없던 외부공급업체 등록 입구가 생겼다** (**미배포**) | ✅ 완료 |
| **P4-1 결제 컬럼** | `payment_status`/`payment_method`/`paid_at`/`paid_by_user_id`/`cash_movement_id` + `cash_movements.purchase_order_id` + `source` expandEnum. 추가형·멱등·**백필 0**(결제 사실을 모르는데 기록하면 거짓) | ✅ 운영 배포 |
| **P4-2 수령 단일화** | 같은 일을 두 라우트가 따로 적던 것을 서비스 하나로(-255/+45). **응답 diff 0** 으로 동작 무변경 증명 | ✅ 운영 배포 |
| **P4-3 결제·되돌리기** | 현금은 **열린 시프트 1개일 때만** 드로어 출금, 2개면 거절. 되돌리기는 삭제가 아니라 반대 이동. `receive-and-pay` 는 한 트랜잭션 | ✅ 운영 배포 |
| **P4-4 돈 안전망** | health-check `cash` 8건 + 고장주입 2회(되돌리기 제거 → 마감 기대금액 실패) | ✅ (**미배포**) |
| **P4-5 결제 화면** | "받을 발주" 블록 · [수령+결제]/[수령만] · [결제 취소] · **3효과 확인창** · 4언어 | ✅ (**미배포**) |
| P3 중 잡은 결함 | `products.category` 가 NOT NULL 인데 ②서버가 `null` 을 넣어 **카테고리 칸 없는 화면에선 항상 500** 이었다 → `'Uncategorized'` | ✅ 완료 |

### 검증 (Fable 게이트 PASS ×2)
- 고장주입 6회 전부 성립(차감 게이트 / 입고 경로 / 출고 1회 가드 / 알림 호출 / **P3 판매가 가드** / **P3 중복 409**)
- 실호출: 발주→수령 0→10→판매 7·원장 2줄 / 삭제 6/6 / P2 계약 3종 / P3 22/22(데모 매장·데모 BG, 만든 행 전부 원복)
- P3 계약 2건을 **health-check 영구 안전망**으로 추가(RA·BG 대칭) → inventory 19/19
- `verify-all --full` 16/16 ×2 · 인쇄 보호파일 8/8 무접촉 · 운영 배포 6회 스모크 10/10
- **확인 불가 1건 명시**: BG 판매자 ship 실차감 — 데모에 계약이 없어 실호출 불가. 운영 실발주 출고 때 증명 예정

### 수정된 파일 (주요)
- `dev-backend/utils/stockTarget.js`(신규) · `services/inventoryDeductionService.js` · `routes/seller-orders.js`
- `routes/purchase-orders-workflow.js` · `routes/purchase-orders-crud.js` · `routes/po-returns.js`
- `routes/restaurants-ingredients.js` · `routes/product-ingredients.js` · `routes/inventory-core.js`
- `services/poNotifications.js` · `utils/notificationTemplates.js` · `locales/*/email.json`
- `scripts/migrate-po-item-product-target.js`(신규·deploy) · `scripts/migrate-git-merge-packaging.js`(신규·manual)
- `scripts/inspection/suites/supply-chain.js`(R-SC-008~012) · `scripts/health-check.js` · `scripts/fable-gate.js`
- 프론트: 재고·메뉴·발주·직원 화면의 재고추적 토글 제거, `IncomingOrdersView`·`PurchaseOrdersPage` 배지

---

## ✅ 완료: 판매 차감 계약 불일치 수정 + K-DINE 레시피 반영 + 레시피 다운로드 재구성 (2026-09-02 #4)

> **P1~P4 가 전제하던 "팔면 재고가 빠진다"가 실제로는 한 번도 성립한 적이 없었다.**
> K-DINE IPC 매장에 브랜드 레시피를 연결하기 전, Fable 이 요구한 **"이미 연결된 1건이 정말 깎는가"**
> 라는 사전 실증에서 드러났다. 연결을 먼저 했으면 그대로 묻혔을 결함이다.

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| 판매 차감 결함 수정 | POS 라인에 `product_id` 가 없어 카트 임시 id(`order-<ts>`)를 상품 id 로 오인 → **POS 주문 전체가 전 매장에서 한 번도 차감 안 됨**. 인쇄가 쓰던 `resolveProductId` 재사용 | ✅ 완료 |
| 관측 구멍 분리 | "상품 식별 불가"(`unresolved_line`)를 "레시피 없음"과 분리 — 이 혼동이 결함을 오래 숨겼다 | ✅ 완료 |
| POS 모양 계약 신설 | 기존 3건은 `product_id` 를 손으로 넣어 호출해 못 잡았다. 라우트 실호출 계약 + 고장주입 반증 | ✅ 완료 |
| K-DINE 레시피 운영 반영 | 브랜드 2 레시피 **45건 보완 · 7건 신규**(해물비빔밥·육개장·Pre-cook 5). 이름 변경 0, 재료 링크 267→267 무변화, 스펙 밖 23건 `updated_at` 불변 | ✅ 완료 |
| 레시피 다운로드 재구성 | "재료 1건=1줄" 구조라 **조리법 열이 아예 없었다** → **레시피 1건=1줄** + 재료요약·조리법 열 추가, 재료는 한 칸에 합침 | ✅ 완료 |
| 재료 대조표(읽기 전용) | 스펙 재료 문자열 ↔ 브랜드 재료 146건 대조. 연결 29줄·신규 후보 4종·결정 필요 3줄. **자동 연결·생성은 하지 않음** | ✅ 완료 |

### 실측 (운영)
```
완료 주문 라인 1,444건 중 product_id 를 가진 라인      0건
로그 "Deducted N ingredients"                        38건 전부 N=0
     "No recipe or stock link"                        56건
매장8 order_deduct 원장 0건 (그 사이 완료 주문 13,833건)
세트 경로는 정상 — set_components 는 숫자 product_id 를 103/103 보유
```

### 소급하지 않는다
과거 주문을 되돌려 깎지 않는다. 해당 매장 재고가 전부 0 이라 전부 부족분이 될 뿐이고,
**원장은 사실의 기록이지 재구성이 아니다.**

### 수정된 파일
- `dev-backend/services/inventoryDeductionService.js`
- `dev-backend/utils/stationEnrichment.js` (🔒 보호파일 — export 1줄, 인쇄 계약 10/10 로 무변화 증명 후 bless)
- `dev-backend/scripts/health-check.js` · `dev-backend/scripts/print-guard.manifest.json`
- `dev-frontend/src/pages/RecipeManagement/RecipesTab.tsx` · `dev-frontend/public/sw.js`(4.73)
- `docs/PURCHASE_ORDER_SYSTEM.md` §9 신설

### 남은 것
- **재료 연결 적용** — Irene 답 대기 3건: 떡 1개 g수 · `cooking oil` 후보 2개 중 어느 것 · `해물볶음`이 구매재료인지 중간재인지
- **IPC 메뉴↔레시피 연결**(드라이런 준비됨, 적용은 Fable 승인)
- 운영 `[INVENTORY]` 실동작 확인 — 배포 후 첫 완료 주문 발생 시 (현재 **확인 불가**)
...
## ✅ 완료: 헤더 2열 반응형 + 발주↔인보이스 대조 설계 (2026-09-08)

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| 헤더 태블릿 세로 규칙 | 769~1024px 구간 규칙이 아예 없어 80px 고정에 잘렸다. **56곳** 전수 | ✅ 검증완료·미배포 |
| 헤더 폰 폭 최대높이 | `height:auto` 는 풀면서 `max-height:80px` 을 안 풀어 삐져나갔다. **54곳** | ✅ 검증완료·미배포 |
| `HeaderRight` 공용 컴포넌트 | 제목 왼쪽 / 나머지 오른쪽 2줄. 대시보드 4개 적용 | ✅ 검증완료·미배포 |
| 발주↔인보이스 원가 대조 | Fable 설계 2회 + Irene 컨펌 4건. 전문 = `docs/PURCHASE_ORDER_SYSTEM.md` 끝 | 📐 설계확정·미구현 |
| P0 결함 | 외부 공급업체 가격 → 원가 전파 누락 (`supplier-directory.js:1204`) | 🔴 확인됨·미수정 |

### 실측으로 뒤집힌 것 두 가지
- **업로드된 인보이스는 전부 이미지다.** 운영 2장 모두 `이미지 1 · 글꼴 0 · 텍스트 0 · JPEG` — 종이를 찍은 사진. 자동 텍스트 추출은 보조 경로일 뿐이고, 주 경로는 "사진 옆에 발주 라인 기본값, 다른 줄만 수정"이다.
- **매장 원가 오버레이는 범위 안이다.** 1차 설계가 "범위 밖"으로 뒀는데, dev 12행이지만 **운영 73행·3개 매장**이고 거의 전부 외부 공급업체 매핑을 갖는다.

### 안전 확인
- 외부 공급업체 청구서의 `Pay Now` 는 눌러도 **우리 계정에서 돈이 안 나간다** — `getStripeForIssuer` 에 플랫폼 폴백이 없고 해당 공급업체는 결제 설정이 없어 에러로 끝난다. 버튼 숨기는 것은 P1.
- 헤더 전수 스윕이 🔒 인쇄 보호파일 2개(`MainLayout`·`POSTerminalPage`)를 잘못 건드렸다. 가드가 잡아 즉시 원복, **8/8 무변경** 확인.
...
## ✅ 완료: 브랜드 메뉴 레시피 표시 + 구조 이해 정정 (2026-09-14)

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| 연결된 레시피 표시 | 브랜드 메뉴 카드에 레시피 이름(보라) / «레시피 없음»(회색) · 4언어 · 운영 배포 SW 5.22 | ✅ 완료 |
| 레시피 태그 클릭 이동 | 태그를 누르면 `/pos/recipes?tab=recipes&brandId=..&search=<이름>` 로 이동(검색된 상태) · **개발서버만, 미배포** | ✅ 완료 |
| 구조 이해 정정 | 팀원이 단정했다가 틀린 2건을 문서에서 실측값으로 교체(왜 뒤집혔는지도 기록) | ✅ 완료 |
| 레시피 재푸시 규칙 | 전수 실측 후 **바꾸지 않기로 결정**(이득 0건·위험만) | ✅ 판단 완료 |
| Fable 구조정리 접수 | 5건을 `docs/TRADE_STRUCTURE.md` 에 사실만 기록 | ✅ 완료 |
| 운영 건강 점검 | 주문·결제·인쇄 실데이터 확인 | ✅ 완료 |

### 실측으로 확정한 사실
- 브랜드2 메뉴 104건 중 **65건이 브랜드2 소유 레시피에 연결**(소유 불일치 0건) — 데이터는 정상, 화면만 안 보였음
- 브랜드 메뉴에 연결된 매장 상품 **492건 전수**: 브랜드O/매장X 1건 · 둘 다 있는데 다름 **0건** · 브랜드X/매장O **0건**
- 브랜드 메뉴 삭제는 **매장 메뉴를 지우지 않음** — `brand_menu_id=null` · `link_status='unlinked'` · 잠금 해제(`brandMenuSyncService.js:467`), 확인창 문구도 이미 있음
- 운영 7일: 주문 completed 269 / cancelled 46 · **이중결제 의심 0건** · 인쇄 대기 2,813건은 자동인쇄 미사용 매장(매장8 2,587)의 알려진 패턴
- 매장8 이월렛 249건이 완납 표시인데 `amount_paid`·원장 모두 0 (2026-03부터) — 원인 미추적

### 수정된 파일
- `dev-frontend/src/pages/BrandGeneral/BrandMenusPage.tsx` (레시피 태그 + 타입)
- `dev-frontend/public/locales/{en,ko,zh,ms}/brand.json` (키 4개)
...
## ✅ 완료: K-DINE 레시피 연동 실측 + 브랜드↔매장 «서로 공유» 검토자료 (2026-09-16 #3)

> **코드 변경 0건.** 조사·문서만. 조치는 Fable 판정 대기.

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| K-DINE 브랜드 레시피 도달 실측 | 운영 DB SELECT + 코드 실측 + 개발서버 실호출(200·건수 일치·익명 401). 레시피 72건 도달 정상, 끊긴 연결 0 | ✅ 완료 |
| 연동 문제 6건 식별 | 재료 든 레시피 21건 메뉴 미연결 · Add-on 10건은 옵션 경로 · 재료 0인데 연결된 레시피 8건 · 메뉴 39개 레시피 없음 · 상속 누락 1 · 이름 중복 8쌍 | ✅ 완료(조치 미착수) |
| Fable 검토자료 A~E 절 | Irene 원문 · 코드 구조 실측 · 운영 수치 3표 · 기계적 제약 · Irene 이 말한 두 모드. 판단·체크리스트 없이 사실만 | ✅ 완료 |
| Fable 판정 요청 | 한도(429)로 미수령 — 5회 연속. 우회(`.fable-gate-skip`) 안 함 | ⚠ 미수령 |
| 검토표 F 절 2개 | K-DINE 옵션 짝(6/7 이미 브랜드에 존재, 연결만 끊김) · with MIN Cafe 227건 미리보기(메뉴→옵션 순서 제약) | ✅ 완료 |

### 수정된 파일
- `docs/BRAND_MENU_SYSTEM.md` (A~E 절 + F 절 추가)
- `.claude/session-state.md`
- `DEVELOPMENT_PLAN.md` · `CHANGELOG.md`

### 확인하지 않은 것
- verify-all · build: 코드 변경이 없어 **돌리지 않았다**(대상 없음).
- 운영 화면 육안 확인: 하지 않았다.
...
## ✅ 완료: 브랜드 프로덕트 마진 — 단위 환산 · 편집창 통일 (2026-09-20)

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| 마진 단위 환산 | `unit_cost` 는 기준양만큼의 값 — 판매 단위로 환산해 비교(김치 RM 4.80 / 36%) | ✅ 완료 |
| 계산기 단일화 | `utils/productMargin.ts` 신설, 목록 카드·편집창이 같은 숫자 | ✅ 완료 |
| 「단위 환산 불가」 | kg ↔ 개 처럼 못 바꾸는 조합은 숫자를 지어내지 않음(운영 6건) | ✅ 완료 |
| 편집창 마진 | 사는 값(재고 단위)과 파는 값(판매 단위)을 섞어 +124537.7% 로 보이던 것 수정 | ✅ 완료 |
| 회귀 박제 | `productMargin.test.ts` 5건 · 고장주입 4/5 실패로 반증 | ✅ 완료 |
| 운영 데이터 | K-DINE IPC ← GIT 소급 발주 #59 (RM 131.00, 수령완료·미결제) · GIT 쌀 재고 15 | ✅ 완료 |

### 수정된 파일
- `dev-frontend/src/utils/productMargin.ts` (신설) · `productMargin.test.ts` (신설)
- `dev-frontend/src/pages/BrandProductManagement/BrandProductsTab.tsx`
...
## ✅ 완료: 인보이스 대조 — 이름 학습 + 총액대로만 저장 (2026-09-24, 미배포)

### 완료된 작업

| 작업 | 설명 | 상태 |
|------|------|:----:|
| 이름 사전 정규화 | 저장값을 매칭과 **같은 함수**로 정규화(`aliasFor`) — 줄번호·잡음 제거 | ✅ 완료 |
| 양방향 매칭 | 어느 한쪽 낱말이 다른 쪽에 전부 있으면 짝 + 전역 greedy | ✅ 완료 |
| 학습 퇴화 방지 | 사전으로 붙은 줄은 사전을 갱신하지 않음(`aliasToRemember`) | ✅ 완료 |
| 총액대로만 저장 | 줄 값 비움 · 원가/전파/소급/사전 전부 건너뜀 · 청구서 차액 줄 | ✅ 완료 |
| 총액만 모드 표시 | 배너 + 버튼(조건 `totals.blocked \|\| totalMismatch`) · i18n 4언어 | ✅ 완료 |
| 디스크 정리 | `BACKUP_KEEP_DAYS` 30→7 · 운영 113개·112GB 확보(81%→32%) | ✅ 완료 |

### 수정된 파일
- `dev-frontend/src/utils/invoiceMatcher.ts` · `invoiceMatcher.guard.test.ts`
