...
> **최종 업데이트:** 2026-09-02 #4 (**v3.80 유지 · 운영 배포 9회**). **판매 차감이 한 번도 작동하지 않고 있었다.** K-DINE IPC 에 브랜드 레시피를 연결하기 전 Fable 이 요구한 사전 실증("이미 연결된 1건이 정말 깎는가")에서 드러났다 — 육개장이 60일 43회 팔렸는데 차감 원장 0건. 원인은 재고 부족이 아니라 **읽는 쪽과 쓰는 쪽의 계약 불일치**: POS 는 라인에 `product_id` 를 싣지 않고 `menuItem.id` 에 상품을 담는데 차감 서비스만 `product_id || id` 를 읽어 **카트 임시 id(`order-<ts>`)를 상품 id 로 오인**했다. 운영 완료 라인 **1,444건 중 product_id 를 가진 라인 0건** = POS 주문 전체·전 매장. 세트는 `set_components` 가 숫자 id 를 103/103 갖고 있어 정상이었다(그래서 세트만 깎여 왔다). 수정은 새 규칙을 만들지 않고 **인쇄가 이미 쓰던 `resolveProductId` 재사용**(보호파일은 export 1줄만, 인쇄 계약 10/10 로 무변화 증명 후 bless). 기존 계약 3건이 못 잡은 이유는 **`product_id` 를 손으로 넣어 서비스를 직접 호출**했기 때문 — POS 모양 라우트 실호출 계약을 신설하고 고장주입으로 반증했다. 소급 없음. 함께: **K-DINE with MIN 레시피 45건 보완·7건 신규 운영 반영**(재료 링크 267→267 무변화·스펙 밖 23건 무접촉), **레시피 다운로드가 "재료 1건=1줄"이라 조리법 열이 아예 없던 것**을 레시피 1건=1줄로 재구성. 아래는 같은 날 #3 내역.
>
