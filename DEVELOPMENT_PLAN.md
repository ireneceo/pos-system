...
> **최종 업데이트:** 2026-09-09 — **레시피 재료명·줄 원가가 화면에서 깨지던 결함 수정(dev · 미배포)**.
> 화면이 재료 이름·원가를 API 가 준 값이 아니라 «선택기 목록»에서 다시 찾았고, 그 목록은 일부러 걸러진 목록이라
> 저장된 줄의 재료를 못 찾았다 → 이름 «Ingredient #123», 줄 원가 0, **그 상태로 저장하면 원가가 0 으로 덮임**.
> 운영 실측: 브랜드 레시피 **337줄 중 334줄(99%)이 거울 재료**를 가리켜 사실상 전부 깨져 있었고, zerocost 21줄이 이미 생겼다.
> 수정은 프론트 2파일(조회용/선택기 목록 분리 · 서버 값 우선 · 저장식 통일 · 0 덮어쓰기 금지). 백엔드 무접촉.
> 검증: 신규 e2e 2건 3/3 · 고장주입 반증(옛 코드로 되돌리니 화면에 `Ingredient #3943 · RM 0.00` 재현) · verify-all · health 227/227.
>
> 이전 기록: 2026-09-08 (**운영 배포 7회** — v3.86 · SW 4.91~4.97. 발주↔인보이스 원가 대조 **P0·P1·P2 전부 완료**).
...
> 검증: `verify-all --full` **18/18** · health-check **221/221** · 실브라우저 mount sweep 8역할 크래시 0 · 인쇄 보호파일 8/8 · 고장주입 6건 반증.
> 이전 기록: 2026-09-06 (SW 4.85 · 하루 3회 배포 — 메일 반송 근절 / 배포 침묵 결함 / 안정화 묶음).
...
> **확정 모델**: 재료 목록은 **Stock Items 하나**, 매장 재료 행은 그 **거울**(기계 유지·한 방향). 출처는 **둘 중 하나** — GIT 이 **사는 것**=Stock Item, **파는 것**(프로덕트)=프로덕트 자체가 재고(2026-09-01 결정과 정합).
> 재료를 만드는 입구 4곳을 막고, 재발을 **게이트로** 잠갔다(인스펙션 불변식 4 + 계약 검사 4, 고장주입 8건 전부 반증).
> `verify-all --full` **17/17** · health-check **212/212** · 인쇄 보호파일 **8/8 무변경** · 인쇄 라우트 34/34.
> **드라이런이 내 결함 7건을 잡았다** — 크로스 테넌트 매칭 · `K-Soy` 다중 매칭 · **소주 복숭아·딸기가 오리지널로 합쳐질 뻔** · 이름마다 재고아이템 개별 생성 · 계산 순서 불일치 · 빈 키 truthy 로 76건 무더기 · 전 브랜드 배포 프로덕트 누락. 전부 운영 적용 전에 걸렸다.
> 새 문서 `docs/TRADE_STRUCTURE.md`(구입·판매·브랜드 구조 단일 기준) · `docs/INGREDIENT_UNIFICATION_DESIGN.md`. 아래는 이전 회차 내역.
