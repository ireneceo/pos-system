...
> **최종 업데이트:** 2026-09-01 #3 (**v3.80 유지 — Irene "올리지마" · 운영 배포 3회** — CHANGELOG `[v3.80-3]`·`[v3.80-4]`·`[v3.80-5]`). ①**레시피 목록 CSV 다운로드**(`RecipesTab.tsx` 1파일 · 백업 `20260901_143912`). ②**목록 표 가로 잘림 근본 수정** — 공용 `DataTableContainer` 의 `overflow:hidden` 탓에 열 많은 표가 1025~1400px(13~14인치 노트북·태블릿 가로)에서 **스크롤이 아니라 잘려** 오른쪽 Actions 열에 손이 닿지 않았다. 세로 hidden 유지 + 가로만 auto → 이 부품을 쓰는 **27개 화면 일괄 정상화**. + **재고 대시보드 "입고 예정"**(재주문 제안 위 · 목록의 `on_order_quantity` 재사용, 백엔드 변경 0 · 백업 `20260901_164201`). ③**전 역할 마무리** — StaffPage 가 **역할별 3벌 복사**돼 있어 공용 부품 수정만으론 브랜드·푸드코트가 안 고쳐졌다(셋 다 공용 컨테이너로 통일). 8역할×47라우트×2폭=**94 측정 → `SupplierDashboard` 1건 검출·수정 → 재측정 0건**. 대시보드 "최근 주문" 3곳 동일 계약 적용(백업 `20260901_183315`). 셋 다 안전게이트 9/9 · mount sweep 크래시 0 · 스모크 10/10 · 번들해시 일치. 실측 도구 `dev-frontend/scripts/responsive-audit.js` 신설(8역할) — **도구 자체의 거짓 통과(무효 토큰이면 로그인 화면을 재고 "0건")를 `AUTH_FAIL` 로 차단**하고 고장주입으로 증명. 아래는 같은 날 #2 내역.
>
