...
### 4.3-1 필수는 «서버가 막는 것»만 (2026-09-12 Irene 지시)

> 원문: 「모든 폼들 필수랑 아닌 거 표시가 없어. 필수를 최소화하고 불편함 없게 하자.」

- **기준**: 화면의 필수 = **서버가 400 으로 되돌리는 칸**만. 서버가 안 막으면 별표를 붙이지 않는다(실측 사례: 브랜드 상품 화면은 별표 4곳인데 서버는 이름·단위 정도).
- **표시는 한 방식**: 공용 `components/Form/FormComponents.tsx` 의 `FormLabel` 에 `required` 속성 → 라벨 뒤 빨간 `*`. **화면마다 로컬 `RequiredStar` 를 새로 만들지 않는다.**
  - **구현됨(2026-09-13)**: `FormLabel` 은 `styled.label<{ required?: boolean }>` 이고 `&::after { content:' *' }` 로 별을 붙인다. 라벨 안에 `<span>*</span>` 를 손으로 넣지 않는다.
    `components/UI/Modal.tsx` 의 `FormLabel` 도 같은 모양으로 맞췄다(두 곳 다 같은 색 `#EF4444`).
  - 로그인 전 화면처럼 공용 부품을 못 쓰는 곳(가입)은 **로컬 라벨에 같은 `required` 속성**을 달아 모양을 맞춘다 — 별표를 손으로 넣는 방식으로 돌아가지 않는다.
  - 적용 순서: ①무료 발주 등급 경로(가입·매장 회사정보) ✅ ②자주 쓰는 폼(상품·재료·재고·매장 설정) ③나머지.
- **선택 칸에는 아무 표시도 하지 않는다.** 별표 없음 = 선택. 「선택」 배지를 달면 화면만 복잡해진다.
- **막지 않는다.** 저장·다음 버튼은 항상 활성 — 누르면 누락 칸으로 데려간다(아래 §4.4).
- **자동저장 폼**(설정·계약 상세)은 빈 값도 저장되므로, 별표는 «아직 안 채운 칸» 표시일 뿐 저장을 막는 장치가 아니다.
- ⚠ 2026-09-12 실측: 표시를 쓰는 파일 **5개** / 폼이 있는 파일 **약 178개** — 대부분의 폼에 표시가 없다. 폼을 새로 만들거나 고칠 때 이 규칙으로 정리한다.
