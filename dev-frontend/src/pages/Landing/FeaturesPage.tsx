...
const ROLE_TABS: RoleTab[] = [
  {
    ...
    features: [
      ...
      { code: 'ingredients', title: 'Ingredients', description: 'Ingredient catalog with cost and category tracking', points: ['Ingredient library', 'Category organization', 'Cost per unit tracking', 'Recipe linkage'], category: 'advanced', images: getImages('ingredients') },
      { code: 'suppliers', title: 'Suppliers', description: 'Supplier directory with contact and purchase history', points: ['Supplier profiles', 'Contact details', 'Purchase tracking per supplier', 'Category filtering'], category: 'advanced', images: getImages('suppliers') },
      ...
      { code: 'work_manuals', title: 'Work Manuals', description: 'Access work manuals shared by brand or foodcourt', points: ['Manual feed with search', 'Category filtering', 'Version tracking', 'Comment threads'], category: 'advanced', images: getImages('work_manuals') },
  ...
  },
  {
    ...
    features: [
      ...
      { code: 'brand_admin_staff', title: 'Admin & Staff Management', description: 'Manage restaurant admins and staff across all brand locations', points: ['Admin assignment per restaurant', 'Admin overview across locations', 'Role management', 'Access control'], category: 'basic', images: getImages('brand_admin_staff', 3) },
      ...
      { code: 'brand_ingredients', title: 'Ingredients', description: 'Brand-wide ingredient catalog shared across franchise locations', points: ['Standardized ingredient library', 'Cost per unit tracking', 'Category organization', 'Distribution to restaurants'], category: 'advanced', images: getImages('brand_ingredients') },
      { code: 'brand_suppliers', title: 'Suppliers', description: 'Brand-level supplier directory and contract management', points: ['Approved supplier list', 'Contact management', 'Contract visibility', 'Category filtering'], category: 'advanced', images: getImages('brand_suppliers') },
      ...
      { code: 'brand_activity_logs', title: 'Change History', description: 'Track all changes across brand operations for accountability', points: ['Activity audit trail', 'User-level tracking', 'Date range filtering', 'Change detail view'], category: 'advanced', images: getImages('brand_activity_logs') },
      { code: 'brand_work_manuals', title: 'Work Manuals', description: 'Centralized work manuals for franchise operations', points: ['Manual creation and editing', 'Category organization', 'Version tracking', 'Distribution to restaurants'], category: 'advanced', images: getImages('brand_work_manuals') },
  ...
  },
  {
    ...
    features: [
      ...
      { code: 'fc_admin_staff', title: 'Admin & Staff Management', description: 'Manage restaurant admins and staff across foodcourt tenants', points: ['Admin assignment', 'Admin overview', 'Access management', 'Role configuration'], category: 'basic', images: getImages('fc_admin_staff') },
      ...
      // Advanced
      { code: 'fc_floor_plan', title: 'Floor Plan & Store Management', description: 'Drag-and-drop visual floor plan with store placement and status overlay', points: ['Drag-and-drop store placement', '4 shape options (rectangle, rounded, circle, triangle)', 'Resize handles and undo/redo', 'Unplaced stores panel for new units', 'Click-to-view contract and tenant info'], category: 'advanced', images: getImages('fc_floor_plan', 4) },
      { code: 'fc_products', title: 'Product Management', description: 'Foodcourt-level product catalog (common service items, rental products)', points: ['Foodcourt-wide product catalog', 'Pricing and unit setup', 'Tenant-shared catalog', 'Active/inactive toggle'], category: 'advanced', images: getImages('fc_products') },
      { code: 'fc_inventory', title: 'Inventory Management', description: 'Stock tracking for foodcourt-owned inventory', points: ['Common stock tracking', 'Low-stock alerts', 'Receipt and dispatch logs', 'Multi-branch inventory'], category: 'advanced', images: getImages('fc_inventory') },
      ...
      { code: 'fc_work_manuals', title: 'Work Manuals', description: 'Shared work manuals for foodcourt tenants', points: ['Manual creation and editing', 'Category organization', 'Distribution to tenants', 'Version tracking'], category: 'advanced', images: getImages('fc_work_manuals') },
      { code: 'fc_activity_logs', title: 'Change History', description: 'Track all changes across foodcourt operations for accountability', points: ['Activity audit trail', 'User-level tracking', 'Date range filtering', 'Change detail view'], category: 'advanced', images: getImages('fc_activity_logs') },
    ]
  },
  {
    key: 'owner',
  ...
  },
  {
    ...
    features: [
      ...
      { code: 'supplier_orders', title: 'Order Management', description: 'Full purchase-order lifecycle tracking — pending, confirmed, shipped, delivered, returned', points: ['Order list with multi-status filter', 'Lifecycle action buttons', 'Per-buyer order history', 'Bulk export'], category: 'basic', images: getImages('supplier_orders', 2) },
      { code: 'supplier_shipping', title: 'Shipping & Delivery', description: 'Carrier integration with tracking URL generation and delivery confirmation', points: ['Carrier catalog (Lalamove, Grab, J&T, Ninja Van, Pos Laju)', 'Auto tracking URL', 'Edit tracking after ship', 'Webhook-based status updates'], category: 'basic', images: getImages('supplier_shipping') },
      { code: 'supplier_directory', title: 'Directory Listing', description: 'Public supplier directory entry visible to PurpleHere buyers', points: ['Public profile with logo and category', 'Featured products', 'Service area listing', 'Lead inquiry form'], category: 'basic', images: getImages('supplier_directory') },
      // Advanced supplier modules
      { code: 'supplier_admin_staff', title: 'Admin & Staff Management', description: 'Manage supplier admin and staff accounts with role-based permissions', points: ['Admin / Staff role separation', 'Permission grid', 'Activity tracking', 'Account activation control'], category: 'advanced', images: getImages('supplier_admin_staff') },
      { code: 'supplier_performance', title: 'Performance Analytics', description: 'Sales, fulfillment, and customer-retention metrics for the supplier', points: ['Revenue trend by month', 'Fulfillment SLA per buyer', 'Top-buying customers', 'Repeat-order rate'], category: 'advanced', images: getImages('supplier_performance') },
      { code: 'supplier_activity_logs', title: 'Change History', description: 'Audit trail of supplier-side changes for accountability', points: ['Per-record change diff', 'User-level tracking', 'Date range filter', 'Compliance export'], category: 'advanced', images: getImages('supplier_activity_logs') },
  ...
  },
  {
    ...
    heading: 'Buy from suppliers — purchase orders, contracts, and trade invoices in one flow',
    description: 'Restaurants, brands, and food courts share the same buyer-side B2B modules. Browse the supplier directory, sign supply contracts, raise purchase orders, and reconcile incoming trade invoices — all without leaving PurpleHere. Auto-receive on PO completion drives both buyer inventory and supplier trade invoices in a single transaction.',
    features: [
      { code: 'buyer_supplier_directory', title: 'Supplier Directory', description: 'Discover and contact suppliers active on PurpleHere', points: ['Filter by category, region, currency', 'Featured products preview', 'Direct contact / inquiry form', 'Saved suppliers shortlist'], category: 'basic', images: getImages('buyer_supplier_directory') },
      { code: 'buyer_supplier_contracts', title: 'Supplier Contracts', description: 'Active supply contracts with pricing terms — gates PO submission to contracted items', points: ['Contract draft → active → expired lifecycle', 'Per-line item pricing locked by contract', 'Auto-renewal options', 'PO submission limited to active contracts'], category: 'basic', images: getImages('buyer_supplier_contracts', 2) },
      { code: 'buyer_purchase_orders', title: 'Purchase Orders', description: 'Raise purchase orders to suppliers — confirmed, shipped, delivered all tracked', points: ['Cart-first PO creation', 'Per-line received quantity', 'Auto-receive into inventory', 'Returns with credit-note generation'], category: 'basic', images: getImages('buyer_purchase_orders', 3) },
      { code: 'buyer_purchase_invoices', title: 'Purchase Invoices', description: 'Trade invoices received from suppliers, with payment status and reconciliation', points: ['Per-supplier trade invoice list', 'Payment status', 'SOA bundling', 'Reconciliation against PO'], category: 'basic', images: getImages('buyer_purchase_invoices', 2) }
    ]
  }
];
