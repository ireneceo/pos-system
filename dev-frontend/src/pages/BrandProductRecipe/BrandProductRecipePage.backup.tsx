...
const BrandProductRecipePage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const productsResponse = await fetchAPI('/api/brand-products');

      // Fetch categories
      const categoriesResponse = await fetchAPI('/api/brand-product-categories');

      if (productsResponse.success) {
        // Transform products with recipe status
        const productsWithRecipe = (productsResponse.data || []).map((p: any) => ({
          id: p.id,
          name: p.name,
          sku: p.sku,
          ...
          ingredient_cost: p.recipe?.total_ingredient_cost || 0,
          profit_margin: p.recipe && p.unit_price > 0
            ? ((p.unit_price - (p.recipe.total_ingredient_cost || 0)) / p.unit_price * 100).toFixed(1)
            : null
        }));
  ...
  const fetchAvailableRecipes = async () => {
    try {
      const response = await fetchAPI(`/api/brands/${brandId}/recipes`);
      if (response.success) {
  ...
  const fetchIngredients = async () => {
    try {
      const response = await fetchAPI(`/api/brands/${brandId}/ingredients`);
      if (response.success) {
  ...
  const fetchProductRecipe = async (productId: number) => {
    try {
      const response = await fetchAPI(`/api/brand-products/${productId}/recipe`);
  ...
  const openRecipeModal = async (product: BrandProduct) => {
  ...
  const openLinkModal = async (product: BrandProduct) => {
    setSelectedProduct(product);
  ...
  const handleLinkRecipe = async () => {
    ...
    try {
      setSaving(true);
      const response = await fetchAPI(`/api/brand-products/${selectedProduct.id}/recipe`, {
        method: 'PUT',
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      ...
      if (selectedProduct.has_recipe && selectedProduct.recipe_id) {
        ...
        const response = await fetchAPI(`/api/brand-products/${selectedProduct.id}/recipe/ingredients`, {
          method: 'PUT',
          body: JSON.stringify({
  ...
  const filteredProducts = products.filter(item => {
    ...
    const matchesCategory = categoryFilter === 'all' ||
                           (item.category && item.category.id.toString() === categoryFilter);
          ...
          {loading ? (
            ...
            <EmptyState>
              ...
              <div style={{ fontSize: '14px' }}>
                Add products in the Brand Products page first.
              </div>
            </EmptyState>
          ) : (
            <Table>
                  ...
                  <MobileGrid>
                    ...
                    <MobileValue>
                      ...
                      <div style={{ fontWeight: 600, color: '#0A2540' }}>
                        {formatCurrency(product.unit_price, selectedCurrency)}
                      </div>
                    </MobileValue>
                    <MobileValue>
        ...
        {selectedProduct && (
          <>
            ...
            <div style={{ marginBottom: '16px' }}>
              <strong>Product Price:</strong> {formatCurrency(selectedProduct.unit_price, selectedCurrency)}
            </div>
            ...
            <CostSummary>
              ...
              <CostRow>
                ...
                <span>{formatCurrency(selectedProduct.unit_price, selectedCurrency)}</span>
              </CostRow>
              <CostRow>
                <span>Profit Margin:</span>
                <span>
                  {selectedProduct.unit_price > 0
                    ? `${((selectedProduct.unit_price - calculateTotalCost()) / selectedProduct.unit_price * 100).toFixed(1)}%`
                    : '0%'}
                </span>
              </CostRow>
            </CostSummary>
              ...
              >
                ...
                {availableRecipes.map(recipe => (
                  <option key={recipe.id} value={recipe.id}>
                    {recipe.name} - {formatCurrency(recipe.total_ingredient_cost, selectedCurrency)}
                  </option>
                ))}
              </FormSelect>
            </UIFormGroup>
