          ...
          {issuedPg.pageItems.map(invoice => (
            <InvoiceTableRow columns="1.6fr 1.3fr 1.2fr 0.9fr 0.9fr 0.7fr 0.8fr 0.8fr minmax(180px, 220px)" key={invoice.id}>
              <MobileGrid>
                ...
                <MobileValue className="col-period"><MobileLabel>{t('foodcourt:foodcourtInvoicesPage.period')}</MobileLabel><div style={{ fontSize: '12px' }}>{(invoice as any).invoiceCategory === 'trade' && (invoice as any).purchaseOrderId ? <TradeInvoiceDates variant="cell" orderedAt={(invoice as any).poOrderedAt} receivedAt={(invoice as any).poReceivedAt} formatDate={formatDate} /> : (invoice.billingPeriod || '-')}</div></MobileValue>
                      ...
                      {(invoice.status === 'sent' || invoice.status === 'pending_payment' || invoice.status === 'overdue') && Number(invoice.total) > 0 && (
                        ...
                        (invoice as any).issuerIsExternal ? (
                          <ExternalInvoicePayAction
                            ...
                            renderTrigger={(open) => (<LocalActionButton variant="success" onClick={open}>{t('settings:invoicesPage.markPaid', 'Mark paid')}</LocalActionButton>)}
                          />
