...
import { invoiceService, foodcourtService } from '../../services/invoiceService';
...
const FoodcourtInvoicesPage: React.FC = () => {
  ...
  // Foodcourt ID - 실제로는 user context나 API에서 가져와야 함
  const foodcourtId = user?.foodcourt_id?.toString() || '1';

  const fetchInvoices = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch invoices issued by this foodcourt
      const issued = await invoiceService.getIssuedInvoices('foodcourt', foodcourtId);
      setIssuedInvoices(issued);

      // Fetch invoices this foodcourt needs to pay
      const toPay = await invoiceService.getInvoicesToPay();
      setInvoicesToPay(toPay);

      // Fetch payment settings
      const settings = await foodcourtService.getPaymentSettings(foodcourtId);
      ...
      <Container>
        ...
        {activeTab === 'issued' ? (
          <>
            ...
            <Content>
              <ContentHeader>
                <ContentTitle>Invoices Issued to Tenants</ContentTitle>
