...
const FoodcourtChargesSection = styled.div`
  margin: 8px 0;
  padding: 12px 16px;
  background: #EFF6FF;
  border-radius: 8px;
...
const FoodcourtChargeItem = styled.div`
  display: flex;
  justify-content: space-between;
  align-items: center;
  font-size: 13px;
...
const FoodcourtPlansPage: React.FC = () => {
  ...
  const fetchPlans = useCallback(async (fcId: number) => {
    try {
      setLoading(true);
      const token = getAuthToken();
      const response = await fetch(`/api/foodcourts/${fcId}/plans`, {
  ...
  const fetchFoodcourtRestaurants = async (fcId: number) => {
    try {
      const token = getAuthToken();
      const response = await fetch(`/api/foodcourts/${fcId}/restaurants`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
        setFoodcourtRestaurants(data.success ? data.data : (Array.isArray(data) ? data : []));
      }
    } catch (error) {
  ...
  const fetchPlanPrices = async (fcId: number, planId: number) => {
    try {
      const token = getAuthToken();
      const response = await fetch(`/api/foodcourts/${fcId}/plans/${planId}/prices`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  useEffect(() => {
    if (foodcourtId) fetchPlans(foodcourtId);
    fetchCurrencyConfig();
    fetchSupportedCurrencies();
  }, [foodcourtId, fetchPlans]);
  ...
  const updatePlan = async () => {
    if (!foodcourtId || !selectedPlan) return;
    try {
      const token = getAuthToken();
      ...
      const response = await fetch(`/api/foodcourts/${foodcourtId}/plans/${selectedPlan.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify(planData)
      });
      ...
      if (response.ok) {
        ...
        if (editFormData.charge_type === 'fixed' && Object.keys(editFormData.currency_prices).length > 0) {
          ...
          await fetch(`/api/foodcourts/${foodcourtId}/plans/${selectedPlan.id}/prices`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const confirmDeletePlan = async () => {
    if (!foodcourtId || !deletingPlanId) return;
    setShowDeleteConfirm(false);
    try {
      const token = getAuthToken();
      const response = await fetch(`/api/foodcourts/${foodcourtId}/plans/${deletingPlanId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
  ...
  const savePlanPrices = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/foodcourts/${foodcourtId}/plans/${selectedPlan.id}/prices`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ prices })
      });
  ...
  const assignRestaurant = async (restaurantId: number) => {
    ...
    try {
      const token = getAuthToken();
      const response = await fetch(`/api/foodcourts/${foodcourtId}/plans/${selectedPlan.id}/restaurants`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
      ...
      if (response.ok) {
        fetchPlans(foodcourtId);
        const updatedResponse = await fetch(`/api/foodcourts/${foodcourtId}/plans/${selectedPlan.id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const removeRestaurant = async (restaurantId: number) => {
    ...
    try {
      const token = getAuthToken();
      const response = await fetch(`/api/foodcourts/${foodcourtId}/plans/${selectedPlan.id}/restaurants/${restaurantId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
  ...
  const handlePlanPrices = (plan: EntityPlan) => {
    setSelectedPlan(plan);
    if (foodcourtId) fetchPlanPrices(foodcourtId, plan.id);
    setShowPricesModal(true);
  };

  const handleManageRestaurants = (plan: EntityPlan) => {
    setSelectedPlan(plan);
    if (foodcourtId) fetchFoodcourtRestaurants(foodcourtId);
  ...
  const renderChargeTypeFields = (
    ...
    <>
      ...
      <FormGroup>
        ...
        >
          <option value="fixed">{t('foodcourt:foodcourtPlansPage.fixedAmount', 'Fixed Amount')}</option>
          <option value="percentage">{t('foodcourt:foodcourtPlansPage.percentageOfRevenue', '% of Revenue')}</option>
        ...
        <div style={{ fontSize: 12, color: '#4B5563', marginTop: 4 }}>
          ...
          {formData.charge_type === 'combined' && 'Rent = max(fixed amount, revenue × rate). Min-guarantee pattern.'}
        </div>
      </FormGroup>
      ...
      <Container>
        ...
        {showCreateModal && (
                ...
                <FormGroup>
                  ...
                  <FormInput
                    type="text"
                    placeholder="e.g., Monthly Rent, Management Fee, Revenue Share"
        ...
        {showDetailsModal && selectedPlan && (
                ...
                <DetailSection>
                  <h4>Assigned Tenants ({getSubscriptionCount(selectedPlan)})</h4>
                ...
                {/* Phase 2-F: Originating Contracts — reverse lookup section */}
        ...
        {showRestaurantModal && selectedPlan && (
                ...
                <RestaurantList>
                  {foodcourtRestaurants.map(restaurant => {
