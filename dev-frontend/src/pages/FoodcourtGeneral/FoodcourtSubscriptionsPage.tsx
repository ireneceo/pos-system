...
const StatCard = styled.div<{ color?: string }>`
  ...
  border-left: 4px solid ${props => props.color || '#EA580C'};
  transition: all 0.2s;
...
const Badge = styled.span<{ variant: 'green' | 'yellow' | 'red' | 'gray' | 'blue' | 'purple' | 'orange' }>`
  padding: 5px 10px;
  border-radius: 6px;
  font-size: 11px;
  font-weight: 600;
  ...
  ${props => {
    switch (props.variant) {
      ...
      case 'orange': return 'background: #FFEDD5; color: #EA580C;';
...
interface FoodcourtPlanSubscription {
...
const FoodcourtSubscriptionsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'foodcourt_plans' | 'pos_subscriptions'>('foodcourt_plans');
  const [loading, setLoading] = useState(true);
  const [generating, setGenerating] = useState(false);
  const [foodcourtSubs, setFoodcourtSubs] = useState<FoodcourtPlanSubscription[]>([]);
  ...
  const fetchFoodcourtSubscriptions = useCallback(async () => {
    if (!foodcourtId) return;
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/foodcourts/${foodcourtId}/subscriptions`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
        setFoodcourtSubs(data.data || []);
      }
    } catch (error) {
  ...
  const handleGenerateInvoices = async () => {
    ...
    setGenerating(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/foodcourts/${foodcourtId}/generate-invoices`, {
        method: 'POST',
        headers: {
  ...
  // Stats
  const totalTenants = foodcourtSubs.length;
  const withPlan = foodcourtSubs.filter(s => s.plan !== null).length;
  const totalMonthRevenue = foodcourtSubs.reduce((sum, s) => sum + (s.current_month?.revenue || 0), 0);
  const totalEstimatedCharges = foodcourtSubs.reduce((sum, s) => sum + (s.current_month?.estimated_charges?.totalAmount || 0), 0);
  const currency = defaultCurrency || 'MYR';

  const getInvoiceStatusBadge = (status: string): 'green' | 'yellow' | 'red' | 'gray' | 'blue' | 'purple' | 'orange' => {
    switch (status) {
  ...
  return (
    <MainLayout>
      <Container>
        ...
        <Content>
          <TabContainer>
            <Tab active={activeTab === 'foodcourt_plans'} onClick={() => setActiveTab('foodcourt_plans')}>
              Foodcourt Plans ({withPlan})
            </Tab>
            <Tab active={activeTab === 'pos_subscriptions'} onClick={() => setActiveTab('pos_subscriptions')}>
              POS Subscriptions ({totalTenants})
            </Tab>
          </TabContainer>
            ...
            <>
              <StatsGrid>
                <StatCard color="#059669">
                  ...
                  <StatLabel>Foodcourt Tenants</StatLabel>
                ...
                <StatCard color="#2563EB">
                  ...
                  <StatSub>All tenants combined</StatSub>
                </StatCard>
                ...
                <StatCard color="#D97706">
                  <StatValue>{foodcourtSubs.filter(s => s.latest_invoice?.status === 'pending_payment').length}</StatValue>
              ...
              ) : foodcourtSubs.length === 0 ? (
                <EmptyState>
                  <EmptyIcon>&#x1F3EA;</EmptyIcon>
                  <EmptyTitle>No Tenants</EmptyTitle>
                  <EmptyDescription>No restaurants are assigned to this foodcourt yet.</EmptyDescription>
                </EmptyState>
              ) : (
                <SubscriptionGrid>
                  {foodcourtSubs.map(sub => (
                    <Card key={sub.restaurant_id}>
                      ...
                      {sub.plan ? (
                        <>
                          <PlanSection>
                            ...
                            {parseFloat(sub.plan.subscription_fee) > 0 && (
                              <PlanDetail>
                                <PlanDetailLabel>Management Fee</PlanDetailLabel>
                            ...
                            {parseFloat(sub.plan.revenue_percentage) > 0 && (
                              <PlanDetail>
                                <PlanDetailLabel>Revenue Share</PlanDetailLabel>
                          ...
                          {sub.current_month.estimated_charges && sub.current_month.estimated_charges.items.length > 0 && (
                            <ChargesSection>
                              ...
                              {sub.current_month.estimated_charges.items.map((item, idx) => (
                                <ChargesItem key={idx}>
                                  <span>{item.item_type === 'subscription_fee' ? 'Management Fee' : item.item_type === 'revenue_percentage' ? 'Revenue Share' : 'Rent'}</span>
                        ...
                        <NoPlanTag>No foodcourt plan assigned - Go to Plans page to assign</NoPlanTag>
                      )}
      ...
      <ModalOverlay show={showGenerateModal}>
        <ModalContent>
          <ModalTitle>Generate Foodcourt Invoices</ModalTitle>
          <ModalSubtitle>
            This will generate invoices for the previous month based on each tenant's assigned plan and actual revenue.
          </ModalSubtitle>
          ...
          <div style={{ padding: '16px', background: '#F0FDF4', borderRadius: '8px', marginBottom: '16px' }}>
            ...
            <div style={{ fontSize: '14px', color: '#374151' }}>
              <div>{withPlan} tenant(s) with active foodcourt plans</div>
          ...
          <div style={{ padding: '12px 16px', background: '#FEF3C7', borderRadius: '8px' }}>
            <div style={{ fontSize: '13px', color: '#92400E' }}>
              Revenue-based charges (revenue share %, rent %) are calculated from completed orders in the billing period.
            </div>
          </div>
