...
const SupplierInventoryPage: React.FC = () => {
  ...
  const fetchSummary = useCallback(async () => {
    try {
      ...
      const res = await fetch('/api/supplier-inventory/summary', {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      const url = `/api/supplier-inventory${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  const submitAdjust = async (e: React.FormEvent) => {
    ...
    try {
      const token = getAuthToken();
      const res = await fetch('/api/supplier-inventory/adjust', {
        method: 'POST',
        headers: {
  ...
  const submitReceive = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/supplier-inventory/receive', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    <Container>
              ...
              {loading ? (
                ...
                rows.map(row => (
                  <DataTableRow key={row.product_id}>
                    ...
                    <DataTableCell data-label={t('common:category', 'Category') as string}>{row.category_name || '-'}</DataTableCell>
      ...
      >
        <form id="receive-form" onSubmit={submitReceive}>
          {activeRow && (
            <UIFormGroup>
              <FormLabel>{t('common:currentStock', 'Current Stock')}</FormLabel>
              <FormInput
