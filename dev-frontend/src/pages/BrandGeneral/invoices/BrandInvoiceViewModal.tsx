...
interface BrandInvoiceViewModalProps {
  invoice: Invoice;
...
const BrandInvoiceViewModal: React.FC<BrandInvoiceViewModalProps> = ({
  ...
  // isReceivedInvoice is not defined in the original code either - it evaluates to undefined (falsy)
  const isReceivedInvoice = false;
    ...
    <>
      <CommonModal isOpen={true} onClose={onClose} title="Invoice Details" size="large" footer={<><Button variant="secondary" onClick={onClose}>{t('brand:brandInvoicesPage.close')}</Button></>}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
          {/* Bill To */}
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '12px', fontWeight: '600', color: '#4B5563', marginBottom: '8px', textTransform: 'uppercase' }}>{t('brand:brandInvoicesPage.billTo')}</div>
            {isReceivedInvoice ? (
              <>
                ...
                {(() => {
                  ...
                  return oneline ? <div style={{ fontSize: '13px', color: '#4B5563', marginTop: '4px' }}>{oneline}</div> : null;
                })()}
                {companySettings?.email && (
                  <div style={{ fontSize: '13px', color: '#4B5563', marginTop: '2px' }}>{companySettings.email}</div>
                )}
              </>
              ...
              <>
                ...
                {invoice.payerType === 'restaurant' && invoice.restaurantName && invoice.restaurantName !== 'Unknown' && (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #C7CED6' }}>
                <th style={{ textAlign: 'left', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('brand:brandInvoicesPage.description')}</th>
                <th style={{ textAlign: 'center', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('brand:brandInvoicesPage.qty')}</th>
                <th style={{ textAlign: 'right', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('brand:brandInvoicesPage.unitPrice')}</th>
        ...
        {displayCompany?.bankName && (
          <div style={{ background: '#F1F4F8', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
            <div style={{ fontSize: '12px', fontWeight: '600', color: '#4B5563', marginBottom: '8px', textTransform: 'uppercase' }}>{t('brand:brandInvoicesPage.paymentDetails')}</div>
            ...
            <div style={{position: 'relative'}}>
              ...
              {showLinkSearchDropdown && linkSearchResults.restaurants.length > 0 && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid #C7CED6', borderRadius: '8px', maxHeight: '300px', overflowY: 'auto', zIndex: 1000, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                  <div>
                    <div style={{padding: '8px 12px', background: '#F1F4F8', fontSize: '12px', fontWeight: '600', color: '#4B5563'}}>{t('brand:brandInvoicesPage.restaurants')}</div>
...
export default BrandInvoiceViewModal;
