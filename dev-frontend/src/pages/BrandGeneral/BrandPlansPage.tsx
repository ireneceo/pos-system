...
const RestaurantItem = styled.div<{ selected?: boolean }>`
  ...
  &:hover {
    border-color: #635BFF;
    background: ${props => props.selected ? '#F0F0FF' : '#FAFAFE'};
  }
`;
...
const BrandPlansPage: React.FC = () => {
  ...
  const fetchBrandRestaurants = useCallback(async () => {
    if (!brandId) return;
    try {
      const response = await fetch(`/api/brands/${brandId}/restaurants`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  }, [brandId, fetchPlans, fetchBrandRestaurants]);
  ...
  const handleSavePlan = async () => {
    ...
    try {
      const url = editingPlan
        ? `/api/brands/${brandId}/plans/${editingPlan.id}`
        : `/api/brands/${brandId}/plans`;
  ...
  const handleDeletePlan = async () => {
    if (!brandId || !deletingPlan) return;
    try {
      const response = await fetch(`/api/brands/${brandId}/plans/${deletingPlan.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
  ...
  const handleToggleActive = async (plan: EntityPlan) => {
    if (!brandId) return;
    try {
      await fetch(`/api/brands/${brandId}/plans/${plan.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleSaveRestaurants = async () => {
    if (!brandId || !managingPlan) return;
    setSaving(true);
    try {
      ...
      // Restaurants to remove
      const toRemove = currentAssigned.filter(id => !selectedRestaurants.includes(id));

      // Add new assignments
      if (toAdd.length > 0) {
        await fetch(`/api/brands/${brandId}/plans/${managingPlan.id}/restaurants`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
      ...
      for (const rid of toRemove) {
        await fetch(`/api/brands/${brandId}/plans/${managingPlan.id}/restaurants/${rid}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${token}` }
        });
      }
      ...
      <Container>
        ...
        <Content>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '24px' }}>
            ...
            <div style={{ background: 'white', padding: '20px', borderRadius: '12px', border: '1px solid #E6EBF1' }}>
              <div style={{ fontSize: '13px', color: '#6B7C93', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Assigned Restaurants</div>
              <div style={{ fontSize: '28px', fontWeight: 700, color: '#635BFF', marginTop: '4px' }}>{totalAssigned}</div>
            </div>
            <div style={{ background: 'white', padding: '20px', borderRadius: '12px', border: '1px solid #E6EBF1' }}>
              <div style={{ fontSize: '13px', color: '#6B7C93', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Brand Restaurants</div>
              <div style={{ fontSize: '28px', fontWeight: 700, color: '#0A2540', marginTop: '4px' }}>{brandRestaurants.length}</div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <EmptyState>
              <EmptyIcon>&#128203;</EmptyIcon>
              <EmptyText>{searchTerm ? 'No plans match your search' : 'No subscription plans yet'}</EmptyText>
              <EmptySubText>Create your first plan to start managing franchise fees and royalties</EmptySubText>
                  ...
                  <PlanCard key={plan.id} isActive={plan.is_active}>
                    ...
                    <CardBody>
                      ...
                      <RestaurantCount onClick={() => handleOpenRestaurants(plan)}>
                        <span>{activeRestaurants.length} restaurant(s) assigned</span>
            ...
            <ModalBody>
              <FormGrid>
                <FormGroup fullWidth>
                  ...
                  <Input
                    placeholder="e.g., Standard Franchise Plan"
                    value={formData.name}
                ...
                <FormGroup>
                  <Label>Subscription Fee ({formData.currency})</Label>
                  <Input
                    type="number"
                    min="0"
                    step="0.01"
              ...
              {/* Estimate Preview */}
              <EstimateBox>
                <EstimateTitle>Estimated Monthly Charge (per restaurant, RM 50,000 revenue)</EstimateTitle>
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      {est.royalty > 0 && (
                        <EstimateLine>
                          <span>Revenue Royalty ({formData.revenue_percentage}%)</span>
            ...
            <ModalBody>
              <p style={{ color: '#374151', fontSize: '14px', lineHeight: 1.6 }}>
                ...
                {(deletingPlan.planRestaurants?.filter(pr => pr.is_active)?.length || 0) > 0 &&
                  ' This plan has active restaurant assignments. Please unassign all restaurants first.'
                }
              </p>
            </ModalBody>
            <ModalFooter>
      ...
      {showRestaurantModal && managingPlan && (
        <ModalOverlay onClick={() => setShowRestaurantModal(false)}>
          <Modal onClick={e => e.stopPropagation()} style={{ maxWidth: '560px' }}>
            <ModalHeader>
              <ModalTitle>Manage Restaurants - {managingPlan.name}</ModalTitle>
              <CloseButton onClick={() => setShowRestaurantModal(false)}>&times;</CloseButton>
            </ModalHeader>
            <ModalBody>
              <p style={{ fontSize: '13px', color: '#6B7C93', marginBottom: '16px' }}>
                Select restaurants to assign to this plan. {selectedRestaurants.length} selected.
              </p>
              <RestaurantList>
                {brandRestaurants.length === 0 ? (
                  <EmptyState style={{ padding: '30px' }}>
                    <EmptyText>No restaurants in this brand</EmptyText>
                  </EmptyState>
                ) : (
                  brandRestaurants.map(r => {
                    const isSelected = selectedRestaurants.includes(r.id);
                    // Check if assigned to another plan
