  ...
  return (
    <>
      ...
      {showViewModal && viewNotice && (
        <ModalOverlay onClick={() => { setShowViewModal(false); setViewNotice(null); }}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            <ModalHeader>
              <ModalTitle>{viewNotice.title}</ModalTitle>
              <CloseButton onClick={() => { setShowViewModal(false); setViewNotice(null); }}>×</CloseButton>
            </ModalHeader>
            <ModalBody>
              <NoticeDetailMeta>
                <MetaItem>
