            ...
            <div class="bill-to-section">
                ...
                ${(() => {
                  const html = formatAddressHtml({
                    address: payerCompany?.address,
                    city: payerCompany?.city,
                    state: payerCompany?.state,
                    postal_code: payerCompany?.postalCode,
                    country: payerCompany?.country
                  }, (i18n.language as AppLocale) || 'en');
        ...
        {showViewModal && selectedInvoice && (() => {
          ...
          <CommonModal isOpen={true} onClose={() => setShowViewModal(false)} title="Invoice Details" size="large" footer={<>{(selectedInvoice.status === 'sent' || selectedInvoice.status === 'pending_payment' || selectedInvoice.status === 'overdue') && Number(selectedInvoice.total) > 0 && ( <Button variant="success" onClick={() => { setShowViewModal(false); handlePayInvoice(selectedInvoice); }}> Pay Now </Button> )}{(selectedInvoice.status === 'sent' || selectedInvoice.status === 'pending_payment' || selectedInvoice.status === 'overdue') && Number(selectedInvoice.total) === 0 && ( <Button variant="success" onClick={() => { setShowViewModal(false); handleConfirmFreeInvoice(selectedInvoice); }}> Confirm </Button> )} <Button onClick={() => generateInvoicePDF(selectedInvoice)}> Download PDF </Button><Button onClick={() => handlePrintInvoice(selectedInvoice)}> Print </Button><Button variant="secondary" onClick={() => setShowViewModal(false)}> Close </Button></>}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '12px', fontWeight: '600', color: '#4B5563', marginBottom: '8px', textTransform: 'uppercase' }}>{t('owner:ownerInvoicesPage.billTo')}</div>
                    ...
                    {payerCompany?.address && (
                      <div style={{ fontSize: '13px', color: '#4B5563', marginTop: '4px' }}>{payerCompany.address}</div>
                    )}
                    ...
                    {payerCompany?.country && (
                      <div style={{ fontSize: '13px', color: '#4B5563', marginTop: '2px' }}>{payerCompany.country}</div>
                    )}
                    {payerCompany?.email && (
                      <div style={{ fontSize: '13px', color: '#4B5563', marginTop: '2px' }}>{payerCompany.email}</div>
                    )}
                  </div>
                  <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', color: '#4B5563', marginBottom: '12px', textTransform: 'uppercase' }}>{t('owner:ownerInvoicesPage.items')}</div>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom: '2px solid #C7CED6' }}>
                        <th style={{ textAlign: 'left', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('owner:ownerInvoicesPage.description')}</th>
                        <th style={{ textAlign: 'center', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('owner:ownerInvoicesPage.qty')}</th>
                        <th style={{ textAlign: 'right', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('owner:ownerInvoicesPage.unitPrice')}</th>
                        <th style={{ textAlign: 'right', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('owner:ownerInvoicesPage.amount')}</th>
                      </tr>
                    </thead>
                    <tbody>
                ...
                {issuerInfo?.bankName && (
                  <div style={{ background: '#F1F4F8', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                    <div style={{ fontSize: '12px', fontWeight: '600', color: '#4B5563', marginBottom: '8px', textTransform: 'uppercase' }}>{t('owner:ownerInvoicesPage.paymentDetails')}</div>
        ...
        {showPaymentSubmitModal && selectedInvoice && (
          <CommonModal isOpen={true} onClose={() => setShowPaymentSubmitModal(false)} title="Submit Payment" footer={<><Button variant="secondary" onClick={() => setShowPaymentSubmitModal(false)}> Cancel </Button> {paymentData.paymentMethod && paymentData.paymentMethod !== 'stripe' && paymentData.paymentMethod !== 'paypal' && ( <Button variant="success" onClick={handleSubmitPayment} disabled={isSubmittingPayment || availablePaymentMethods.length === 0} > {isSubmittingPayment ? 'Submitting...' : 'Submit Payment'} </Button> )}</>}>
                ...
                {loadingPaymentMethods ? (
                  <div style={{ textAlign: 'center', padding: '20px', color: '#4B5563' }}>{t('owner:ownerInvoicesPage.loadingPaymentMethods')}</div>
                        ...
                        {availablePaymentMethods.map(method => (
                          <button
                            ...
                            style={{
                              ...
                              borderRadius: '8px',
                              border: `1px solid ${paymentData.paymentMethod === method.id ? '#635BFF' : '#C7CED6'}`,
                              background: paymentData.paymentMethod === method.id ? 'rgba(99, 91, 255, 0.1)' : 'white',
                              color: paymentData.paymentMethod === method.id ? '#635BFF' : '#1F2937',
