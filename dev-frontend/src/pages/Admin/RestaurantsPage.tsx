...
import { formatCurrency, getPlanPrice, getActivePlanCurrencies, COUNTRY_TO_CURRENCY, normalizeCurrencyCode, getCurrencySymbol } from '../../utils/currency';
import { formatPhoneForDisplay } from '../../utils/phoneUtils';
import PhoneInput from '../../components/Common/PhoneInput';
import { AddressFields } from '../../components/Form';
import SubscriptionFormFields, { SubscriptionValues } from '../../components/Subscription/SubscriptionFormFields';
...
interface Restaurant {
  ...
  reservation_settings?: { enabled?: boolean; [k: string]: any };
  is_demo?: boolean;
  is_test?: boolean;
}
...
// Adapter: legacy camelCase form state ↔ snake_case SubscriptionValues used by
// the unified <SubscriptionFormFields> component (v3.27).
function toSubscriptionValues(r: any): SubscriptionValues {
  return {
    currency: normalizeCurrencyCode(r.currency || 'MYR'),
    plan_type: r.planType || '',
    plan_amount: String(r.planAmount ?? '0'),
    billing_cycle: (r.billingCycle === 'annual' ? 'annual' : r.billingCycle === 'monthly' ? 'monthly' : '') as SubscriptionValues['billing_cycle'],
    payment_model: (r.paymentModel || 'restaurant') as SubscriptionValues['payment_model'],
    subscription_start: r.subscriptionStart || new Date().toISOString().split('T')[0],
    subscription_end: r.subscriptionEnd || '',
    auto_renew: r.autoRenew !== undefined ? !!r.autoRenew : true,
    treat_as_trial: r.status === 'trial',
    activate_now: true, // hidden — outer activateSubscription toggle owns this
    discount_type: (r.discount_type || 'none') as SubscriptionValues['discount_type'],
    discount_value: Number(r.discount_value) || 0,
    discount_reason: r.discount_reason || ''
  };
}

function fromSubscriptionPatch(patch: Partial<SubscriptionValues>): any {
  const merged: any = {};
  if (patch.currency !== undefined) merged.currency = patch.currency;
  if (patch.plan_type !== undefined) merged.planType = patch.plan_type;
  if (patch.plan_amount !== undefined) merged.planAmount = patch.plan_amount;
  if (patch.billing_cycle !== undefined) merged.billingCycle = patch.billing_cycle;
  if (patch.payment_model !== undefined) merged.paymentModel = patch.payment_model;
  if (patch.subscription_start !== undefined) merged.subscriptionStart = patch.subscription_start;
  if (patch.subscription_end !== undefined) merged.subscriptionEnd = patch.subscription_end;
  if (patch.auto_renew !== undefined) merged.autoRenew = patch.auto_renew;
  if (patch.treat_as_trial !== undefined) merged.status = patch.treat_as_trial ? 'trial' : 'active';
  if (patch.discount_type !== undefined) merged.discount_type = patch.discount_type;
  if (patch.discount_value !== undefined) merged.discount_value = patch.discount_value;
  if (patch.discount_reason !== undefined) merged.discount_reason = patch.discount_reason;
  return merged;
}
...
const RestaurantsPage: React.FC = () => {
  ...
  const [newRestaurant, setNewRestaurant] = useState({
    ...
    subscriptionEnd: '',
    // When false, creates restaurant without generating subscription invoice.
    // Use case: hardware-only customers, placeholder accounts — activate later.
  ...
  const handleSubmitAdd = async () => {
    try {
      ...
      const restaurantData: any = {
        ...
        branch_name: newRestaurant.branch_name || null,
        adminAction,
        ...
        activate_subscription: newRestaurant.activateSubscription !== false
      };
    ...
    <>
      <Container>
        ...
        {showAddModal && (
                ...
                <FormGrid>
                  ...
                  <FormGroup style={{ gridColumn: '1 / -1' }}>
                    ...
                    <FormInput
                      type="text"
                      placeholder="e.g. Bukit Bintang, Sunway Pyramid"
                      value={newRestaurant.branch_name}
                      onChange={(e) => setNewRestaurant({...newRestaurant, branch_name: e.target.value})}
                    />
                  </FormGroup>
                  ...
                  {/* Activate Subscription toggle — uncheck for hardware-only customers */}
                  <FormGroup style={{gridColumn: '1 / -1'}}>
                    <label style={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: '12px',
                      padding: '16px 20px',
                      background: newRestaurant.activateSubscription ? '#ECFDF5' : '#FEF3C7',
                      borderRadius: '12px',
                      border: newRestaurant.activateSubscription ? '2px solid #10B981' : '2px solid #F59E0B',
                      cursor: 'pointer',
                      transition: 'all 0.2s'
                    }}>
                      <input
                        type="checkbox"
                        checked={newRestaurant.activateSubscription}
                        onChange={(e) => setNewRestaurant({ ...newRestaurant, activateSubscription: e.target.checked })}
                  ...
                  {newRestaurant.activateSubscription && (
                    <div style={{gridColumn: '1 / -1'}}>
                      <SubscriptionFormFields
                        userType="restaurant"
                        mode="add"
                        values={toSubscriptionValues(newRestaurant)}
                        onChange={(patch) => setNewRestaurant({ ...newRestaurant, ...fromSubscriptionPatch(patch) })}
                        availablePlans={availablePlans}
                        planCurrencies={planCurrencies}
                        options={{ hideActivateNow: true, hideCurrency: true, hideSectionHeader: true }}
                      />
                    </div>
                  )}
                </FormGrid>
                ...
                <FormGrid>
                  ...
                  <FormGroup style={{ gridColumn: '1 / -1' }}>
                    ...
                    <FormInput
                      type="text"
                      placeholder="e.g. Bukit Bintang, Sunway Pyramid"
                      value={editingRestaurant.branch_name || ''}
                      onChange={(e) => setEditingRestaurant({...editingRestaurant, branch_name: e.target.value})}
                    />
                  </FormGroup>
                  ...
                  {editingRestaurant.activateSubscription && (
                    <div style={{gridColumn: '1 / -1'}}>
                      <SubscriptionFormFields
                        userType="restaurant"
                        mode="edit"
                        values={toSubscriptionValues(editingRestaurant)}
                        onChange={(patch) => setEditingRestaurant({ ...editingRestaurant, ...fromSubscriptionPatch(patch) })}
                        availablePlans={availablePlans}
                        planCurrencies={planCurrencies}
                        options={{ hideActivateNow: true, hideCurrency: true, hideSectionHeader: true }}
                      />
        ...
        {showViewModal && selectedRestaurant && (
          <CommonModal isOpen={true} onClose={() => setShowViewModal(false)} title={selectedRestaurant.name + (selectedRestaurant.branch_name ? ` (${selectedRestaurant.branch_name})` : '')} size="large" footer={<><ThemedButton variant="secondary" onClick={() => { setShowViewModal(false); handleEditRestaurant(selectedRestaurant); }}>Edit</ThemedButton><ThemedButton onClick={() => setShowViewModal(false)}>{t('admin:restaurantsPage.close')}</ThemedButton></>}>

            <DetailSection>
              <DetailSectionTitle>Basic Information</DetailSectionTitle>
              <DetailRow>
                <DetailLabel>Restaurant Name</DetailLabel>
                <DetailValue>{selectedRestaurant.name}</DetailValue>
              </DetailRow>
              {selectedRestaurant.branch_name && (
                <DetailRow>
                  <DetailLabel>Branch</DetailLabel>
                  <DetailValue>{selectedRestaurant.branch_name}</DetailValue>
                </DetailRow>
              )}
              <DetailRow>
                <DetailLabel>Status</DetailLabel>
                <DetailValue><StatusBadge status={selectedRestaurant.status}>{selectedRestaurant.status}</StatusBadge></DetailValue>
              </DetailRow>
              <DetailRow>
                <DetailLabel>Cuisine</DetailLabel>
                <DetailValue>{selectedRestaurant.cuisine || 'Various'}</DetailValue>
              </DetailRow>
              <DetailRow>
                <DetailLabel>Currency</DetailLabel>
                <DetailValue>{selectedRestaurant.currency || 'MYR'}</DetailValue>
              </DetailRow>
              {(selectedRestaurant.is_demo || selectedRestaurant.is_test) && (
                <DetailRow>
                  <DetailLabel>Account Type</DetailLabel>
                  <DetailValue>{selectedRestaurant.is_demo ? 'Demo' : ''}{selectedRestaurant.is_test ? 'Test' : ''}</DetailValue>
                </DetailRow>
              )}
              <DetailRow>
            ...
            <DetailSection>
              <DetailSectionTitle>Admin & Managers</DetailSectionTitle>
              <DetailRow>
                <DetailLabel>Restaurant Admin</DetailLabel>
                <DetailValue>
                  {selectedRestaurant.admin
                    ? `${selectedRestaurant.admin.name} (${selectedRestaurant.admin.email}${selectedRestaurant.admin.phone ? ' / ' + selectedRestaurant.admin.phone : ''})`
                    : <span style={{ color: '#DC2626' }}>No Admin Assigned</span>}
                </DetailValue>
              </DetailRow>
              <DetailRow>
                <DetailLabel>Oversight Managers</DetailLabel>
                <DetailValue>
                  {selectedRestaurant.managers && selectedRestaurant.managers.length > 0
                    ? selectedRestaurant.managers.map(m => `${m.name} (${m.role})`).join(', ')
                    : 'None'}
                </DetailValue>
              </DetailRow>
            </DetailSection>
            ...
            <DetailSection>
              <DetailSectionTitle>Contact & Address</DetailSectionTitle>
              <DetailRow>
                <DetailLabel>Email</DetailLabel>
                <DetailValue>{selectedRestaurant.email || 'N/A'}</DetailValue>
              </DetailRow>
              <DetailRow>
                <DetailLabel>Phone</DetailLabel>
                <DetailValue>{formatPhoneForDisplay(selectedRestaurant.phone) || 'N/A'}</DetailValue>
              </DetailRow>
              <DetailRow>
            ...
            <DetailSection>
              <DetailSectionTitle>Business Details</DetailSectionTitle>
              <DetailRow>
                <DetailLabel>Business Registration</DetailLabel>
                <DetailValue>{selectedRestaurant.businessRegistration || 'N/A'}</DetailValue>
              </DetailRow>
              <DetailRow>
                <DetailLabel>Tax ID / GST No.</DetailLabel>
                <DetailValue>{selectedRestaurant.taxId || 'N/A'}</DetailValue>
              </DetailRow>
            </DetailSection>

            <DetailSection>
              <DetailSectionTitle>Subscription</DetailSectionTitle>
              <DetailRow>
                <DetailLabel>Plan</DetailLabel>
                <DetailValue>{selectedRestaurant.planType || 'Basic Plan'}</DetailValue>
              </DetailRow>
              <DetailRow>
              ...
              <DetailRow>
                ...
                <DetailValue>{selectedRestaurant.paymentModel === 'brand_manager' ? 'Brand Manager' : selectedRestaurant.paymentModel === 'foodcourt_manager' ? 'Foodcourt Manager' : 'Restaurant Admin'}</DetailValue>
              </DetailRow>
              <DetailRow>
              ...
              <DetailRow>
                ...
                <DetailValue>{selectedRestaurant.autoRenew !== false ? 'Yes' : 'No'}</DetailValue>
              </DetailRow>
