...
interface InvoiceViewModalProps {
  ...
  onCloseLinkAccountModal: () => void;
  linkSearchQuery: string;
  onLinkSearch: (query: string) => void;
  showLinkSearchDropdown: boolean;
  onShowLinkSearchDropdown: (show: boolean) => void;
  linkSearchResults: { managers: Manager[]; restaurants: Restaurant[] };
  onLinkAccount: (targetType: 'restaurant' | 'manager', targetData: Restaurant | Manager) => void;
}
...
const InvoiceViewModal: React.FC<InvoiceViewModalProps> = ({
  ...
  return (
    <>
      <CommonModal isOpen={true} onClose={onClose} title="Invoice Details" size="large" footer={<><Button variant="secondary" onClick={onClose}>{t('admin:invoicesPage.close')}</Button></>}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
          {/* Bill To */}
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '12px', fontWeight: '600', color: '#4B5563', marginBottom: '8px', textTransform: 'uppercase' }}>{t('admin:invoicesPage.billTo')}</div>
            {invoice.payerType === 'external' ? (
              ...
              <>
                ...
                {invoice.payerType === 'restaurant' && invoice.restaurantName && invoice.restaurantName !== 'Unknown Restaurant' && (
                ...
                {invoice.companyName && invoice.companyName !== invoice.customerName && (
                  <div style={{ fontSize: '13px', color: '#4B5563', marginTop: '4px' }}>Company: {invoice.companyName}</div>
                )}
              </>
            )}
          </div>
        ...
        <div style={{ marginBottom: '24px' }}>
          <div style={{ fontSize: '12px', fontWeight: '600', color: '#4B5563', marginBottom: '12px', textTransform: 'uppercase' }}>{t('admin:invoicesPage.items')}</div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #C7CED6' }}>
                <th style={{ textAlign: 'left', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('admin:invoicesPage.description')}</th>
                <th style={{ textAlign: 'center', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563' }}>{t('admin:invoicesPage.qty')}</th>
                <th style={{ textAlign: 'right', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563', whiteSpace: 'nowrap' }}>{t('admin:invoicesPage.unitPrice')}</th>
                <th style={{ textAlign: 'right', padding: '12px 8px', fontSize: '12px', fontWeight: '600', color: '#4B5563', whiteSpace: 'nowrap' }}>{t('admin:invoicesPage.amount')}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '24px' }}>
          <div style={{ width: '280px' }}>
            <InvoiceSummary>
              ...
              {invoice.discountType && invoice.discountType !== 'none' && invoice.discountAmount && invoice.discountAmount > 0 && (
                <SummaryRow>
        ...
        {(() => {
          ...
          return (
            <div style={{ background: '#F1F4F8', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
              <div style={{ fontSize: '12px', fontWeight: '600', color: '#4B5563', marginBottom: '8px', textTransform: 'uppercase' }}>{t('admin:invoicesPage.paymentDetails')}</div>
      ...
      {/* Link Account Modal */}
      {showLinkAccountModal && invoice && (
        <CommonModal isOpen={true} onClose={onCloseLinkAccountModal} title="Link to Member Account" footer={<Button variant="secondary" onClick={onCloseLinkAccountModal}>{t('admin:invoicesPage.cancel')}</Button>}>
          <FormGroup>
            ...
            <div style={{position: 'relative'}}>
              <FormInput
                ...
                onFocus={() => onShowLinkSearchDropdown(true)}
                onBlur={() => setTimeout(() => onShowLinkSearchDropdown(false), 200)}
              ...
              {showLinkSearchDropdown && (linkSearchResults.managers.length > 0 || linkSearchResults.restaurants.length > 0) && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid #C7CED6', borderRadius: '8px', maxHeight: '300px', overflowY: 'auto', zIndex: 1000, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                  {linkSearchResults.managers.length > 0 && (
                    <div>
                      <div style={{padding: '8px 12px', background: '#F1F4F8', fontSize: '12px', fontWeight: '600', color: '#4B5563'}}>{t('admin:invoicesPage.managers')}</div>
                      {linkSearchResults.managers.map(manager => (
                        <div key={manager.id} onClick={() => onLinkAccount('manager', manager)} style={{ padding: '12px', cursor: 'pointer', borderBottom: '1px solid #F1F4F8' }} onMouseEnter={(e) => e.currentTarget.style.background = '#F1F4F8'} onMouseLeave={(e) => e.currentTarget.style.background = 'white'}>
                          <div style={{fontWeight: '500', color: '#0A2540'}}>{manager.fullName}</div>
                          <div style={{fontSize: '13px', color: '#4B5563'}}>{manager.companyName || manager.email} - {manager.role}</div>
                        </div>
                      ))}
                    </div>
                  )}
                  {linkSearchResults.restaurants.length > 0 && (
                    <div>
                      <div style={{padding: '8px 12px', background: '#F1F4F8', fontSize: '12px', fontWeight: '600', color: '#4B5563'}}>{t('admin:invoicesPage.restaurants')}</div>
                      {linkSearchResults.restaurants.map(restaurant => (
                        <div key={restaurant.id} onClick={() => onLinkAccount('restaurant', restaurant)} style={{ padding: '12px', cursor: 'pointer', borderBottom: '1px solid #F1F4F8' }} onMouseEnter={(e) => e.currentTarget.style.background = '#F1F4F8'} onMouseLeave={(e) => e.currentTarget.style.background = 'white'}>
