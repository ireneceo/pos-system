...
const GeneralStockCategoriesTab: React.FC<GeneralStockCategoriesTabProps> = ({ isBrandGeneralMode, restaurantId: propsRestaurantId, onCountChange, onCategoryChange }) => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        if (isBrandGeneralMode || isBrandUser) {
          ...
          const response = await fetch(`/api/restaurants/${effectiveRestaurantId}/general-stock-categories`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
    ...
    try {
      ...
      if (isBrandGeneralMode || isBrandUser) {
        ...
        url = editingCategory
          ...
          : `/api/restaurants/${effectiveRestaurantId}/general-stock-categories`;
      }
  ...
  if (loading) {
    return (
      <Container>
        <div style={{ textAlign: 'center', padding: '40px', color: '#4B5563' }}>{t('recipes:generalStockCategoriesTab.loading')}</div>
      </Container>
    );
  }
  ...
  return (
    <Container>
      ...
      <Modal
        ...
        title={`${editingCategory ? 'Edit' : 'New'} General Stock Category`}
        size="medium"
        footer={
          <>
        ...
        <form onSubmit={handleSubmit}>
          <UIFormGroup>
            ...
            <FormInput
              ...
              placeholder="e.g., Packaging, Cleaning Supplies"
              autoFocus
              required
            />
          </UIFormGroup>
...
export default GeneralStockCategoriesTab;
