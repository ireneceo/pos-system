        ...
        return (
          <>
            ...
            <div style={{ borderTop: '1px solid #E6EBF1', paddingTop: '16px' }}>
              ...
              {currentProof ? (
                <>
                  {currentProof.reference && (<div style={{ fontSize: '13px', marginBottom: '6px' }}><span style={{ color: '#6B7C93' }}>Reference: </span><span style={{ fontFamily: 'monospace', fontWeight: 600, color: '#0A2540' }}>{currentProof.reference}</span></div>)}
                  {currentProof.file_name && (<div style={{ fontSize: '13px', marginBottom: '6px', color: '#6B7C93' }}>File: {currentProof.file_name}</div>)}
                  {currentProof.uploaded_at && (<div style={{ fontSize: '12px', color: '#9CA3AF', marginBottom: '6px' }}>Submitted: {formatDateTimeUtil(currentProof.uploaded_at, operationSettings)}</div>)}
            ...
            {proofHistory.length > 0 && (
              <div style={{ borderTop: '1px solid #E6EBF1', paddingTop: '16px', marginTop: '16px' }}>
                <div style={{ fontSize: '13px', fontWeight: 600, color: '#6B7C93', marginBottom: '10px' }}>Previous Attempts ({proofHistory.length})</div>
                {proofHistory.map((entry: any, idx: number) => (
                  <div key={idx} style={{ padding: '10px', background: '#F9FAFB', borderRadius: '6px', marginBottom: idx < proofHistory.length - 1 ? '8px' : 0, border: '1px solid #E5E7EB' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                      <span style={{ fontSize: '12px', color: '#DC2626', fontWeight: 600 }}>Rejected #{entry.reject_count || idx + 1}</span>
                      {entry.rejected_at && (<span style={{ fontSize: '11px', color: '#9CA3AF' }}>{formatDateTimeUtil(entry.rejected_at, operationSettings)}</span>)}
                    </div>
                    {entry.reference && (<div style={{ fontSize: '12px', color: '#6B7C93' }}>Ref: <span style={{ fontFamily: 'monospace' }}>{entry.reference}</span></div>)}
