...
const CashDrawerOps: React.FC<Props> = ({ restaurantId, onChange, compact }) => {
  ...
  const submit = async (type: 'in' | 'out') => {
    ...
    setBusy(false);
    if (status === 200) { setMovements(j?.movements || movements); setAmt(''); setReason(''); onChange?.(); }
        ...
        <Box>
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 16, paddingTop: 12 }}>
            <div style={{ fontSize: 12.5, color: C.subtle, lineHeight: 1.5 }}>
              {t('cash:settleInDailyHint', { defaultValue: 'Close the shift in Daily Settlement (Floor Plan / Live Orders). Full history & corrections: Cash Management (sidebar).' })}
            </div>
          </div>
        </Box>
      )}
