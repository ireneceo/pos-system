...
// 할인 승인 PIN 모달 (#5) — 터치 numpad. 세션 전환 없이 verify-pin-permission 으로 권한만 확인.
// window.prompt(터치 부적합) 대체.
...
interface Props {
  ...
  title?: string;
  onClose: () => void;
  onApproved: (by: string | null) => void;
}
...
const DiscountPinModal: React.FC<Props> = ({ show, restaurantId, title, onClose, onApproved }) => {
  ...
  const verify = useCallback(async (full: string) => {
    ...
    try {
      const res = await fetch('/api/staff/verify-pin-permission', {
        ...
        body: JSON.stringify({ pin_code: full, permission: 'discount_authorize', restaurant_id: restaurantId })
      });
      const j = await res.json();
      if (j?.data?.authorized) { onApproved(j.data.by || null); }
  ...
  return (
    <Overlay show={show} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <Box onClick={(e) => e.stopPropagation()}>
        <Title>{title || 'Discount Approval'}</Title>
        <Subtitle>Enter manager 4-digit PIN</Subtitle>
...
export default DiscountPinModal;
