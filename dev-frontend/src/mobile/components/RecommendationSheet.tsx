...
const Card = styled.button`
  ...
  &:active { transform: scale(0.97); box-shadow: 0 4px 14px rgba(99, 91, 255, 0.14); }
`;
...
const AddPill = styled.span`
  ...
  background: #635BFF; color: #fff; font-size: 18px; line-height: 1;
...
const RecommendationSheet: React.FC<Props> = ({ isOpen, items, currency, cartCount, onClose, onAdd, onGoToCart }) => {
  const { t } = useTranslation();
  if (!isOpen || !items.length) return null;
  return (
    <Overlay onClick={onClose}>
      <Sheet onClick={e => e.stopPropagation()} role="dialog" aria-label={t('itemDetail.recommend.title', 'You might also like') as string}>
        <Handle />
        <Header>
          <Heading>
            <Title>{t('itemDetail.recommend.title', 'You might also like')}</Title>
            <Subtitle>{t('itemDetail.recommend.subtitle', 'Add a little extra to your order')}</Subtitle>
          </Heading>
        ...
        <Cards>
          {items.map(it => (
            <Card key={it.id} type="button" onClick={() => onAdd(it)} aria-label={`${t('itemDetail.addToCart', 'Add to Cart')}: ${it.name}`}>
              <Thumb $img={it.image || null} />
              <CardBody>
                <Name>{it.name}</Name>
                <PriceRow>
                  <Price>{fmtPrice(currency, it.price)}</Price>
                  <AddPill aria-hidden="true">+</AddPill>
                </PriceRow>
              </CardBody>
            </Card>
          ))}
        ...
        <Footer>
          <FootBtn type="button" onClick={onClose}>{t('itemDetail.recommend.keepAdding', 'Keep adding')}</FootBtn>
