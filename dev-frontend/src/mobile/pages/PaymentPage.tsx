    ...
    try {
      ...
      console.log('🔵 Step 4: Processing payment for method:', paymentMethod);
        ...
        if (paymentMethod === 'payAtCounter' || paymentMethod === 'counter') {
          ...
          console.log('🔵 Processing QR / Bank Transfer payment...');
          ...
          try {
            ...
            console.log('💾 Saving QR/Bank order to DATABASE...');
            const response = await fetch('/api/orders', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(dbOrderData)
            ...
            sessionStorage.setItem('paymentPageData', JSON.stringify(orderData));

            // Set order as current but don't clear cart yet (will clear after payment proof upload)
            setCurrentOrder({
              id: savedOrder.id,
              pickupNumber: backendPickupNumber,
              items: cartItems,
