            ...
            <RadioGroup>
              {group.options.map((option: any) => {
                ...
                >
                  ...
                  {so ? (
                    <div style={{ fontSize: '11px', fontWeight: 700, color: '#DC2626', marginTop: '2px' }}>{t('itemDetail.soldOut', { defaultValue: 'Sold out' })}</div>
                  ) : option.price > 0 && (
            ...
            <CheckboxGroup>
              {group.options.map((option: any) => {
                ...
                <CheckboxLabel key={option.id} style={so ? { opacity: 0.6 } : undefined}>
                  ...
                  {so ? (
                    <CheckboxPrice style={{ color: '#DC2626', fontWeight: 700 }}>{t('itemDetail.soldOut', { defaultValue: 'Sold out' })}</CheckboxPrice>
                  ) : option.price > 0 && (
