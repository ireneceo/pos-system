...
- **발주 수령·배송 양방향 확인 + 결제/현금 연결 + 판매자 재고 차감** (2026-09-01 Irene 지시:
  *"다른 건 다 다음 섹션에 설계해서 하게 저장해"*) — **설계부터**.
  ...
  - **A. GIT 이 팔아도 GIT 자기 재고가 안 빠진다 — 프로덕트↔자기재고 연결 부재**
    ...
    · **재고아이템 등록 루트 (Irene 지시)** — 입구 3개, 결과는 하나. 물건이 둘로 갈라지는 걸 등록 단계에서 차단:
      ①프로덕트/메뉴 등록 → 재고아이템 함께 등록  ②공급업체 상품 등록 → 재고아이템으로도 등록
      ③재고아이템 등록 → 공급업체 상품 + 프로덕트/메뉴로도 등록
        (공급업체 목록에 없으면 **그 자리에서 신규 등록**, 프로덕트/메뉴 연결 시 **판매가 등** 추가 입력)
      원칙: 한 물건 = 한 재고아이템(중복 생성 방지 검사) · 레시피 없는 것은 수량을 프로덕트/메뉴에 둔다
      ⛔ 공급업체 원가를 판매가 자리에 복사 금지 · 건수기반 채번(SKU 재발급) 같이 손보기
