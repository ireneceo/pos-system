...
### 확정된 방향 (Irene)
1. **취급단위(g)는 그대로 둔다** — 레시피가 쓰는 단위. 바꾸면 레시피가 다 틀어진다.
2. **발주 응답에 판매자 단위를 싣는다** — `seller_source` 에 판매자 상품의 `unit`·`base_quantity`.
3. **발주 화면은 판매자 단위로 주문받는다** — 가격이 그 단위의 가격이므로.
4. **환산값이 판매자 단위 ↔ 취급단위를 잇는다** (1 kg = 1000 g).
