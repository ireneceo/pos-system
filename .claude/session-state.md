## 현재 작업 상태
**마지막 업데이트:** 2026-09-13 #3 (포장단위 «ea» — 운영 배포 완료 · SW 5.20)
**버전:** 변경 없음 (Irene 미지시) · SW 5.20-package-unit-ea-20260913
**운영:** 번들 `main.90d6117f.js` · SW `5.20-package-unit-ea-20260913` · 백업 `/var/www/backups/20260913_103714` · 10:42 UTC 배포 · 스모크 10/10 · 오늘 배포 2회(10:01 상품 종류 3갈래 / 10:42 포장단위 ea)
**작업 상태:** 배포 완료. ⚠ Fable 판정 미수령(한도 429 · 4회 시도) — 두 배포 기록 `fable_note` 에 명시. `.claude/.fable-gate-skip` 생성은 이 세션 권한 분류기가 차단 → Irene 터미널에서 `! touch /var/www/.claude/.fable-gate-skip` 필요(훅 정지 해제용).
...
### 0-A번. (새로 배포됨 2026-09-13 #2·#3) 상품 종류 3갈래 + 포장단위 ea — 약 5분
1. **판매자 상품 등록·수정 창**에 «상품 종류» 세 개(재고 상품 / 주문제작 / 서비스·기타)가 보이는지
2. «서비스 / 기타» 를 고르면 **취급단위 목록에 `hour`** 가 나오는지
3. **4시간 기본 서비스 넣어 보기**: 종류 «서비스/기타» · 취급단위 `hour` · 포장당 용량 `4` · 기준단위(포장) `ea` → 규격이 «4 hour/ea» 로 뜨고, 1 ea 주문이 4시간인지
4. 기준단위(포장) 칸 목록에 **`ea`** 가 있는지
5. 서비스만 담긴 주문의 판매자 화면에 «**완료 처리**» 버튼만 나오고(«출고 기록» 안내 없음), 누르면 완료로 바뀌는지
- **끝나면 Claude 에게**: «상품 종류 확인했어» (이상하면 번호만)
- 참고: 운영 상품은 전부 «재고 상품» 으로 시작합니다 — 소스류·컨설팅에 종류를 지정하는 일은 지시 대기입니다.
...
- **(2026-09-12~13) B2B 무료 발주 등급 — 운영 배포 4회 완료.** 설계 단일기준 `docs/BUYER_FREE_TIER_DESIGN.md`(§7 구현현황 · §7-0 링크 자리 · §7-1-1 타입검사 되살리는 법).
  - **1차 SW 5.15** — 요금제 «발주 전용(무료)» · 무료는 체험 없음 · 화면 잠금 + 업그레이드 안내 · 판매자 «계약 필요 여부» 설정 · 가맹점 밖 구매자 판매(`distribution_mode='external_buyers'`) · 공급형 계약(`contract_type='supply'`) + 구분 탭 · 주문용 상품 링크(`/shop/:slug` 공개) · 발주 전 법인정보 안내 · 사이드바 잠금 확대 · 폼 필수표시 1단계.
  - **2차 SW 5.16** — 링크 카드 접어두기 · 저장 후 링크가 사라지던 결함(브랜드 조회가 표준 형식이 아니었음) · 접힌 줄에서 바로 복사.
  - **3차 SW 5.17** — 접힌 줄에 «Open»(새 창, noopener).
  - **4차 SW 5.18** — 링크 화면 «가게 이름» 을 판매자가 정함(적은 이름 → 회사명 → 이름). GIT Consulting 링크에 브랜드 «with MIN» 이 뜨던 것 해소(운영 확인: `GIT Consulting`).
  - **고친 급소(전부 고장주입으로 증명)**: ①0원 무료 등급이 요금제 «변경» 목록에 섞여 유료 매장을 내릴 수 있었음 → 가격 0 기준 제외(가입 화면은 유지) ②브랜드 회사정보 저장이 **안 보낸 칸까지 덮어씀**(address_line_2 → null) → 보낸 칸만 수정 ③설정 JSON 통째 덮어쓰기 2곳 → 병합 저장 ④`VALID_MODES` 하드코딩이 ENUM 과 따로 살아 새 값이 저장에서 걸러짐 ⑤사이드바 잠금이 모바일 렌더러 한 곳에만 있어 데스크탑에선 잠긴 칸이 원래 주소로 감.
  - 검증 누계: 실호출 48건 PASS · 고장주입 7건 성립(전부 원복 sha256 일치) · 실브라우저 43건 PASS · `verify-all --full` 최종 **19/19** · mount sweep 크래시 0 · 배포 스모크 10/10 ×4.
  - ⚠ **Fable 판정 미수령**(한도 소진 429). 각 릴리즈 기록 `fable_note` 에 미수령 사실과 팀원 판단 항목을 명시함.
  - ⚠ **타입 검사가 두 겹으로 막혀 있었음**(힙 2048MB OOM + i18next d.ts 가 TS 5.x 문법). 되살리는 법과 기준 439건을 설계 문서 §7-1-1 에 남김. 이번 변경 **신규 타입 오류 0**(파일별 분포 동일).
  - ⚠ 내 검증 프로브가 카드의 «Copy link» 가 아니라 상품 줄 «Copy»(상품 복제)를 눌러 dev 에 복제 상품 11건 생성 → 전부 삭제·잔재 0 확인. 운영 무관.
...
- **포장단위 제안에 `ea` 추가** (SW 5.20 배포) — `utils/unitConversion.PACKAGE_UNIT_SUGGESTIONS` 한 줄, 상품 창 4곳 공용. 서버는 포장단위를 자유 문자열로 저장하므로 값이 고쳐지지 않음. 화면 단위 테스트 20/20.
- **상품 종류 3갈래** (`brand_products.product_kind` ENUM stock/made_to_order/service · SW 5.19 배포) — 재고 안 세는 종류는 출고에서 자체재고 무접촉 · 서비스 전용 발주는 `POST /api/seller-orders/:id/complete` 한 번으로 완료(섞이면 400) · 판정은 `utils/orderFulfillment.isServiceOnlyOrder` 한 곳 · service 일 때만 단위에 hour
...
- 🟡 **주문용 상품 링크의 «배송·픽업 안내» + 기준위치 — 다음 섹션 (2026-09-13 Irene 지시 · 접수만, 착수 안 함)**
  - Irene 지시: 「상품링크에 이런 배송안내 가능해? 그리고 기준위치가 있어야하지? … 넣을 수 있는지 보고 자동으로 배송안내 표시 되는지 확인해줘. 이것도 다음 섹션에서 체크해줘.」 · 문구 확정 「직접 픽업 시 배송비 무료라고 명확하게」
  - 표시 문구(한/영 원문)·실측 6건·갈리는 길 3갈래는 `docs/BUYER_FREE_TIER_DESIGN.md` §6-4 에 그대로 들어 있다. **다음 섹션에서 그 절부터 읽는다.**
  - 실측 요지: 공급업체는 칸이 **이미 있다**(`delivery_policy`·`min_order_amount`·`latitude/longitude`) — 다만 ①구매자가 «외부 공급업체» 등록할 때만 쓰이고 판매자 설정 화면엔 없음 ②공개 링크 응답(`routes/public.js:665`)이 안 실어서 **자동 표시 안 됨** ③**브랜드는 칸 자체가 없음**(Irene 링크가 브랜드 줄이라 이게 관문) ④운영: 공급업체 41건 좌표·정책·최소주문 전부 0, 매장 22곳 중 좌표 11.
  - 거리 구간 자동 판정은 공개 링크(로그인 전)에서는 구매자 위치를 몰라 불가 — 링크는 문구+기준위치, 구간 판정은 로그인 후 발주 화면 범위.
  - ⛔ 최소주문·배송비는 돈에 닿고 설정 위치(새 칸 vs operation_settings)는 되돌리기 어려움 → **Fable 판정 대상**으로 표시.

- 🟡 **판매자 자체 재고의 «재고 단위» 칸 — Fable 판정 대기 (2026-09-13 접수, 착수 금지)**
  - Irene 원문: 「프로덕트 상세에 재고관리에 단위를 왜 또 따로 넣어? 여기서 선택하면 그 기준으로 베이스 유닛 단위가 맞춰져야 하는 거 아니야? 아니면 패키지 단위? 이거 어떻게 되는거야? 이것도 중요하니까 fable 에게 남겨두자.」
  - 단일 기준 문서 = `docs/TRADE_STRUCTURE.md` §5-12 — Irene 원문 · 팀원 실측 6건 · 갈리는 길 3갈래(A 칸 없애고 주문단위로 못 박기 / B 환산 구현 / C 라벨 유지+안내) · 공통 쟁점 4개가 그 절에 들어 있다. **Fable 에는 그 절을 통째로 넘긴다.**
  - 요지: `stock_unit` 은 환산에 안 쓰이는 **라벨**이고, 실제 차감은 `current_stock -= quantity_ordered`(주문 단위=포장단위 기준)다. 운영 81행 중 78행이 빈 칸 — 아무도 다르게 쓰지 않는다. 공급업체 쪽은 칸 자체가 없고 원장 라벨만 내용물 단위다.
  - ⛔ 팀원은 길을 고르지 않는다(재고 숫자의 의미 변경 = 비가역 · 길 갈림 = 3축 성립).

- 🟡 **준비된 재고(1차 가공 재고) — Fable 설계 판정 대기 (2026-09-12 접수, 착수 금지)**
  - Irene 지시: 「이거 제대로 저장해둬서 fable 다음 작업으로 준비해놔」 → **다음에 Fable 을 띄울 사안은 이것.**
  - 단일 기준 문서 = `docs/RECIPE_MANAGEMENT_SYSTEM.md` «준비된 재고(1차 가공 재고)» 절 — Irene 원문 2건 · 팀원 실측 9건 · 갈리는 길 3갈래 · 공통 쟁점 4개가 그대로 들어 있다. **Fable 에는 그 절을 통째로 넘긴다.**
  - 요지: 재고아이템으로 **준비된 재고**(예: 소고기+간장+마늘 → 불고기 양념육)를 만들고, 그 준비된 재고도 재고관리가 되어야 한다. 3kg 재면 실제 2.5kg = **수율**. 브랜드 레시피·매장 메뉴 레시피가 재고아이템 말고 이 준비된 재고도 재료로 쓸 수 있어야 한다.
  - **범위(Irene 확정)**: `recipes` 표 한 곳(owner_type brand·restaurant). ⛔ **프로덕트 레시피(`product_recipes`) 는 제외** — 준비된 재고 필요 없음. (팀원이 한 번 프로덕트 레시피를 끌어들여 정정받음 — 다시 끌어들이지 말 것.)
  - 실측 급소 3개: ①`recipe_ingredients` 는 **재료만 가리킨다**(레시피를 가리키는 칸 없음) ②`recipes.yield_amount/yield_unit` 칸은 **이미 있는데 원가·차감 어디에도 안 쓰이는 죽은 칸** ③차감은 1단계뿐(`inventoryDeductionService`, 수율 개념 0).
  - ⛔ 팀원은 길을 고르지 않는다(재고·원가 = 파급 크고 비가역 · 길 갈림 = 3축 성립). Fable 판정 뒤 문서 절을 고쳐 쓰고 착수한다.
  - Irene 순서 지시: **위 «Irene 님이 하실 일» 체크가 끝난 뒤** 이 안건을 진행한다.

- ✅ **무료 구매자 등급(B2B 유입) — 2026-09-13 운영 배포 완료.** 아래 기록은 설계 근거·Irene 원문 보관용(손댈 것 없음).
- **다음 후보 (Irene 지시 대기 — 자동 착수 금지)**
  - **링크 화면이 빈 이유 안내**: 운영 GIT 은 «가맹점 밖 구매자» 로 내놓은 상품이 **0개**라 링크 화면이 «No products are listed yet.» 이다(실측: `all` 77 · `specific_brands` 29 · `external_buyers` 0). 상품에서 고르면 바로 나온다. 카드에 «아직 내놓은 상품 0개 — 상품에서 고르세요» 를 띄우는 것을 Irene 이 「여기까지」 하라 하여 **착수하지 않고 기록만** 함.
  - **링크를 어느 단위에 붙일지**: 지금은 브랜드 줄(`brands.shop_slug`)에 붙는데 **보여 주는 상품은 주인 계정 전체**다. 회사에 브랜드가 여럿이면 어느 줄에 붙일지가 갈린다 — 구조 판단 필요(Fable 대상).
  - **폼 «필수» 표시 전수 정리 2단계**: 자주 쓰는 폼(상품·재료·재고·매장 설정) → 나머지.
  - **타입 검사 상시화 여부**: 되살리는 법과 기준 439건은 문서에 있음. 게이트로 세울지는 별도 판단.
  ...
  - **Irene 답(2026-09-12, 확정)**: ①**매장 1곳 = 가입 1건** — 「이게 우리 구조잖아… 여러 레스토랑 등록시켜서 발주시키게? 그러면 나중에 레스토랑으로 연동 어떻게 해?」(내가 물은 이유 = 오너 다매장 구조 때문이었으나, 발주 주체가 매장이고 유료 전환 시 그 매장이 그대로 올라가야 하므로 1매장 1가입이 맞다. 재질문 금지) ②**공급형과 공급업체는 기준이 같아야 하고, 계약 필요 여부는 설정으로** — 「무조건 계약해야 판매하지 않는 경우도 많잖아」 → 판매자(공급업체·브랜드)가 «계약 필요/불필요»를 설정으로 고른다. 실측: 두 표 모두 전용 칸 없음, 그러나 **`operation_settings` JSON 칸이 양쪽에 이미 있음**(같은 패턴) → 새 칸 신설 없이 설정 키 하나. ③**폼 필수 표시 전수 정리 — 「알아서 다 해」**.
