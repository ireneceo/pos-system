...
## 현재 작업 상태
**마지막 업데이트:** 2026-05-18
**버전:** **v3.33** (운영 배포 완료) + dev 누적 (Recipe ReferenceError fix · 모바일 footer 링크 · Reports menu aggregation, **운영 미반영 — RA Recipe 시급 hotfix**)
**작업 상태:** 완료 (헤드리스 sweep 95 페이지 mount 검증 / 23/23 API 검증 / health-check 80/80 / 빌드 main.ed61afb7.js), Irene `/배포` 명령 대기
...
**v3.33 직후 dev 누적 (운영 미반영, /배포 대기)**
- **RA Recipe Management 5 탭 ReferenceError 시급 fix** — v3.32 alert→Modal sweep 잔여 결함. 운영 매장 4곳 모두 Recipe Management 진입 시 ErrorBoundary 폴백. 5 파일에 `infoModal` useState 한 줄씩 추가
- 모바일 OrderTypePage Footer 링크 — 로그인 상태별 Back to Dashboard / Visit Homepage
- BG/FG/Owner Reports `/api/menu` aggregation fix — restaurantId 없이 호출하던 결함. allowedRestaurantIds 순회 + 카테고리 dedup
- 헤드리스 자동 mount sweep 도구 (Playwright) — RA/BG/Admin/FG/Owner/Supplier/FCM/BM 95 페이지 검증 도구화

### 다음 확정 작업
- 없음 — 지시 대기
  (Irene: 이번 세션 dev 누적 변경 운영 hotfix 배포는 `/배포` 명령 시점에 진행. Recipe ReferenceError 가 운영 매장 4곳에 영향 중 — 시급도 높음)
...
- 다른 역할(Admin/FG/Owner/Manager/Supplier)의 인터랙션 후 발생 가능한 결함 — 헤드리스 sweep 으로 잡지 못함. 별도 검증 도구 필요 시
- zh/ms i18n 영어 잔존 690건 sweep (v3.31 운영 critical 아님)
