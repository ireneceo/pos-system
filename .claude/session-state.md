...
### 진행 중인 작업
- 없음. (임대료 청구 **v3.69 운영 배포 완료** · 설정 가짜탭 정리분은 dev·미배포)
...
2. **Fable 사후 점검** — v3.68·v3.69(임대료) 둘 다 게이트 대상이었으나 Irene 지시로 배포. 대조 항목 = 아래 "★ Fable 검증 대상" 블록.
3. **미배포 dev분 배포** — 설정 가짜탭 제거(매니저 Company/Brands/Billing) + 매니저 구독 'Manage Billing' 제거. 검증 후 `/배포`.

#### B. 배포 인프라 결함 (다음 배포 전 반드시)
4. **🔴 마이그레이션이 배포 중 실행되지 않았다** (2026-07-11 v3.69) — `migrate-rent-category.js` 가 운영에 복사됐으나 **실행 안 됨**(수동 실행으로 복구). 배포 로그를 `tail -40` 으로만 저장해 원인 구간이 잘려 진단 실패. **다음 배포 때 마이그레이션 구간 전체 로깅 후 원인 규명.** 이번엔 카테고리 행 1개라 수동 복구가 쉬웠지만, 스키마 변경이었으면 위험했다.
