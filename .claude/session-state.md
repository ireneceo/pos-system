## 현재 작업 상태
**마지막 업데이트:** 2026-08-31 #2 (발주·인보이스 단위/공급업체명 · dev 완료 미배포)
**버전:** v3.80 (2026-08-28 배포). 8/30 배포 5회는 전부 버전 미상승 — Irene 지시
**작업 상태:** **오늘 4건 전부 운영 배포 완료. 개발 잔여 없음. 수동 조치 대기 0.**
  ① 알림 3종 (06:53) ② 발주·인보이스 단위/공급업체명 (08:20, 마이그 64/64) ③ SW 4.66 (08:37)
  ④ 실브랜드 운영자 자동복구 대칭화 (10:03) — **손으로 할 일이 사라졌다**

> ### 🔔 2026-08-31 #2 — 발주·인보이스 단위/이름 (최신)
> Irene 원문: *"리스트에 숫자 위에 단위도 표시해야 할 것 같아. 인보이스랑 모든 곳에 다 단위 따라다녀야 할 것 같아.
> 그리고 공급업체에 보내는 건 우리 표시이름은 없어도 되지 않아? 그리고 여기에 보이는 리스트도 공급업체 상품명으로 표시
> 또는 우리 재고명으로 표시 라는 표시방법 2가지 따로 있으면 안돼? 인보이스는 모두 공급업체에 보내는 건 모두 공급업체 상품표시"*
> 추가: *"이 내용은 푸터에 하단에 붙어서 계속 하단 따라다녀야지. 1 POs / RM 81.00 / Submit All"*
> 기존 인보이스 처리 방침: *"B로 해. 아직 발주관리 제대로 안해서 바꿔도 돼."* → 기존분도 전환.
>
> **착수 전 실측이 절반을 줄였다** — 공급업체 판매품목명 유틸(`utils/sellerProductIdentity`)이 이미 있었고
> **staging 목록 API 는 이미 `seller_product_name`·SKU 를 내려주고 있었다**(화면이 안 쓰고 있었을 뿐).
> 발주 라인 `unit` 도 이미 저장돼 있었다(dev 127건 중 119건). 인쇄본은 이미 공급업체명 우선이었다.
>
> **완료 5건 (dev, 커밋 `2f0b1e0` 계열 — 미배포)**
> 1. **단위 표시** — staging 목록 `× 2.5 kg`. 인보이스 화면·인쇄본 수량 옆에 단위.
> 2. **`invoice_items.unit` 신설** — 발주 라인 단위를 인보이스로 운반(그전엔 통째로 버려짐).
> 3. **공급업체 문서 = 공급업체 상품명** — 거래 인보이스(`purchaseOrderService.createTradeInvoice`)와
>    공급업체 메일(`utils/poEmailItems`)이 `seller_product_name` 사용, **우리 내부명 제거**.
>    매핑 없는 라인만 내부명 폴백. 이름이 실제로 다름: `Beef Rib`↔`Australian Beef Rib`, `Tomato`↔`Fresh Tomato`.
> 4. **표시방법 토글** — `공급업체 상품명`/`우리 재고명`, 기본 seller, localStorage 기억.
> 5. **푸터 하단 고정** — `sticky bottom:0` 이 있었는데도 안 붙던 이유 = 이 바가 Container 의 마지막 자식이라
>    **콘텐츠 끝이 곧 정착점**이었다. Container 를 flex column + Content flex:1 로 전환. safe-area 패딩 추가.
>
> **🔴 덤으로 잡은 잠재 결함 — `invoice_items.quantity` 가 `int` 였다**
> 단위만 붙이고 수량이 정수면 `2.5 kg` 를 못 적는다. 방금 배포한 BG 단위주문(2.5kg→RM75)이 인보이스로
> 넘어가는 순간 **2 로 조용히 깎일** 상태였다(피해 0 — 소수 발주 라인이 아직 0건).
> DECIMAL(10,2) 로 확장. **금액 무영향 확인**: 인보이스 합계는 `utils/invoiceCalculation` 이
> `calculated_amount` 를 더해서 낸다(quantity × unit_price 아님). 실증: 2.5 저장 → 2.50 조회.
> 표시 회귀 방지로 API 직렬화 6곳을 `Number(item.quantity ?? 1)` 로(안 하면 기존 `1` 이 `1.00` 으로 보임).
>
> **기존 인보이스 전환 (B 지시)** — `purchase_orders.trade_invoice_id` FK 로 짝을 맞춰 재작성.
> **인보이스 22건 / 라인 29건 변경**, 재실행 0건(멱등). 짝 맞는 라인 13건 전수 PASS(이름 13/13·단위 13/13·수량 13/13).
> ⛔ **라인 수 불일치 6건은 의도적 스킵** — 짝을 못 맞춘 채 옮기면 엉뚱한 줄이 바뀐다. ⛔ 금액은 무접촉.
>
> **검증** — `verify-all --full` **16/16**. 실브라우저 확인(dev, 픽스처 생성→확인→삭제, 잔재 0):
> 기본 `Australian Beef Rib × 2.5 kg` / 토글 후 `Beef Rib × 2.5 kg` · 푸터 800/800 고정 ·
> 스크롤 끝에서도 고정 · 콘솔 에러 0.
>
> **⚠ 검증 중 내 실수 3건(자진 보고)**
> - 첫 인보이스 판정이 **교차곱**이라 "13/28 실패" 오판 → ROW_NUMBER 로 라인 짝 맞춰 재측정, 13/13 PASS.
> - 첫 브라우저 확인에서 단위 0건 → **dev draft 발주에 품목 줄이 아예 없어서**였다(코드 문제 아님).
> - **디자인 규칙 위반**: 토글에 로컬 버튼 스타일 컴포넌트를 새로 만들었다 → design-guard 적발,
>   **우회 없이 공용 `components/UI/Button` 으로 교체 + 재빌드 + 화면 재확인**.
>   교체 후에도 1건 남길래 보니 **내 주석 문구**가 검사 패턴에 걸린 것 → 문구 수정(예외 등록 아님).
>
> **📌 Fable 게이트 대상** (기준 ② 돈·주문 무결성 + ③ DB 마이그). Fable 한도 소진 → Opus 판정, 마커에 명시.
>
> **✅ 운영 배포 완료 (2026-08-31 08:20 UTC)** — 스모크 10/10 · 마이그 **64/64** · ENUM 소실 0 ·
> 백업 `/var/www/backups/20260831_080523`. 운영 반영 실측:
> `invoice_items.unit` varchar(50) 신설 · `quantity` **decimal(10,2)** 확장 · 4언어 도달 ·
> 지연청크(`4670.c8c6c4e6.chunk.js`)에서 `Supplier product name`·`seller_product_name` 검출.
>
> **📊 운영 실태 — 기존 인보이스 전환 대상은 0건이었다** (dev 는 22건). 이유: 운영 발주 10건 중
> **`trade_invoice_id` 보유 0건** = 아직 발주에서 인보이스가 만들어진 적이 없다(Irene "아직 발주관리
> 제대로 안해서"와 일치). **앞으로 발행되는 인보이스부터 적용**된다.
> 운영 발주 라인 24건은 **전부 공급업체 연결 보유 + 전부 단위 보유**(can/pack/piece) → 화면에 즉시 반영.
>
> **⚠ 내 실수 4번째 — SW 버전을 안 올리고 배포했다.** 운영·배포분 둘 다 4.65 라 캐시한 기기가
> 옛 캐시를 안 비우는 상태였다(직전 세션에 내가 지적하고 고쳤던 바로 그 사고를 내가 반복).
> 발견 즉시 **4.66 으로 올려 재배포**(08:37 UTC, 스모크 10/10) — 운영 SW `4.66-po-unit-seller-name-20260831` 확인.
> 📌 **규칙화: 프론트가 바뀐 배포는 SW_VERSION 을 반드시 같이 올린다.**

---

> ### 🔔 2026-08-31 #1 — 알림 3종 (배포 완료)
...
> - 운영 2행 UPDATE = **수작업 자체가 없어졌다** (2026-08-31 10:03 배포).
>   Irene *"배포할 때 운영서버가 알아서 해?"* → 실측하니 **두 축이 비대칭**이었다:
>   매장 축은 REAL 절에 있어 이미 찍힌 is_test 를 배포가 자동으로 풀어 줬는데(08:20 배포에서
>   r8:Wai·r8:James 자동 복구가 그 증거), 브랜드 축은 TEST 절 제외 조건에만 있어
>   "새로 안 찍는다"까지였고 **푸는 경로가 없었다.** → REAL 절에 브랜드 레그 추가로 대칭화.
>   ⛔ "test 전부를 real 로" 같은 넓은 복원은 넣지 않았다(시험계정 실메일 = 역방향 사고).
>   사전 폭발반경 실측 = 정확히 2계정. 리허설 4/4(복구·연속3회·역방향 안전).
>   **운영 결과: id 11·29 `is_test` 1→0 · 총계 32→30 · 지정 데모/테스트 10개 표시 보존 ·
>   알림 관문 판정 "통과 — 알림 발송 가능".** 인수인계 문서는 불필요해져 삭제.
>
