...
- **발주 수령·배송 양방향 확인 + 결제/현금 연결 + 판매자 재고 차감** (2026-09-01 Irene 지시:
  *"다른 건 다 다음 섹션에 설계해서 하게 저장해"*) — **설계부터**.
  ...
  - **A. GIT 이 팔아도 GIT 자기 재고가 안 빠진다 — 프로덕트↔자기재고 연결 부재**
    ...
    · **재료 vs 재고 아이템 구분 (Irene)**: *"브랜드 레시피에 연결되는 재료가 해당 브랜드의 재고가 되는 것은
      별개의 문제야. 브랜드 재료를 가져가서 레시피 연결해서 재고가 되는 건 재고 아이템인거잖아."*
      - **브랜드 재료** = 레시피에 연결되는 것 → `ingredients`(brand 2) **146건, 수량>0 = 0건**
      - **재고 아이템** = 실제 재고, 발주 입고가 쌓임 → `product_ingredients`(owner 23) **288건, 수량>0 = 23건**
      - 브랜드 레시피(Recipe)→브랜드 재료 / 프로덕트 레시피(ProductRecipe)→재고 아이템. **이름 겹침 63건**
      ⇒ **①지금 할 일** = 판매상품 → **재고 아이템** 연결 / **②별개** = 브랜드 재료가 재고 아이템이 되는 경로
