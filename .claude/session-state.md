...
## ⏳ 고침 (dev·미배포·**Fable 2차 판정 대기**) — 판매자 발주 가시성 + 상태 표시

### 함께 들어간 것: 발주 상태 표시 (Irene 지시)
> "배송완료 뒤에 recived가 상태표시는 이상해. 브랜드제너럴에게는 완료가 되어야지
> 그리고 결제가 안되었으면 표시해야 하고 레스토랑 관리자처럼 주문상태랑 결제상태 따로 해줘."

`IncomingOrdersView.tsx` — **프론트만**(백엔드 무변경, `payment_status` 는 이미 `po.toJSON()` 으로 실려 온다):
- 판매자 관점 라벨: `received`→**완료** · `partial_received`→**부분 완료**. **상태값 자체는 안 건드린다**(표시만).
- **결제 열 신설** — 미결제(주황)/결제완료(녹색)/환불(빨강). i18n 4언어(brand·supplier·foodcourt).

### ⚠ Irene 계정 정정 — 내가 틀렸다
Irene 은 **`help@gitconsulting.group`(user 23)** 으로 로그인한다. 이 계정은 `brands.owner_id=23` 으로
**브랜드 1·2 의 소유자**다. 내가 `irene@gitconsulting.group`(user 11, 소유 0)을 근거로 "통일할까요?"를
물었고 Irene 은 질문 자체를 이해하지 못했다("이게 무슨 말인지 모르겠어") — **설명 실패**.
→ Q3(`brandScope` 통일)은 지금 실익이 없어 손대지 않았다. 형제 로직은 유지(비소유 계정이 생기면 같은 사고).
→ **"그래도 안 나온다"의 답은 "아직 배포를 안 해서"** 다. 고친 것은 dev 에만 있다.

### 최종 검증
`verify-all --full` **18/18 전 게이트 통과** · health-check **227/227**(신규 2건) ·
mount sweep 8역할 크래시 0 · 인쇄 8/8 무변경 · SW **4.87-seller-scope-status-20260907**

### Fable 1차 지시 반영
- **반품 화면 빈틈**(Fable 적발) — `po-returns.js` 2곳이 단일 id 비교라 "목록엔 보이는데 반품은 404" 였다
- 세 벌 복제된 판정 → `ownsPurchaseOrder()` **함수 하나**로 (`sellerScope.js` export)
- **BM 은 소속 브랜드 하나 유지** — 2026-09-06 판정과 갈리지 않게 BG/BM 분기 분리
- 회귀 케이스 2건 영구 추가 · **고장주입 2/2**(형제 제거→노출 FAIL · 주인조건 제거→**타 조직 100건 누출** FAIL)
...
**⚠ 1차 수정만으로는 Irene 계정이 여전히 못 본다** — 운영 실측:
| 계정 | brand_id | 소유 | 1차 후 |
|---|---|---|---|
| `irene@gitconsulting.group` | 1 | **[]** | **[1] — 여전히 안 보임** |
| `help@gitconsulting.group` | 1 | [1,2] | [1,2] 보임 |
(현행 코드에서는 **두 계정 다 안 보임**. `/brands?owner=me` → irene@ 0건 · help@ 2건.)

**2차 — Irene 요구 "여긴 모든 브랜드 주문 다 나와야 해"**: 범위를 세 갈래 합집합으로.
① `brands.owner_id = 나` ② `user.brand_id`(소속) ③ **②와 주인이 같은 형제 브랜드**

**증명(dev)**: 소속만 있고 소유 0 인 계정으로 — 대표 브랜드에 0건인 상태(`shipped`)로 좁힌 목록에
형제 브랜드 발주 **1건 정확히 노출** · **남의 조직 브랜드 누출 0건** · health-check **225/225** · 인쇄 8/8.
⚠ 목록 전체 조회로는 판별이 안 된다(브랜드 1 발주가 100건 상한을 채운다) — 상태로 좁혀야 보인다.

**Fable 에 올린 판단 3건 (내가 정할 게 아니라서)**
1. `siny@purplehere.com`(브랜드 4 소속·소유 0)이 새 규칙이면 **브랜드 10·12 발주까지** 보게 된다. 의도인가?
2. **Brand Manager 도 같은 범위**가 된다 — 2026-09-06 "BM 은 `user.brand_id` 하나" 판정과 갈린다.
3. 범위를 `sellerScope` 에만 넓혀서, `irene@` 은 판매자 화면은 열리는데 `brandScope` 쪽은 여전히 403
   (`/brands?owner=me` 0건). **한 사람이 화면마다 다른 범위**를 갖는다. 통일할지 결정 필요.
...
> 취급단위는 g이어도 상관없잖아. 발주하는 단위때문에 문제인데.
> 발주화면은 무조건 판매자 단위어야 가격이랑 맞지"**

### 신고
GIT 프로덕트 `K-Kimchi Sauce for Fried Rice 1kg`(kg·1.00·RM27.90, `order_mode='pack'`)를
K-DINE 관리자가 발주하면 화면에 **`g`** 으로 뜬다. 가격은 kg 가격 그대로.

### 실측한 원인 (코드)
`routes/purchase-orders-crud.js:418` 발주 제안 응답이
`ingredient.unit`(= **취급단위** g)만 내려보내고 **판매자 단위를 아예 안 싣는다**.
`seller_source` 에는 `unit_price · unit_conversion · min_order_quantity · lead_time_days` 뿐.
→ 화면이 가진 단위가 취급단위 하나라 g 을 띄우고 kg 가격을 붙인다.
기준단위(`package_unit`)는 쓰지 않는다(K-DINE 재료는 아예 NULL, with MIN 은 `piece`).

**두 번째 원인**: `utils/catalogLink.js:56 resolveUnitConversion` — 값이 없으면 **무조건 1**.
단위 짝(kg↔g)을 보고 1000 을 유도하는 코드가 없다. 그래서 매핑 19건이 환산 1로 남았다.

### 확정된 방향 (Irene — 2차 정정이 최종)
> Irene 원문(최종): **"취급단위 바꾸는 게 아니네. 포장단위, 발주정보는 공급업체 기준.
> 발주페이지는 다 포장단위여야 하네"**

1. **취급단위(`unit`, g)는 그대로 둔다** — 레시피가 쓰는 단위. 바꾸면 레시피가 다 틀어진다.
2. **발주 페이지 전체가 포장단위(`package_unit`) 기준** — 수량·단가·합계 다. 지금은 응답에
   `ingredient.unit`(취급단위)만 실려 포장단위도 판매자 단위도 화면이 못 본다.
3. **포장단위·기준양은 공급업체(판매자) 기준으로 채워진다** — 매핑을 만들 때 판매자 상품의
   `unit`·`base_quantity` 를 매장 재료의 `package_unit`·`package_quantity` 로 가져와야 한다. 지금은 안 가져온다.
4. **환산값이 포장단위 ↔ 취급단위를 잇는다** (1 kg = 1000 g). 지금은 비면 1.

⚠ 내가 1차에 "취급단위를 바꾼다"로 잘못 적었다가 Irene 이 정정했다. **바꾸는 것은 포장단위 쪽이다.**

### 포장단위 실측 (판매자 단위와 맞나)
| 매장 | 비어 있음 | 판매단위와 다름 | 일치 | 전체 |
|---|---:|---:|---:|---:|
| **K-DINE IPC (브랜드 구매)** | 3 | **7** | 1 | 11 |
| with MIN Cafe (브랜드 구매) | 21 | **14** | 84 | 119 |
| with MIN Cafe (공급업체 구매) | 23 | **110** | 234 | 367 |
| 브랜드 자체 행 | 0 | 0 | 9 | 10 |

→ **브랜드 자체 행은 9/10 정상**이라 BG 화면에서는 멀쩡해 보였다. 매장 쪽만 안 따라와 있다.
→ 공급업체 구매까지 합치면 **어긋난 것이 131건 · 빈 것이 47건**. 브랜드만의 문제가 아니다.

### 영향 범위 (운영 실측)
| 매장 | 환산이 1로 비어 있는 매핑 | 품목 |
|---|---|---|
| K-DINE IPC Branch | **5** | Glass Noddle · K-Gochujang · K-Mannani · K-Soybean · K-Sundubu |
| with MIN Cafe | **14** | K-소스 12종 + Glass Noddle 등 |

- 브랜드 자체 행 10건은 환산 **1000 으로 정상**. K-DINE `K-Kimchi`(재료 #1106)는 취급단위가 kg 이라 환산 1 이 맞다.
- **수령된 건 0건** — 수령했으면 재고가 1000배 적게 들어갔다.

### ⚠ 지금 나가 있는 발주 `PO-R8-20260907-001` — 라인 전수 실측
**금액은 전부 맞다** (라인 합계 1,338.40 = 헤더). 틀린 것은 **환산값 2건뿐**:
| 품목 | 주문 | 라인단위 | 환산 | 수령하면 |
|---|---:|---|---:|---|
| K-Sundubu Sauce 1kg | 6 | g | **1** | 6 g 만 입고 ⚠ |
| K-Mannani Sauce 1kg | 2 | g | **1** | 2 g 만 입고 ⚠ |
| K-Jjamppong·Gochujang·Soy·Bulgogi | | g | 1000 | 정상(→ ×1000 g) |
| K-Kimchi(취급 kg) · MW R12 · 소스통 | | kg/pack/piece | 1 | 정상 |

**Irene 지시: "너가 취소하고 새로 다시 만들어줘. 똑같이"**
→ ⛔ **지금 똑같이 만들면 똑같이 틀린다** — 새 발주도 같은 매핑을 복사한다.
   순서: **환산 2건(매핑 #2 K-Sundubu · #4 K-Mannani) 수정 → 재생성 → 옛 발주 취소.**
   (재생성을 먼저 하고 취소를 나중에 — 중간에 발주가 없는 순간을 만들지 않는다.)
   운영 DB 쓰기가 차단이라 미실행. **수령만 안 하면 손해 없다.**
...
- 이미 제출된 발주 라인 처리
- 발주 화면이 판매자 단위로 바뀔 때 기존 발주 이력·재고 계산과의 정합
