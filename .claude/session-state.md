## 현재 작업 상태
**마지막 업데이트:** 2026-08-31 #3 (발주 운영 실사용 피드백 · 배포 7회 · 미배포 3건)
...
>
> **🔴 미배포 — dev 완료, 빌드 대기 (메모리 게이트)**
> `deploy-dev.sh` 메모리 게이트가 막음: `/opt/planq/dev-frontend` 의 `tsc -b` 가 **4.1GB** 점유,
> 가용 1.5GB < 필요 2.5GB. **PlanQ 는 같은 서버의 별개 프로젝트라 임의 종료 금지.**
> Irene 지시 = "A(기다렸다 자동 빌드) 하고 배포하고 /개발완료".
> ⛔ `SKIP_MEMGATE=1` 우회 금지 — 서버 프리즈 방지 장치([[reference_server_freeze_memory_guard]]).
>
> 미배포 변경 3건 (`PurchaseOrderStagingPage.tsx` · `PurchaseOrdersPage.tsx`):
...
### 다음 확정 작업
- **재고 대시보드 "입고 예정" 표시** (Irene: "2번 다음 섹션에 하자") — 위 🔔 #3 참조. 설계부터.
- 미배포 3건 빌드·배포 완료 (메모리 게이트 해소 후)
