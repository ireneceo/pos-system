...
- **발주 수령·배송 양방향 확인 + 결제/현금 연결 + 판매자 재고 차감** (2026-09-01 Irene 지시:
  *"다른 건 다 다음 섹션에 설계해서 하게 저장해"*) — **설계부터**.
  ...
  - **A. 판매자 재고 차감 근본 — 입고 버킷과 출고 버킷이 다르다**
    ⚠ 내 1차 진단("연결이 아예 없다")은 **틀렸다.** Irene 정정: *"깃 프로덕트를 팔았으면 당연히 깃의
    재고에서 나가야지. 재고관리가 프로덕트랑 연결되는 거잖아. … 레시피 있고 없고에 따라 다르고
    레시피 연결은 재료 연결된 것에서 빼는 거고"*
    **연결은 실재한다** — `ingredients.brand_product_id` (brand 1 은 **63/63 전부 연결됨**).
    **문제는 수량이 그 행에 없다는 것**: 연결된 `ingredients` = 전부 0.00 /
    실제 수량은 `product_ingredients`(투명컵 #307·뚜껑 #308 = 18 pack)에 쌓여 있다.
    GIT 이 UGS 에서 받으면 `product_ingredients` 로 들어가고, 팔 때는 다른 버킷을 본다.
    = [[project_brand_stock_two_lists_split]] 와 같은 뿌리. **어느 쪽을 단일 소스로 삼을지가 핵심 결정.**
    추가 결함: 6품목 중 **4개는 미러 재고 행 자체가 없다**(8/28 등록 스크립트가 sync 를 안 불렀다.
    `sync_to_ingredients=1` 인데 미러 0 = 플래그와 실제가 어긋남).
    ⛔ `brand_products.current_stock` 은 재판매 경로에서 쓰지 않는다.
