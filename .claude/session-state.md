## 현재 작업 상태
**마지막 업데이트:** 2026-09-06 16:00 UTC
**버전:** SW `4.84-cost-propagation-20260906` (프론트 무변경 — 버전 미상승)
**작업 상태:** **운영 배포 2회 완료** — ①메일 반송+배포 침묵(14:51, 커밋 `2e879df10`) ②잔여 정리(15:49, 커밋 `7be034d36`)
배포 스냅샷 = git 완전 일치(check-sensitive-diff **0건/0건**). 워킹트리 clean.
...
### 2차 배포 — 잔여 정리 (2026-09-06 15:49, 커밋 `7be034d36`) — 위 잔여 4건 **전부 해결**
Irene "다 해줘" 지시. 마이그 78/78 · PM2 재시작 확인 · 스모크 10/10 · 백업 `20260906_154455` · 마이그 로그 78개 보존. **프론트 번들 무변경**(`main.bdaeea90.js`) → SW 미상승·릴리즈 노트 생략(Irene 지시).

- **수렴 검산 확대** — 매핑만 보던 것을 **레시피 줄·재고**까지(`QTY_MAX` DECIMAL(10,4) · `STOCK_MAX` DECIMAL(10,2)). 직전 배포는 반쪽이라 줄 수량이 넘치면 여전히 멈췄을 것.
- **요약 카운터** `changed && !plan.review` — 강등 행을 "바뀌는 행"으로 세던 표시 결함.
- **채번 잔여 7곳 이관** — general-stock·inventory-extra·supplier-products·supplier-directory·system-products·subscriptions·invoiceScheduler. 인보이스 형식 보존 위해 `generateCode` 에 **`separator` 옵션** 신설(`INV-2026090001` 유지).
  ⚠ **Fable 적발**: `whereClause.owner_id` 범위 추출이 없어 general_stock 이 **전 BG 시퀀스 하나를 공유**. 한 줄 추가로 수정, 2-owner 증명(둘 다 `GS-003`)·고장주입(`GS-004` 재현).
- **달력 실브라우저 spec** `dev-frontend/e2e/calendar-month-label.spec.js`.
  ⚠ Fable 이 지시한 `Asia/Seoul` 로는 **고장주입이 통과했다** — 밀림은 UTC 보다 **뒤진** 존에서만 난다. `America/Los_Angeles` 로 바꿔 `August` 기대에 `July` 로 잡음. 기대값은 트리거 라벨의 날짜 문자열에서 뽑아 Date 를 안 거친다.
- **배포 준비 검사** — Playwright 스펙까지 "프론트 변경"으로 세어 SW 상승을 요구하던 것. `^dev-frontend/e2e/` 만 최소 제외.
- **rsync `--exclude 'logs/'`** — `*.log` 만으로는 운영 감사 로그(`.json`)가 `--delete` 에 지워졌다.
- **heavy-task-gate `build` 분기에 `heavy_node_running()`** — 08-20 에 만들어 놓고 emulator 분기에만 걸어 둬 PlanQ 빌드(4GB)를 못 봤다.
- **구조 문서** `docs/TRADE_STRUCTURE.md` §5-1 갱신 + §5-8(카탈로그 상한)·§5-9(채번)·§5-10(BM 접근, 돈 경계 경고)·§5-11(수렴이 사람 몫으로 넘기는 규칙) 신설.

**검증**: verify-all 17/17 · 인쇄 보호파일 8/8 무변경 · 고장주입 6묶음 전부 원복 확인 · 인보이스 실발행 1회 실호출(`INV-2026090001`, 발행분 삭제) · 채번 동시 10건 고유 10/10. **Fable 게이트 PASS**(마커 `d0c28bbe78a6`).

**이번에도 내가 저지른 것**: 인보이스 검증 스크립트가 반환값 모양을 짐작해 `undefined` 로 던져 정리 단계를 못 밟았다(인보이스가 잠시 남음 → DB 직접 조회로 확인·삭제). 달력 spec 은 시간대 방향·기대값 기준·단언 범위를 **세 번 고쳐서야** 결함을 잡았다.
...
### 다음 확정 작업
1. ~~구조도 갱신~~ — **완료**(2026-09-06, §5-1 갱신 + §5-8~5-11 신설).
2. 다음 묶음 — 아래 3건이 **미착수**로 남음(채번 잔여 7곳은 완료, 중복 코드 백필은 Irene 지시로 안 함):
   - **BM 접근 모델(안전한 투영)** — `GET /api/brands` 가 결제 설정·은행 계좌·구독 금액까지 내려주므로 그냥 넓히면 돈 경계가 뚫린다. `brands?owner=me` 호출부 전수 조사 선행. 상세 = `TRADE_STRUCTURE.md §5-10`.
   - **카탈로그 서버 검색** — 지금은 전량(상한 2,000)을 받아 브라우저에서 거른다.
   - **가격변동 알림**.
