...
> **[AUTO-SAVE STALE] (2026-07-12 03:55, idle 1826s)** — narrative 가 마지막 편집된 이후 작업 파일이 변경됐는데 narrative 가 미갱신 상태로 자동저장됨. /개발시작 진입 시 git HEAD 와 대조해 진행/완료를 정정하고 이 블록을 삭제할 것.
> 변경된 작업 파일: deploy-manifest.json,deploy-to-production.sh
<!-- /AUTOSAVE-STALE-BANNER -->

## 현재 작업 상태
**마지막 업데이트:** 2026-07-11 #3 (임차인 임대료 청구 신규기능 **운영 배포** + 설정 가짜탭 제거(dev·미배포))
**버전:** **v3.68 운영 배포됨**(리포트·구독 정확성). 임대료 청구도 **운영 배포됨**(Backup 20260711_194035) — **버전 미상승, Irene 확인 대기**.
**작업 상태:** 완료 — verify-all 13/13. 미배포 dev분 = 설정 가짜탭 제거.
...
### 완료 (이번 세션 2026-07-11 #3)
- **임차인 임대료 청구 신규 기능 — `/기능설계` 6단계 → 운영 배포** (Backup 20260711_194035). 계약 임대조건 → 매월 임대료 인보이스 자동발행(멱등 1장/월) → 임차인별 납부·연체 현황 → 계약 종료 시 중단. **신규 테이블 0개**. 이메일 알림·연체 전환은 기존 인보이스 파이프라인이 그대로 커버(실증). 설계 = `docs/TENANT_RENT_BILLING.md`, 메모리 = [[reference_tenant_rent_billing]].
  - 🔴 발견·수정: 임대 관리 라우트가 `Foodcourt Manager` 만 허용 → **임대사업자인 총괄이 자기 화면에 못 들어갔다**
  - ⚠ **마이그레이션이 배포 중 실행 안 됨** → 운영 수동 실행으로 복구. 원인 미규명(로그 tail 로만 저장) — 다음 배포 전 반드시 확인
- **설정 매니저 탭 3개 제거** (dev·미배포) — Company/Brands/Billing 이 전부 하드코딩("January 15, 2025", 브랜드 상수)이고 저장 API 0건. 게다가 `requireRestaurantMatch` 로 **아무도 도달 못 하던 죽은 코드**. 매니저 구독 'Manage Billing'(일괄청구) 도 제거.
  - ⚠ 정리 중 **RA 설정 크래시 회귀**를 냈고 실브라우저 검증에서 잡아 수정(프린터 설정 포함 운영 critical). 현재 기본탭·프린터·주방스테이션 정상.
- 검증: verify-all **13/13**(8역할 mount 657초 크래시0) · 실호출 13/13 + 유저흐름 11/11 · health-check pos **19/19** · 인쇄 8/8·라우트 34/34
...
### 다음 확정 작업 / 남은 할 일 (2026-07-11 기준 전수)

#### A. 즉시 대기 (사람 필요)
1. **with MIN 데스크탑앱 0.1.7 매장 1회 인쇄 확인** — 코드로 증명 불가한 유일 항목. 앱 업데이트 → 스테이션 프린터 지정(BAR 포함) → 빌/오더티켓 디자인 출력 확인 → 백지면 `Ctrl+Shift+D → Render check`. 결과 판단 = **Fable**. 백지여도 `printFormat=auto` 원격복구(재방문 불필요). 절차는 아래 "⏳ 여전히 대기(물리)" 블록.
2. **Fable 사후 점검** — v3.68·임대료 청구 배포분 둘 다 게이트 대상이었으나 Irene 지시로 배포. 대조 항목 = 아래 "★ Fable 검증 대상" 블록.
3. **미배포 dev분 배포** — 설정 가짜탭 제거(매니저 Company/Brands/Billing) + 매니저 구독 'Manage Billing' 제거. 검증 후 `/배포`.

#### B. 배포 인프라 결함 (다음 배포 전 반드시)
4. **🔴 마이그레이션이 배포 중 실행되지 않았다** (2026-07-11 임대료 배포) — `migrate-rent-category.js` 가 운영에 복사됐으나 **실행 안 됨**(수동 실행으로 복구). 배포 로그를 `tail -40` 으로만 저장해 원인 구간이 잘려 진단 실패. **다음 배포 때 마이그레이션 구간 전체 로깅 후 원인 규명.** 이번엔 카테고리 행 1개라 수동 복구가 쉬웠지만, 스키마 변경이었으면 위험했다.
5. 배포 직후 **스모크 7/9** (재현 시 전부 정상 → 재시작 순간 전이적 실패). 스모크에 재검 로직 추가 검토.

#### C. 미구현·반쪽 기능
6. **임대료 청구 후속** — 연차별 인상(`rent_schedule`) 자동 적용(현재 입력만·청구는 base_rent) · 매출연동 임대료(percentage rent) 정산 · 보증금 관리/반환
7. **비전 AI B2** — 진짜 이미지 AI 연결(Irene API 키 필요). 현재 색 기반 로컬 인식(B1)
8. **오프라인 편집 배선** · **#38 고객 분석** · **인벤토리 브랜드 모드**(BG ProductIngredient 기준)
9. **설정의 'Add Brand' 버튼** — 가짜탭 제거로 함께 사라짐. 브랜드 추가는 `/pos/brand/general/management` 가 실기능(별도 확인 필요)

#### D. 구조 부채 (사고 없음, 점진 정리)
10. **서비스워커 콜드스타트** — 첫 로드에서 `/api` 요청이 실패할 수 있음(AuthContext 로그아웃은 방어 완료). 다른 화면은 첫 진입에 빈 데이터를 볼 수 있음 → SW fetch 전략 재검토
11. timezone baseline · design baseline — **신규 위반 0**, 기존분 점진 교체
12. **POS 터미널의 죽은 'Coming Soon' 모달** — `setShowFeatureAlert(true)` 호출 0건(열리지 않음). **POSTerminalPage 는 인쇄 보호파일이라 의도적으로 손대지 않음**(사용자 이득 0인 정리를 위해 생명선 파일을 건드리지 않는다)
