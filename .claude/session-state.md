...
## 현재 작업 상태
**마지막 업데이트:** 2026-06-27 #2 (🆕 thefire 브랜드메뉴 3매장 완전동기화 + "수요일 세트 안나옴" 근본수정(LUNCH MENU 카테고리 활성화) + 요일 런치스케줄 3매장 동일 — **운영 데이터 작업, 코드 무수정**. 세트 전파 영구수정(brandMenuSyncService.js)은 미커밋·**Irene "이따 배포" 대기**.)
...
>
> **(이어서) 브랜드메뉴 이름 한글 제거 — 운영 적용 완료**: brand5 메뉴 119개 이름 + 3매장 상품 357개에서 한글 제거(한글잔존 0 검증). 규칙=(a)한글+`l`구분자 제거→영문 (b)한글만 제거 (c)한글 뒤 수량(\d pcs)이면 유지("8pcs l Fried…"), 잔여조각(100/&/UFO/(2인))은 영문측만. 빈이름0·RA커스텀0. 백업 `/var/www/backups/thefire-brandmenu-names-20260627.json`.
> **(이어서 2차) 옵션·카테고리·치킨윙 l 까지 전부 한글 제거 완료**: Irene 추가지시. 브랜드옵션 24 + 매장옵션 72 + 메뉴(치킨윙 l)2 + 매장상품(치킨윙)6. 옵션 구분자는 "I"(대문자i)였음(메뉴는 "l")→둘다 처리, 이모지(🌶) 보존. 옵션그룹·카테고리·Product.category 는 원래 한글0. **전 테이블(메뉴/옵션/그룹/카테고리×브랜드+3매장) 한글잔존 0 검증.** 백업 `/var/www/backups/thefire-options-names-20260627.json`. 치킨윙="8pcs Fried Chicken Wing (2 flavors)".
...
- **thefire 브랜드메뉴 3매장 완전동기화 (운영 데이터 수정, 코드 무수정)**: BG가 넣은 브랜드메뉴(brand 5 "The Fire", 126개)가 thefire01(rid16)·02(rid24)·03(rid25)에 옵션/세트가 동일하게 안 들어간 신고. **실데이터 정밀비교로 진단**: 메뉴 누락0, 그러나 ①옵션그룹 누락(rid16=8·rid24=2: 치킨윙소스/찌개메인/닭갈비BBQ애드온 등 — manual 배포모드라 pending만 뜨고 미반영) ②세트구성 stale(rid24·25 SET1/2/5/6 — set_groups가 켤수없는 locks.set_items에 묶여 미전파, dev 미배포 수정본 존재) ③세트플래그(bm#240). **수정**: 매장별 백업(`/var/www/backups/thefire{16,24,25}-products-20260627_brandmenu_resync.json`) → dev의 수정된 brandMenuSyncService(set 항상미러) 로직을 자체완결 스크립트로 운영서 실행, 126개×3매장 재동기화. **RA커스텀(이름/가격/카테고리/is_active) 보존**(rid24 active2→2·rid25 126→126). 최종검증 3매장 모두 opt/setGroup/setFlag mismatch=0. 세트 전수비교(SET1~6+월화수목+녹차)도 구성품·수량까지 3매장 동일 확인. 운영 소스 무수정·임시파일 정리. **⚠️ 운영코드엔 아직 세트버그 잔존(수정본 dev 미배포) — BG 재편집 시 재발, /배포 필요.** 관련 [[project_brand_set_groups_verify]].
  - **후속 "수요일 세트 안나옴" 진단·해결**: 데이터 문제 아님. 근본=**thefire01(rid16) "LUNCH MENU" 카테고리가 isActive=0**(rid24/25는 켜져있었음)이라 월/화/수/목 런치세트 전 화면 숨김(POS=POSTerminalPage:1278 비활성카테고리 숨김 / 모바일=isActive필터). **rid16 LUNCH MENU 활성화**로 해결(POS/플로어플랜 즉시표시·POS는 요일스케줄 무시). 모바일은 카테고리활성+요일스케줄(월=days[1]/화[2]/수[3]/목[4] 11:30-14:00 disable) 둘다적용→수요일세트는 수요일점심에만(의도된 요일특선, Irene "스케줄 유지" 결정). **요일스케줄은 rid16에만 있었음→Irene "3매장 동일" 결정대로 rid24/25 런치세트4개에 동일 복사**(3매장 avail 일치). is_active 무수정(rid24 월/화/목 비활성=매장 미오픈상태, 그 매장선 여전히 안보임). 모바일: 리스트 set_groups 누락이나 상세 /menu/item/:id 가 set_groups 반환=세트 정상렌더. **목요일 THU(bm#238)는 브랜드에서 is_set_menu=false(구성無)=세트아님 — 세트로 원하면 BG 구성 필요.**
