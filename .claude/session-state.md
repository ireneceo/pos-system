...
- **발주 수령·배송 양방향 확인 + 결제/현금 연결 + 판매자 재고 차감** (2026-09-01 Irene 지시:
  *"다른 건 다 다음 섹션에 설계해서 하게 저장해"*) — **설계부터**.
  ...
  - **A. GIT 이 팔아도 GIT 자기 재고가 안 빠진다 — 프로덕트↔자기재고 연결 부재**
    ...
    · **레시피 없는 프로덕트 = 수량이 프로덕트에 그대로 있어야 한다** (Irene 확정).
      ⇒ 지금 차감 코드가 `brand_products.current_stock` / `products.current_stock` 을 보는 것은 **맞다.**
      ⚠ 내가 이 자리를 "레거시라 틀렸다"고 한 것은 **오판**(그 주석은 "별도 재고아이템도 만들면 이원화"라는 뜻).
      🔴 **진짜 결함은 입고 쪽** — GIT 이 UGS 에서 산 포장재가 프로덕트가 아니라 **따로 만든 재고아이템**으로
      입고됐다(투명컵/뚜껑 18 pack). 발주 화면이 재고아이템만 주문 대상으로 삼아서
      (PO 라인이 `product_ingredient_id` 참조) 레시피 없는 프로덕트를 **직접 보충할 경로가 없다.**
      ⇒ ①발주·입고가 프로덕트 수량으로 들어가게 ②이미 재고아이템에 있는 수량을 프로덕트로 이관 후 중복 정리
        ③`track_stock` 게이트 재검토(현재 0 이라 수량 있어도 차감 안 걸림)
