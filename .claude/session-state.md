...
**마지막 업데이트:** 2026-07-13
**버전:** v3.68 (운영).
**작업 상태:** 완료 — ①브랜드 재료 실사·발주제안·매장별 PAR ②발주 오너 승인 우회 봉인 + 판매자 유출 차단 ③**반품 net 0 수정**. **전부 운영 배포 완료** (반품분: Backup 20260713_093857 · 스모크 9/9 · **운영 health-check pos 37/37**).

### 진행 중인 작업
- 없음

### 다음 확정 작업
- **🔴 배포 스크립트가 원인 없이 죽는다 — 다음 세션 최우선.** 반품 배포 1차 시도가 **안전 게이트 9/9 통과 후 스키마 비교 단계에서 조용히 exit 2**(에러 메시지 0). 운영은 손대기 전이라 무피해, 재시도는 정상 통과(dev 152 = 운영 152).
  - **원인 규명 완료**: `deploy-to-production.sh:164` 가 `node compare-schema.js --export ... 2>/dev/null` 로 **stderr 를 버린다**. `compare-schema.js:187-189` 는 export 실패 시 `console.error('✗ Export failed:', e.message); process.exit(2)` → 배포 스크립트의 `set -e` 가 그 2 를 받아 **아무 말 없이 종료**. 다음 줄 `DEV_TABLES=$(node -e ...)` 도 파일이 없으면 같은 식으로 죽는다.
  - **고칠 것**: ①`2>/dev/null` 제거(또는 stderr 를 배포 로그로) ②실패 시 `error "스키마 export 실패: <원인>"` 로 **fail-loud** ③export 산출물 존재·유효 확인 후 진행 ④1차 실패의 진짜 원인(일시적 DB 연결/풀 고갈?) 재현 시도.
  - **왜 중요한가**: fail-closed 는 맞지만 **fail-silent 는 안 된다** — 배포가 왜 안 됐는지 알 수 없으면 다음 사람이 `--skip-safety` 로 우회하게 된다.
