...
### 다음 확정 작업
- **인보이스 대조 화면 «줄 고르기»가 인보이스와 전혀 안 맞음** (2026-09-11 Irene 지시 — 다음 세션에서 본다)
  - 화면: https://purplehere.com/pos/purchase-orders/32/reconcile (**운영** 발주 32)
  - Irene 원문: 「여기 인보이스에 나오는 항목이랑 `Pick this line from the invoice…` `0.80` `kg` 여기서 고르라는 항목이 전혀 맞지도 않아. 그리고 고르라는 건 5개도 넘는데 리스트는 2-3개 나와. 맞지도 않은 걸? 이미지 제대로 스캔을 못하는 거야? 수준이 낮아? 이거 같이 다음 섹션에 보게 해줘.」
  - 증상 두 가지(아직 원인 미확정 — 추측 금지, 실측부터):
    1. 선택 목록에 뜨는 인보이스 줄이 실제 인보이스 항목과 **내용이 안 맞는다**
    2. 인보이스 항목은 **5개 넘게** 있는데 선택 목록에는 **2~3개만** 뜬다
  - 어디부터 잴지(구조 사실만): 자동 읽기는 브라우저 무료 OCR(`dev-frontend/src/utils/invoiceOcr.ts`, tesseract, 긴 변 1800px 축소) → 줄 짝짓기 `invoiceMatcher.ts`(사전 우선 → 전역 greedy 1:1) → 선택 목록은 **다른 줄이 이미 가져간 인보이스 줄을 뺀다**(09-10 배포).
    - ①OCR 이 뽑은 원문 줄 수·내용 ②파서가 «품목 줄»로 인정한 줄 수 ③이미 가져간 줄 제외로 줄어든 수를 **단계별로 따로** 재야 «스캔이 못 읽음 / 줄 나누기 실패 / 목록 필터» 중 어디인지 갈린다. 메모리 [[reference_invoice_ocr_browser]] 에 «약한 고리는 이름 매칭».
  - 필요한 것: 발주 32 에 올린 **인보이스 원본 파일**(운영 `external_invoice_url`) — 운영 읽기 전용 계정(«Irene 님이 하실 일» 1번)이 있으면 경로 조회가 쉽다. 없으면 Irene 에게 그 인보이스 사진/PDF 를 받아 dev 에서 같은 파일로 재현.
  - 돈 칸(원가)에 닿는 화면이라 원인 확정 뒤 수정 방향이 갈리면 Fable 판정.
