...
## 현재 작업 상태
**마지막 업데이트:** 2026-05-03 (v3.24 운영 배포)
**버전:** **v3.24** (운영, 2026-05-03 배포 — 결제 시스템 표준화 + External QR ↔ Coupon 매핑)
**작업 상태:** 중단 (이어서 재개 예정)
...
**마지막 작업:** 라운드 4 (B9 v2 sales-chart 통합 + B10 v2 27 tests + CI MySQL service container) 커밋 `c484db3d`. 사용자가 다음 개발 계획 점검 후 1시간 휴식.

**바로 다음 작업 (사용자 결정 대기)** — 미완료 영역 5개 + 가성비 권고:
- (가) Overdue 자동 전환 Cron (1~2h, ★★★) → 즉시 운영 가치
- (나) Kitchen Display Item View (4~6h) — 명확 기획 + 사용자 가치
- (다) Invoice Payment Page 공용 (4~6h) — 결제 전환율
- 또는 사용자가 신규 기능 명세

**맥락 유지할 것:**
- `V3_18_BASIC_TIER_GAPS.md` 문서 outdated (코드는 fix 완료) — 갱신 필요
- B10 v3 후속: `tests/fixtures/seed.js` 만들면 CI backend-tests 활성화 가능
- `.github/workflows/ci.yml` 이동은 사용자 sudo 영역
- 미배포 4 라운드 누적: 8acc8f48 + dca93e6a + b687a58c + c484db3d (사용자가 이따 `/배포` 예정)
