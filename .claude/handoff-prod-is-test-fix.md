...
그 수정이 배포되면 더는 뒤집히지 않는다. 지금 필요한 건 **이미 잘못 붙어 있는 2행을 되돌리는 것**뿐이다.
...
3. **`id IN (11,29)` 이외의 조건으로 UPDATE 하지 마라.** `WHERE is_test=1` 같은 광범위 조건 금지 —
   운영에 정상적으로 테스트 표시된 계정이 34건 있고, 그건 전부 그대로 둬야 한다.
...
5. **스키마 변경 금지.** 컬럼 추가·ALTER 하지 마라. `is_real_customer` 컬럼은 다음 배포 때
   마이그레이션(`scripts/migrate-real-customer-flag.js`)이 만든다. 지금 손으로 만들지 마라.
...
- 전체 분포 대조: 작업 전 `users` 49건 중 `is_test=1` **34건** → 작업 후 **32건** 이어야 한다.
